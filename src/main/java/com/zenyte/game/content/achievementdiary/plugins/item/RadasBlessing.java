...
public class RadasBlessing extends DiaryItem {
    ...
    public void handle() {
        bind("Kourend Woodland", (player, item, slotId) -> {
            val limit = item.getId() == ItemId.RADAS_BLESSING_1 ? 3 : item.getId() == ItemId.RADAS_BLESSING_2 ? 5 : -1;
            val usedTeleports = player.getVariables().getKourendWoodlandTeleports();
        ...
        bind("Mount Karuulm", (player, item, slotId) -> {
            val limit = item.getId() == ItemId.RADAS_BLESSING_3 ? 3 : -1;
            val usedTeleports = player.getVariables().getMountKaruulmTeleports();
            if (limit != -1 && usedTeleports >= limit) {
                player.sendMessage("You have used up your daily teleports for today.");
                return;
            }
    ...
    @Override
    public int[] getItems() {
        return new int[]{ItemId.RADAS_BLESSING_1, ItemId.RADAS_BLESSING_2, ItemId.RADAS_BLESSING_3,
