...
public class DesertAmulet extends DiaryItem {
    ...
    private void nardahTeleport(final Player player, final Item item) {
        if (item.getId() == 13133) { //has teleport option but it doesn't mention the teleport feature on the wikia (drunk)
            player.sendMessage("Nothing interesting happens.");
            return;
        }
        val limit = item.getId() == 13133 ? 0 : item.getId() == 13136 ? -1 : 1;
        val usedTeleports = player.getVariables().getNardahTeleports();
