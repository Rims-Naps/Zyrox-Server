...
public class WildernessDragonBonesProcessor extends DropProcessor {
    ...
    public void attach() {
        ...
        put(LAVA_DRAGON_BONES, new PredicatedDrop("This drop will be noted in the Wilderness if you have the elite Wilderness diary completed, you can toggle this off by talking to Lesser Fanatic."));

    }

    @Override
    public Item drop(NPC npc, Player killer, Drop drop, Item item) {
        if ((item.getId() == DRAGON_BONES || item.getId() == LAVA_DRAGON_BONES) && killer.inArea("Wilderness") && DiaryReward.WILDERNESS_SWORD4.eligibleFor(killer)) {
            if (item.getId() == LAVA_DRAGON_BONES && !killer.getAttributes().containsKey("NOTED_LAVA_DRAGON_BONES")) {
                return item;
            }
