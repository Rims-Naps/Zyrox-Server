...
public enum MorytaniaDiary implements Diary {
    ...
    CRAFT_SNELM(EASY, true, "Craft any Snelm from scratch in Morytania."),
    COOK_A_THIN_SNAIL(EASY, true, "Cook a thin Snail on the Port Phasmatys range."),
    GET_A_SLAYER_TASK_FROM_MAZCHNA(EASY, NO_PREDICATE, "Get a slayer task from Mazchna."),//done
    ...
    KILL_A_WEREWOLF(EASY, true, "Kill a werewolf in its human form using the Wolfbane", "Dagger."),
    RESTORE_PRAYER_POINTS_AT_NATURE_ALTAR(EASY, true, "Restore your prayer points at the nature altar."),

    CATCH_SWAMP_LIZARD(MEDIUM, NO_PREDICATE, "Catch a swamp lizard."),//done
    COMPLETE_CANIFIS_COURSE_LAP(MEDIUM, NO_PREDICATE, "Complete a lap of the Canifis agility course."),//done
    OBTAIN_BARK(MEDIUM, true, "Obtain some Bark from a Hollow tree."),
    TRAVEL_TO_DRAGONTOOTH_ISLE(MEDIUM, true, "Travel to Dragontooth Isle."),
    KILL_A_TERRORDOG(MEDIUM, true, NO_PREDICATE, "Kill a Terror Dog."),//done but disabled as terrordogs are unavailable.
    COMPLETE_TROUBLE_BREWING_GAME(MEDIUM, true, "Complete a game of trouble brewing."),
    BOARD_THE_SWAMPY_BOAT(MEDIUM, true, "Board the Swampy boat at the Hollows."),
    MAKE_CANNONBALLS(MEDIUM, PORT_PHASMATYS::inArea, "Make a batch of cannonballs at the Port Phasmatys", "furnace."),//done
    KILL_A_FEVER_SPIDER(MEDIUM, true, "Kill a Fever Spider on Braindeath Island."),
    USE_ECTOPHIAL(MEDIUM, true, "Use an ectophial to return to Port Phasmatys."),
    MIX_A_GUTHIX_BALANCE(MEDIUM, "Mix a Guthix Balance potion while in Morytania."),//done
    ...
    HARVEST_WATERMELON(HARD, p -> p.inArea(HarmonyIsland.class), "Harvest some Watermelon from the Allotment patch on", "Harmony Island."),
    CHOP_AND_BURN_MAHOGANY_LOGS(HARD, true, "Chop and burn some mahogany logs on Mos Le'Harmless."),
    COMPLETE_HARD_TEMPLE_TREK(HARD, true, "Complete a temple trek with a hard companion."),
    KILL_A_CAVE_HORROR(HARD, NO_PREDICATE, "Kill a Cave Horror."),//done
    ...
    USE_SALVE_BRIDGE_SHORTCUT(HARD, true, "Use the shortcut to get to the bridge over the Salve."),
    MINE_MITHRIL_IN_ABANDONED_MINE(HARD, true, "Mine some Mithril ore in the Abandoned Mine."),

    CATCH_SHAKE_IN_BURGH_DE_ROTT(ELITE, true, "Catch a shark in Burgh de Rott with your bare hands."),
    CREMATE_SHADE_REMAINS(ELITE, true, "Cremate any Shade remains on a Magic or Redwood pyre."),
    FERTILIZE_MORYTANIA_HERB_PATCH(ELITE,  "Fertilize the Morytania herb patch using Lunar Magic."),
    CRAFT_BLACK_DRAGONHIDE_BODY(ELITE, CANIFIS_BANK::inArea, "Craft a Black dragonhide body in Canifis bank."),//done
    KILL_ABYSSAL_DEMON(ELITE, p -> p.inArea("Slayer Tower"), "Kill an Abyssal demon in the Slayer Tower."),//done
    LOOT_BARROWS_CHEST(ELITE, NO_PREDICATE, "Loot the Barrows chest while wearing any complete", "barrows set.");//done
    ...
    public DiaryChunk[] chunks() {
        return new DiaryChunk[] { new DiaryChunk(6315, 11, 4527), new DiaryChunk(6316, 11, 4528), new DiaryChunk(6317, 10, 4529), new DiaryChunk(6318, 6, 4530) };
    }
