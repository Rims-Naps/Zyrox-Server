...
public enum DesertDiary implements Diary {
	...
	GET_HERB_CLEANED(EASY, true, "Have the Nardah Herbalist clean a Herb for you."),
	COLLECT_POTATO_CACTUS(EASY, true, "Collect 5 Potato Cactus from the Kalphite Hive."),
	SELL_ARTIFACTS(EASY, true, "Sell some artifacts to Simon Templeton."),
	OPEN_SARCOPHAGUS(EASY, true, "Open the Sarcophagus in the first room of Pyramid", "Plunder."),
	CUT_CACTUS(EASY, true, "Cut a desert cactus open to fill a waterskin."),
	CARPET_TO_POLLNIVNEACH(EASY, true, "Travel from the Shantay Pass to Pollnivneach by Magic", "Carpet."),

	CLIMB_AGILITY_PYRAMID(MEDIUM, NO_PREDICATE, "Climb to the summit of the Agility Pyramid."), //done
	SLAY_DESERT_LIZARD(MEDIUM, "Slay a desert lizard."), // done
	CATCH_ORANGE_SALAMANDER(MEDIUM, "Catch an Orange Salamander."),//done
	STEAL_DARK_PHOENIX_FEATHER(MEDIUM, true, "Steal a feather from the Desert Phoenix."),
	CARPET_TO_UZER(MEDIUM, true, "Travel to Uzer via Magic Carpet."),
	TRAVEL_TO_DESERT_VIA_EAGLE(MEDIUM, true, "Travel to the Desert via Eagle."),
	PRAY_AT_ELIDINIS_STATUETTE(MEDIUM, "Pray at the Elidinis statuette in Nardah."), // done
	CREATE_COMBAT_POTION(MEDIUM, "Create a combat potion in the desert."), // done
	TELEPORT_TO_ENAKHRAS_TEMPLE(MEDIUM, true, "Teleport to Enakhra's Temple with the Camulet."),
	VISIT_THE_GENIE(MEDIUM, true, "Visit the Genie."),
	TELEPORT_POLLNIVNEACH_HOUSE(MEDIUM, true, "Teleport to Pollnivneach with a redirected teleport to", "house tablet."),
	CHOP_TEAK_LOGS(MEDIUM, "Chop some Teak logs near Uzer."), // done

	PICKPOCKET_MENAPHITE_THUG(HARD, true, "Knock out and pickpocket a Menaphite Thug."),
	MINE_GRANITE(HARD, true, "Mine some Granite."),
	REFILL_WATERSKINS_WITH_HUMIDIFY(HARD, "Refill your waterskins in the Desert using Lunar magic."), // done
	KILL_KALPHITE_QUEEN(HARD, NO_PREDICATE, "Kill the Kalphite Queen."), // done
	COMPLETE_POLLNIVNEACH_AGILITY_COURSE_LAP(HARD, true, "Complete a lap of the Pollnivneach agility course."),
	SLAY_DUST_DEVIL(HARD, p -> p.inArea(SmokeDungeonArea.class), "Slay a Dust Devil with a Slayer helmet equipped."), // done
	ACTIVATE_ANCIENT_MAGICKS(HARD, true, "Activate Ancient Magicks at the altar in the Jaldraocht", "Pyramid."),
	DEFEAT_LOCUST_RIDER_WITH_KERIS(HARD, true, "Defeat a Locust Rider with Keris."),
	BURN_YEW_LOGS(HARD, "Burn some yew logs on the Nardah Mayor's balcony."), // done
	...
	SPEAK_TO_KQ_HEAD(ELITE, true, "Speak to the KQ head in your POH."),
	STEAL_FROM_GRAND_GOLD_CHEST(ELITE, true, "Steal from the Grand Gold Chest in the final room of", "Pyramid Plunder."),
	RESTORE_85_PRAYER_POINTS(ELITE, "Restore at least 85 Prayer points when praying at the", "Altar in Sophanem.");//done
	...
	public DiaryChunk[] chunks() {
		return new DiaryChunk[] { new DiaryChunk(6295, 11, 4523), new DiaryChunk(6296, 12, 4524), new DiaryChunk(6297, 10, 4525), new DiaryChunk(6298, 6, 4526) };
	}
