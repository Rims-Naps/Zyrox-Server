...
public enum KourendDiary implements Diary {
    ...
    HAND_A_BOOK_AT_LIBRARY(EASY, true, "Hand in a book at the Arceuus Library."),
    STEAL_FROM_FOOD_STALL(EASY, "Steal from a Hosidius Food Stall."),//done
    ...
    DIG_SALTPETRE(EASY, true, "Dig up some Saltpetre."),
    ENTER_POH(EASY, true, "Enter your Player Owned House from Hosidius."),
    HEAL_A_WOUNDED_SOLDIER(EASY, true, "Heal a wounded Shayzien soldier."),
    CREATE_STRENGTH_POTION(EASY, p -> LOVAKENGJ_PUB.inArea(p), "Create a Strength potion in the Lovakengj Pub."), //done
    FISH_A_TROUT(EASY, p -> p.inArea(GreatKourend.class), "Fish a Trout from the River Molch."),

    FAIRY_RING_TO_MT_KARUULM(MEDIUM, NO_PREDICATE, "Travel to the Fairy Ring south of Mount Karuulm."),//done
    KILL_A_LIZARDMAN(MEDIUM, "Kill a Lizardman."),//done
    USE_KHAREDST_MEMOIRS(MEDIUM, true, "Use Kharedst's memoirs to teleport to all five cities in", "Great Kourend."),
    MINE_VOLCANIC_SULPHER(MEDIUM, true, "Mine some Volcanic sulphur."),
    ENTER_FARMING_GUILD(MEDIUM, "Enter the Farming Guild."),//done
    SWITCH_TO_NECROMANCY_SPELLBOOK(MEDIUM, "Switch to the Necromancy Spellbook at Tyss."),//done
    REPAIR_A_PISC_CRANE(MEDIUM, true, "Repair a Piscarilius crane."),
    DELIVER_SOME_INTELLIGENCE_TO_GINEA(MEDIUM, true, "Deliver some intelligence to Captain Ginea."),
    CATCH_A_BLUEGILL(MEDIUM, true, "Catch a Bluegill on Molch Island."),
    USE_BOULDER_LEAP(MEDIUM, "Use the boulder leap in the Arceuus essence mine."),//done
    SUBDUE_WINTERTODT(MEDIUM, "Subdue the Wintertodt."),//done
    ...
    PLANT_SOME_LOGOVANO_SEEDS(HARD, true,"Plant some Logavano seeds at the Tithe Farm."),
    KILL_A_ZOMBIE(HARD, true, "Kill a Zombie in the Shayzien Crypts."),
    TELEPORT_TO_XERICS_HEART(HARD, NO_PREDICATE, "Teleport to Xeric's Heart using Xeric's Talisman."), //done
    DELIVER_AN_ARTIFACT_TO_CAPTAIN_KHALED(HARD, true,"Deliver an artefact to Captain Khaled."),
    KILL_A_WYRM(HARD, NO_PREDICATE, "Kill a Wyrm in the Karuulm Slayer Dungeon."), //done
    ...
    DEFEAT_SKOTIZO(ELITE, true, "Defeat Skotizo in the Catacombs of Kourend."),
    CATCH_ANGLERFISH(ELITE, 0x1 | 0x2, "Catch an Anglerfish and cook it whilst in Great Kourend.") {
        @Override
    ...
    CREATE_APE_ATOLL_TELEPORT(ELITE, true, "Create an Ape Atoll teleport tablet."),
    COMPLETE_A_RAID(ELITE, NO_PREDICATE, "Complete a Raid in the Chambers of Xeric."),
    CREATE_BATTLESTAFF(ELITE, false, 0x1 | 0x2, p -> p.inArea(FarmingGuildArea.class), "Create your own Battlestaff from scratch within the", "Farming Guild.") {
        @Override
        public boolean flagging() {
            return true;
        }
    ...
    public DiaryChunk[] chunks() {
        return new DiaryChunk[]{new DiaryChunk(7933, 12, 7929),
                new DiaryChunk(7934, 13, 7930),
                new DiaryChunk(7935, 10, 7931),
                new DiaryChunk(7936, 8, 7932)};
    }
