...
public class ExplorersRing extends DiaryItem {
    ...
    public static final IntList rings = new IntArrayList(Arrays.asList(ItemId.EXPLORERS_RING_1, ItemId.EXPLORERS_RING_2,
        ...
        bind("Functions", (player, item, slotId) -> {
            player.getDialogueManager().start(new Dialogue(player) {
                ...
                public void buildDialogue() {
                    options(TITLE, "Recharge run energy", "Alchemy")
                            .onOptionOne(() -> energy(player, item))
                            .onOptionTwo(() -> alchemy(player));
                }
            });
        });
        bind("Teleport", (player, item, slotId) -> {
            val usedTeleports = player.getVariables().getCabbageFieldTeleports();
    ...
    private void energy(final Player player, final Item item) {
        val limit = item.getId() == ItemId.EXPLORERS_RING_1 ? 2 : item.getId() == ItemId.EXPLORERS_RING_2 || item.getId() == ItemId.EXPLORERS_RING_4 ? 3 : 4;
        val replenishments = player.getVariables().getRunReplenishments();
    ...
    private void alchemy(final Player player) {
        val hasCast = player.getVariables().getFreeAlchemyCasts() < FREE_ALCHEMY_CASTS;
        if (!hasCast) {
    ...
    public static boolean hasAnyRing(final Player player) {
        return player.carryingAny(ItemId.EXPLORERS_RING_1, ItemId.EXPLORERS_RING_2, ItemId.EXPLORERS_RING_3,
