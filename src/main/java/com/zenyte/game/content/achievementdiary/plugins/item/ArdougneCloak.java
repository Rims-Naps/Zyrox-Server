...
public class ArdougneCloak extends DiaryItem {
    ...
    private void ardougneFarmTeleport(final Player player, final Item item) {
        val limit = item.getId() == 13122 ? 3 : item.getId() == 13123 ? 5 : -1;
        val usedTeleports = player.getVariables().getArdougneFarmTeleports();
    ...
    @Override
    public int[] getItems() {
        return new int[]{13121, 13122, 13123, 13124, MaxCape.ARDOUGNE.getCape()};
    }
}
