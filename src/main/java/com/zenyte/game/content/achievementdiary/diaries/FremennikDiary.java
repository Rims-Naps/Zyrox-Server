...
public enum FremennikDiary implements Diary {
    ...
    CHANGE_YOUR_BOOTS(EASY, true, "Change your boots at Yrsa's Shoe Store."),
    KILL_ROCK_CRABS(EASY, 5, "Kill 5 Rock crabs."),//done
    CRAFT_A_TIARA(EASY, 0x1 | 0x2 | 0x4, "Craft a tiara from scratch in Rellekka.") {
        @Override
    ...
    CHOP_AND_BURN_OAK_LOGS(EASY, 0x1 | 0x2, "Chop and burn some oak logs in the Fremennik Province.") {
        @Override
        public boolean flagging() {
            return true;
        }
    },//done
    SLAY_BRINE_RAT(MEDIUM, true, "Slay a Brine rat."),
    TRAVEL_TO_SNOWY_AREA_VIA_EAGLE(MEDIUM, true, "Travel to the Snowy Hunter Area via Eagle."),
    MINE_COAL_IN_RELLEKKA(MEDIUM, "Mine some coal in Rellekka."),//done
    ...
    CATCH_A_SNOWY_KNIGHT(MEDIUM, true, "Catch a Snowy knight."),
    PICK_UP_YOUR_PET_ROCK(MEDIUM, true, "Pick up your Pet Rock from your POH Menagerie."),
    VISIT_THE_LIGHTHOUSE(MEDIUM, true, "Visit the Lighthouse from Waterbirth island."),
    MINE_SOME_GOLD(MEDIUM, true, "Mine some gold at the Arzinian mine."),

    TELEPORT_TO_TROLLHEIM(HARD, NO_PREDICATE, "Teleport to Trollheim."),//done
    ...
    CRAFT_FREMENNIK_SHIELD(HARD, true, "Craft a Fremennik shield on Neitiznot."),
    MINE_ADAMANTITE_ORE(HARD, false, 5, JATISZO_MINE::inArea, "Mine 5 Adamantite ores on Jatizso."),//done
    OBTAIN_SUPPORT_FROM_KINGDOM(HARD, true, "Obtain 100% support from your kingdom subjects."),
    TELEPORT_TO_WATERBIRTH_ISLAND(HARD, NO_PREDICATE, "Teleport to Waterbirth Island."),//done
    OBTAIN_BLAST_FURNACE_PERMISSION(HARD, true, "Obtain the Blast Furnace Foreman's permission to use the", "Blast Furnace for free."),

    KILL_DAGANNOTH_KINGS(ELITE, false, 0x1 | 0x2 | 0x4, NO_PREDICATE, "Kill each of the Dagannoth Kings.") {
        @Override
        public boolean flagging() {
            return true;
        }
    ...
    COMPLETE_RELLEKKA_AGILITY_COURSE_LAP(ELITE, "Complete a lap of the Rellekka agility course."),
    KILL_GODWARS_GENERALS(ELITE, false, 0x1 | 0x2 | 0x4 | 0x8, NO_PREDICATE, "Kill each of the Godwars generals.") {
        @Override
        public boolean flagging() {
            return true;
        }
    },//done
    SLAY_A_SPIRITUAL_MAGE(ELITE, NO_PREDICATE, "Slay a Spiritual mage within the Godwars Dungeon.");//done
    ...
    public DiaryChunk[] chunks() {
        return new DiaryChunk[] { new DiaryChunk(6303, 10, 4531), new DiaryChunk(6304, 9, 4532), new DiaryChunk(6305, 9, 4533), new DiaryChunk(6306, 6, 4534) };
    }
