...
public class Wintertodt {
    ...
    private static boolean allFourBraziersDownedDuringGame;
    private static boolean aPyromancerDownedDuringGame;
    ...
    private static final IntSet warmClothing = new IntArraySet(new int[]{
            ...
            // pyro gear
            RewardCrate.PYROMANCER_HOOD,
            RewardCrate.PYROMANCER_GARB,
            RewardCrate.PYROMANCER_ROBE,
            RewardCrate.PYROMANCER_BOOTS,
            RewardCrate.WARM_GLOVES,
            ...
            12773,
            20056,
            LightSourceItem.LightSource.BRUMA_TORCH.getLitId(),
            
            // misc shield slot
            TomeOfFire.TOME_OF_FIRE,
            TomeOfFire.TOME_OF_FIRE_EMPTY,
            7053
    });
    private static final RSPolygon safeArea = new RSPolygon(new int[][]{
            {1624, 3988},
            {1625, 3987},
            {1625, 3978},
            {1628, 3975},
    ...
    static {
        for (val potion : RejuvenationPotion.Potion.items) {
    ...
    static void tickRoundTimer() {
        ...
        roundTimer--;
        if (roundTimer == 0) { // round wait time has finished
            setEnergyToMax();
            refreshOverlayForAllPlayers();
            allFourBraziersDownedDuringGame = false;
            aPyromancerDownedDuringGame = false;
        }
    }
    ...
    static void addPoints(final Player player, final int pointAmount) {
        ...
        val initialPoints = getPoints(player);
        val multiplier = getPointsMultiplier();
        val points = initialPoints + (pointAmount*multiplier);
    ...
    static void refreshOverlayForAllPlayers() {
        for (val player : GlobalAreaManager.get("Wintertodt").getPlayers()) {
            refreshOverlay(player);
        }
    }
    ...
    static void refreshOverlay(final Player player, final boolean forceTimerUpdate) {
        player.getPacketDispatcher().sendClientScript(1421,
                getPoints(player),
                Wintertodt.getCurrentEnergy(),
                Corner.SOUTH_WEST.pyromancer.isHealthy() ? 1 : 0,
                Corner.NORTH_WEST.pyromancer.isHealthy() ? 1 : 0,
                Corner.NORTH_EAST.pyromancer.isHealthy() ? 1 : 0,
                Corner.SOUTH_EAST.pyromancer.isHealthy() ? 1 : 0,
                Corner.SOUTH_WEST.brazier.getState().getCs2Value(),
                Corner.NORTH_WEST.brazier.getState().getCs2Value(),
                Corner.NORTH_EAST.brazier.getState().getCs2Value(),
                Corner.SOUTH_EAST.brazier.getState().getCs2Value());
    ...
    private static void finishRound() {
        for (val player : GlobalAreaManager.get("Wintertodt").getPlayers()) {
            ...
            if (points < 500) {
                ...
                val rewardExp = player.getSkills().getLevelForXp(Skills.FIREMAKING) * 100;
                val crate = generateSupplyCrate(player, points);
                ...
                val kills = player.getNotificationSettings().getKillcount("Wintertodt");
            ...
            if (getNumberOfWarmClothingWorn(player) == 4 && !player.getBooleanAttribute("easy-combat-achievement26")) {
                player.putBooleanAttribute("easy-combat-achievement26", true);
                EasyTasks.sendEasyCompletion(player, 26);
            }
            if (!allFourBraziersDownedDuringGame && !player.getBooleanAttribute("medium-combat-achievement29")) {
                player.putBooleanAttribute("medium-combat-achievement29", true);
                MediumTasks.sendMediumCompletion(player, 29);
            }
            if (!aPyromancerDownedDuringGame && !player.getBooleanAttribute("medium-combat-achievement33")) {
                player.putBooleanAttribute("medium-combat-achievement33", true);
                MediumTasks.sendMediumCompletion(player, 33);
            }
            if (points >= 3000 && !player.getBooleanAttribute("hard-combat-achievement48")) {
                player.putBooleanAttribute("hard-combat-achievement48", true);
                HardTasks.sendHardCompletion(player, 48);
            }
    
            val highestPoints = player.getNumericAttribute("wintertodt_highest_points").intValue();
            player.addAttribute("wintertodt_highest_points", Math.max(highestPoints, points));
    
            val accumulatedPoints = player.getNumericAttribute("wintertodt_total_accumulated_points").intValue();
    ...
    static Item generateSupplyCrate(final Player player, final int points) {
        val pointsPerRoll = 500;
        val crate = new Item(RewardCrate.SUPPLY_CRATE, 1);
        var rolls = (points + pointsPerRoll) / pointsPerRoll; // base rolls is always 2 for 500+ points
        
        rolls++; // add another extra roll, we're an rsps
        if (player.getMemberRank().eligibleTo(MemberRank.EMERALD_MEMBER)) { // emerald+ members get an extra roll
            rolls++;
        }
        if (player.getMemberRank().eligibleTo(MemberRank.DRAGONSTONE_MEMBER)) { // dragonstone+ members get another extra roll
            rolls++;
        }
         ...
         */
        val remainder = points % pointsPerRoll;
        val chanceForExtraRoll = (remainder * 100) / pointsPerRoll;
    ...
    static void dealDamage(final double amount) {
        ...
        val potentialEnergy = currentEnergy - amount;
    ...
    static int getNumberOfWarmClothingWorn(final Player player) {
        int amount = 0;
        for (val item : player.getEquipment().getContainer().getItems().values()) {
    ...
    static int getNumberOfBraziersBroken() {
        int amount = 0;
        for (val corner : Corner.VALUES) {
    ...
    enum Corner {
        ...
        static {
            for (Corner corner : VALUES) {
                ...
                val middle = new Location(corner.brazier.getX(), corner.brazier.getY()).transform(Direction.NORTH_EAST, 1);
    ...
    public static void failFourBrazierTask() {
        allFourBraziersDownedDuringGame = true;
    }

    public static void failPyromancerTask() {
        aPyromancerDownedDuringGame = true;
    }
