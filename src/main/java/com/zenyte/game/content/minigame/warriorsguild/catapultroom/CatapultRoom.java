...
public final class CatapultRoom extends WarriorsGuildArea implements FullMovementPlugin, CycleProcessPlugin {
    ...
    private static final InterfacePosition[] tabs = new InterfacePosition[] {
      InterfacePosition.JOURNAL_TAB_HEADER, InterfacePosition.INVENTORY_TAB, InterfacePosition.PRAYER_TAB, InterfacePosition.SPELLBOOK_TAB, InterfacePosition.SETTINGS_TAB, InterfacePosition.EMOTE_TAB
    };

    private static final String[] PROJ_NAMES = new String[]{"spiky ball", "flung anvil", "slashing blades", "magic missile"};

    private static final Projectile[] PROJS = new Projectile[]{new Projectile(679, 60, 15, 0, 45, 40 + 60, 0, 5),
            new Projectile(680, 60, 15, 0, 45, 40 + 60, 0, 5), new Projectile(681, 60, 15,
            0, 45, 40 + 60, 0, 5), new Projectile(682, 60, 15, 0, 45, 40 + 60, 0, 5)};
    ...
    private final void unequipShield(final Player player) {
        ...
        val i = player.getInterfaceHandler();
        i.openJournal();
    ...
    private final void checkStance(final Player player) {
        val value = player.getNumericTemporaryAttribute("catapultStance").intValue();
            ...
            if (!players.isEmpty()) {
                for (Player p : players) {
                    if (p.getLocation().getPositionHash() == LOCATION.getPositionHash()) {
                        if (!p.getAppearance().getForcedAppearance().isEmpty())
                            checkStance(p);
                        else {
