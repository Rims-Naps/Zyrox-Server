...
public class TzKalZukCutscene extends Cutscene {
    ...
    public void build() {
        addActions(0, () -> {
            ...
            for (val rocks : inferno.getFallingRocks()) {
                World.spawnObject(rocks);
            }
            for (val patch : inferno.getWallPatches()) {
        ...
        addActions(2,
                new CameraLookAction(player, inferno.getLocation(new Location(2270, 5364, 0)), 0, 100, 100),
                new CameraPositionAction(player, inferno.getLocation(new Location(2274, 5347, 0)), 800, 100, 100),
                new CameraShakeAction(player, CameraShakeType.UP_AND_DOWN, (byte) 10, (byte) 5, (byte) 0));
        addActions(3, () -> {
            for (val rocks : inferno.getFallingRocks()) {
