...
public class TzKalZuk extends InfernoNPC {
    ...
    private static final Location[] healerSpawnLocations = {
            new Location(2262, 5363, 0), new Location(2266, 5363, 0),
            new Location(2276, 5363, 0), new Location(2280, 5363, 0)
    };
    ...
    private boolean started, spawnedJad, spawnedHealers, increasedDelay;
    private long xilAndZekSpawnDelay;

    public TzKalZuk(final Inferno instance) {
        super(NpcId.TZKALZUK, instance.getLocation(spawnLocation), instance);
        ...
        combat = new NPCCombat(this) {
            ...
            public int combatAttack() {
                ...
                var distance = npc.getAttackDistance();
    ...
    public void processNPC() {
        ...
        if (xilAndZekSpawnDelay == 0) {
            val xil = (JalXil) new JalXil(inferno.getLocation(jalXilSpawnLocation), inferno).spawn();
            val zek = (JalZek) new JalZek(inferno.getLocation(jalZekSpawnLocation), inferno).spawn();
        ...
        if (!spawnedJad && getHitpoints() <= 480) {
            val jad = new JalTokJad(inferno.getLocation(jadSpawnLocation), inferno);
            ...
            for (val location : healerSpawnLocations) {
        ...
        if (healing) {
            ...
            for (val healer : healers) {
                val target = healer.getCombat().getTarget();
    ...
    private boolean isProtected(final Player player) {
        ...
        val distanceX = Math.sqrt(Math.pow(player.getX() - (glyph.getNextPosition(1).getX() + 1), 2));
        return distanceX <= 2;
    }
