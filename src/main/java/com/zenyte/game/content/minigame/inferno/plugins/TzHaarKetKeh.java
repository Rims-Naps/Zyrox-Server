...
public class TzHaarKetKeh extends NPCPlugin {
    ...
    private static final Item practiceFee = new Item(ItemId.COINS_995, 1_500_000);
    ...
    public void handle() {
        bind("Talk-to", (player, npc) -> player.getDialogueManager().start(new Dialogue(player, npc) {
            ...
            public void buildDialogue() {
                val hasSpoken = player.getAttributes().get("spokenToTzhaarKetKeh");
                if (hasSpoken == null) {
                    player("Wow what has happened here? That doesn't look good!");
                    npc("We pushed it too far! Way too far. JalYt would not understand. The memories, we needed them. Now big Inferno.")
                            .executeAction(() -> player.getAttributes().put("spokenToTzhaarKetKeh", true));
                }
                options(TITLE, new DialogueOption("What is this place?", key(WHAT_IS_THIS_PLACE)),
                        new DialogueOption("What happened here?", key(WHAT_HAPPENED)),
                        new DialogueOption("Can I go down there?", key(CAN_I_GO_DOWN)),
                        new DialogueOption("Talk about Practice Mode", () -> practiceMode(player, npc)),
                        new DialogueOption("Nevermind.", key(NEVERMIND)));
                ...
                options(TITLE, new DialogueOption("What happened here?", key(WHAT_HAPPENED)),
                        new DialogueOption("Can I go down there?", key(CAN_I_GO_DOWN)),
                        new DialogueOption("Nevermind.", key(NEVERMIND)));
                ...
                if (player.getNumericAttribute("infernoVar").intValue() == 2) {
                    ...
                    options("Sacrifice your Fire cape?", new DialogueOption("Yes, I want to sacrifice my cape.", key(SACRIFICE_CAPE)),
                    ...
                    if (player.getInventory().containsItem(TzHaar.FIRE_CAPE_ITEM)) {
                        ...
                        options("<col=ff0000>Really sacrifice your Fire cape?", new DialogueOption("Yes, hand it over.", key(HAND_OVER_CAPE)),
                                new DialogueOption("No, I want to keep it!")
                        );
        ...
        bind("Exchange", (player, npc) -> player.getDialogueManager().start(new Dialogue(player, npc) {
            @Override
            public void buildDialogue() {
                player("I have a spare infernal cape, perhaps I could offer it back to you for a littlun to take with me?");
                npc("Maybe. Hand it over and if you make lucky TokKul toss, it can be yours JalYt.");
                npc("But do know, you do not get cape back.");
                options("Sacrifice infernal cape for a chance at Jal-Nib-Rek?","Yes, I accept that I won't get my cape back.", "No, I want to keep my cape.").onOptionOne(() -> gambleInfernalCape(player));
            }
        }));
    }
    ...
    private static void practiceMode(final Player player, final NPC npc) {
        player.getDialogueManager().start(new Dialogue(player, npc) {
            ...
            public void buildDialogue() {
                ...
                val feeLocation = mode.equals(GameMode.ULTIMATE_IRON_MAN) ? "inventory" : "bank";
                if (player.getNumericAttribute("infernoVar").intValue() < 2) {
                ...
                options("Continue with the Inferno Practice Mode (costs " + Utils.format(practiceFee.getAmount()) + " coins).", "Yes - continue.", "No.")
                        .onOptionOne(() -> setKey(10))
                        .onOptionTwo(() -> setKey(20));
                player(10, "Yes, let's carry on.").executeAction(() -> {
                    if ((mode.equals(GameMode.ULTIMATE_IRON_MAN) && player.getInventory().containsItem(practiceFee)) ||
                            (!mode.equals(GameMode.ULTIMATE_IRON_MAN) && player.getBank().containsItem(practiceFee))) {
                        setKey(15);
                    } else {
                        setKey(25);
                    }
                ...
                npc(16, "At which wave would you like to start your practice run?").executeAction(() -> {
                    ...
                    player.sendInputInt("Start the Inferno Practice run at wave (1 - 69):", value -> {
                        ...
                        val wave = InfernoWave.get(value);
                        if (wave == null) {
                            setKey(30);
                            return;
                        }
                        player.getDialogueManager().start(new Dialogue(player, npc) {
                            ...
                            public void buildDialogue() {
                                ...
                                options("Do you wish to start at wave " + wave.getWave() + "?", "Yes.", "No.")
                                        .onOptionOne(() -> setKey(10))
                                        .onOptionTwo(() -> setKey(20));
                                ...
                                npc("Very well, JalYt.").executeAction(() -> {
                                    try {
                                        ...
                                        val inferno = new Inferno(player, area, true);
    ...
    public void gambleInfernalCape(Player player) {
        if(!player.getInventory().containsItem(ItemId.INFERNAL_CAPE)) {
            player.sendMessage(Colour.RS_RED.wrap("You have no infernal cape to exchange!"));
            return;
        }
        val roll = Utils.random(1, 100);
        player.getInventory().deleteItem(new Item(ItemId.INFERNAL_CAPE));
        if(roll == 1) {
            player.getInventory().addItem(new Item(ItemId.JALNIBREK));
            player.getDialogueManager().start(new NPCChat(player, 7690, "Luck be a TzHaar tonight. Jal-Nib-Rek is yours."));
            WorldBroadcasts.broadcast(player, BroadcastType.GAMBLE_CAPE, new Item(ItemId.INFERNAL_CAPE));
        } else {
            player.getDialogueManager().start(new NPCChat(player, 2180, "No Jal-Nib-Rek for you."));
        }
    }
