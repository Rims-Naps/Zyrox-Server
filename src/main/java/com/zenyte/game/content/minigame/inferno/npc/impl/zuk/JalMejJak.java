...
public class JalMejJak extends InfernoNPC {
    ...
    public JalMejJak(final Location location, final Inferno inferno, final TzKalZuk zuk) {
        super(NpcId.JALMEJJAK, location, inferno);
        this.zuk = zuk;
        setAoEArea();
    ...
    public int attack(final Entity target) {
        ...
        if (target.equals(zuk)) {
            ...
            World.scheduleProjectile(this, target, healingProjectile).schedule(() -> {
                zuk.setHealing(true);
                zuk.applyHit(new Hit(Utils.random(15, 25), HitType.HEALED));
                World.sendSoundEffect(target, hittingSound);
            });
        } else {
            val targetLocation = new Location(target.getLocation());
            val tiles = new ArrayList<Location>(3);
            var count = 1000;
            while (--count > 0) {
                val tile = getRandomPoint();
                if (!tiles.contains(tile)) {
                    val location = targetLocation.getDistance(getLocation()) <= 6 && Utils.random(5) == 0 && !tiles.contains(targetLocation) ? targetLocation : tile;
                    tiles.add(location);
                }
                if (tiles.size() == 3) {
                    break;
    ...
    private void setAoEArea() {
        val x = getX();
        val y = getY();
        val originalX = inferno.getStaticLocation(getLocation()).getX();
        switch (originalX) {
            case 2262:
