...
public class CatapultInterface extends Interface {
    ...
    private static final int ZAMORAK_X = 86, ZAMORAK_Y = 113, ZAMORAK_OFFSET_MULTIPLIER = 2;
    private static final int SARADOMIN_X = 193, SARADOMIN_Y = 222, SARADOMIN_OFFSET_MULTIPLIER = -2;
    ...
    protected void build() {
        ...
        bind("Fire", player -> {
            ...
            val destination = getDestination(player);
            ...
            WorldTasksManager.schedule(() -> {
                ...
                World.sendGraphics(new Graphics(287), destination);
    ...
    private void refreshDigits(@NotNull final Player player) {
        val horizontalClicks = getHorizontalClicks(player);
        val verticalClicks = getVerticalClicks(player);
        assert horizontalClicks >= 0 && horizontalClicks <= MAX_CLICKS;
        assert verticalClicks >= 0 && verticalClicks <= MAX_CLICKS;
        val horizontalCoordsString = Integer.toString(horizontalClicks);
        val verticalCoordsString = Integer.toString(verticalClicks);
        assert !horizontalCoordsString.isEmpty();
        assert !verticalCoordsString.isEmpty();
        val horizontalLength = horizontalCoordsString.length();
        val verticalLength = verticalCoordsString.length();
        val firstHorizontalDigit = horizontalLength == 1 ? 0 : Character.getNumericValue(horizontalCoordsString.charAt(0));
        val secondHorizontalDigit = Character.getNumericValue(horizontalCoordsString.charAt(horizontalLength - 1));
        val firstVerticalDigit = verticalLength == 1 ? 0 : Character.getNumericValue(verticalCoordsString.charAt(0));
        val secondVerticalDigit = Character.getNumericValue(verticalCoordsString.charAt(verticalLength - 1));
    ...
    private void refreshCross(@NotNull final Player player) {
        ...
        assert horizontalClicks >= 0 && horizontalClicks <= MAX_CLICKS;
        assert verticalClicks >= 0 && verticalClicks <= MAX_CLICKS;
        val startX = team == CastleWarsTeam.SARADOMIN ? SARADOMIN_X : ZAMORAK_X;
        val startY = team == CastleWarsTeam.SARADOMIN ? SARADOMIN_Y : ZAMORAK_Y;
        val multiplier = team == CastleWarsTeam.SARADOMIN ? SARADOMIN_OFFSET_MULTIPLIER : ZAMORAK_OFFSET_MULTIPLIER;
    ...
    private Location getDestination(@NotNull final Player player) {
        ...
        assert horizontalClicks >= 0 && horizontalClicks <= MAX_CLICKS;
        assert verticalClicks >= 0 && verticalClicks <= MAX_CLICKS;
        val startTile = team == CastleWarsTeam.SARADOMIN ? SARADOMIN_CATAPULT_LOWEST_LAND_COORDS : ZAMORAK_CATAPULT_LOWEST_LAND_COORDS;
        return new Location(startTile.getX() + (horizontalClicks / (team == CastleWarsTeam.SARADOMIN ? SARADOMIN_OFFSET_MULTIPLIER : ZAMORAK_OFFSET_MULTIPLIER)),
                startTile.getY() + (verticalClicks / (team != CastleWarsTeam.SARADOMIN ? SARADOMIN_OFFSET_MULTIPLIER : ZAMORAK_OFFSET_MULTIPLIER)), startTile.getPlane());
    }
}
