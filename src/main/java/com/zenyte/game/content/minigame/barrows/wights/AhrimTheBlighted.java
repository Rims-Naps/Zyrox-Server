...
public class AhrimTheBlighted extends BarrowsWightNPC implements Spawnable, CombatScript {
	...
	public int attack(final Entity target) {
		val spell = Utils.random(6) != 0 ? CombatSpell.FIRE_WAVE
				: Utils.getRandomElement(CombatSpell.CONFUSE, CombatSpell.WEAKEN, CombatSpell.CURSE);
		setAnimation(spell.getAnimation());
		setGraphics(spell.getCastGfx());
        this.delayHit(World.sendProjectile(this, target, spell.getProjectile()), target, new Hit(this,
                this.getRandomMaxHit(this, combatDefinitions.getMaxHit(), MAGIC, target), HitType.MAGIC).onLand(hit -> {
            if (hit.getDamage() <= 0) {
	...
	public boolean validate(final int id, final String name) {
		return id == NpcId.AHRIM_THE_BLIGHTED;
	}
