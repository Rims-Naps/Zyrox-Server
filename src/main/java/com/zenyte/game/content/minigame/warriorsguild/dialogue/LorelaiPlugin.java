    ...
    public void handle() {
        bind("Talk-to", (player, npc) -> player.getDialogueManager().start(new Dialogue(player, npc) {
            ...
            public void buildDialogue() {
                ...
                npc("I'm training this gorup of special Cyclopes to be "
                        + "stronger and more powerful for Kamfreena.");
                options();
                player(10, "What's special about these Cyclopes?");
                npc("They have a chance of dropping a new, more powerful "
                        + "defender. Kamfreena doesn't want just anyone getting "
                        + "their hands on one so she moved them down here and "
                        + "tasked me with training them.");
                options();
                player(30, "How exactly are you training them?");
                npc("With a punishing routine of combat, fighting and the "
                        + "odd death!");
                player("So, you just let them fight each other and hope they get "
                        + "stronger?");
                npc("Of course not, that would just be cruel! Kamfreena and "
                        + "I only allow the most worthy of adventurers to fight "
                        + "these cyclopes.");
                player("I am a mighty adventurer! Can I fight them?");

                val carryingRuneDefender = player.getInventory().containsItem(ItemId.RUNE_DEFENDER, 1) || player.getInventory().containsItem(ItemId.RUNE_DEFENDER_T, 1)
                        || player.getEquipment().getId(EquipmentSlot.SHIELD) == ItemId.RUNE_DEFENDER || player.getEquipment().getId(EquipmentSlot.SHIELD) == ItemId.RUNE_DEFENDER_T;
