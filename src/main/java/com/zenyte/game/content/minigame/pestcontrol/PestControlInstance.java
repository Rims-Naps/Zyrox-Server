 ...
 */
@Slf4j
public final class PestControlInstance extends DynamicArea
        implements DeathPlugin, CycleProcessPlugin, LogoutPlugin, CannonRestrictionPlugin {

    private static final int ACTIVITY_FLAG = 0x1;
    private static final int DAMAGE_FLAG = 0x2;

    public static final int KNIGHT_FLAG = 0x8;
    private static final int SHIELDS_FLAG = 0x4;
    private static final int TIME_FLAG = 0x10;
    ...
    public PestControlInstance(final PestControlGameType type, final Map<Player, PestControlStatistic> initialPlayers,
                               final AllocatedArea area) {
    ...
    public void addActivity(final Player player, final int amount) {
        val statistic = mappedPlayers.get(player);
        if (statistic == null) {
            return;
        }
    ...
    void addDamageDealt(final Player player, final PestNPC npc, final int damage) {
        ...
        statistic.incrementActivity(
                npc instanceof PestPortalNPC || npc instanceof SpinnerNPC ? HIGH_ACTIVITY_POINTS : MODERATE_ACTIVITY_POINTS);
    ...
    public void constructed() {
        ...
        for (val portal : PestPortal.VALUES) {
            portals.put(portal, new PestPortalNPC(this, portal));
        }

        for (val entry : mappedPlayers.entrySet()) {
    ...
    public final WorldObject destroyRavagableObject(WorldObject trackedObject) {
        if (trackedObject.getId() < 14233) {
            val id = trackedObject.getId() + 3;
            World.removeObject(trackedObject);
            World.spawnObject(trackedObject = new WorldObject(id, id >= 14230 ? 22 : trackedObject.getType(), trackedObject.getRotation(),
                    trackedObject));
        } else {
            val id = trackedObject.getId() + 4;
            World.removeObject(trackedObject);
            World.spawnObject(trackedObject = new WorldObject(id, id >= 14245 ? 22 : trackedObject.getType(), trackedObject.getRotation(),
                    trackedObject));
        }
    ...
    private void flagPlayer(final Player player, final int flag) {
        val mask = flagMap.getOrDefault(player, (byte) 0);
        if ((mask & flag) == 0) {
    ...
    private int getMinutesRemaining() {
        return (int) Math.ceil((GAME_DURATION - timer) / 100F);
    }

    public final List<WorldObject> getRavagableObjects() {
        val ravagableObjects = new ObjectArrayList<WorldObject>();

        for (val tile : barricadeTiles) {
        ...
        for (val tile : gateTiles) {
            val object = World.getObjectWithType(tile, 0);
            if (object != null) {
                if (ArrayUtils.contains(PestControlUtilities.VALID_RAVAGABLE_OBJECTS, object.getId())) {
                    ravagableObjects.add(object);
                }
    ...
    public void finish(final boolean successful) {
        ...
        WorldTasksManager.schedule(new WorldTask() {
            ...
            public void run() {
                ...
                for (val npc : monsters.toArray(new NPC[0])) {
                    npc.finish();
                }
                stop();
            }
    ...
    private void exportPlayer(final Player player, final boolean successful) {
        ...
        val activity = this.getActivity(player);
        WorldTasksManager.schedule(() -> {
            if (successful) {
                ...
                player.getDialogueManager().start(new NPCChat(player, type.getSquireId(),
                        "Congratulations! You managed to " + (timer == GAME_DURATION ? "protect the Void Knight" : "destroy all the portals") +
                                "! We've awarded you " + points + " Void Knight Commendation points. Please also accept these coins as a reward."));
            } else {
    ...
    public void process() {
        ...
        if (timer >= 10 && Utils.random(2) == 0) {
            val count = availablePortals.size();
        ...
        if (!flagMap.isEmpty()) {
            for (val entry : flagMap.object2ByteEntrySet()) {
                val player = entry.getKey();
                if (player.isNulled()) {
                    continue;
                }
                val mask = entry.getByteValue();
    ...
    private Location getRandomSpawnTile(final int npcId, final PestPortal portal) {
        ...
        val size = npcDefinitions.getSize();
        val baseTile = portal.getNpcSpawnTile();
        int attempts = 50;
        while (--attempts > 0) {
            val hash = getHash(Location.getHash(baseTile.getX() + Utils.random(3), baseTile.getY() + Utils.random(3), 0));
            val x = (hash >> 14) & 0x3FFF;
            val y = hash & 0x3FFF;
    ...
    private void spawn() {
        ...
        val type = PestNPCTable.VALUES[Utils.random(PestNPCTable.VALUES.length - 1)];
        val array = this.type == PestControlGameType.NOVICE ? type.getNoviceIds()
                : this.type == PestControlGameType.INTERMEDIATE ? type.getIntermediateIds() : type.getVeteranIds();
        val id = array[Utils.random(array.length - 1)];
        val portal = availablePortals.get(Utils.random(availablePortals.size() - 1));
        val tile = getRandomSpawnTile(id, portal);
        if (tile == null) {
            return;
        }
        val npcClass = type.getNpcClass();
        try {
    ...
    private void dropShield() {
        val index = timer == 25 ? 0 : (timer / 50);
        val portal = sequence.getPortals()[index];
        flag(SHIELDS_FLAG);
        val portalNPC = portals.get(portal);
