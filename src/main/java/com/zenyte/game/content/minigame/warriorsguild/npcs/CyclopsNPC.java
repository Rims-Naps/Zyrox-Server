...
public final class CyclopsNPC extends NPC implements Spawnable {
	...
	private final void checkDefender(final Entity source) {
		...
		if (Utils.random(24) != 0) {
			return;
		}
		final Player player = (Player) source;
		val currentDefender = ItemChain.DEFENDERS.getHighestCarrying(player);
		val nextDefender = getNextDefender(currentDefender);
	...
	private final int getNextDefender(final int currentDefender) {
		...
		if (currentDefender == ItemId.RUNE_DEFENDER_T || currentDefender == ItemId.RUNE_DEFENDER
				|| currentDefender == ItemId.DRAGON_DEFENDER || currentDefender == ItemId.DRAGON_DEFENDER_T) {
