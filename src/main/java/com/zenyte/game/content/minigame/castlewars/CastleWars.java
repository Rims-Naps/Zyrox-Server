...
public class CastleWars {
    ...
    @Getter @Setter private static boolean active;
    @Getter @Setter private static boolean timerStarted;
    ...
    private static final Item[] CASTLE_WARS_ITEMS = new Item[] {
            EXPLOSIVE_POTION, new Item(4049), CATAPULT_ROCK,
            new Item(954), new Item(590), TOOLKIT,
            new Item(4053), new Item(1925), new Item(1265),
            SARADOMIN_FLAG, ZAMORAK_FLAG, SARADOMIN_CLOAK, ZAMORAK_CLOAK
    };
    ...
    public static void preloadItemMap() {
        for(final ItemDefinitions defs : ItemDefinitions.definitions) {
            ...
            if(defs.getName().equalsIgnoreCase("castlewars hood") || defs.getName().equalsIgnoreCase("castlewars cloak") ||
                defs.getId() == SARADOMIN_FLAG.getId() || defs.getId() == ZAMORAK_FLAG.getId()) {
                continue;
            }
            ...
            if(!defs.getName().contains("rune")) {
                ...
                val nameArray = defs.getName().split(" ");
        ...
        allowedItems.add(12791); // rune pouch
    ...
    private static final void processBracelet(@NotNull final Player player) {
        val braceletId = player.getEquipment().getId(EquipmentSlot.HANDS);
        if (braceletId == ItemId.CASTLE_WARS_BRACELET3 || braceletId == ItemId.CASTLE_WARS_BRACELET2 || braceletId == ItemId.CASTLE_WARS_BRACELET1) {
            player.getEquipment().set(EquipmentSlot.HANDS, braceletId == ItemId.CASTLE_WARS_BRACELET1 ? null : braceletId == ItemId.CASTLE_WARS_BRACELET3 ? new Item(ItemId.CASTLE_WARS_BRACELET2) :
                    new Item(ItemId.CASTLE_WARS_BRACELET1));
    ...
    public static void processPlayerGameFinish(final Player player) {
        val tickets = getTicketAmount(getTeam(player));
        ...
        val saradomin = getTeam(player).equals(CastleWarsTeam.SARADOMIN);
        val saraScore = getVarbit(CastleWarsTeam.SARADOMIN, CWarsOverlayVarbit.SARADOMIN_SCORE);
        val zamScore = getVarbit(CastleWarsTeam.SARADOMIN, CWarsOverlayVarbit.ZAMORAK_SCORE);
    ...
    public static int getTicketAmount(final CastleWarsTeam team) {
        val saradomin = team.equals(CastleWarsTeam.SARADOMIN);
        val saraScore = getVarbit(team, CWarsOverlayVarbit.SARADOMIN_SCORE);
        val zamScore = getVarbit(team, CWarsOverlayVarbit.ZAMORAK_SCORE);
    ...
    public static void setVarbit(final CastleWarsTeam team, final CWarsOverlayVarbit k, final int v) {
        ...
        if(!k.isUniversal()) {
            val teamList = team.equals(CastleWarsTeam.SARADOMIN) ? SARADOMIN_TEAM : ZAMORAK_TEAM;
