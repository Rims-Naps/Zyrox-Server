...
public class Inferno extends DynamicArea implements CycleProcessPlugin, LogoutPlugin, DeathPlugin, CannonRestrictionPlugin, TeleportPlugin, DropPlugin {
    ...
    @Getter
    private WorldObject[] fallingRocks, wallPatches;
    private Location nibblerSpawnLocation;
    @Getter
    private Location[] tripleJadSpawnLocations, spawnLocations, revivalLocations, dragBlockTiles;
    @Getter
    ...
    @Getter
    private Location middle, singleJadSpawnLocation;
    private int targetIndex;
    private int cycle;
    private long duration;
    @Setter
    private boolean resuming, onAssignment;
    @Getter
    private boolean practiceMode;
    @Getter private final int minShieldY = getY(5356), maxShieldY = getY(5358);
    ...
    public Inferno(final Player player, final AllocatedArea area, final boolean practiceMode) {
        ...
        spawnLocations = new Location[]{
                middle.transform(3, 2, 0), middle.transform(-3, -2, 0),
                middle.transform(-3, -4, 0), middle.transform(5, -4, 0),
                middle.transform(7, 4, 0), middle.transform(-7, 6, 0)
        };
        revivalLocations = new Location[]{
                middle, getLocation(2274, 5339, 0), getLocation(2268, 5339, 0),
                getLocation(2274, 5347, 0), getLocation(2268, 5347, 0), getLocation(2273, 5345, 0)
        };
        nibblerSpawnLocation = getLocation(2266, 5345, 0);
        tripleJadSpawnLocations = new Location[]{
                getLocation(2274, 5346, 0),
                getLocation(2265, 5347, 0),
                getLocation(2268, 5335, 0)
        };
        dragBlockTiles = new Location[] {
                getLocation(2264, 5359, 0),
                getLocation(2278, 5359, 0)
        };
        wallPatches = new WorldObject[]{
                new WorldObject(30339, 10, 3, getLocation(2275, 5364, 0)),
                new WorldObject(30340, 10, 1, getLocation(2267, 5364, 0)),
                new WorldObject(30341, 10, 3, getLocation(2275, 5366, 0)),
                new WorldObject(30342, 10, 1, getLocation(2267, 5366, 0)),
        };
        fallingRocks = new WorldObject[]{
                new WorldObject(30343, 10, 3, getLocation(2273, 5364, 0)),
                new WorldObject(30344, 10, 3, getLocation(2268, 5364, 0))
        };
    ...
    public void constructed() {
        ...
        if (resuming) {
            ...
            player.putBooleanAttribute("jalimkotBurrowed", false);
            player.putBooleanAttribute("has_taken_hit_from_zuk_during_inferno", false);
            player.putBooleanAttribute("has_taken_damage_from_jad_during_inferno", false);
            player.putBooleanAttribute("jadOnWave69Died", false);
            player.putBooleanAttribute("hitInInfernoWasIceSpell", false);
            player.putBooleanAttribute("hpUnder10", false);
        ...
        val lastPosHash = player.getNumericAttribute("inferno_player_location").intValue();
        if (lastPosHash > 0) {
        ...
        if (!resuming) {
            ...
            val wave = player.getNumericAttribute("inferno_progress").intValue();
            if (wave > 0) {
                val duration = player.getNumericAttribute("inferno_duration").longValue();
            ...
            for (int i = 0; i < spawnLocations.length; i++) {
                val bitpacked = player.getNumericAttribute("inferno_npc_spawn_location_" + i).intValue();
                if (bitpacked == 0) {
                    continue;
                }
        ...
        if (wave.getWave() < InfernoWave.WAVE_67.getWave()) {
            for (val location : RockySupportLocation.values) {
                val hitpoints = player.getNumericAttribute("inferno_rocky_support_" + location).intValue();
                if (hitpoints >= 0) {
                    val support = new RockySupport(location, this);
                    rockySupports.add((RockySupport) support.spawn());
                    support.setHitpoints(hitpoints == 0 ? support.getMaxHitpoints() : hitpoints);
                }
            }
        }
        for (val tile : dragBlockTiles) {
    ...
    public static final void onLogin(final LoginEvent event) {
        ...
        val attribute = player.getAttributes().get("inferno_progress");
        if (attribute == null) {
            return;
        }
        player.lock();
        try {
            val practiceMode = player.getBooleanAttribute("inferno_practice_mode");
            val area = MapBuilder.findEmptyChunk(8, 8);
            val inferno = new Inferno(player, area, practiceMode);
    ...
    private final int calculateHealthOfAllSlaughteredMonsters() {
        int totalHealth = 0;
        try {
            for (val wave : InfernoWave.getValues()) {
                if (wave.ordinal() >= this.wave.ordinal()) {
                    break;
                }
                for (val entry : wave.getEntries()) {
    ...
    public void leave(final boolean completed) {
        player.lock(2);
        val amount = completed ? 16440 : (calculateHealthOfAllSlaughteredMonsters() / 2);
        val tokkul = new Item(ItemId.TOKKUL, amount);
        ...
        if (completed) {
            if (!practiceMode) {
                ...
                if (!player.getBooleanAttribute("master-combat-achievement33")
                        && player.getBooleanAttribute("nibblersBegone")) {
                    player.putBooleanAttribute("master-combat-achievement33", true);
                    MasterTasks.sendMasterCompletion(player, 33);
                }
                if (!player.getBooleanAttribute("grandmaster-combat-achievement6")
                        && !player.getBooleanAttribute("jalimkotBurrowed")) {
                    player.putBooleanAttribute("grandmaster-combat-achievement6", true);
                    GrandmasterTasks.sendGrandmasterCompletion(player, 6);
                }
                if (!player.getBooleanAttribute("grandmaster-combat-achievement10")
                        && !player.getBooleanAttribute("has_taken_damage_from_jad_during_inferno")
                        && !player.getBooleanAttribute("has_taken_hit_from_zuk_during_inferno")) {
                    player.putBooleanAttribute("grandmaster-combat-achievement10", true);
                    GrandmasterTasks.sendGrandmasterCompletion(player, 10);
                }
                if (!player.getBooleanAttribute("grandmaster-combat-achievement14")
                        && !player.getBooleanAttribute("jadOnWave69Died")) {
                    player.putBooleanAttribute("grandmaster-combat-achievement14", true);
                    GrandmasterTasks.sendGrandmasterCompletion(player, 14);
                }
                if (!player.getBooleanAttribute("grandmaster-combat-achievement16")
                        && !player.getBooleanAttribute("hitInInfernoWasIceSpell")) {
                    player.putBooleanAttribute("grandmaster-combat-achievement16", true);
                    GrandmasterTasks.sendGrandmasterCompletion(player, 16);
                }
                if (!player.getBooleanAttribute("grandmaster-combat-achievement17")
                        && !(player.getInventory().containsItem(ItemId.TWISTED_BOW) || player.getWeapon().getId() == ItemId.TWISTED_BOW)) {
                    player.putBooleanAttribute("grandmaster-combat-achievement17", true);
                    GrandmasterTasks.sendGrandmasterCompletion(player, 17);
                }
                if (!player.getBooleanAttribute("grandmaster-combat-achievement18")
                        && !player.getBooleanAttribute("hpUnder10")) {
                    player.putBooleanAttribute("grandmaster-combat-achievement18", true);
                    GrandmasterTasks.sendGrandmasterCompletion(player, 18);
                }
                if (player.getBossTimer().getBossTimer("Inferno") < 4200 && !player.getBooleanAttribute("grandmaster-combat-achievement29")) {
                    player.putBooleanAttribute("grandmaster-combat-achievement29", true);
                    GrandmasterTasks.sendGrandmasterCompletion(player, 29);
                }
            }
            for (val npc : npcs) {
            ...
            WorldTasksManager.schedule(() -> {
                if (!practiceMode) {
                    ...
                    BossPet.JAL_NIB_REK.roll(player, onAssignment ? 50 : 75);
                }
    ...
    private void removeSupports() {
        if (wave.equals(InfernoWave.WAVE_66)) {
            int count = 0;
            for (val support : rockySupports) {
                if (support.getHitpoints() > 0 || !support.isDead()) {
                    count++;
                }
            }
            if (count == 3) {
                player.putBooleanAttribute("nibblersBegone", true);
            } else {
                player.putBooleanAttribute("nibblersBegone", false);
            }
        }
        for (val support : rockySupports) {
    ...
    private void executeWave(final InfernoWave wave) {
        if(!(wave.getWave() == 1 && (boolean) player.getTemporaryAttributes().getOrDefault("inferno_wave_32_skip", false))) {
        ...
        setNibblerTarget();
        val entries = wave.getEntries();
        val tiles = new Int2IntOpenHashMap(6);
        for (int index = 0; index < entries.length; index++) {
            val entry = entries[index];
            val amount = entry.getCount();
            for (int count = 0; count < amount; count++) {
                ...
                try {
                    ...
                    if (n.equals(WaveNPC.JAL_NIB)) {
                        ...
                        val x = nibblerSpawnLocation.getX() - 1;
                        val y = nibblerSpawnLocation.getY() - 1;
                        for (int i = 0; i < 100; i++) {
                    ...
                    val npc = entry.getNpc().getClazz().getDeclaredConstructor(Location.class, Inferno.class).newInstance(location, this);
                    npc.spawn();
        ...
        appendInfernoSettings();
        if(wave.getWave() == 1 && (boolean) player.getTemporaryAttributes().getOrDefault("inferno_wave_32_skip", false)) {
            skip(InfernoWave.WAVE_32);
            player.getTemporaryAttributes().remove("inferno_wave_32_skip");
        }
    }

    public void setNibblerTarget() {
        val supports = new ArrayList<RockySupport>(3);
