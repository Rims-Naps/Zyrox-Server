...
public abstract class FightCavesNPC extends NPC {
    ...
    public enum TzHaarNPC {
        ...
        YT_MEJ_KOT(YtMejKot.class, 3123),
        KET_ZEK(KetZek.class, 3125),//Sound effects: Strike: 598(magic)
        TZ_TOK_JAD(TzTokJad.class, 3127),

        /* Secondary monsters encountered, needs to be separated from the primary. */
        TZ_KIH_ALT(TzKih.class, 3117),
        TZ_KEK_ALT(TzKek.class, 3119),
        TZ_KEK_SPLIT(SplitTzKek.class, 3120),
        TOK_XIL_ALT(TokXil.class, 3122),
        YT_MEJ_KOT_ALT(YtMejKot.class, 3124),
        KET_ZEK_ALT(KetZek.class, 3126),
        YT_HUR_KOT(YtHurKot.class, 3128);//Sound effects: Strike: 608, block: 610, death: 609
        ...
        static {
            for (val tzhaar : allValues) {
                map.put(tzhaar.id, tzhaar);
            }
        }

        private static final TzHaarNPC[] values = new TzHaarNPC[] {
                TZ_KIH, TZ_KEK, TOK_XIL, YT_MEJ_KOT, KET_ZEK, TZ_TOK_JAD
        };
        ...
        private static final Class<?>[] arguments = new Class<?>[] {
                TzHaarNPC.class, Location.class, FightCaves.class
        };

        public static boolean isAlternative(final NPC npc) {
            val constant = map.get(npc.getId());
        ...
        public static void populate(final List<TzHaarNPC> list, final int wave) {
            float remaining = wave, multiplier;
            while (remaining >= 1) {
                TzHaarNPC npc;
    ...
    public void handleIngoingHit(final Hit hit) {
        ...
        if (source instanceof Player) {
            ...
            if (!player.getBooleanAttribute("elite-combat-achievement58")) {
                val type = hit.getHitType();
                val weapon = player.getWeapon();
                if (!(type == HitType.MELEE || type == HitType.REGULAR || (type == HitType.MISSED && player.getBooleanAttribute("used_special")))) {
                    player.putBooleanAttribute("hitInFightcavesWasNotMelee", true);
                }
            }
        }
