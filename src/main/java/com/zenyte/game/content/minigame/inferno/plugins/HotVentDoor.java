...
public final class HotVentDoor implements ObjectAction {
	...
	public void handleObjectAction(final Player player, final WorldObject object, final String name, final int optionId, final String option) {
		val proven = player.getNumericAttribute("infernoVar").intValue() >= 1;
		if (!proven) {
			player.getDialogueManager().start(new Dialogue(player, TzHaar.TZHAAR_KET) {

				@Override
				public void buildDialogue() {
					val npcs = CharacterLoop.find(player.getLocation(), 10, NPC.class, n -> n.getId() == TzHaar.TZHAAR_KET);
					val npc = npcs.get(0);
					if (npc != null) {
					...
					if (player.getInventory().containsItem(TzHaar.FIRE_CAPE, 1) || player.getInventory().containsItem(MaxCape.FIRE.getCape(), 1)) {
						val item = player.getInventory().containsItem(TzHaar.FIRE_CAPE_ITEM) ? TzHaar.FIRE_CAPE_ITEM : new Item(MaxCape.FIRE.getCape());
