...
public class JalImKot extends InfernoNPC {
    ...
    public JalImKot(final Location location, final Inferno inferno) {
        super(NpcId.JALIMKOT, location, inferno);
    }
    ...
    public void processNPC() {
        if (!isDead() && Utils.currentTimeMillis() - getAttackingDelay() >= TimeUnit.TICKS.toMillis(10) && !hasWalkSteps()) {
            ...
            player.putBooleanAttribute("jalimkotBurrowed", true);
            burrowing = true;
            lock(9);
            setAnimation(burrowStartAnimation);
            setAttackingDelay(Utils.currentTimeMillis() + TimeUnit.TICKS.toMillis(25));
            val burrowLocation = getBurrowLocation();
    ...
    private Location getBurrowLocation() {
        val player = inferno.getPlayer();
        val optionalLocation = Utils.findEmptySquare(player.getLocation(), player.getSize() + getSize(), getSize(),
                Optional.of(l -> CombatUtilities.isWithinMeleeDistance(l, getSize(), player)));
