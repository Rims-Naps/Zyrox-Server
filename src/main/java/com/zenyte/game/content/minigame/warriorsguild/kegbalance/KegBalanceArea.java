...
public final class KegBalanceArea extends WarriorsGuildArea implements CycleProcessPlugin {
    ...
    public static final boolean pickKeg(final Player player, final WorldObject object, final int kegCount) {
        if (player.getWeapon() != null || player.getShield() != null
                || player.getGloves() != null || kegCount == 0 && player.getHelmet() != null) {
    ...
    public final void loseBalance(final Player player) {
        val kegCount = player.getNumericTemporaryAttribute("WG Kegs Count").intValue();
        if (kegCount <= 0) {
            return;
        }
        val ticks = player.getNumericTemporaryAttribute("WG Kegs Ticks").intValue();
    ...
    public void process() {
        ...
        for (val player : players) {
            val kegs = player.getNumericTemporaryAttribute("WG Kegs Count").intValue();
            if (kegs > 0) {
                val ticks = player.getNumericTemporaryAttribute("WG Kegs Ticks").intValue();
