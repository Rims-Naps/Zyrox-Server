...
public class JalAk extends InfernoNPC {
    ...
    public static final Map<Direction, byte[][]> bloblingOffsets = ImmutableMap.<Direction, byte[][]>builder()
            .put(Direction.NORTH, new byte[][]{
                    new byte[]{0, 1}, new byte[]{-1, 0}, new byte[]{1, 0}
            })
            .put(Direction.NORTH_EAST, new byte[][]{
                    new byte[]{1, -1}, new byte[]{-1, 1}, new byte[]{1, 1}
            })
            .put(Direction.EAST, new byte[][]{
                    new byte[]{0, 0}, new byte[]{1, 1}, new byte[]{1, -1}
            })
            .put(Direction.NORTH_WEST, new byte[][]{
                    new byte[]{-1, -1}, new byte[]{-1, 1}, new byte[]{1, 1}
            })
            .put(Direction.SOUTH, new byte[][]{
                    new byte[]{0, -1}, new byte[]{-1, 0}, new byte[]{1, 0}
            })
            .put(Direction.SOUTH_WEST, new byte[][]{
                    new byte[]{-1, -1}, new byte[]{-1, 1}, new byte[]{1, 1}
            })
            .put(Direction.WEST, new byte[][]{
                    new byte[]{-1, 0}, new byte[]{0, 1}, new byte[]{0, -1}
            })
            .put(Direction.SOUTH_EAST, new byte[][]{
                    new byte[]{-1, -1}, new byte[]{1, -1}, new byte[]{1, 1}
            })
            .build();

    private static final WaveNPC[] bloblings = {
            WaveNPC.JAL_AKREK_MEJ, WaveNPC.JAL_AKREK_XIL, WaveNPC.JAL_AKREK_KET
    };

    public JalAk(final Location location, final Inferno inferno) {
        super(NpcId.JALAK, location, inferno);
    ...
    private void switchStyle(@NotNull final Player player) {
        val prayer = player.getPrayerManager();
    ...
    public void onFinish(Entity source) {
        ...
        val direction = getRoundedDirection(Utils.getFaceDirection(source.getX() - middle.getX(), source.getY() - middle.getY()), 1024);
        val directionConstant = Utils.findMatching(Direction.values, dir -> dir.getNPCDirection() == direction);
        assert directionConstant != null;
        val offsets = bloblingOffsets.get(directionConstant);
        try {
            for (int index = 0; index < bloblings.length; index++) {
                val offset = offsets[index];
                val blobling = bloblings[index].getClazz().getDeclaredConstructor(Location.class, Inferno.class).newInstance(middle.transform(offset[0], offset[1], 0), inferno);
                blobling.freeze(1);
                blobling.spawn();
