...
public class JalMejRah extends InfernoNPC {
    ...
    private static final int[] drainableStats = {
     ...
     */
    private static final StatType[] statTypes = new StatType[] {
        StatType.ATTACK, StatType.DEFENCE, StatType.STRENGTH, StatType.RANGED, StatType.MAGIC, null
    };

    public JalMejRah(final Location location, final Inferno inferno) {
        super(NpcId.JALMEJRAH, location, inferno);
        setAttackDistance(3);
    }
        ...
        WorldTasksManager.schedule(() -> {
            if (sapCount < 10 && Utils.random(9 + sapCount) == 0) {
                ...
                for (int i = 0; i < drainableStats.length; i++) {
                    val skill = drainableStats[i];
                    if (skill == Skills.HITPOINTS) {
                        val drained = Math.min(sapCount, 4);
                        ...
                        val currentLevel = player.getSkills().getLevel(skill);
                        player.getSkills().setLevel(skill, Math.max(0, currentLevel - 1));
                        val statType = statTypes[i];
                        if (statType == null) {
                            continue;
                        }
