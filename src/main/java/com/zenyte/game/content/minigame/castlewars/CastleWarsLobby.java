 ...
 */
public class CastleWarsLobby extends Area implements LoginPlugin, CannonRestrictionPlugin, RandomEventRestrictionPlugin, EquipmentPlugin, CycleProcessPlugin {

    private boolean started;

    @Getter @Setter private static int saradominInvites;
    @Getter @Setter private static int zamorakInvites;
    ...
    @Getter @Setter private static int ticks;
    ...
    public void dispatchLateInvites(final Set<Player> lobby) {
        val team = lobby.equals(SARADOMIN_LOBBY) ? CastleWarsTeam.SARADOMIN : CastleWarsTeam.ZAMORAK;
    ...
    public void enter(final Player player) {
       ...
       if(started) {
           val timer = Math.round( (int) Math.ceil(ticks / 100) );
