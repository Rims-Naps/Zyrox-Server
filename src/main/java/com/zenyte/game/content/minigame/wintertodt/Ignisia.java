...
public class Ignisia extends NPCPlugin {
    ...
    public void handle() {
        bind("Trade-in", (player, npc) -> {
            ...
            for (val entry : tradeableItems.int2ObjectEntrySet()) {
            ...
            player.getDialogueManager().start(new Dialogue(player, npc) {
                ...
                public void buildDialogue() {
                    ...
                    options("Select an Option",
                            new DialogueOption("Trade in " + tradeableItems.get(finalItemToTrade.getId()) + ".", key(10)),
                            new DialogueOption("I've changed my mind.", key(15)));
                    
                    options(10, "Exchange your " + finalItemToTrade.getName() + "?",
                            new DialogueOption("Yes.", key(25)),
                            new DialogueOption("No.")).onOptionOne(() -> {
    ...
    private void tradeItem(final Player player, final Item itemToTrade) {
        val crate = new Item(RewardCrate.SUPPLY_CRATE);
        var rolls = 1;
