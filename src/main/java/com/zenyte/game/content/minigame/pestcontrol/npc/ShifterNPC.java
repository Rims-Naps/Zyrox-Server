...
public final class ShifterNPC extends PestNPC {
	...
	private void teleport() {
		val platform = Utils.random(2) == 0;
		int attempts = 50;
		while (--attempts > 0) {
			val tile = instance.getLocation(platform ? PestControlUtilities.getRandomPlatformLocation() : PestControlUtilities.getRandomShifterLocation());
			if (tile.withinDistance(getLocation(), 3)) {
				continue;
			}
			if (World.isTileFree(tile, getSize())) {
				val currentTile = new Location(getX(), getY() - 1, getPlane());
	...
	private final class ShifterCombat extends NPCCombat {
		...
		public int combatAttack() {
			...
			if (Utils.collides(npc.getX(), npc.getY(), size, target.getX(), target.getY(), target.getSize())) {
				return 0;
			}
		...
		public boolean checkAll() {
			...
			if (npc.isDead() || npc.isFinished() || npc.isLocked() || target.isDead() || target.isFinished()
			...
			if (!target.isMultiArea()) {
				...
				if (attackedBy != null && attackedBy != npc && !attackedBy.isDead() && !attackedBy.isFinished()
			...
			if (!target.hasWalkSteps() && Utils.collides(npc.getX(), npc.getY(), size, target.getX(), target.getY(), targetSize)) {
			...
			npc.resetWalkSteps();
			if (npc.isProjectileClipped(target, maxDistance == 0)
					|| (!Utils.isOnRange(npc.getX(), npc.getY(), size, target.getX(), target.getY(), targetSize, maxDistance))) {
				if (!npc.calcFollow(target, npc.isRun() ? 2 : 1, true, npc.isIntelligent(), npc.isEntityClipped()) && getCombatDelay() < 3
						&& melee) {
					setCombatDelay(3);
				}
				return true;
