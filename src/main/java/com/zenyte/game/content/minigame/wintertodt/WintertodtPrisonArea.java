...
public class WintertodtPrisonArea extends WintertodtCampArea implements CannonRestrictionPlugin, LayableTrapRestrictionPlugin, DropPlugin, RandomEventRestrictionPlugin, LogoutPlugin, LoginPlugin, CycleProcessPlugin {
    ...
    private static void startAOEAttack(final Player player) {
        val aoeMiddleLocation = getAOEAttackMiddleLocation(player);
        if (aoeMiddleLocation == null) {
            return;
        }
        
        val middleLocation = new Location(aoeMiddleLocation.getX(), aoeMiddleLocation.getY());
        
        val subLocations = getSubLocations(middleLocation);
        for (Location subLocation : subLocations) {
            aoeAttackLocations.add(subLocation.getPositionHash());
        }
    
        val cornerLocations = new HashSet<Location>(Direction.interCardinalDirections.length);
        ...
        WorldTasksManager.schedule(() -> {
            ...
            for (val p : CharacterLoop.find(middleLocation, 1, Player.class, p -> !Wintertodt.insideSafeArea(p))) {
                val damageAmount = 3 * Wintertodt.getBaseAOEDamageAmount(p);
    ...
    private static Location getAOEAttackMiddleLocation(final Player player) {
        var middleLocation = player.getMiddleLocation();
        var subLocations = getSubLocations(middleLocation);
    ...
    private static HashSet<Location> getSubLocations(final Location middleLocation) {
        val subLocations = new HashSet<Location>(9);
    ...
    public void process() {
        ...
        for (val player : players) {
            if (Wintertodt.insideSafeArea(player)) { // if player is standing inside the cold-free safe area
                continue;
            }
        ...
        if (!Wintertodt.atFullEnergy()) {
            for (Wintertodt.Corner corner : Wintertodt.Corner.VALUES) {
                val brazier = corner.getBrazier();
                val pyromancer = corner.getPyromancer();
    
                spawnBrazierProjectile(brazier, pyromancer);
                brazierAttack(brazier);
                pyromancerAttack(pyromancer);
                if (!pyromancer.isHealthy()) {
                    Wintertodt.failPyromancerTask();
                }
            }
        }
        if (Wintertodt.getNumberOfBraziersBroken() == 4 && !Wintertodt.atFullEnergy() && !Wintertodt.betweenRounds()) {
            Wintertodt.failFourBrazierTask();
        }
    }
    ...
    private void spawnBrazierProjectile(final Brazier brazier, final Pyromancer pyromancer) {
        ...
        if (brazier.getTicks() < 2) { // only spawn every other tick
            return;
        } else {
            brazier.setTicks(0);
        }
        val pointingDirection = pyromancer.getSpawnDirection();
        WorldTasksManager.schedule(() -> {
            val fromlocation = new Location(brazier.getX(), brazier.getY()).transform(1, 1, 0).transform(pointingDirection, -1);
            val toLocation = fromlocation.transform(pointingDirection, 9);
            
            val npc = World.spawnNPC(new NPC(7373, fromlocation, pointingDirection, 0));
    ...
    private void randomColdDamage(final Player player) {
        ...
        var braziersLit = Wintertodt.getNumberOfBraziersLit();
        val brazierModifier = braziersLit >= 3 ? 3 : 0;
        
        val isBelowFiftyPercentEnergy = Wintertodt.getCurrentEnergyPercentage() <= 50;
        val chanceToHit = 35 + (braziersLit * brazierModifier) + (isBelowFiftyPercentEnergy ? 20 : 0);
        
        if (Utils.random(0, chanceToHit) == 0) {
            braziersLit = Math.min(Wintertodt.getNumberOfBraziersLit(), 3);
            val randomHitAmount = ((16 - Wintertodt.getNumberOfWarmClothingWorn(player) - (2 * braziersLit)) * (player.getMaxHitpoints() + 1))
                                          / player.getSkills().getLevelForXp(Skills.FIREMAKING);
    ...
    private void brazierAttack(final Brazier brazier) {
        ...
        val braziersLit = Wintertodt.getNumberOfBraziersLit();
        val isBelowFiftyPercentEnergy = Wintertodt.getCurrentEnergyPercentage() <= 50;
    ...
    private void areaAttack(final Player player) {
        val isBelowFiftyPercentEnergy = Wintertodt.getCurrentEnergyPercentage() <= 50;
        val playerMultiplier = (int) Math.min(Math.pow(Wintertodt.getAmountOfPlayers(), 1.5), 250);
