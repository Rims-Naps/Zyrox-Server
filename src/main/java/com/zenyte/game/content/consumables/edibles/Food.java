...
public enum Food implements Edible {
    ...
    JANGERBERRIES(247, 0) {
        ...
        public Boost[] boosts() {
            return new Boost[] {
                    ...
                    new Boost(Skills.STRENGTH, 0, 1),
                    new Restoration(Skills.PRAYER, 0, 1),
                    new Debuff(Skills.DEFENCE, 0, 1)
            };
        }
    },
	SHRIMP(315, 3),
    ...
    KEBAB(1971, 0) {
        ...
        public void onConsumption(final Player player) {
            ...
            if (roll <= 66) {
                ...
                healAmount = (int) (player.getSkills().getLevelForXp(Skills.HITPOINTS) * 0.10);
            } else if (roll <= 87) {
    ...
    HALF_OF_A_MEAT_PIE(2331, 6, new Item(2313)),
    ...
    STACKABLE_PURPLE_SWEETS(10476, 0) {
        ...
        public boolean canConsume(Player player) {
            if (player.getArea() instanceof Inferno) {
                player.sendMessage("The heat of the inferno causes the wrapper to stick to the sweet. You cannot see any way to open it.");
                return false;
            }
            return super.canConsume(player);
        }
    },
    STACKABLE_HALLOWEEN_SWEETS(24565, 0) {
        @Override
        public int healedAmount(final Player player) {
            return Utils.random(1, 3);
        }
    ...
    GOLOVANOVA_FRUIT(13426, 0) {
        @Override
        public boolean canConsume(Player player) {
            player.sendMessage("The scent doesn't appeal to you.");
            return false;
        }
    },//TODO: Find out amount
    BOLOGANO_FRUIT(13427, 0) {
        @Override
        public boolean canConsume(Player player) {
                player.sendMessage("The scent doesn't appeal to you.");
                return false;
