 ...
 */
public class Aviansie extends KillcountNPC implements Spawnable, CombatScript {

    private static final int[] javelinAviansie = new int[] {
            3163, 3164, 3182, 3179, 3171, 3167, 3181, 3178, 3180, 3175, 3170, 3173, 3172
    };
    ...
    public int attack(final Entity target) {
        ...
        if (ArrayUtils.contains(javelinAviansie, getId())) {
            ...
            delayHit(npc, World.sendProjectile(npc, target, npc.getId() == 3168 ? magic : javelin), target, new Hit(npc, getRandomMaxHit(npc, npc.getCombatDefinitions().getMaxHit(), npc.getId() == 3168 ?
                                                                                                                MAGIC :
                                                                                                                RANGED, target), npc.getId() == 3168 ?
                                                                                                                                 HitType.MAGIC :
                                                                                                                                 HitType.RANGED));
        } else {
            npc.setAnimation(npc.getCombatDefinitions().getAttackAnim());
