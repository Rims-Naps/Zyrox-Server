...
public class GodwarsAltarObject implements ObjectAction {
    ...
    private static final Location[] ALTAR_TELEPORT_LOCS = new Location[] {
            new Location(2925, 5333, 2),
            new Location(2909, 5265, 0),
            new Location(2839, 5294, 2),
            new Location(2862, 5354, 2)
    };

    @Override
    public void handleObjectAction(Player player, WorldObject object, String name, int optionId, String option) {
        if (option.equals("Pray")) {
            val altar = object.getName().split(" ")[0] + "AltarDelay";
            ...
            if (player.getArea() instanceof GodwarsInstance) {
                val altar = Utils.findMatching(GodwarsInstancePortal.values(), portalConstant -> portalConstant.getAltarId() == object.getId());
                Preconditions.checkArgument(altar != null);
                val instance = (GodwarsInstance) player.getArea();
                player.lock(1);
                player.setLocation(instance.getLocation(altar.getAltarTeleportLocation()));
                player.blockIncomingHits();
                return;
            }
            val location = ALTAR_TELEPORT_LOCS[object.getId() - 26363];
