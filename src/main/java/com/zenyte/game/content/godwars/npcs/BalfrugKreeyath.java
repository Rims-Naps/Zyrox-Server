...
public class BalfrugKreeyath extends GodwarsBossMinion implements Spawnable, CombatScript {
    ...
    public int attack(final Entity target) {
        ...
        val preciseDelay = projectile.getProjectileDuration(npc.getMiddleLocation(), target);
        World.sendSoundEffect(new Location(target.getLocation()), hitSound.withDelay(preciseDelay));
        delayHit(npc,  World.sendProjectile(npc, target, projectile), target,
                new Hit(npc, getRandomMaxHit(npc, npc.getCombatDefinitions().getMaxHit(), MAGIC, target), HitType.MAGIC).onLand(hit -> target.setGraphics(hit.getDamage() > 0 ? targetGraphics : splashGraphics)));
