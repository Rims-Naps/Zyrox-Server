...
public class SaradominSteppingStone implements Shortcut {
    ...
    public Location getRouteEvent(final Player player, final WorldObject object) {
        val hasWesternStone = World.getObjectWithType(object.transform(Direction.WEST, 2), 10) != null;
        val hasEasternStone = World.getObjectWithType(object.transform(Direction.EAST, 2), 10) != null;
    ...
    public void startSuccess(Player player, WorldObject object) {
        ...
        val western = isWesternStone(object);
        if (western) {
            val tile = object.transform(Direction.WEST, 2);
        ...
        WorldTasksManager.scheduleOrExecute(() -> {
            player.faceObject(object);
            val direction = object.getX() > player.getX() ? Direction.EAST : Direction.WEST;
            for (int i = 0; i < 4; i++) {
