 ...
 */
public class KaruulmSlayerDungeon extends Area implements CycleProcessPlugin {
    ...
    public static boolean isProtectedAgainstHeat(final Player player) {
        ...
        if (DiaryReward.RADAS_BLESSING4.eligibleFor(player)) {
            return true;
        }
        val boots = player.getBoots();
        val id = boots == null ? -1 : boots.getId();
