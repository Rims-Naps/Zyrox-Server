...
public class Hydra extends NPC implements CombatScript, Spawnable {
    ...
    private int attacks, ticks, processTicks;
    ...
    public int attack(Entity target) {
        if (poisonAttack) {
            ...
            val hydraCenter = getMiddleLocation();
            setAnimation(poisonAttackAnim);
            for (val tile : poisonTiles) {
                World.sendProjectile(this, tile, poisonAttackProj);
                val direction = getRoundedDirection(Utils.getFaceDirection(tile.getX() - hydraCenter.getX(), tile.getY() - hydraCenter.getY()), 1024);
