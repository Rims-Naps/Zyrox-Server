...
public interface HydraPhaseSequence {
    ...
    static final Projectile magicAttackProj = new Projectile(1662, 22, 12, 30, 10, 30, 0, 5);
    static final Projectile rangedAttackProj = new Projectile(1663, 42, 32, 30, 2, 15, 0, 5);
    ...
    void handleIngoingHit(final AlchemicalHydra hydra, final Hit hit);

    int getHitpointsThreshold(final AlchemicalHydra hydra);
    ...
    default void applyPoison(final AlchemicalHydra hydra, final Player player) {
        ...
        CombatUtilities.delayHit(hydra, -1, player, new Hit(hydra, Utils.random(5, 12), HitType.POISON).onLand(hit -> player.getToxins().applyToxin(Toxins.ToxinType.POISON, 6)));
    }
}
