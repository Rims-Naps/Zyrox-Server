...
public class DrakeProcessor extends DropProcessor {
    ...
    public void attach() {
        ...
        put(8612, ItemId.DRAKES_TOOTH, new DropProcessor.PredicatedDrop("This drop becomes 5 times more common during a slayer assignment."));
        put(8612, ItemId.DRAKES_CLAW, new DropProcessor.PredicatedDrop("This drop becomes 5 times more common during a slayer assignment."));
        put(8612, ItemId.DRAGON_THROWNAXE, new DropProcessor.PredicatedDrop("This drop becomes 5 times more common during a slayer assignment."));
        put(8612, ItemId.DRAGON_KNIFE, new DropProcessor.PredicatedDrop("This drop becomes 5 times more common during a slayer assignment."));
    }

    @Override
    public Item drop(final NPC npc, final Player killer, final Drop drop, final Item item) {
        if (!drop.isAlways()) {
            var rate = killer.getSlayer().isCurrentAssignment(npc) ? 384 : 1920;
            if (random(rate) == 0) {
