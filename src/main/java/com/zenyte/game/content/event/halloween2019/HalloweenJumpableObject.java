...
public class HalloweenJumpableObject implements ObjectAction {
    ...
    public void handleObjectAction(Player player, WorldObject object, String name, int optionId, String option) {
        if (option.equalsIgnoreCase("Jump-over")) {
            val dir = player.getX() < object.getX() ? Direction.EAST : player.getX() > object.getX() ? Direction.WEST : player.getY() > object.getY() ? Direction.SOUTH : Direction.NORTH;
            val destination = player.getLocation().transform(dir, 2);
    ...
    public Object[] getObjects() {
        return new Object[] {
                HalloweenObject.STRANGE_FLOOR.getRepackedObject(), HalloweenObject.PILE_OF_RUBBLE.getRepackedObject()
        };
    }
}
