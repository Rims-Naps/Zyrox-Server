...
public class FutureScourgeCutscene extends Cutscene {
    ...
    private static final List<GameInterface> closedTabs = Arrays.asList(COMBAT_TAB, SKILLS_TAB, JOURNAL_HEADER_TAB, INVENTORY_TAB,
            EQUIPMENT_TAB, PRAYER_TAB_INTERFACE, SPELLBOOK, SETTINGS, EMOTE_TAB, GAME_NOTICEBOARD);
    private Emote lastSelectedEmote;

    @Override
    public void build() {
        val impName = ChristmasUtils.getImpName(player);
        addActions(0, player::lock, () -> closedTabs.forEach(tab -> player.getInterfaceHandler().closeInterface(tab)),
                () -> player.getInterfaceHandler().openGameTab(GameTab.FRIENDS_TAB));
        ...
        addActions(4, new CameraPositionAction(player, instance.getLocation(2471, 5407, 1), 350, -128, 0),
                new CameraLookAction(player, instance.getLocation(2471, 5390, 1), 200, -128, 0), () -> {
        ...
        addActions(8, new CameraPositionAction(player, instance.getLocation(2471, 5407 - 3, 1), 350, 5, 0),
                new CameraLookAction(player, instance.getLocation(2471, 5390 - 3, 1), 200, 5, 0));

        addActions(12, () -> World.sendObjectAnimation(new WorldObject(46180, 10, 2, instance.getLocation(2470, 5398, 1)), new Animation(15001)),
                () -> dummy.setInvalidAnimation(new Animation(15001)));

        addActions(21, new CameraPositionAction(player, instance.getLocation(2471, 5400, 1), 550, 255, 100),
                new CameraLookAction(player, instance.getLocation(2471, 5403, 1), 75, 255, 100),
                () -> {
        ...
        addActions(30, new CameraPositionAction(player, instance.getLocation(2475 + 1, 5404 + 1, 1), 500, 255, 100),
                new CameraLookAction(player, instance.getLocation(2465, 5396, 1), 0, 255, 100));
        ...
        addActions(34, new DialogueAction(new Dialogue(player) {
            @Override
            public void buildDialogue() {
                npcWithId(ChristmasConstants.PAJAMAS_SCOURGE_NPC_ID, "Well, if you don't wish to speak, allow me. We've had the Ghost of Christmas Past " +
                        "and the Ghost of Christmas Present - that much leave the Ghost of Christmas Yet to Come. Am I correct?", Expression.HIGH_REV_MAD);
                ...
                npcWithId(ChristmasConstants.PAJAMAS_SCOURGE_NPC_ID, "Well, what have you come to show me, ghost? More " +
                        "spoilt brats? Another tale of lost love?", Expression.HIGH_REV_MAD);
        ...
        addActions(101, new CameraPositionAction(player, instance.getLocation(2475 + 1, 5404 + 1, 1), 500, 255, 100),
                new CameraLookAction(player, instance.getLocation(2465, 5396, 1), 0, 255, 100), new DialogueAction(new Dialogue(player) {
                    @Override
                    public void buildDialogue() {
        ...
        addActions(201, new CameraPositionAction(player, instance.getLocation(2475 + 1, 5404 + 1, 1), 500, 255, 100),
                new CameraLookAction(player, instance.getLocation(2465, 5396, 1), 0, 255, 100), new DialogueAction(new Dialogue(player) {
                    @Override
                    public void buildDialogue() {
        ...
        val object = new WorldObject(46226, 10, 3, instance.getLocation(2469, 5401, 1));
        addActions(303, () -> {
        ...
        addActions(316, new CameraPositionAction(player, instance.getLocation(2472, 5401, 1), 220, 255, 100),
                new CameraLookAction(player, instance.getLocation(2467, 5402, 1), 25, 255, 100));

        addActions(319, new CameraPositionAction(player, instance.getLocation(2475 + 1, 5404 + 1, 1), 500, 255, 100),
                new CameraLookAction(player, instance.getLocation(2465, 5396, 1), 0, 255, 100));
        ...
        val pLoc = instance.getLocation(2464, 5401, 0);
        val iLoc = instance.getLocation(2465, 5401, 0);
        addActions(325, () -> {
        ...
        },  new CameraPositionAction(player, instance.getLocation(2464, 5406, 0), 550, 255, 100),
                new CameraLookAction(player, instance.getLocation(2464, 5400, 0), 75, 255, 100));
        ...
        val fs = new FadeScreen(player);
        addActions(331, fs::fade);
        addActions(334, player::unlock, new CameraResetAction(player), () -> player.getVarManager().sendBit(4606, 0), () -> closedTabs.forEach(tab -> tab.open(player)),
                () -> player.setLocation(new Location(2098, 5405, 0)), () -> player.getPacketDispatcher().sendClientScript(10700, 0));
    ...
    private void executeOptions(final int startTime, final Emote[] options, final Emote correctOption, final int jumpStage) {
        overrideActions(startTime, new DialogueAction(new Dialogue(player) {
            @Override
            public void buildDialogue() {
                val optionsList = new ObjectArrayList<DialogueOption>();
        ...
        }), new CameraPositionAction(player, instance.getLocation(2475 + 1, 5404 + 1, 1), 500, 255, 100),
                new CameraLookAction(player, instance.getLocation(2465, 5396, 1), 0, 255, 100));
        for (int i = 0; i < 3; i++) {
            overrideActions(startTime + 1 + (i * 10), getEmoteRunnables(options[i]));
            val duration = options[i].getAnimation().getDuration() / 600;
    ...
    private final Runnable[] getEmoteRunnables(@NotNull final Emote emote) {
        return new Runnable[] {
                new CameraPositionAction(player, instance.getLocation(2471, 5400, 1), 550, 255, 100),
                new CameraLookAction(player, instance.getLocation(2471, 5403, 1), 75, 255, 100),
                () -> Emote.play(player, emote.name().toLowerCase().replace(" ", "_"))
        };
    }
