...
public class HalloweenWeb implements ObjectAction, ItemOnObjectAction {
    ...
    public static final Predicate<String> sharpBladePredicate = name -> name.contains("scythe") || name.contains("sword") || name.contains("dagger")
            || name.contains("scimitar") || name.contains("axe") || name.equals("knife") || name.contains("whip") || name.contains("bludgeon") || name.contains("tentacle");
    ...
    public static final void slash(@NotNull final Player player, @NotNull final WorldObject web, @NotNull final Item weapon) {
        ...
        WorldTasksManager.scheduleOrExecute(() -> {
            if (player.matches(web)) {
                val dest = web.transform(web.getRotation() == 1 ? Direction.NORTH : web.getRotation() == 0 ? Direction.WEST : web.getRotation() == 2 ? Direction.EAST : Direction.SOUTH, 1);
            ...
            val random = weapon.getName().contains("wilderness sword") ? 0 : Utils.random(2);
            if (weapon.getId() != 946) {
                val defs = weapon.getDefinitions();
                player.getAppearance().setRenderAnimation(new RenderAnimation(defs.getStandAnimation(), defs.getStandTurnAnimation(), defs.getWalkAnimation(),
