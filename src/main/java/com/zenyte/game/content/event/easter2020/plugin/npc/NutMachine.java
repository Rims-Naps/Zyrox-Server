...
public class NutMachine extends NPCPlugin {
    ...
    public void handle() {
        bind("Look-at", new OptionHandler() {
                ...
                if (SplittingHeirs.getProgress(player) == Stage.RETRAIN_SQUIRRELS) {
                    ...
                    player.getDialogueManager().start(new Dialogue(player, EasterConstants.BIG_BEN) {
                        ...
                        public void buildDialogue() {
                            val amountOfGold = player.getBank().getAmountOf(ItemId.COINS_995);
                            val goldString = Utils.format(amountOfGold);
                            ...
                            options(TITLE,
                                    new DialogueOption("Hmm, okay, but only 40% of them.", key(FOURTY)),
                                    new DialogueOption("Hmm, okay, but only 20% of them.", key(TWENTY)),
                                    new DialogueOption("Hmm, okay, but only 5% of them.", key(FIVE)),
                                    new DialogueOption("Bye then.", this::finish)
                            );
                            ...
                            
                            {
                                ...
                                npc("When yer ready.", Expression.EASTER_BUNNY_NORMAL)
                                        .executeAction(() -> {
            ...
            public void click(final Player player, final NPC npc, final NPCOption option) {
                val object = World.getObjectWithType(nutMachine, 10);
    ...
    public int[] getNPCs() {
        return new int[]{
                EasterConstants.NUT_MACHINE, EasterConstants.NUT_MACHINE_BASE, EasterConstants.NUT_MACHINE_START, EasterConstants.NUT_MACHINE_WORKING
        };
    }
}