...
public class HalloweenSteppingStone implements ObjectAction {
    ...
    private static final Location[] permittedLocations = new Location[] {
            new Location(1748, 4709, 0),
            new Location(1749, 4710, 0),
            new Location(1750, 4709, 0),
            new Location(1751, 4709, 0),
            new Location(1752, 4710, 0)
    };
    ...
    public void handleObjectAction(Player player, WorldObject object, String name, int optionId, String option) {
        if (option.equalsIgnoreCase("Cross")) {
            val dir = player.getX() < object.getX() && player.getY() < object.getY() ? Direction.NORTH_EAST :
                    player.getX() < object.getX() && player.getY() > object.getY() ? Direction.SOUTH_EAST :
                            player.getX() > object.getX() && player.getY() < object.getY() ? Direction.NORTH_WEST :
                    player.getX() > object.getX() && player.getY() > object.getY() ? Direction.SOUTH_WEST :

                    player.getX() < object.getX() ? Direction.EAST : player.getX() > object.getX() ? Direction.WEST : player.getY() > object.getY() ? Direction.SOUTH : Direction.NORTH;
            val destination = player.getLocation().transform(dir, 1);
    ...
    public Object[] getObjects() {
        return new Object[] {
                HalloweenObject.STEPPING_STONES.getRepackedObject()
        };
    }
}