...
public class EasterBird extends NPCPlugin implements ItemOnNPCAction {
    ...
    public void handle() {
        bind("Talk-to", (player, npc) -> player.getDialogueManager().start(new Dialogue(player, npc) {
            ...
            public void buildDialogue() {
                ...
                val requiredFood = SeedBowl.getRequiredBowl(player);
                ...
                if (!SplittingHeirs.progressedAtLeast(player, Stage.FEED_BIRD)) {
                    ...
                    options(TITLE,
                            new DialogueOption("Blow a raspberry at the bird.", () -> {
                    ...
                    if (!Varbit.EMPTY_BOWL.exists(player)) {
                        ...
                        npc("Squawk! Aye, aye, cap'n!", Expression.EASTER_BIRD_HAPPY)
                                .executeAction(() -> SplittingHeirs.advanceStage(player, Stage.BIRD_FED_SPEAK_TO_BUNNY_JR));
    ...
    public Object[] getItems() {
        val food = SeedBowl.Contents.byFoodItemId.keySet().stream().filter(i -> i != -1).toArray();
