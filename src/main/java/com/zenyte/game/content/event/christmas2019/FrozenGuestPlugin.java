...
public class FrozenGuestPlugin implements ObjectAction, ItemOnObjectAction {
    ...
    private final void throwWater(@NotNull final Player player, @NotNull final WorldObject object, @NotNull final OptionalInt bucketSlot) {
        val guest = Utils.findMatching(FrozenGuest.getValues(), g -> g.getBaseObject() == object.getId());
        Preconditions.checkArgument(guest != null);
        val previousHash = player.getNumericAttribute(UNFROZEN_GUESTS_HASH_KEY).intValue();
        val hash = previousHash | (1 << guest.ordinal());
        ...
        val npc = World.findNPC(guest.getBaseNPC(), guest.getTile(), 1).orElseThrow(RuntimeException::new);
        ...
        WorldTasksManager.schedule(() -> {
            ...
            player.getDialogueManager().start(new Dialogue(player) {
                ...
                public void buildDialogue() {
                    ...
                    npcWithId(ChristmasConstants.PERSONAL_SNOW_IMP, impName, "We's been wasting our time digging up Scourge's past." +
                            " Past is way too long ago. Everyone knows misers have rubbish memory! This weeping boy, he's from presents.", Expression.HIGH_REV_NORMAL);
                    player("Bu-");
                    npcWithId(ChristmasConstants.PERSONAL_SNOW_IMP, impName, "If there is anyone who can melt the frozen heartstrings " +
                            "of that crankerous ol' miser, it's this angelic boy. We can use this boy to show Scourge how he's hurting people in the presents!", Expression.HIGH_REV_NORMAL);
    ...
    public Object[] getItems() {
        return new Object[] {
                ChristmasConstants.ICY_WATER_BUCKET
        };
    }
    ...
    public Object[] getObjects() {
        ...
        for (val guest : FrozenGuest.getValues()) {
            val baseObject = guest.getBaseObject();
            set.add(baseObject);
        }
        return set.toArray();
    }
