    ...
    public void build() {
        ...
        addActions(0, () -> player.setLocation(instance.getLocation(2079, 5404, 0)), () -> player.getAppearance().setInvisible(true), () -> player.getVarManager().sendBit(4606, 1),
                () -> {
                    ...
                    for (val guest : FrozenGuest.getValues()) {
                        val npc = new NPC(guest.getCutsceneBaseNPC(), instance.getLocation(guest.getTile()), guest.getDirection(), 0);
                        npc.spawn();
                        frozenGuestsMap.put(guest, npc);
                        player.getVarManager().sendBit(ObjectDefinitions.getOrThrow(guest.getBaseObject()).getVarbitId(), 0);
                    }
                });
        addActions(1, new CameraPositionAction(player, instance.getLocation(2079, 5398, 0), 1600, -128, 0),
                new CameraLookAction(player, instance.getLocation(2079, 5401, 0), 200, -128, 0));
        ...
        for (int i = 0; i < length; i++) {
            ...
            addActions(16 + (i * 2), () -> {
                ...
                val tile = instance.getLocation(guest.getTile());
