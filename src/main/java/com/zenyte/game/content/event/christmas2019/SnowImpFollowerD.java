...
public class SnowImpFollowerD extends Dialogue {
    ...
    public void buildDialogue() {
        val progress = AChristmasWarble.getProgress(player);
        val impName = ChristmasUtils.getImpName(player);
        if (progress == null) {
            return;
        }
        
        val options = new ArrayList<DialogueOption>();
        
        switch (progress) {
            case FROZEN_GUESTS:
                npc(impName, "Quick! We need to help the Queen's mates at the feastie!", Expression.HIGH_REV_SHOCKED);
                options(TITLE,
                        new DialogueOption("What do we need to do?", key(20)),
                        new DialogueOption("How can I put out the fires?", key(40)),
                        new DialogueOption("Actually, never mind.", this::finish)
                );
            ...
            case CHRISTMAS_PRESENT_FAILED:
                ...
                npc(impName, "We lies! Don't worry, leave it to me. Just get back dem stairs.", Expression.HIGH_REV_NORMAL)
                        .executeAction(() -> AChristmasWarble.progress(player, AChristmasWarble.ChristmasWarbleProgress.GHOST_OF_CHRISTMAS_FUTURE));
                return;
        ...
        }
        {
            ...
            options(TITLE,
                    new DialogueOption("I've got time for it.", key(QUEEN_OF_SNOW_AND_SANTA + 5)),
                    new DialogueOption("Actually, never mind.", this::finish)
            );
