...
public class RedCarpetCutscene extends Cutscene {
    ...
    public void build() {
        ...
        addActions(4, new CameraPositionAction(player, instance.getLocation(2199, 4396, 0), 250, -128, 0),
                new CameraLookAction(player, instance.getLocation(2204, 4396, 0), 200, -128, 0), () -> {
        ...
        addActions(13, player::unlock, new CameraResetAction(player), () -> player.getVarManager().sendBit(4606, 0), () -> closedTabs.forEach(tab -> tab.open(player)),
                () -> player.setLocation(new Location(2208, 4394, 0)), () -> player.getPacketDispatcher().sendClientScript(10700, 0),
                () -> player.getPacketDispatcher().sendMinimapState(MinimapState.ENABLED),
                () -> SplittingHeirs.advanceStage(player, Stage.POST_MEETING_CUTSCENE));
