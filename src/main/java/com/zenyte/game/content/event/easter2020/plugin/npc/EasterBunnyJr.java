...
public class EasterBunnyJr extends NPCPlugin {
            ...
            public void handle(Player player, NPC npc) {
                ...
                player.getDialogueManager().start(new Dialogue(player, EasterConstants.EASTER_BUNNY_JR) {
                    ...
                    public void buildDialogue() {
                        val progress = SplittingHeirs.getProgress(player);
                        if (progress == Stage.BIRD_FED_SPEAK_TO_BUNNY_JR) {
                            ...
                            options(TITLE,
                                    new DialogueOption("It's a goose that lays eggs made of gold.", key(GOLD_GOOSE)),
                                    new DialogueOption("It's a highly intelligent life form, which provides profit through chocolate.", key(INTELLIGENT_LIFEFORM)),
                                    new DialogueOption("It's just a dumb parrot that has a chocolatey gift.", key(DUMB_PARROT)),
                                    new DialogueOption("I give up!", key(GIVE_UP))
                            );
                            ...
                            
                            {
                                ...
                                npc("Dad keeps nets and jars in his office for when they go running off, it's quite tiresome. Apparently he catches them in the net then stuffs them in a jar. Anyway, I'm going to take a nap.", Expression.EASTER_BUNNY_NORMAL)
                                        .executeAction(() -> SplittingHeirs.advanceStage(player, Stage.GATHER_IMPLINGS));
                            ...
                            options(TITLE,
                                    new DialogueOption("...but not just painted, the'yre hand-painted by master artists.", key(MASTER_ARTIST)),
                                    new DialogueOption("An impling painting an egg twice its own size is pretty cool.", key(TWICE_ITS_SIZE)),
                                    new DialogueOption("I give up!", key(GIVE_UP))
                            );
    ...
    
                            {
                                ...
                                player("Eggsellent!");
                                npc("This has quite tired me out, I'm going to go sit down. Oh, and see what you can do about that silly impling over at that broken machine, it keeps bugging me about some missing parts.", Expression.EASTER_BUNNY_NORMAL)
                                        .executeAction(() -> SplittingHeirs.advanceStage(player, Stage.FIX_INCUBATOR));
...

                            {
                                ...
                                player("Vermin?");
                                npc("Yes, the ones over on that machine with the fluffy tails.", Expression.EASTER_BUNNY_NORMAL)
                                        .executeAction(() -> SplittingHeirs.advanceStage(player, Stage.RETRAIN_SQUIRRELS));
                            ...
                            npc("Hmm...you summed it up in a nutshell. Meet me in dad's office, I've made a decision.", Expression.EASTER_BUNNY_NORMAL)
                                    .executeAction(() -> SplittingHeirs.advanceStage(player, Stage.MEET_ME_IN_MY_OFFICE));
                            player("I wish he wouldn't do that!");
                            ...
                            npc("Yes indeedy! I just need to...", Expression.EASTER_BUNNY_VERY_HAPPY).executeAction(() -> {
                                try {
                                    val area = MapBuilder.findEmptyChunk(3, 3);
                                    val instance = new RedCarpetInstance(area);
                            ...
                            options(TITLE,
                                    new DialogueOption("I think that's very lazy of you.", key(LAZY_OF_YOU)),
                                    new DialogueOption("You should clean this place up.", key(CLEAN_THIS_PLACE)),
                                    new DialogueOption("Why aren't you fixing the factory?", key(WHY_ARENT_YOU_FIXING)),
                                    new DialogueOption("Can't hang around here all day, I'm off.", key(CANT_HANG_AROUND_ALL_DAY))
                            );
    ...
    
                            {
                                ...
                                npc("Oh, I wouldn't know the first thing about where to start, but I know my dad has a big to-do list in his office.", Expression.EASTER_BUNNY_NORMAL)
                                        .executeAction(() -> SplittingHeirs.advanceStage(player, Stage.CHECK_TODO));
                                options(TITLE,
                                        new DialogueOption("I think that's very lazy of you.", key(LAZY_OF_YOU)),
                                        new DialogueOption("You should clean this place up.", key(CLEAN_THIS_PLACE)),
                                        new DialogueOption("Can't hang around here all day, I'm off.", key(CANT_HANG_AROUND_ALL_DAY))
    ...
    public int[] getNPCs() {
        return new int[]{
                EasterConstants.EASTER_BUNNY_JR,
                EasterConstants.EASTER_BUNNY_JR_CHAIR,
                EasterConstants.EASTER_BUNNY_JR_NEXT_TO_BIRD,
                EasterConstants.EASTER_BUNNY_JR_NEXT_TO_COAL,
                EasterConstants.EASTER_BUNNY_JR_NEXT_TO_EGGPAINTING_MACHINE,
                EasterConstants.EASTER_BUNNY_JR_NEXT_TO_NUT_MACHINE,
                EasterConstants.EASTER_BUNNY_JR_ON_RED_CARPET,
                EasterConstants.EASTER_BUNNY_JR_NEXT_TO_BIRD_VIS,
                EasterConstants.EASTER_BUNNY_CUTSCENE
        };
    }
}
