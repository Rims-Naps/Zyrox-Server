...
public class ChristmasConstants {
    ...
    public static final String[] SNOW_IMP_NAMES = {
            "Harry",
            "Stuart",
            "Risto",
            "Luke",
    ...
    public static final String[] FROZEN_GUEST_ORDERS = {
            "twbacp",
            "twbpac",
            "tabcwp",
            "tpcbaw",
    ...
    public static final void refreshAllVarbits(@NotNull final Player player) {
        val imp = ChristmasUtils.getImp(player);
        if (imp != null) {
        ...
        if (AChristmasWarble.progressedAtLeast(player, AChristmasWarble.ChristmasWarbleProgress.GHOST_OF_CHRISTMAS_PRESENT)) {
            ...
            val unfrozenGuestsHash = player.getNumericAttribute(UNFROZEN_GUESTS_HASH_KEY).intValue();
            val characters = ChristmasUtils.getFrozenGuestOrder(player);
            for (val c : characters.toCharArray()) {
                val guest = Utils.findMatching(FrozenGuest.getValues(), g -> g.getConstant() == c);
                Preconditions.checkArgument(guest != null);
                player.getVarManager().sendBit(ObjectDefinitions.getOrThrow(guest.getBaseObject()).getVarbitId(), ((unfrozenGuestsHash >> guest.ordinal()) & 0x1) == 1 ? 2 : 3);
            }
        }
