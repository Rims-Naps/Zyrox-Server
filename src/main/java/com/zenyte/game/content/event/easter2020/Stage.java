...
public enum Stage {
    ...
    public void sendVarbit(final Player player) {
        if (varbitValue != -1) {
            val currentValue = player.getVarManager().getBitValue(EasterConstants.GENERAL_EVENT_VARBIT);
            player.getVarManager().sendBit(EasterConstants.GENERAL_EVENT_VARBIT, varbitValue);
            val position = mappedPositions.get(currentValue);
            if (position != null) {
                player.getPacketDispatcher().sendGraphics(poof, position);
            }
            val newPosition = mappedPositions.get(varbitValue);
            if (newPosition != null) {
