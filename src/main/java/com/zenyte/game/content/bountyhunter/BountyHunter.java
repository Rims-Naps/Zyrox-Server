...
public class BountyHunter {
    ...
    public static final void onInit(final InitializationEvent event) {
        ...
        val savedBounty = savedPlayer.getBountyHunter();
    ...
    @Subscribe
    public static final void onDeath(final PlayerDeathEvent event) {
        val player = event.getPlayer();
        val source = event.getSource();
        val bh = player.getBountyHunter();
        val target = bh.target;
        if (source == null) {
            bh.reset();
     ...
     */
    private static final void pair(@NotNull final Player player, @NotNull final Player target) {
        availablePlayers.remove(player);
     ...
     */
    private static final void setTargetIndividually(@NotNull final Player player) {
        val bh = player.getBountyHunter();
        val target = Objects.requireNonNull(bh.target);
     ...
     * @param player the player whose information to update.
     */
    private static final void requestTargetInformationUpdate(@NotNull final Player player) {
    ...
    public void searchTarget() {
        ...
        val combat = player.getSkills().getCombatLevel();
        for (val target : availablePlayers) {
            if (target == player) {
                continue;
            }
            val targetCombat = target.getSkills().getCombatLevel();
    ...
    public void abandonTarget() {
        ...
        val count = getLastSkipsCount();
        player.sendMessage(Colour.RED.wrap("You have abandoned your target "
                + (count == 1 ?
                   "once" :
                   (count + " times")) + " in the last 30 minutes."));
        requestTargetInformationUpdate(player);
        requestTargetInformationUpdate(target);
    }
