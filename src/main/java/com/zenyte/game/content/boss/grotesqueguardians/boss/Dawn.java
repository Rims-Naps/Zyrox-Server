...
public class Dawn extends NPC implements CombatScript {
    ...
    private static final byte[][] OFFSETS = new byte[][] {
            new byte[] { -1, -1 }, new byte[] { -1, 0 }, new byte[] { -1, 1 },
    ...
    private long sphereAttackDelay, freezeAttackDelay;
    @Getter
    ...
    public int attack(Entity target) {
        ...
        if (sphereAttack && sphereAttackDelay <= Utils.currentTimeMillis()) {
            ...
            val sphereLocations = new ArrayList<Location>(3);
            var count = 1000;
            while (--count > 0 && sphereLocations.size() != 3) {
                val location = instance.getRandomPoint();
            ...
            for (int index = 0; index < sphereLocations.size(); index++) {
                val sphere = new EnergySphere(index, sphereLocations.get(index), player, this, instance);
        ...
        if (attacks >= 3 && freezeAttackDelay <= Utils.currentTimeMillis()) {
            val tile = getFreezeLocation(player);
    ...
    public void sendDeath() {
        ...
        if ((attr != null && (boolean) attr) || (getHitpoints() <= 9 && isUnlocked && (source.getInventory().containsItem(Gargoyle.ROCK_HAMMER) || source.getInventory().containsItem(Gargoyle.GRANITE_HAMMER) || source.getInventory().containsItem(21754, 1)))) {
            if (isUnlocked) {
                val routeEvent = new EntityEvent(source, new DistancedEntityStrategy(this, 1), () -> deathSequence(source), true);
            ...
            public void run() {
                switch (ticks++) {
                    case 1:
                        ...
                        for (val corner : getCorners(getMiddleLocation())) {
                            World.sendGraphics(FLAME_CORNER_GFX, corner);
                        }
                        for (val corner : getCorners(dusk.getMiddleLocation())) {
                            World.sendGraphics(FLAME_CORNER_GFX, corner);
                        }
                        if (!instance.getEnergySpheres().isEmpty()) {
                            for (val entry : instance.getEnergySpheres().int2ObjectEntrySet()) {
    ...
    private Location getFreezeLocation(final Player player) {
        ...
        for (val offset : OFFSETS) {
            val location = base.transform(offset[0], offset[1], 0);
