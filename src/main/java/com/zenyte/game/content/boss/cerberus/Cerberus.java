...
public final class Cerberus extends NPC implements CombatScript, Spawnable {
    ...
    private boolean tookLavaDamage;
    private boolean spawnedSouls;
     ...
     */
    private static final int COMBAT_CERBERUS = NpcId.CERBERUS, SLEEPING_CERBERUS = NpcId.CERBERUS_5863;
     ...
     * Do not access the variable directly, instead call {@link Cerberus#getLair()} for type-safety.
     */
    private CerberusLair _area;
    /**
    ...
    public Cerberus(int id, Location tile, Direction facing, int radius) {
        ...
        this.cerberusRoom = Utils.findMatching(CerberusRoom.getValues(), value -> tile == null || value.getRegionId() == tile.getRegionId());
        if (tile != null) {
            if (this.cerberusRoom == null) {
        ...
        this.combat = new NPCCombat(this) {
            ...
            protected boolean checkAll() {
                return super.checkAll() && getLair().getPlayers().contains(target);
            }
        };
    ...
    @Override
    public NPC spawn() {
        this._area = GlobalAreaManager.getArea(cerberusRoom.getClazz());
        this.attackCount = 0;
    ...
    private void processSleepingCerberus() {
        ...
        if (location.matches(respawnTile)) {
            ...
            val possibleTargets = getPossibleTargets(targetType);
            if (!possibleTargets.isEmpty()) {
                setAnimation(leaningFromSleep);
                forcedDelay += 2;
                WorldTasksManager.schedule(() -> {
                    val possibleRemainingTargets = new ObjectArrayList<>(getLair().getPlayers());
                    ...
                    val target = possibleRemainingTargets.get(Utils.random(possibleRemainingTargets.size() - 1));
    ...
    private void processFightingCerberus() {
        ...
        if (getLair().getPlayers().isEmpty() || (combat.getTarget() == null && getPossibleTargets(targetType).isEmpty())) {
            combat.reset();
            attackCount = 0;
            setAnimation(Animation.STOP);
    ...
    public void onFinish(@NotNull final Entity source) {
        ...
        player.getBossTimer().finishTracking("Cerberus");
        if (player.getBossTimer().getBossTimer("Cerberus") < 40 && !player.getBooleanAttribute("master-combat-achievement53")) {
            player.putBooleanAttribute("master-combat-achievement53", true);
            MasterTasks.sendMasterCompletion(player, 53);
        }
        if (player.getBossTimer().getBossTimer("Cerberus") < 50 && !player.getBooleanAttribute("elite-combat-achievement71")) {
            player.putBooleanAttribute("elite-combat-achievement71", true);
            EliteTasks.sendEliteCompletion(player, 71);
        }
        if (player.getNumericAttribute("succesful_summoned_soul_protections").intValue() >= 6 && !player.getBooleanAttribute("elite-combat-achievement23")) {
            player.putBooleanAttribute("elite-combat-achievement23", true);
            EliteTasks.sendEliteCompletion(player, 23);
        }
        player.getAttributes().remove("succesful_summoned_soul_protections");
        if (!player.getBooleanAttribute("has_taken_melee_damage_in_cerb") && !player.getBooleanAttribute("elite-combat-achievement41")) {
            player.putBooleanAttribute("elite-combat-achievement41", true);
            EliteTasks.sendEliteCompletion(player, 41);
        }
        if (!tookLavaDamage && !player.getBooleanAttribute("elite-combat-achievement42")) {
            player.putBooleanAttribute("elite-combat-achievement42", true);
            EliteTasks.sendEliteCompletion(player, 42);
        }
        tookLavaDamage = false;
        if (!spawnedSouls && !player.getBooleanAttribute("master-combat-achievement14")) {
            player.putBooleanAttribute("master-combat-achievement14", true);
            MasterTasks.sendMasterCompletion(player, 14);
        }
        spawnedSouls = false;
    }
    ...
    private void checkMarkerCollisions(@NotNull final ImmutableLocation marker) {
        for (val player : getLair().getPlayers()) {
            val distance = player.getLocation().getTileDistance(marker);
    ...
    private void applyMarkerDamage(@NotNull final Entity target, final boolean standingOntop) {
        delayHit(-1, target, new Hit(Utils.random(standingOntop ? MARKER_DIRECT_MIN_DAMAGE : MARKER_DISTANCE_MIN_DAMAGE,
                standingOntop ? MARKER_DIRECT_MAX_DAMAGE : MARKER_DISTANCE_MAX_DAMAGE), HitType.DEFAULT));
        tookLavaDamage = true;
    }
    ...
    private void spawnFlames() {
        val westernFire = cerberusRoom.getWesternFireLocation();
        val lair = GlobalAreaManager.getArea(CerberusLair.class);
        val players = getLair().getPlayers();
        for (val p : players) {
            p.getBossTimer().startTracking("Cerberus");
            p.getAttributes().remove("succesful_summoned_soul_protections");
            p.putBooleanAttribute("has_taken_melee_damage_in_cerb", false);
        }
        tookLavaDamage = false;
        spawnedSouls = false;
        for (int i = 0; i < FLAMES_COUNT; i++) {
            val fireLocation = westernFire.transform(Direction.EAST, i);
            val existingFire = World.getObjectWithType(fireLocation, 10);
            ...
            for (val player : lair.getPlayers()) {
    ...
    public int attack(Entity target) {
        ...
        val possibleStyles = new ObjectArrayList<>(possibleOrderedAttackTypes);
        if (!isWithinMeleeDistance(this, target)) {
            possibleStyles.remove(STAB);
        }
        val style = Utils.getRandomCollectionElement(possibleStyles);
    ...
    private void executeSummonedSoulsAttack(@NotNull final Entity target) {
        ...
        val randomizedSoulIds = new IntArrayList(new int[]{NpcId.SUMMONED_SOUL, NpcId.SUMMONED_SOUL_5868, NpcId.SUMMONED_SOUL_5869});
        for (int i = randomizedSoulIds.size() - 1; i >= 0; i--) {
            val soulId = randomizedSoulIds.removeInt(Utils.random(randomizedSoulIds.size() - 1));
            val soul = new SummonedSoul(soulId, cerberusRoom.getWesternSoulLocation().transform(Direction.EAST, i), this, target, i);
            soul.spawn();
    ...
    private void executeComboAttack(@NotNull final Entity target) {
        val leftHeadFirst = Utils.random(1) == 0;
    ...
    private void executeLavaPoolsAttack(@NotNull final Entity target) {
        ...
        val leftHeadPosition = getProjectileStartPosition(target, CerberusHead.LEFT);
        val rightHeadPosition = getProjectileStartPosition(target, CerberusHead.RIGHT);
        val leftHeadTargetedPosition = getProjectileStartPosition(target, CerberusHead.LEFT, SIZE_OFFSET_LAVA_POOL);
        val rightHeadTargetedPosition = getProjectileStartPosition(target, CerberusHead.RIGHT, SIZE_OFFSET_LAVA_POOL);
        val leftHeadDestination = translateRandomReachablePosition(leftHeadTargetedPosition);
        ...
        val rightHeadDestination = translateRandomReachablePosition(rightHeadTargetedPosition);
    ...
    private Location translateRandomReachablePosition(@NotNull final Location location) {
        ...
        val possibleLocationsList = new ObjectArrayList<Location>((int) Math.pow(((MARKER_RANDOMIZATION_DISTANCE << 1) + 1), 2));
        for (int x = -MARKER_RANDOMIZATION_DISTANCE; x <= MARKER_RANDOMIZATION_DISTANCE; x++) {
            for (int y = -MARKER_RANDOMIZATION_DISTANCE; y <= MARKER_RANDOMIZATION_DISTANCE; y++) {
                val transformedLocation = location.transform(x, y, 0);
                if (!isProjectileClipped(transformedLocation, true) && Utils.findMatching(markers, marker -> marker.matches(transformedLocation)) == null) {
    ...
    private void sendLavaPool(@NotNull final Location headLocation, @NotNull final Location tile) {
        val delay = World.sendProjectile(headLocation, tile, markerProj);
        val preciseDelay = markerProj.getProjectileDuration(headLocation, tile);
        val marker = new ImmutableLocation(tile);
        markers.add(marker);
    ...
    private void executeAttack(@NotNull final Entity target, @NotNull final AttackType style, @NotNull final CerberusHead head) {
        ...
        val headLocation = getProjectileStartPosition(target, head);
        if (style.isRanged()) {
            val delay = World.sendProjectile(headLocation, target, rangedProj);
            val preciseDelay = rangedProj.getProjectileDuration(headLocation, target.getLocation());
            ...
            val destinationLocation = new Location(target.getLocation());
            val delay = World.sendProjectile(headLocation, target, magicProj);
            val preciseDelay = magicProj.getProjectileDuration(headLocation, destinationLocation);
    ...
    boolean isCancelled(@NotNull final Entity target) {
        return target.isFinished() || target.isNulled() || target.isDead() || !getLair().getPlayers().contains(target);
    }
