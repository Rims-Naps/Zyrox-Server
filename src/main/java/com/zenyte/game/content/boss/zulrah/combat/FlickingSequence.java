	...
	public void attack(final ZulrahNPC zulrah, final ZulrahInstance instance, final Player target) {
		...
		WorldTasksManager.schedule(new WorldTask() {
			...
			public void run() {
				...
				val mage = ((amount + (startingStyle == ZulrahNPC.MAGIC ? 1 : 0)) & 0x1) != 0;
				val hit = new Hit(zulrah, CombatUtilities.getRandomMaxHit(zulrah, 41, mage ? CombatScript.MAGIC : CombatScript.RANGED, target), mage ? HitType.MAGIC : HitType.RANGED);
