...
public class KolodionNPC extends NPCPlugin implements ItemOnNPCAction {
    ...
    public void handle() {
        bind("Talk-to", (player, npc) -> player.getDialogueManager().start(new Dialogue(player, npc) {
            ...
            public void buildDialogue() {
                npc("Hello.");
                options(TITLE, new DialogueOption("Are there any challenges available?", key(5)), new DialogueOption("What have I brought you so far?", key(500)), new DialogueOption(
                        "Cancel."));
                player(500, "What have I brought you so far?");
                val handCount = player.getNumericAttribute("handed in " + ItemDefinitions.getOrThrow(21797).getName() + " count").intValue();
                val entCount = player.getNumericAttribute("handed in " + ItemDefinitions.getOrThrow(21798).getName() + " count").intValue();
                val demonCount = player.getNumericAttribute("handed in " + ItemDefinitions.getOrThrow(21799).getName() + " count").intValue();

                npc("You've currently brought me " +
                        "" + handCount + " x Justiciar's hand, " +
                        "" + demonCount + " x Demon's heart, " +
                        "" + entCount + " x Ent's roots.");
                ...
                npc("I am one of the most powerful mages in existence. But even my power has limitations." +
                        " There are some brings however that have power exceeding even my own.");
                options(TITLE, new DialogueOption("Great, I've been waiting for an improvement!", key(100)),
                        new DialogueOption("Actually, my current cape is fine.", key(25)));
                ...
                npc("I will need you to kill these three beings and bring me their remains." +
                        " Even in death, their power should be great enough for me to harness it for our own use.");
    ...
    public void handleItemOnNPCAction(final Player player, final Item item, final int slot, final NPC npc) {
        ...
        val entCount = player.getNumericAttribute("handed in " + ItemDefinitions.getOrThrow(21798).getName() + " count").intValue();
        val demonCount = player.getNumericAttribute("handed in " + ItemDefinitions.getOrThrow(21799).getName() + " count").intValue();
        val completedOnce = player.getNumericAttribute("claimed one mage arena 2 cape").intValue() != 0;
        val idToBringIfCompletedOnce = id == ItemId.SARADOMIN_CAPE ? ItemId.JUSTICIARS_HAND : id == ItemId.GUTHIX_CAPE ? ItemId.ENTS_ROOTS : ItemId.DEMONS_HEART;
        val ifCompletedOnceAmount = player.getNumericAttribute("handed in " + ItemDefinitions.getOrThrow(idToBringIfCompletedOnce).getName() + " count").intValue();
        if (completedOnce ? (ifCompletedOnceAmount == 0) : (handCount < 1 || entCount < 1 || demonCount < 1)) {
            player.getDialogueManager().start(new Dialogue(player, npc) {
                @Override
                public void buildDialogue() {
                    if (completedOnce) {
                        npc("I'll need 1 x " + ItemDefinitions.getOrThrow(idToBringIfCompletedOnce).getName() + " to imbue that!<br>You've currently brought me " +
                                "" + handCount + " x Justiciar's hand, " +
                                "" + demonCount + " x Demon's heart, " +
                                "" + entCount + " x Ent's roots.");
                    } else {
                        npc("You need to bring me a Justiciar's hand, A Demon's heart and some Ent's roots!<br>You've currently brought me " +
                                "" + handCount + " x Justiciar's hand, " +
                                "" + demonCount + " x Demon's heart, " +
                                "" + entCount + " x Ent's roots.");
                    }
        ...
        val nextId = id == 2412 ? 21791 : id == 2413 ? 21793 : 21795;
        player.getDialogueManager().start(new Dialogue(player, npc) {
            @Override
            public void buildDialogue() {
                ...
                options(TITLE, new DialogueOption("Imbue it.", () -> {
                    ...
                    if (completedOnce) {
                        player.addAttribute("handed in " + ItemDefinitions.getOrThrow(idToBringIfCompletedOnce).getName() + " count",
                                (player.getNumericAttribute("handed in " + ItemDefinitions.getOrThrow(idToBringIfCompletedOnce).getName() + " count").intValue()) - 1);
                    } else {
    ...
    public Object[] getItems() {
        return new Object[] {
            21797, 21798, 21799, 2412, 2413, 2414
        };
    }
