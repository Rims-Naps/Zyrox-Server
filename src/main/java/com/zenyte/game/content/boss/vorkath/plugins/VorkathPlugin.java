                ...
                WorldTasksManager.schedule(new WorldTask() {
                    ...
                    public void run() {
                        if (ticks++ == 0) {
                            ...
                            val middle = vorkath.getMiddleLocation();
                            vorkathWakeSound.sendGlobal(middle);
                            vorkathWakeSoundContinue.sendGlobal(middle);
                            return;
                        }
                        player.getBossTimer().startTracking("Vorkath");
                        player.putBooleanAttribute("drainedPrayerDuringVorkath", false);
                        player.getTemporaryAttributes().remove("melee only");
                        player.putBooleanAttribute("hitByZombifiedSpawn", false);
