...
public class Porazdir extends MageArenaBossBase {
    ...
    public Porazdir(@NotNull final Player owner, @NotNull final Location tile) {
        super(NpcId.PORAZDIR_7860, owner, tile);
    }
    ...
    int special(final Entity target) {
        ...
        val delay = World.sendProjectile(this, target, specialProjectile);
        ...
        WorldTasksManager.schedule(() -> {
            val distance = target.getLocation().getDistance(getLocation());
            val damage = (int) ((distance >= 8 ? 0 : ((25 / distance) + Utils.random(5))) * (!player.getPrayerManager().isActive(Prayer.PROTECT_FROM_MAGIC) ? 1.5F : 1F));
