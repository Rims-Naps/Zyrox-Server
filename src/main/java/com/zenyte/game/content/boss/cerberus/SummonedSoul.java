...
public class SummonedSoul extends NPC {
     ...
     */
    private void attack() {
        val spiritShieldType = hasElysianShield() ? SpiritShieldType.ELYSIAN : hasSpectralShield() ? SpiritShieldType.SPECTRAL : SpiritShieldType.NONE;
        val targetPlayer = target.get();
        val cerberus = this.cerberus.get();
        ...
        val player = ((Player) targetPlayer);
        faceEntity(player);
        if (isMelee()) {
    ...
    private void meleeAttack(@NotNull final SpiritShieldType spiritShieldType, @NotNull final Player player) {
        ...
        val delay = World.sendProjectile(this, player, soulProjectile);
    ...
    private void magicAttack(@NotNull final SpiritShieldType spiritShieldType, @NotNull final Player player) {
        ...
        val delay = World.sendProjectile(this, player, fireProjectile);
    ...
    private void rangedAttack(@NotNull final SpiritShieldType spiritShieldType, @NotNull final Player player) {
        ...
        val delay = World.sendProjectile(this, player, dartProjectile);
    ...
    private void scheduleHit(final int delay, @NotNull final Player player, @NotNull final Prayer prayer, @NotNull final SpiritShieldType spiritShieldType) {
        player.addPostProcessRunnable(() -> {
            val hasPrayerUp = player.getPrayerManager().isActive(prayer);
            if (hasPrayerUp) {
                player.getPrayerManager().drainPrayerPoints(spiritShieldType == SpiritShieldType.SPECTRAL ? 15 : 30);
                if (!player.getAttributes().containsKey("succesful_summoned_soul_protections")) {
                    player.getAttributes().put("succesful_summoned_soul_protections", 1);
                }
                player.getAttributes().put("succesful_summoned_soul_protections", player.getNumericAttribute("succesful_summoned_soul_protections").intValue() + 1);
            } else {
    ...
    private boolean hasElysianShield() {
        ...
        if (player instanceof Player) {
            return ((Player) player).getEquipment().getId(EquipmentSlot.SHIELD) == ItemId.ELYSIAN_SPIRIT_SHIELD;
        }
        return false;
    }
