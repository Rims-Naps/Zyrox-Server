...
public final class ZulrahInstance extends DynamicArea implements DeathPlugin, PartialMovementPlugin, CannonRestrictionPlugin, LootBroadcastPlugin {
    ...
    public static final void launch(final Player player) {
        try {
            ...
            val instance = new ZulrahInstance(player, fadeScreen, area);
            ...
            public void run() {
                switch (ticks++) {
                    case 0:
                        ...
                        return;
                    case 1:
                        val tile = player.getLastLoadedMapRegionTile();
                        player.getPacketDispatcher().sendCameraPosition(camPos.getLocalX(tile),
                                camPos.getLocalY(tile), 1000, -128, 0);
                        player.getPacketDispatcher().sendCameraLook(camLook.getLocalX(tile), camLook.getLocalY(tile),
                                1000, -128, 0);
                        return;
                    case 2:
    ...
    public boolean sendDeath(Player player, Entity source) {
        ...
        val resurrections = player.getVariables().getZulrahResurrections();
