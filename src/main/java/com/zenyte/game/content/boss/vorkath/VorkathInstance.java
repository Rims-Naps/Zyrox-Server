 ...
 */
public final class VorkathInstance extends DynamicArea implements CycleProcessPlugin, FullMovementPlugin,
        LogoutPlugin, DeathPlugin, CannonRestrictionPlugin, DropPlugin, LootBroadcastPlugin {
    ...
    public void constructed() {
        ...
        player.getAttributes().put("vorkath_kc_on_instance_creation", player.getNotificationSettings().getKillcount("vorkath"));
    ...
    public void leave(final Player player, boolean logout) {
        ...
        player.getAttributes().remove("vorkath_kc_on_instance_creation");
    }
    ...
    @Override
    public void process() {
        if (vorkath == null || player.hasWalkSteps() || !containsPool(player.getX(), player.getY())) {
            return;
        }
    ...
    public boolean processMovement(final Player player, final int x, final int y) {
        ...
        player.applyHit(new Hit(vorkath, damage, HitType.REGULAR));
        vorkath.applyHit(new Hit(damage, HitType.HEALED));
        vorkath.setHitByPoisonPool(true);
        return true;
    }

    private final boolean containsPool(final int x, final int y) {
        val pools = vorkath.getAcidPools();
        if (pools.isEmpty()) {
            return false;
        }
        ...
        WorldTasksManager.schedule(new WorldTask() {
            ...
            public void run() {
                ...
                if (ticks == 0) {
                    ...
                    player.getDeathMechanics().service(ItemRetrievalService.RetrievalServiceType.TORFINN, source);
                    player.sendMessage("Oh dear, you have died.");
                    player.reset();
                    player.setAnimation(Animation.STOP);
                    player.sendMessage("Torfinn has retrieved some of your items. You can collect them from him " +
                            "in Rellekka.");
