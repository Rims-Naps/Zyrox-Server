...
public final class SOSPortal implements ObjectAction {
	@AllArgsConstructor
	private static enum Portal {
		FIRST_FLOOR(20786, new Location(1914, 5222, 0), player -> player.getBooleanAttribute("sos first claimed")),
		SECOND_FLOOR(19005, new Location(2021, 5223, 0), player -> player.getBooleanAttribute("sos second claimed")),
		THIRD_FLOOR(23707, new Location(2146, 5287, 0), player -> player.getBooleanAttribute("sos third claimed")),
		FOURTH_FLOOR(23922, new Location(2341, 5219, 0), player -> player.getBooleanAttribute("sos fourth claimed"));
	...
	public void handleObjectAction(final Player player, final WorldObject object, final String name, final int optionId,
			final String option) {
		val portal = Utils.findMatching(Portal.VALUES, p -> p.id == object.getId());
		if (portal == null) {
