...
public class DrewDepositDialogue extends Dialogue {
    ...
    private static final int MAXIMUM_BUCKET_AMOUNT = 25_000;
    ...
    public void buildDialogue() {
        val currentBucketHeldAmount = player.getNumericAttribute(DREW_BUCKET_AMOUNT_KEY).intValue();
        val notedBucketId = ItemDefinitions.getOrThrow(ItemId.BUCKET).getNotedOrDefault();
        if (currentBucketHeldAmount >= MAXIMUM_BUCKET_AMOUNT) {
            ...
            player.sendInputInt("How many buckets do you wish to deposit?", (depositAmount) -> {
                if (depositAmount > 0) {
                    ...
                    val amountOfCarriedBuckets = inventory.getAmountOf(ItemId.BUCKET) + inventory.getAmountOf(notedBucketId);
                    ...
                    val cappedByInventory = Math.min(amountOfCarriedBuckets, depositAmount);
                    val cappedDepositAmount = Math.min(MAXIMUM_BUCKET_AMOUNT - currentBucketHeldAmount, cappedByInventory);
                    val deletedUnnotedBuckets = inventory.deleteItem(ItemId.BUCKET, cappedDepositAmount).getSucceededAmount();
                    inventory.deleteItem(notedBucketId, cappedDepositAmount - deletedUnnotedBuckets);
                    val totalBucketHeldAmount = player.incrementNumericAttribute(DREW_BUCKET_AMOUNT_KEY, cappedDepositAmount);
