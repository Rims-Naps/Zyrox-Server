...
public class DrewClaimDialogue extends Dialogue {
    ...
    public void buildDialogue() {
        val currentBucketsHeld = player.getNumericAttribute(DrewDepositDialogue.DREW_BUCKET_AMOUNT_KEY).intValue();
        val currentBucketsOfSand = player.getNumericAttribute(Grinder.BUCKET_OF_SAND_AMOUNT_KEY).intValue();
        val costString = COST_PER_BUCKET.toPrettyString();
        val notedBucketId = ItemDefinitions.get(ItemId.BUCKET_OF_SAND) == null ? ItemId.BUCKET_OF_SAND + 1 : ItemDefinitions.get(ItemId.BUCKET_OF_SAND).getNotedOrDefault();
        if (currentBucketsHeld <= 0 || currentBucketsOfSand <= 0) {
            npc("Ya don't have any bucket of sand to withdraw. Ya need to give me empty buckets, and grind sandstone in my grinder, then I'll sell ya buckets of sand.");
            ...
            player.sendInputInt("How many buckets of sand do ya want? (" + costString + " each)", (bucketsToBuy) -> {
                ...
                val maxAffordableBuckets = inventory.getAmountOf(COST_PER_BUCKET.getId()) / COST_PER_BUCKET.getAmount();
                val cappedAmount = Math.min(maxAffordableBuckets, Math.min(currentBucketsOfSand, Math.min(currentBucketsHeld, bucketsToBuy)));
