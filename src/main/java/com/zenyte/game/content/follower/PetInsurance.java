...
public class PetInsurance {
    ...
    public boolean isInsured(final int petItemId) {
        if (!insuredPets.contains(petItemId)) {
            val variablePet = InsurableVariablePet.getPet(petItemId);
            if (variablePet == null) {
                return false;
            }
            for (val id : variablePet.getIds()) {
    ...
    public List<Pet> getInsuredPets() {
        val list = new ArrayList<Pet>();
        for (val id : insuredPets) {
            val pet = PetWrapper.getByItem(id);
            if (pet == null) {
                continue;
            }
            list.add(pet);
