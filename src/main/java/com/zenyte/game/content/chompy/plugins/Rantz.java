...
public class Rantz extends NPCPlugin {
        ...
        bind("Talk-to", (player, npc) -> {
            player.getDialogueManager().start(new Dialogue(player, npc) {
                ...
                public void buildDialogue() {
                    ...
                    npc("Huh, dat's a true thing but you's godda be da good shooter wiv da stabbie chucker and stick lotsa" +
                            "chompies for da hatsies.");

                    val bow = new Item(ItemId.OGRE_BOW);
                    val offset = ChompyBirdHat.values.length;
                    options(TITLE,
                            new DialogueOption("Where can I get an ogre bow?", key(5)),
                            new DialogueOption("How many chompies do I have to stick?", key(15)),
                            new DialogueOption("Can I have a hat please?", key(20)),
                            new DialogueOption("Can I have an ogre bellows please?", key(35 + offset)),
                            new DialogueOption("Ok thanks.")
                    );


                    val amount = Utils.random(500, 550);
                    ...
                    options(TITLE,
                            new DialogueOption("Yes, I'll buy the bow", key(10)),
                            new DialogueOption("No thanks...")
                    );

                    val coins = new Item(ItemId.COINS_995, amount);
                    ...
                    npc("We'll dat's da fing wiv da hatsies, dey's many hatsies for da many chompies dat you stick. But " +
                            "you's godda get at least this many for a hat. <col=0000FF>~Rantz flashes fingers at you, " +
                            "you think he means 30.~");

                    val hats = getHats(player);
                    ...
                    if (hats.isEmpty()) {
                        ...
                        hats.iterator().forEachRemaining(hat -> {
                            val item = new Item(hat.getItemId(), 1);
                            if (player.getInventory().hasFreeSlots()) {
                                item(item, "<col=0000FF> ~ Rantz shows you a new hat that you can take ~<br>" +
                                        "~ as a reward for your chompy bird kill total. ~<br>" +
                                        "~ This is the hat for <col=FF0000>" + hat.getTitle() + " ~").executeAction(() -> {
                    ...
                    val bellows = new Item(ItemId.OGRE_BELLOWS, 1);
    ...
    private ObjectArrayList<ChompyBirdHat> getHats(final Player player) {
        val list = new ObjectArrayList<ChompyBirdHat>();
        for (ChompyBirdHat hat : ChompyBirdHat.values) {
            if (!player.containsItem(hat.getItemId()) &&
                    player.getNumericAttribute(Chompy.KILL_ATTRIB).intValue() >= hat.getKills()) {
                list.add(hat);
            }
        }
        return list;
