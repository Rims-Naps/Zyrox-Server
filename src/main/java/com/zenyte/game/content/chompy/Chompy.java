...
public class Chompy extends NPC {
    ...
    public void drop(final Location location) {
        val deadChompy = new NPC(DeadChompy.DEAD_CHOMPY_ID, location, true);
    ...
    private boolean hasArrow(final Player player) {
        val ammunition = player.getEquipment().getItem(EquipmentSlot.AMMUNITION);
        if (ammunition == null) {
            return false;
        }
    ...
    public void finish() {
        ...
        World.getPlayer(playerName).ifPresent(p -> {
            val arrow = p.getTemporaryAttributes().get("last hint arrow");
            if (arrow instanceof HintArrow) {
                val hint = (HintArrow) arrow;
