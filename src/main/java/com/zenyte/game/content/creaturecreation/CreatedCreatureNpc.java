...
public class CreatedCreatureNpc extends NPC {
    ...
    public void dropItem(final Player killer, Item item, final Location tile, boolean guaranteedDrop) {
        val symbol = SymbolOfLife.of(getId());
        val diary = symbol.getDiaryForNoted();
        if (diary.isPresent() && diary.get().eligibleFor(killer) && symbol.getAllMaterials().contains(item.getId())) {
