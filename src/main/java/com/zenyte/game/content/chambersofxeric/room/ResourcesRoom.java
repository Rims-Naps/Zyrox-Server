...
public final class ResourcesRoom extends RaidArea implements RoomController, CycleProcessPlugin {
     ...
     */
    private static final Location[][] bats = new Location[][]{
            new Location[]{
                    new Location(3282, 5463, 1), new Location(3288, 5461, 1), new Location(3285, 5457, 1)
            },
            new Location[]{
                    new Location(3308, 5455, 1), new Location(3301, 5458, 1), new Location(3305, 5462, 1)
            },
            new Location[]{
                    new Location(3348, 5455, 1), new Location(3344, 5459, 1), new Location(3343, 5464, 1)
            },
    };
     ...
     */
    private static final Location[][] fishObjects = new Location[][]{
            new Location[]{
                    new Location(3274, 5457, 0), new Location(3280, 5451, 0), new Location(3281, 5454, 0)
            },
            new Location[]{
                    new Location(3302, 5459, 0), new Location(3308, 5449, 0), new Location(3306, 5461, 0)
            },
            new Location[]{
                    new Location(3337, 5453, 0), new Location(3345, 5445, 0), new Location(3343, 5461, 0)
            }
    };
     ...
     */
    private static final Location[][] snakeCenterTiles = new Location[][]{
            new Location[]{
                    new Location(3275, 5457, 0), new Location(3279, 5451, 0), new Location(3280, 5454, 0)
            },
            new Location[]{
                    new Location(3302, 5458, 0), new Location(3309, 5449, 0), new Location(3306, 5460, 0)
            },
            new Location[]{
                    new Location(3338, 5453, 0), new Location(3345, 5446, 0), new Location(3343, 5460, 0)
            }
    };
     ...
     */
    private static final Location[] resourcesAStorageUnits = new Location[]{
            new Location(3275, 5465, 0), new Location(3312, 5463, 0),
            new Location(3335, 5458, 0)
    };
    /**
     * The coordinates to the static storage units found in the B type room.
     */
    private static final Location[] resourcesBStorageUnits = new Location[]{
            new Location(3276, 5465, 1), new Location(3317, 5456, 1),
            new Location(3342, 5456, 1)
    };
    /**
     ...
     */
    @Getter private Location caveSnakeCenter;
     ...
     */
    @Getter private final MutableInt attempts = new MutableInt();
    ...
    public void loadRoom() {
        if (getType() == RaidRoom.RESOURCES_B) {
            val level = getHighestLevel(Skills.HUNTER);
            val bats = new ObjectArrayList<Bat.BatType>();
            val bestBat = getBestBat(level);
            for (int i = bestBat.ordinal(); i >= 0; i--) {
                bats.add(Bat.BatType.values[i]);
                if (bats.size() >= 3) {
                    break;
                }
            }
            val locations = new ObjectArrayList<>(Arrays.asList(ResourcesRoom.bats[index]));
            val count = Math.min(6, 2 + (raid.getOriginalPlayers().size() >> 3));
            val type = bats.get(Utils.random(bats.size() - 1));
            for (int i = 0; i < count; i++) {
                try {
                    val bat = new Bat(raid, this, type.getId(), getLocation(locations.get(Utils.random(locations.size() - 1))));
                    bat.spawn();
                } catch (Exception e) {
                    e.printStackTrace();
                }
     ...
     */
    private final Bat.BatType getBestBat(final int level) {
        Bat.BatType bat = null;
        for (val batType : Bat.BatType.values) {
    ...
    public final void riseSnake() {
        ...
        attempts.increment();
        containsSnake = true;
        val snake = new CaveSnake(raid, this, caveSnakeCenter.transform(-1, -1, 0), fishingSpotPosition);
    ...
    public void process() {
        ...
        if (++ticks >= 50 || attempts.intValue() >= 10) {
            ticks = 0;
            attempts.setValue(0);
            val list = new IntArrayList(IntArray.of(0, 1, 2));
