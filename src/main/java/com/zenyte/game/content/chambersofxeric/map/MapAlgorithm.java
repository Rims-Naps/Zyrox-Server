...
public class MapAlgorithm {
    ...
    private void generatePath(@NotNull final LayoutRoom[] rooms, @NotNull final MapPalette palette, final boolean forceLastRoom) {
        ...
        val secondRoomEntryPos = secondRoom.getDirection();
        ...
        for (val room : rooms) {
            val dir = index + 1 >= rooms.length ? Direction.SOUTH : rooms[index + 1].getDirection();
            ChunkDirection direction = dir == Direction.NORTH ? ChunkDirection.NORTH : dir == Direction.SOUTH ? ChunkDirection.SOUTH : dir == Direction.WEST ? ChunkDirection.WEST :
                                                                                                                                       ChunkDirection.EAST;
    ...
    private void calculateBoundaries() {
        ...
        int maxX = 0;
        int maxY = 0;
        for (val floor : Utils.concatenate(topFloorPalette, middleFloorPalette, bottomFloorPalette)) {
            if (floor == null) {
                continue;
            }
            for (val room : floor) {
