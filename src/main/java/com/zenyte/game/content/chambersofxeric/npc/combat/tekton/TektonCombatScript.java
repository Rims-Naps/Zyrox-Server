...
public final class TektonCombatScript implements TektonScript {
	...
	public void execute(final Tekton tekton) {
		WorldTasksManager.schedule(new WorldTask() {
			...
			public void run() {
				...
				/* After 15 consecutive successful attacks, tekton retreats back to the anvil. */
				if (stage >= (enraged ? 45 : 24)) {
				...
				if (stage % 3 == 0) {
					...
					for (final Entity target : reachableTargets) {
						if (Utils.collides(tekton.getX(), tekton.getY(), tekton.getSize(), target.getX(), target.getY(), target.getSize())
								|| list.contains(target.getLocation().getPositionHash())) {
						    val enraged = tekton.isEnraged();
						    val challenge = tekton.getRoom().getRaid().isChallengeMode();
							val max = enraged ? challenge ? 87 : 59 : (challenge ? 78 : 52);

							tekton.getCombatDefinitions().setAttackStyle(attackType == 0 ? "Stab" : attackType == 1 ?
                                    "Slash" : "Crush");
							val hit = CombatUtilities.getRandomMaxHit(tekton, max, CombatScript.MELEE,
                                    CombatScript.MELEE, target);
    ...
    private void fillSlashTiles(final IntList tiles, final Direction direction, final Tekton tekton) {
        ...
        val x = tekton.getX();
        val y = tekton.getY();
        val size = tekton.getSize();
        val plane = tekton.getPlane();
        switch (direction) {
            case SOUTH:
	...
	private void fillReachableTargets(final Tekton tekton, final List<Entity> list) {
		...
		for (final Player player : tekton.getRaid().getPlayers()) {
			...
			if (CombatUtilities.isWithinMeleeDistance(tekton, player)) {
				if (Utils.collides(tekton.getX(), tekton.getY(), tekton.getSize(), player.getX(), player.getY(), player.getSize())) {
					continue;
				}
