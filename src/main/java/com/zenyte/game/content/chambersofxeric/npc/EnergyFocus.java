...
public final class EnergyFocus extends NPC {
	...
	private static final Location[][] crystalTiles = new Location[][] {
		new Location[] { new Location(3283, 5361, 2), new Location(3279, 5366, 2), new Location(3282, 5369, 2), new Location(3287, 5366, 2) },
		new Location[] { new Location(3311, 5356, 2), new Location(3320, 5360, 2), new Location(3306, 5361, 2), new Location(3305, 5356, 2) },
		new Location[] { new Location(3341, 5355, 2), new Location(3337, 5354, 2), new Location(3340, 5363, 2), new Location(3340, 5367, 2) }
	};
	...
	public EnergyFocus(final CrabPuzzleRoom room, final Location tile) {
		...
		this.room = room;
		direction = (room.getIndex() + room.getRotation()) & 0x3;
		for (int i = 0; i < 4; i++) {
	...
	public void processNPC() {
		...
		if (finish) {
			...
			List<Player> eligiblePlayers = getPlayersEligibleForPoints();
			int eligiblePlayerCount = eligiblePlayers.size();

			for (final Location tile : crystals) {
				if (getX() == tile.getX() && getY() == tile.getY()) {
					if (index == 0 && getId() == 7580
							|| index == 1 && getId() == 7583
							|| index == 2 && getId() == 7582
							|| index == 3 && getId() == 7581) {
						finish();
						...
						if(eligiblePlayerCount > 0) {
							for(Player p : eligiblePlayers) {
								Raid.incrementPoints(p, 1600 / eligiblePlayerCount);
							}
						}
						/*if (username != null) {
                        ...
                        val graphics = getGraphics(false);
		...
		while (true) {
			...
			for (final JewelledCrab crab : room.getCrabs()) {
				if (crab.getX() == x && crab.getY() == y) {
					...
					advanceDirection();
					val target = crab.getCombat().getTarget();
		...
		val nextTile = getNextPosition(1);
		if (!nextTile.matches(getLocation())) {
            checkPlayers(nextTile);
        }
	}

	private List<Player> getPlayersEligibleForPoints() {
		List<Player> eligible = new ArrayList<>();
		for(Player p : room.getRaid().getPlayers()) {
			if(p.getArea() instanceof CrabPuzzleRoom) {
				if(p.getLastTarget() instanceof JewelledCrab) {
					eligible.add(p);
				}
			}
		}
