...
public final class MuttadileRoom extends RaidArea implements CycleProcessPlugin {
     ...
     */
    private static final Location[] treeSpawnLocations = new Location[]{
            new Location(3269, 5320, 1), new Location(3301, 5320, 1), new Location(3350, 5317, 1)
    };

    /**
     * A 2D array containing coordinates to the tendrils that block the entrance.
     */
    private static final Location[][] tendrilSpawnLocations = new Location[][]{
            new Location[]{new Location(3279, 5314, 1), new Location(3280, 5314, 1)},
            new Location[]{new Location(3311, 5314, 1), new Location(3312, 5314, 1)},
            new Location[]{new Location(3343, 5313, 1), new Location(3344, 5313, 1)}
    };

    /**
     * An array containing coordinates that the large muttadile cannot reach.
     */
    private static final Location[] ignoredTiles = new Location[]{
            new Location(3280, 5312, 1), new Location(3311, 5312, 1), new Location(3343, 5312, 1)
    };

    /**
     * An array containing coordinates to the crystal that blocks the exit.
     */
    private static final Location[] crystalSpawnLocations = new Location[]{
            new Location(3270, 5328, 1), new Location(3308, 5335, 1), new Location(3352, 5327, 1)
    };

    /**
     * An array containing coordinates to where the large muttadile initially lies at.
     */
    private static final Location[] largeMuttadileSpawnLocations = new Location[]{
            new Location(3282, 5332, 1), new Location(3314, 5330, 1), new Location(3336, 5330, 1)
    };

    /**
     * An array containing coordinates to where the small muttadile spawns.
     */
    private static final Location[] smallMuttadileSpawnLocations = new Location[]{
            new Location(3276, 5328, 1), new Location(3307, 5325, 1), new Location(3344, 5323, 1)
    };
    ...
    public void unleashLargeMuttadile() {
        ...
        val dir = largeMuttadile.getMovementDirection();
        val x = largeMuttadile.getX();
        val y = largeMuttadile.getY();
    ...
    public void clearPath() {
        ...
        WorldTasksManager.schedule(() -> {
            for (val tendril : tendrilSpawnLocations[index]) {
    ...
    public void loadRoom() {
        ...
        val location = getObjectLocation(treeSpawnLocations[index], 2, 2, getRotation());
        ...
        largeMuttadile = new LargeMuttadile(this, Direction.mainDirections[((index == 2 ? 0 : 1) + getRotation()) & 0x3], getNPCLocation(largeMuttadileSpawnLocations[index], 8));
    }
    ...
    public static final class MeatTreeWoodcutting extends Action {
        ...
        private boolean checkAll(final Player player) {
            val optionalAxe = Woodcutting.getAxe(player);
        ...
        public int processWithDelay() {
            val successRate = 20 + (int) Math.floor(player.getSkills().getLevel(Skills.WOODCUTTING) / 1.7F);
