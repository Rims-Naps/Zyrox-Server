...
public final class VasaNistirio extends RaidNPC<VasaNistirioRoom> {
	...
	public VasaNistirio(final Raid raid, final VasaNistirioRoom room, final Location tile) {
		super(raid, room, NpcId.ROCKS_7565, tile);
	...
	private int stage = -1;
	private int substage, ticksTilDestroy = 67;
	private int delay;
		...
		case -1:
			for (final Player p : raid.getPlayers()) {
				if (p.getLocation().withinDistance(center, 7)) {
					stage++;
					room.start();
					this.getTemporaryAttributes().put("vasa teleports", 0);
					return;
				}
			}
			return;
					...
					public void run() {
						...
						for (final Player r : raid.getPlayers()) {
							if (r.getLocation().withinDistance(pTile, 1)) {
								...
								val damage = CombatUtilities.getRandomMaxHit(VasaNistirio.this, getMaxHit(18), CombatScript.RANGED, r);
								val hit = new Hit(VasaNistirio.this, damage, HitType.RANGED);
	...
	private void refreshNextCrystal() {
	    val availableCrystals = new IntArrayList(4);
	...
	private void walkToCenter() {
		...
		val steps = RouteFinder.findRoute(getX(), getY(), getPlane(), getSize(),
				new TileStrategy(tile.getX(), tile.getY()), true);
	...
	private void sendExplosions() {
        ...
        val count = Math.max(nearbyPlayers.size(), CharacterLoop.find(center, 4, Player.class, player -> true).size());
		if (count == 0) {
		    return;
        }
		val damage = damageForNextExplosion / count;
		for (int x = -1; x <= 5; x += 3) {
			for (int y = -1; y <= 5; y += 3) {
				final Location tile = new Location(getLocation().getX() + x, getLocation().getY() + y, center.getPlane());
				if (tile.getPositionHash() == hash) {
		...
		distantPlayers.forEach(player -> {
			if (player.getLocation().withinDistance(center, 4)) {
				val amount = !player.getPrayerManager().isActive(Prayer.PROTECT_FROM_MAGIC) ? damage : (damage / 2);
	...
	protected void drop(final Location tile) {
		...
		if (Integer.parseInt(this.getTemporaryAttributes().get("vasa teleports").toString()) < 2) {
			if (!killer.getBooleanAttribute("master-combat-achievement16")) {
				killer.putBooleanAttribute("master-combat-achievement16", true);
				MasterTasks.sendMasterCompletion(killer, 16);
			}
		}
		announce(killer);
		onDrop(killer);
	...
	private void teleportTargets() {
		this.getTemporaryAttributes().put("vasa teleports", Integer.parseInt(this.getTemporaryAttributes().get("vasa teleports").toString()) + 1);
		setAnimation(swirl);
		final Location middle = getMiddleLocation();
	...
	private Location getNearbyLocation(final Location middle) {
		...
		while (true) {
			...
			final Location tile = new Location(Utils.random(-4, 4) + middle.getX(), Utils.random(-4, 4) + middle.getY(),
					middle.getPlane());
	...
	private Location getDistantLocation(final Location middle) {
		...
		while (true) {
			...
			final Location tile = new Location(Utils.random(-14, 14) + middle.getX(), Utils.random(-14, 14) + middle.getY(),
					middle.getPlane());
