...
public final class CrabPuzzleRoom extends RaidArea implements PlayerCombatPlugin {
     ...
     */
    private static final Location[] lightFocusSpawnLocations = new Location[]{
            new Location(3277, 5353, 2), new Location(3318, 5363, 2),
            new Location(3346, 5358, 2)
    };

    /**
     * A 2D array containing locations for the crab spawns.
     */
    private static final Location[][] crabSpawnLocations = new Location[][]{
            new Location[]{
                    new Location(3286, 5354, 2), new Location(3280, 5358, 2), new Location(3281, 5365, 2),
                    new Location(3287, 5361, 2)
            },
            new Location[]{
                    new Location(3306, 5358, 2), new Location(3311, 5353, 2), new Location(3313, 5361, 2),
                    new Location(3318, 5356, 2)
            },
            new Location[]{
                    new Location(3337, 5356, 2), new Location(3335, 5362, 2), new Location(3341, 5366, 2),
                    new Location(3344, 5361, 2)
            }
    };
    ...
    private static final Location[] crystalSpawnLocations = new Location[]{
            new Location(3272, 5365, 2), new Location(3308, 5366, 2),
            new Location(3349, 5366, 2)
    };
    ...
    public CrabPuzzleRoom(final RaidRoom type, final Raid raid, final int rotation, final int size, final int regionX, final int regionY,
    ...
    private Location getCrystalTile() {
        val defs = Objects.requireNonNull(ObjectDefinitions.get(29753));
    ...
    public void transformCrystal(final int index) {
        val crystal = new WorldObject(crystalId, 10, getRotation(), getCrystalTile());
        ...
        if (focusCount <= 4) {
            for (val player : this.getPlayers()) {
                if (!player.getBooleanAttribute("master-combat-achievement28")) {
                    player.putBooleanAttribute("master-combat-achievement28", true);
                    MasterTasks.sendMasterCompletion(player, 28);
                }
            }
        }
    }
