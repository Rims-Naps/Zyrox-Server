...
public final class CrystalBomb implements OlmCombatScript {
	...
	public void handle(final GreatOlm olm) {
		...
		final int players = olm.getRoom().getRaid().getOriginalPlayers().size();
		int bombCount = players >= 56 ? 5 : players >= 24 ? 4 : players >= 8 ? 3 : 2;
		olm.performAttack();
		final Location head = olm.getFaceCoordinates();
		bombs : while (bombCount-- > 0) {
			int trycount = 50;
			loop : while (trycount-- > 0) {
			...
			public void run() {
				...
				for (final Bomb bomb : crystalBombs) {
					for (final Player p : everyone) {
						...
						if (distance <= 4) {
							val damage = distance <= 0 ? 60 : distance == 1 ? 45 : distance == 2 ? 35 : distance == 3 ? 25 : 15;
