    ...
    public void handleObjectAction(final Player player, final WorldObject object, final String name, final int optionId, final String option) {
        player.getRaid().ifPresent(raid -> raid.ifInRoom(player, MuttadileRoom.class, room -> {
            ...
            if (!room.getTendrilPlayers().add(player.getUsername())) {
                val hit = new Hit((int) ((player.getHitpoints() * 0.15F) + (player.getHitpoints() * 0.075F)), HitType.REGULAR);
                hit.setExecuteIfLocked();
                player.applyHit(hit);
            }
            val destTile = new Location(player.getLocation());
            ...
            val obj = World.getObjectWithType(destTile, 10);
            if (obj != null && obj.getId() == object.getId()) {
                ...
                val secondaryTile = new Location(destTile);
