...
public final class LeftClaw extends GreatOlmClaw {
     ...
     */
    public static final int CRYSTAL_SIGN = 0, LIGHTNING_SIGN = 1, SWIRL_SIGN = 2;
    ...
    LeftClaw(final OlmRoom room, final Location tile) {
        super(room, NpcId.GREAT_OLM_LEFT_CLAW_7555, tile);
    }
    ...
    public float getPointsMultiplier(final Hit hit) {
        if (healedDamage > 0) {
            ...
            val multiplier = healedDamage >= damage ? 0 : (1 - ((float) healedDamage / damage));
    ...
    boolean shouldClench() {
        val collectiveDamage = new MutableInt();
        clenchDamage.removeIf(hit -> {
            val removed = hit.getScheduleTime() + TimeUnit.TICKS.toMillis(10) < System.currentTimeMillis();
            if (!removed) {
        ...
        val threshold = room.getOlm().everyone(GreatOlm.ENTIRE_CHAMBER).size() * 20;
    ...
    protected void spawnClawlessObject() {
        ...
        val defeatedClaw = new WorldObject(29885, 10,
                side == OlmRoom.LEFT ? OlmRoom.leftSideLeftClawObject.getRotation() : OlmRoom.rightSideLeftClawObject.getRotation(),
                room.getLocation(side == OlmRoom.LEFT ? OlmRoom.leftSideLeftClawObject : OlmRoom.rightSideLeftClawObject));
