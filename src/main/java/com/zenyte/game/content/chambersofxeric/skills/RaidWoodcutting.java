...
public final class RaidWoodcutting extends Action {
     ...
     */
    private final int[] milestones = new int[]{
            1, 24, 36, 48, 60, 72, 84, 96
    };
    /**
     * The experience granted at different milestones.
     */
    private final float[] experience = new float[]{
            30F, 45F, 55F, 62.5F, 38.5F, 73.5F, 77.75F, 81.5F
    };
    /**
    ...
    private void addLog(final Player player) {
        player.getRaid().ifPresent(raid -> {
            ...
            val experience = this.experience[amount.intValue() - 1];
     ...
     */
    private final int getMaximumAmountKindling(@NotNull final Player player) {
        val level = Math.max(player.getSkills().getLevel(Skills.WOODCUTTING), player.getSkills().getLevelForXp(Skills.WOODCUTTING));
        int max = 0;
        for (val milestone : milestones) {
            if (level < milestone) {
                break;
            }
            max++;
