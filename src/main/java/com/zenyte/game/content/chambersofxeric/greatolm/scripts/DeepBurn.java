...
public final class DeepBurn implements OlmCombatScript {
	...
	private static final int[] combatSkills = new int[] {
            Skills.ATTACK, Skills.STRENGTH, Skills.DEFENCE, Skills.RANGED, Skills.MAGIC
    };

	@Getter private final Object2IntMap<Player> infectionsMap = new Object2IntOpenHashMap<>();
	...
	public void handle(final GreatOlm olm) {
		...
		WorldTasksManager.schedule(new WorldTask() {
			...
			public void run() {
				...
				val remainingInfections = new Object2IntOpenHashMap<Player>(infectionsMap.size());
				for (val entry : infectionsMap.object2IntEntrySet()) {
                    ...
                    val infectionLevel = entry.getIntValue();
                    if (infectionLevel > 1) {
