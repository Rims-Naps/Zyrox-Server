...
public final class GuardiansRoom extends RaidArea implements CycleProcessPlugin {
     ...
     */
    public static final Location[][] guardianSpawnTiles = new Location[][]{
            new Location[]{new Location(3276, 5265, 2), new Location(3276, 5269, 2), new Location(3276, 5267, 2), new Location(3276, 5268, 2)},
            new Location[]{new Location(3312, 5271, 2), new Location(3308, 5271, 2), new Location(3310, 5271, 2), new Location(3311, 5271, 2)},
            new Location[]{new Location(3344, 5268, 2), new Location(3344, 5272, 2), new Location(3344, 5271, 2), new Location(3344, 5270, 2)}
    };

    /**
     * A 2D array containing the coordinates to which each of the guardian faces.
     */
    public static final Location[][] faceTiles = new Location[][]{
            new Location[]{new Location(4000, 5265, 2), new Location(4000, 5269, 2)},
            new Location[]{new Location(3312, 4000, 2), new Location(3308, 4000, 2)},
            new Location[]{new Location(2000, 5268, 2), new Location(2000, 5272, 2)}
    };

    /**
     * A 2D array containing coordinates to the prevented tiles, standing upon which the player is pushed back.
     */
    private static final Location[][] preventedTiles = new Location[][]{
            new Location[]{new Location(3275, 5267, 2), new Location(3275, 5268, 2)},
            new Location[]{new Location(3310, 5273, 2), new Location(3311, 5273, 2)},
            new Location[]{new Location(3346, 5271, 2), new Location(3346, 5270, 2)}
    };
    ...
    public GuardiansRoom(final RaidRoom type, final Raid raid, final int rotation, final int size, final int regionX, final int regionY,
    ...
    public void loadRoom() {
        ...
        for (int i = 0; i < 2; i++) {
            final RaidGuardianNPC guardian = new RaidGuardianNPC(raid, this, NpcId.GUARDIAN + i, getNPCLocation(loc[i], 2));
            guardian.spawn();
    ...
    public void finish() {
        ...
        for (val object : prevented) {
            World.removeObject(World.getObjectWithType(object, 10));
        }
        for (val player : this.getPlayers()) {
            if (!player.getBooleanAttribute("has_taken_damage_from_guardian_in_raid") && !player.getBooleanAttribute("elite-combat-achievement46")) {
                player.putBooleanAttribute("elite-combat-achievement46", true);
                EliteTasks.sendEliteCompletion(player, 46);
            }
        }
    }
    ...
    private int isPrevented(final Player player) {
        ...
        for (int i = 0; i < 2; i++) {
            ...
            val distance = loc.getDistance(prevented[i]);
            if (distance <= 1) {
                return i;
            }
        }
     ...
     */
    private final void pushBack(final Player player, final int index) {
        player.resetWalkSteps();
        val t = new Location(player.getLocation());
        WorldTasksManager.schedule(() -> {
            if (!t.matches(player)) {
                return;
            }
            final Location tile = getLocation(guardianSpawnTiles[this.index][2 + index]);
            player.setForceMovement(new ForceMovement(tile, 10, tile, 30, Utils.getFaceDirection(tile.getX() - player.getX(),
                    tile.getY() - player.getY())));
            val n = npcs.stream().max((n2, n1) -> n1.getLocation().getDistance(player.getLocation()) > n2.getLocation().getDistance(player.getLocation()) ? 1 : -1)
                    .orElseThrow(RuntimeException::new);
            n.setFaceLocation(player.getLocation());
            n.setAnimation(RaidGuardianNPC.attack);
            val combat = n.getCombat();
            ...
            val oppositeNPC = npcs.get(0) == n ? npcs.get(1) : npcs.get(0);
            if (oppositeNPC.isDead() || oppositeNPC.isFinished()) {
                return;
            }
            val oppositePreventedTile = getLocation(guardianSpawnTiles[this.index][2 + (index == 0 ? 1 : 0)]);
            oppositeNPC.stomp(oppositePreventedTile);
            oppositeNPC.setFaceLocation(player.getLocation());
            val oppositeCombat = oppositeNPC.getCombat();
