...
public final class RaidFarming {
        ...
        patches.forEach((k, v) -> {
            if (v.stage < 4) {
                if (++v.ticks >= 12) {
                    v.stage++;
                    v.ticks = 0;
                    World.spawnObject(v.currentPatch = new WorldObject(v.type.objectId + (v.stage * 3), v.patch.getType(),
                            v.patch.getRotation(), v.patch));
                }
            }
        });
    }
    ...
    public void handle(final Player player, final WorldObject object, final String option) {
        val patch = patches.get(object.getPositionHash());
            ...
            case "Inspect":
                ...
                WorldTasksManager.schedule(() -> {
                    ...
                    switch (patch.stage) {
                        ...
                        case 2:
                            player.sendMessage(
                                    "There's a " + patch.type.toString().toLowerCase() + " plant growing here. It looks to be about half-grown.");
                            return;
                        case 3:
                            player.sendMessage(
                                    "There's a " + patch.type.toString().toLowerCase() + " plant growing here. It's almost ready for harvesting.");
                            return;
                        case 4:
    ...
    private enum PatchType {
        ...
        private final int objectId, seedId, produceId, level, plantingXp, harvestingXp;
    ...
    private static final class HerbHarvestAction extends Action {
        ...
        public boolean process() {
            for (val clearingPlayer : patch.clearingPlayers) {
                val player = World.getPlayer(clearingPlayer);
                if (!player.isPresent()) {
                    continue;
                }
                val p = player.get();
        ...
        public int processWithDelay() {
            ...
            val req = patch.type.level;
            val chance = Math.min(0.95, 0.65 + ((player.getSkills().getLevel(Skills.FARMING) - req) / 250F));
    ...
    private static final class HerbClearAction extends Action {
        ...
        public boolean start() {
            ...
            if (player.getSkills().getLevel(Skills.FARMING) < type.level) {
                player.sendMessage(
                        "You need a Farming level of at least " + type.level + " to clear " + type.toString().toLowerCase() + " patch.");
                return false;
            }
    ...
    private static final class HerbPlantAction extends Action {
        ...
        public boolean start() {
            if (player.getSkills().getLevel(Skills.FARMING) < type.level) {
                player.sendMessage(
                        "You need a Farming level of at least " + type.level + " to plant " + type.toString().toLowerCase() + " seeds.");
                return false;
            }
        ...
        public int processWithDelay() {
            ...
            val herbPatch = new HerbPatch(object, type);
    ...
    private static final class HerbPatch {
         ...
         */
        private Set<String> clearingPlayers = new ObjectOpenHashSet<>();
