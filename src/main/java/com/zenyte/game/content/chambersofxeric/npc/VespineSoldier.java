...
public class VespineSoldier extends RaidNPC<VespulaRoom> implements CombatScript {
    ...
    public static final int FLYING = NpcId.VESPINE_SOLDIER;
    private static final int CRAWLING = NpcId.VESPINE_SOLDIER_7539;
    ...
    @Getter @Setter private boolean grantPoints;
    ...
    public void rise() {
        ...
        val vespula = room.getVespula();
        val portal = room.getAbyssalPortal();
        if (!portal.isDead()) {
    ...
    private void explode() {
        val tile = new Location(getMiddleLocation());
        finish();
    ...
    public void onFinish(final Entity source) {
        ...
        for (val grub : this.room.getGrubs()) {
            val relatedSoldier = grub.getSoldier();
    ...
    public int attack(final Entity target) {
        ...
        if (Utils.random(2) == 0) {
            target.getToxins().applyToxin(Toxins.ToxinType.POISON, 15);
        }
