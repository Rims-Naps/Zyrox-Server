...
public class Vespula extends RaidNPC<VespulaRoom> implements CombatScript {
    ...
    private static final int NORMAL = NpcId.VESPULA, ENRAGED = NpcId.VESPULA_7531, CRAWLING = NpcId.VESPULA_7532;
    ...
    @Getter
    @Setter private boolean skipStinging;
        ...
        if (id != CRAWLING) {
            ...
            if (room.getAbyssalPortal().isUnderAttack()) {
                if (id == ENRAGED) {
                    ...
                    val targetsInMeleeDistance = CharacterLoop.find(getLocation(), getSize() + 1, Player.class,
                            target -> CombatUtilities.isWithinMeleeDistance(this, target) && !Utils.collides(getX(), getY(), getSize(), target.getX(), target.getY(), target.getSize()));
                    stingDelay = 2;
                    Location tile;
                    if (!targetsInMeleeDistance.isEmpty()) {
                        tile = targetsInMeleeDistance.get(Utils.random(targetsInMeleeDistance.size() - 1)).getLocation();
                        for (val target : targetsInMeleeDistance) {
                            if (target.getLocation().withinDistance(tile, 1)) {
                                target.applyHit(new Hit(Utils.random(20), HitType.REGULAR));
                                if (Utils.random(3) == 0) {
                                    target.getToxins().applyToxin(Toxins.ToxinType.POISON, 20);
                                }
                            }
                        }
                    } else {
            ...
            if (id == NORMAL) {
                ...
                val farthestGrub = getFarthestUnstingedGrub();
    ...
    private boolean removeTarget() {
        if (combat.getTarget() != null) {
            if (getAttackedByDelay() < System.currentTimeMillis()) {
                if (room.getPlayers().isEmpty()) {
                    combat.removeTarget();
                    calcFollow(getRespawnTile(), -1, true, true, false);
                    return true;
    ...
    public void kill() {
        ...
        onDeath(source);
        if (getId() != CRAWLING && !room.getPlayers().isEmpty()) {
            val players = room.getPlayers();
            for (val player : players) {
                if (!player.getBooleanAttribute("elite-combat-achievement24")) {
                    player.putBooleanAttribute("elite-combat-achievement24", true);
                    EliteTasks.sendEliteCompletion(player, 24);
                }
            }
        }
    ...
    private final Optional<LuxGrub> getFarthestUnstingedGrub() {
        ...
        val grubs = room.getGrubs();
        LuxGrub farthestGrub = null;
        val location = getMiddleLocation();
        for (val grub : grubs) {
            if (!grub.canBeStung()) {
                continue;
            }
    ...
    public int attack(final Entity target) {
        ...
        val projectile = getId() == CRAWLING ? crawlingAttackProjectile : flyingAttackProjectile;
        ...
        val destTile = new Location(target.getLocation(), 2);
