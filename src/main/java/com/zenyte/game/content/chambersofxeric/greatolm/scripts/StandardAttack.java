...
public final class StandardAttack implements OlmCombatScript {
	...
	public void handle(final GreatOlm olm) {
		...
		val ranged = olm.isRanging();
		val face = olm.getFaceCoordinates();
		val everyone = olm.everyone(olm.getDirection());
		if (everyone.isEmpty()) {
			return;
		}
		olm.performAttack();
		World.sendSoundEffect(olm.getMiddleLocation(), ranged ? rangedSound : magicSound);
		for (val player : everyone) {
			World.sendProjectile(face, player, ranged ? rangedProjectile : magicProjectile);
			CombatUtilities.delayHit(olm, rangedProjectile.getTime(face, player.getLocation()), player, new Hit(olm,
                    CombatUtilities.getRandomMaxHit(olm, 33 + (olm.getRoom().getRaid().getOriginalPlayers().size() / 8), ranged ? CombatScript.RANGED : CombatScript.MAGIC, player),
                    ranged ? HitType.RANGED : HitType.MAGIC));
		}
	}
