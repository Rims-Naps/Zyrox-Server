...
public final class VanguardRoom extends RaidArea {
    ...
    private static boolean vanguardsHealed;
     ...
     */
    private static final Location[][] vanguardPositions = new Location[][]{
            new Location[]{new Location(3285, 5329, 0), new Location(3277, 5334, 0), new Location(3276, 5324, 0)},
            new Location[]{new Location(3317, 5330, 0), new Location(3309, 5334, 0), new Location(3309, 5324, 0)},
            new Location[]{new Location(3347, 5325, 0), new Location(3341, 5334, 0), new Location(3337, 5325, 0)}
    };
    /**
    ...
    private static final Location[] crystalLocations = new Location[]{
            new Location(3268, 5327, 0), new Location(3311, 5339, 0),
            new Location(3350, 5328, 0)
    };
    /**
    ...
    public VanguardRoom(final RaidRoom type, final Raid raid, final int rotation, final int size, final int regionX, final int regionY,
    ...
    public CombatPointCapCalculator buildPointsCap() {
        val calculator = new CombatPointCapCalculator();
    ...
    public void launch() {
        ...
        vanguardsHealed = false;
        val ids = new IntArrayList(VanguardRoom.ids);
        int count = 0;
        while (!ids.isEmpty() && ++count < 1000) {
            ...
            val id = ids.getInt(Utils.random(ids.size() - 1));
    ...
    private void heal() {
        ...
        val one = vanguards[1];
        val two = vanguards[2];
        if ((!zero.isFinished() && !one.isFinished() && (zero.getHitpoints() + amount < one.getHitpoints() || zero.getHitpoints() - amount > one.getHitpoints()))
                || (!one.isFinished() && !two.isFinished() && (one.getHitpoints() + amount < two.getHitpoints() || one.getHitpoints() - amount > two.getHitpoints()))
                || (!two.isFinished() && !zero.isFinished() && (two.getHitpoints() + amount < zero.getHitpoints() || two.getHitpoints() - amount > zero.getHitpoints()))) {
            for (val vanguard : vanguards) {
                val hitpoints = vanguard.getHitpoints();
                if (vanguard.isDead()) {
                    continue;
                }
    ...
    public void switchLocations() {
        ...
        heal();
        stage = 2;
        val clockwise = Utils.random(1) == 0;
        for (val vanguard : vanguards) {
            if (vanguard.isDead()) {
                continue;
            }
    ...
    public void loadRoom() {
        for (int i = 0; i < 3; i++) {
            vanguards[i] = (Vanguard) new Vanguard(raid, this, NpcId.VANGUARD, getLocation(vanguardPositions[index][i]).moveLocation(-1, -1, 0)).spawn();
        }
        for (int i = 0; i < 3; i++) {
    ...
    public static boolean hasHealed() {
        return vanguardsHealed;
    }
