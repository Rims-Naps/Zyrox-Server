...
public abstract class RaidArea extends DynamicArea implements RoomController, DropPlugin, DeathPlugin, HitProcessPlugin, CannonRestrictionPlugin, TeleportPlugin, LogoutPlugin, LayableObjectPlugin {
     ...
     */
    private final int rotation, fromPlane, toPlane;
    ...
    public RaidArea(final RaidRoom type, final Raid raid, final int rotation, final int size, final int regionX, final int regionY,
    ...
    protected Location getCrystalTile(final Location location) {
        val defs = Objects.requireNonNull(ObjectDefinitions.get(30018));
        val sizeX = defs.getSizeX();
        val sizeY = defs.getSizeY();
    ...
    public void refreshStorageUnits(final int storageUnitId) {
        val units = getStorageUnitLocations();
        if (units == null) {
            return;
        }
        ...
        val tile = getLocation(units[index]);
        val existingObject = World.getObjectWithType(tile, 10);
    ...
    protected int getHighestLevel(final int skill) {
        ...
        for (val player : raid.getPlayers()) {
            val lv = player.getSkills().getLevelForXp(skill);
            if (lv > level) {
                level = lv;
            }
        }
    ...
    public Location getLocation(final int x, final int y, final int height) {
        ...
        final int transformedX = (int) Math
                .round(15.5d + ((offsetX - 15.5d) * Math.cos(radians)) - ((offsetY - 15.5d) * Math.sin(radians)));
        final int transformedY = (int) Math
                .round(15.5d + ((offsetX - 15.5d) * Math.sin(radians)) + ((offsetY - 15.5d) * Math.cos(radians)));
    ...
    public String toString() {
        return getClass().getSimpleName() + ", Rotation: " + rotation + ", static: [" + staticChunkX + ", " + staticChunkY + ", " + fromPlane + "] + "
                + "dynamic: [" + chunkX + ", " + chunkY + ", " + toPlane + "], ";
    }
    ...
    private final void updateStage(@NotNull final Player player) {
        ...
        val stage = raid.getStage();
        if (plane >= 3) {
            return;
        }
        val update = plane == 2 && stage <= 1 || plane == 1 && stage <= 2 || type == null;
        if (!update) {
            return;
        }
        val room = type == null ? this : Utils.findMatching(raid.getMap().getRaidChunks(),
                chunk -> chunk.getToPlane() == plane && (chunk.getType() == RaidRoom.FLOOR_START_UPSTAIRS || chunk.getType() == null));
        if (room == null) {
            return;
        }
        val boundTile = room.getBoundTile();
        if (boundTile == null) {
            return;
        }
    ...
    public void leave(final Player player, boolean logout) {
        ...
        if (logout || !(nextArea instanceof RaidArea)) {
            ...
            val contained = new MutableBoolean();
            if (raid != null) {
        ...
        if (this.leaveTime == 0 && !logout && nextArea instanceof RaidArea && (!player.getPrivilege().eligibleTo(Privilege.ADMINISTRATOR) || Constants.WORLD_PROFILE.isDevelopment())) {
            ...
            val thisIndex = raid.getMap().getRaidChunks().indexOf(this);
            val nextIndex = thisIndex == raid.getMap().getRaidChunks().size() - 1 && nextArea instanceof OlmRoom ? (thisIndex + 1) : raid.getMap().getRaidChunks().indexOf(nextArea);
    ...
    public boolean inside(final Location location) {
        ...
        val x = location.getX();
        val y = location.getY();
        val areaX = chunkX * CHUNK_SIZE;
        val areaY = chunkY * CHUNK_SIZE;
        val areaLimitX = areaX + (sizeX * CHUNK_SIZE);
        val areaLimitY = areaY + (sizeY * CHUNK_SIZE);
        ...
        WorldTasksManager.schedule(new WorldTask() {
            ...
            public void run() {
                ...
                if (ticks == 1) {
                    ...
                    val totalPoints = raid.getTotalPoints();
                    if (points >= 8400) {
                    ...
                    val type = getType();
                    val killingBlow = player.getTemporaryAttributes().get("killing blow hit");
                    val source = killingBlow instanceof NPC ? ((NPC) killingBlow).getDefinitions().getName() : "Unknown source";
                    builder.append(player.getUsername()).append(": Death at ")
                            .append(type == null ? "Great Olm" : type.getFormattedName()).append(" room by ").append(source).append(", losing ");
                    if (points >= 8400) {
    ...
    public boolean hit(final Player source, final Entity target, final Hit hit, final float modifier) {
        ...
        if (target instanceof JewelledCrab) {
            val crab = (JewelledCrab) target;
            hit.setDamage(0);
        ...
        if (hit.getDamage() > 0) {
            ...
            val maximum = (Math.min(hit.getDamage(), target.getHitpoints())) * 5F;
            val actualPointsBasedOnDamage = hit.getDamage() * ((target instanceof RaidNPC ? ((RaidNPC<?>) target).getPointsMultiplier(hit) : target.getXpModifier(hit)) * 5F);
