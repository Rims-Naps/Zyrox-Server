...
public abstract class ScavengerRoom extends RaidArea {
    ...
    public void loadRoom() {
        ...
        val amount = size <= 1 ? 2 : size <= 4 ? 3 : size <= 7 ? 4 : size <= 15 ? 5 : size <= 23 ? 6 : size <= 31 ? 6 : size <= 39 ? 7 : 8;
        while (scavengers.size() < amount) {
            val tile = findSpawnTile();
            if (!tile.isPresent()) {
                break;
            }
            val beast = new ScavengerBeast(this, raid, tile.get());
