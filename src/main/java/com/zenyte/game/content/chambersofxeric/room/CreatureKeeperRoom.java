...
public final class CreatureKeeperRoom extends RaidArea implements CycleProcessPlugin {
     ...
     */
    private static final Location[][] scavengerPositions = new Location[][]{
            new Location[]{
                    new Location(3271, 5390, 0), new Location(3272, 5390, 0), new Location(3272, 5391, 0),
                    new Location(3271, 5391, 0), new Location(3271, 5394, 0)
            },
            new Location[]{
                    new Location(3318, 5397, 0), new Location(3319, 5397, 0), new Location(3319, 5398, 0),
                    new Location(3318, 5398, 0), new Location(3321, 5398, 0)
            },
            new Location[]{
                    new Location(3350, 5397, 0), new Location(3351, 5397, 0), new Location(3351, 5398, 0),
                    new Location(3350, 5398, 0), new Location(3351, 5394, 0)
            }
    };
    ...
    private static final Location[] storageUnits = new Location[]{
            new Location(3274, 5388, 0), new Location(3316, 5395, 0),
            new Location(3348, 5400, 0)
    };
    /**
    ...
    public CreatureKeeperRoom(final RaidRoom type, final Raid raid, final int rotation, final int size, final int regionX,
                              final int regionY, final int chunkX, final int chunkY, final int fromPlane, final int toPlane) {
    ...
    public static final void onServerLaunch(final ServerLaunchEvent event) {
        val firstRoomList = new ObjectArrayList<WorldObject>(64);
        val secondRoomList = new ObjectArrayList<WorldObject>(64);
        val thirdRoomList = new ObjectArrayList<WorldObject>(64);
        staticChests.add(firstRoomList);
        staticChests.add(secondRoomList);
        staticChests.add(thirdRoomList);
        val objects = World.getRegion(13140, true).getObjects().values();
        for (val object : objects) {
            if (object == null || object.getId() != 29742) {
                continue;
            }
            val list = object.getX() < 3294 ? firstRoomList : secondRoomList;
            list.add(object);
        }

        for (val object : World.getRegion(13396, true).getObjects().values()) {
            if (object == null || object.getId() != 29742) {
                continue;
            }
    ...
    public int processTrough() {
        ...
        if (trough.stage > 0) {
            val amount = Math.max(Math.min(Math.min(5, trough.stage / 6), trough.stage), 1);
            trough.stage -= amount;
            return amount;
        }
        return 0;
   ...
   /* @Override
    ...
    public void loadRoom() {
        val staticSpawns = staticChests.get(index);
        assert staticSpawns != null;
        val usedChests = new IntOpenHashSet();
        ...
        while (this.poisonHashes.size() < 3) {
            val hash = getLocation(staticSpawns.get(Utils.random(staticSpawns.size() - 1))).getPositionHash();
        ...
        scavenger = new CorruptedScavenger(raid, this, NpcId.CORRUPTED_SCAVENGER, getLocation(scavengerPositions[index][getRotation()]),
                Direction.mainDirections[(index + getRotation()) & 0x3]);
        scavenger.spawn();
        for (int x = scavenger.getX(); x < scavenger.getX() + scavenger.getSize(); x++) {
            for (int y = scavenger.getY(); y < scavenger.getY() + scavenger.getSize(); y++) {
                World.getRegion(Location.getRegionId(x, y), true).addFlag(scavenger.getPlane(), x & 0x3F, y & 0x3F, Flags.FLOOR);
            }
        }
    ...
    private final void recalculateHayPosition() {
        val hay = new Location(Integer.MAX_VALUE);
        for (int x = this.hay.getX() - 4; x < this.hay.getX() + 4; x++) {
            for (int y = this.hay.getY() - 4; y < this.hay.getY() + 4; y++) {
                val region = World.getRegion(Location.getRegionId(x, y), true);
                val objects = region.getObjects(getToPlane(), x & 0x3F, y & 0x3F);
                val matchingObject = Utils.findMatching(objects, obj -> obj != null && (obj.getId() == 3471 || obj.getId() == 3472));
    ...
    private static final class ChestLootingAction extends Action {
        ...
        public int processWithDelay() {
            ...
            val lockpick = player.getInventory().containsAnyOf(1523, 1) || player.getInventory().containsItem(11682, 1);
            val success = (int) (lockpick ? (49F + (player.getSkills().getLevel(Skills.THIEVING) / 4.95F)) : (35F + (player.getSkills().getLevel(Skills.THIEVING) / 7F)));
            if (Utils.random(100) <= success) {
                player.unlock();
                val typeInt = new MutableInt(room.batHash == chest.getPositionHash() ? 20 : room.poisonHashes.contains(chest.getPositionHash()) ? 19 : Utils.random(18));
                //Cannot find anymore grubs once the scavenger has been fed.
                if (room.scavenger.isFed() && typeInt.intValue() < 12) {
                    typeInt.setValue(12);
                }
                val type = typeInt.intValue();
                if (type != 20) {
                ...
                if (type < 10) {
                    ...
                    chest.setId(29745);
                    val thievingLevel = player.getSkills().getLevelForXp(Skills.THIEVING);
                    val amount = new MutableInt(1);
                    val random = Utils.random(49);
                    if (thievingLevel >= 95) {
