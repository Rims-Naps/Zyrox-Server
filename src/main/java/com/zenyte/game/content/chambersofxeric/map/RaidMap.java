...
public class RaidMap {
     ...
     */
    @Getter private final MapAlgorithm algorithm;
    ...
    public void construct() throws OutOfSpaceException {
        ...
        val boundaries = algorithm.boundaries;
        assert boundaries != null;
        val width = (int) (boundaries.getMaxX() - boundaries.getMinX());
        val height = (int) (boundaries.getMaxY() - boundaries.getMinY());
    ...
    private void constructBoss() throws OutOfSpaceException {
        val bossArea = MapBuilder.findEmptyChunk(5, 7);
        ...
        for (int z = 3; z >= 2; z--) {
            for (int x = allocatedArea.getChunkX(); x < (allocatedArea.getChunkX() + 16); x++) {
                for (int y = allocatedArea.getChunkY(); y < (allocatedArea.getChunkY() + 8); y++) {
                    val wrapperChunk = new WrapperChunk(1, 1, 416, 647, x, y, 0, z, Utils.random(3));
    ...
    private void constructRooms(@NotNull final AllocatedArea allocatedArea, @Nullable final MapPalette floorPalette, final int height) {
        ...
        for (val room : floorPalette) {
            val instance = getChunk(room.room, room.chunkDirection, (room.x * 4) + allocatedArea.getChunkX(), (room.y * 4) + allocatedArea.getChunkY(), room.direction.ordinal(), height);
    ...
    private final void bindTeleportationPositions() {
        ...
        for (int index = 0; index < raidChunks.size(); index++) {
            val chunk = raidChunks.get(index);
            ...
            if (type == RaidRoom.RAID_START) {
                chunk.setBoundTile(chunk.getRespectiveTile(chunk.getLocation(3278, 5187, chunk.getToPlane()),
                        chunk.getLocation(3298, 5188, 0), chunk.getLocation(3330, 5188, chunk.getToPlane())));
            } else if (type == RaidRoom.FLOOR_START_UPSTAIRS) {
                val previousRoom = raidChunks.get(index - 1);
                chunk.setBoundTile(previousRoom.getRespectiveTile(previousRoom.getLocation(3278, 5168, previousRoom.getToPlane()),
                        previousRoom.getLocation(3308, 5173, previousRoom.getToPlane()), null));
            } else {
                val nextRoom = index == raidChunks.size() - 1 ?
                               boss :
                               raidChunks.get(index + 1);
                chunk.setBoundTile(nextRoom == boss ?
                                   boss.getEntrance() :
                                   nextRoom.getRespectiveTile(nextRoom.getLocation(3280, 5711, nextRoom.getToPlane()),
                                           nextRoom.getLocation(3312, 5711, nextRoom.getToPlane()), nextRoom.getLocation(3344, 5711, nextRoom.getToPlane())));
                if (nextRoom == boss) {
                    boss.setBoundTile(chunk.getRespectiveTile(chunk.getLocation(3278, 5168, chunk.getToPlane()),
                            chunk.getLocation(3308, 5173, chunk.getToPlane()), null));
                }
            }
        }
    }
    ...
    private RaidArea getChunk(final RaidRoom room, final int direction, final int x, final int y, final int rotation, final int toPlane) {
        try {
            return room.getParentClass().getDeclaredConstructor(arguments)
                    .newInstance(room, raid, rotation, 4, 408 + (4 * (direction)), room.getStaticChunkY(), x, y, room.getHeight(), toPlane);
