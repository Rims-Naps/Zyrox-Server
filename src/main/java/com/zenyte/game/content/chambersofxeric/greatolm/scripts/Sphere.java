...
public final class Sphere implements OlmCombatScript {
	...
	public void handle(final GreatOlm olm) {
		...
		val typelist = new IntArrayList(Arrays.asList(0, 1, 2));
		...
		while (random-- > 0) {
			...
			WorldTasksManager.schedule(new WorldTask() {
				...
				public void run() {
					...
					val hit = new Hit(olm, Math.min(99, player.getHitpoints() / 2), HitType.REGULAR);
					...
					hit.setPredicate(h -> !(type == 0 && !prayer.isActive(Prayer.PROTECT_FROM_MELEE)
                            || type == 1 && !prayer.isActive(Prayer.PROTECT_FROM_MISSILES)
                            || type == 2 && !prayer.isActive(Prayer.PROTECT_FROM_MAGIC)));
					if ((type == 0 && !prayer.isActive(Prayer.PROTECT_FROM_MELEE)
							|| type == 1 && !prayer.isActive(Prayer.PROTECT_FROM_MISSILES)
							|| type == 2 && !prayer.isActive(Prayer.PROTECT_FROM_MAGIC))) {
