...
public final class LizardmanShaman extends RaidNPC<LizardmanShamanRoom> implements CombatScript {
    ...
    public void processNPC() {
        ...
        if (getCombat().getTarget() != null && Utils.random(60) == 0 && lastTime < Utils.currentTimeMillis()) {
            ...
            if (target instanceof Player) {
                ...
                val players = CharacterLoop.find(getMiddleLocation(), 2, Player.class, player -> !player.isDead());
                for (val p : players) {
    ...
    public int attack(final Entity target) {
                    ...
                    public void run() {
                        ...
                        if (ticks == 0) {
                            ...
                            CharacterLoop.forEach(targetTile, 15, Player.class, p -> {
                                val action = p.getActionManager().getAction();
                            ...
                            for (val p : CharacterLoop.find(targetTile, 1, Player.class,
                                    player -> !player.isDead() && (npc.isMultiArea() || player == target))) {
        ...
        switch (style) {
            case 6:
                ...
                delayHit(npc, 0, target, new Hit(npc, getRandomMaxHit(npc, (int) (24 * (raid.isChallengeMode() ? 1.5F : 1F)), CRUSH, target), HitType.MELEE));
                break;
            case 4:
                ...
                val perfectTime = poisonProj.getProjectileDuration(npc.getLocation(), tt);
                ...
                WorldTasksManager.schedule(new WorldTask() {
                    ...
                    public void run() {
                        ...
                        for (val p : CharacterLoop.find(tt, 2, Player.class, player -> !player.isDead() && (npc.isMultiArea() || player == target))) {
                            if (p != target && room.getSafePolygon().contains(p.getLocation())) {
                                continue;
                            }
                            delayHit(-1, p,
                                    new Hit(npc, (int) (Utils.random(20, 35) * com.zenyte.game.world.entity.npc.impl.LizardmanShaman.protectionModifier(p)), HitType.POISON).onLand(hit -> {
                                if (Utils.random(3) == 0) {
            ...
            default:
                ...
                delayHit(npc, rangedProj.getTime(npc.getLocation(), target.getLocation()), target, new Hit(npc, getRandomMaxHit(npc, (int) (24 * (raid.isChallengeMode() ? 1.5F : 1F)), RANGED, target),
                        HitType.RANGED));
                break;
        }
        return 4;
    ...
    public void onDeath(final Entity source) {
        ...
        if (source instanceof Player) {
            ...
            if (!player.getBooleanAttribute("has_taken_damage_from_shaman_in_raid") && !player.getBooleanAttribute("elite-combat-achievement44")) {
                player.putBooleanAttribute("elite-combat-achievement44", true);
                EliteTasks.sendEliteCompletion(player, 44);
            }
        }
    }
