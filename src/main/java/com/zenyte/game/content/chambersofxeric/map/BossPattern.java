...
public enum BossPattern {
    ...
    ROTATION_1(TEKTON, VASA_NISTIRIO, GUARDIANS, DARK_ALTAR_ROOM, LIZARDMEN_SHAMAN, MUTTADILES, VANGUARD, VESPULA),
    ROTATION_2(TEKTON, MUTTADILES, GUARDIANS, VESPULA, LIZARDMEN_SHAMAN, VASA_NISTIRIO, VANGUARD, DARK_ALTAR_ROOM);

    @Getter
    private static final BossPattern[] values = values();
    @Getter private final RaidRoom[] rooms;
    ...
    public final List<RaidRoom> random(final int length) {
        ...
        val list = new ObjectArrayList<RaidRoom>(length);
        val index = Utils.random(rooms.length - 1);
