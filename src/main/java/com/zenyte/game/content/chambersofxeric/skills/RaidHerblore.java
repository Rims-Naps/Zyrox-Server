...
public final class RaidHerblore {
    ...
    public enum RaidPotion {
        ...
        WEAK_ELDER_POTION(47, 6, 20916, 20910, RaidHerb.GOLPAR),
        WEAK_TWISTED_POTION(47, 6, 20928, 20912, RaidHerb.GOLPAR),
        WEAK_KODAI_POTION(47, 6, 20940, 20911, RaidHerb.GOLPAR),
        ELDER_POTION(59, 9, 20920, 20910, RaidHerb.GOLPAR),
        TWISTED_POTION(59, 9, 20932, 20912, RaidHerb.GOLPAR),
        KODAI_POTION(59, 9, 20944, 20911, RaidHerb.GOLPAR),
        STRONG_ELDER_POTION(70, 13, 20924, 20910, RaidHerb.GOLPAR),
        STRONG_TWISTED_POTION(70, 13, 20936, 20912, RaidHerb.GOLPAR),
        STRONG_KODAI_POTION(70, 13, 20948, 20911, RaidHerb.GOLPAR),
        WEAK_REVITALISATION(52, 10, 20952, 20910, RaidHerb.BUCHU),
        WEAK_PRAYER_ENHANCE(52, 10, 20964, 20912, RaidHerb.BUCHU),
        WEAK_XERICS_AID(52, 10, 20976, 20911, RaidHerb.BUCHU),
        REVITALISATION(65, 17, 20956, 20910, RaidHerb.BUCHU),
        PRAYER_ENHANCE(65, 17, 20968, 20912, RaidHerb.BUCHU),
        XERICS_AID(65, 17, 20980, 20911, RaidHerb.BUCHU),
        STRONG_REVITALISATION(78, 26.5, 20960, 20910, RaidHerb.BUCHU),
        STRONG_PRAYER_ENHANCE(78, 26.5, 20972, 20912, RaidHerb.BUCHU),
        STRONG_XERICS_AID(78, 26.5, 20984, 20911, RaidHerb.BUCHU),
        WEAK_OVERLOAD(60, 30, 20988, new int[]{20916, 20928, 20940}, RaidHerb.NOXIFER),
        OVERLOAD(75, 50, 20992, new int[]{20920, 20932, 20944}, RaidHerb.NOXIFER),
        STRONG_OVERLOAD(90, 70, 20996, new int[]{20924, 20936, 20948}, RaidHerb.NOXIFER);
         ...
         */
        private static final RaidPotion[][] groupedValues = new RaidPotion[][]{
                new RaidPotion[]{WEAK_ELDER_POTION, ELDER_POTION, STRONG_ELDER_POTION},
                new RaidPotion[]{WEAK_TWISTED_POTION, TWISTED_POTION, STRONG_TWISTED_POTION},
                new RaidPotion[]{WEAK_KODAI_POTION, KODAI_POTION, STRONG_KODAI_POTION},
                new RaidPotion[]{WEAK_REVITALISATION, REVITALISATION, STRONG_REVITALISATION},
                new RaidPotion[]{WEAK_PRAYER_ENHANCE, PRAYER_ENHANCE, STRONG_PRAYER_ENHANCE},
                new RaidPotion[]{WEAK_XERICS_AID, XERICS_AID, STRONG_XERICS_AID},
        ...
        public static final Item[] getPotions(@NotNull final Player player, @NotNull final Item used, @NotNull final Item usedWith) {
            val vial = used.getId() == 20801 ? used : usedWith.getId() == 20801 ? usedWith : null;
            val other = vial == null ? null : vial == used ? usedWith : used;
            val list = new ObjectArrayList<Item>();
            val level = player.getSkills().getLevel(Skills.HERBLORE);
            for (val pots : RaidPotion.groupedValues) {
                RaidPotion potion = null;
                for (val pot : pots) {
                    if ((vial != null && (pot.herb.clean == other.getId() || ArrayUtils.contains(pot.secondaryIngredient, other.getId())))
                            || (pot.herb.clean == used.getId() && ArrayUtils.contains(pot.secondaryIngredient, usedWith.getId())
                            || pot.herb.clean == usedWith.getId() && ArrayUtils.contains(pot.secondaryIngredient, used.getId()))) {
                        if (potion == null) {
                            potion = pot;
                            continue;
                        }
                ...
                if (potion != null) {
                    val requiredItems = new ArrayList<Item>();
                    for (val secondaries : potion.secondaryIngredient) {
    ...
    public enum RaidHerb {
        ...
        GOLPAR(27, 20904, 20905, 8),
        BUCHU(39, 20907, 20908, 9),
        NOXIFER(55, 20901, 20902, 11);
    ...
    public static final class RaidHerbCleaningAction extends Action {
        ...
        public static final boolean clean(@NotNull final Player player, final int id, @NotNull final OptionalInt slot) {
            val raidHerb = RaidHerb.map.get(id);
            if (raidHerb == null) {
                return false;
            }
            if (player.getSkills().getLevel(Skills.HERBLORE) < raidHerb.level) {
                player.sendMessage("You need a Herblore level of at least " + raidHerb.level + " to clean a "
                        + raidHerb.toString().toLowerCase() + " leaf.");
                return false;
            }
            if (slot.isPresent()) {
                player.getInventory().replaceItem(raidHerb.clean, 1, slot.getAsInt());
            } else {
                val result = player.getInventory().deleteItem(id, 1).getSucceededAmount();
                if (result == 0) {
                    return false;
                }
            ...
            player.getSkills().addXp(Skills.HERBLORE, raidHerb.xp);
    ...
    public static final class RaidMixDialogue extends SkillDialogue {
        ...
        public void run(final int slotId, final int amount) {
            ...
            val pot = RaidPotion.map.get(item.getId());
            if (pot == null) {
                return;
            }
            if (amount <= 0) {
    ...
    private static final class RaidPotionAction extends Action {
        ...
        @Override
        public boolean start() {
            if (player.getSkills().getLevel(Skills.HERBLORE) < potion.level) {
                player.sendMessage(
                        "You need a Herblore level of at least " + potion.level + " to mix a " + potion.toString().toLowerCase() + ".");
                return false;
            }
            return check();
        }
        ...
        private boolean check() {
            ...
            for (val ing : potion.secondaryIngredient) {
            ...
            if (list != null && !list.isEmpty()) {
                ...
                int index = 0;
                for (val missingIngredients : list) {
                ...
                val potName = potion.toString().toLowerCase().replace("_", " ");
        ...
        public int processWithDelay() {
            ...
            val pot = new Item(potion.potionId, 1);
            pot.setAttribute("brewer", player.getUsername());
            inventory.addItem(pot);
            player.getSkills().addXp(Skills.HERBLORE, potion.xp);
            val builder = new StringBuilder();
            ...
            player.sendFilteredMessage("You mix your " + ItemDefinitions.getOrThrow(potion.herb.clean).getName().toLowerCase() + " together with your " + builder.toString() + " into a potion " +
                    "perfectly!");
            if (--amount <= 0) {
                return -1;
            }
