...
public final class VasaNistirioRoom extends RaidArea {
    ...
    public CombatPointCapCalculator buildPointsCap() {
        return new CombatPointCapCalculator().appendNPC(vasa).appendNPC(new Crystal(raid, this, NpcId.GLOWING_CRYSTAL, vasa.getLocation(), index), 4);
    }
    ...
    public boolean hit(final Player source, final Entity target, final Hit hit, final float modifier) {
        ...
        if (target == vasa && vasa.getId() == NpcId.VASA_NISTIRIO_7567) {
            hit.setDamage(0);
        }
        return true;
    }
    ...
    public void resetCrystal(final int index) {
        ...
        crystals[index] = new Crystal(raid, this, NpcId.GLOWING_CRYSTAL, tile, index);
