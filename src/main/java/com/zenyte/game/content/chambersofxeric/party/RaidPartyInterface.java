...
public class RaidPartyInterface extends Interface {
    ...
    public static final void refresh(@NotNull final Player player, @NotNull final Raid raid) {
        val started = raid.getStage() > 0;
        val collection = started ? raid.getPlayers() : raid.getParty().getChannel().getMembers();
        ...
        collection.forEach(p -> player.getPacketDispatcher().sendClientScript(1548, mutableInt.getAndIncrement(),
                (!started || raid.getPlayers().contains(p) ? "<col=ffffff>" : "<col=969696>") + p.getName() + "</col>|"
                        + p.getSkills().getCombatLevel() + "|" + p.getSkills().getTotalLevel()));
    ...
    protected void build() {
        ...
        bind("Start raid", player -> player.getRaid().ifPresent(raid -> player.getDialogueManager().start(new Dialogue(player) {
            ...
            public void buildDialogue() {
                options("No-one may join the party after the raid begins.", new DialogueOption("Begin the raid.", raid::load),
                        new DialogueOption("Don't begin the raid yet."));
            }
        })));
    }
