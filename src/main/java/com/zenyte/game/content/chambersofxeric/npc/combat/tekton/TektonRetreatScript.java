	...
	public void execute(final Tekton tekton) {
		...
		WorldTasksManager.schedule(new WorldTask() {
			...
			public void run() {
				...
				if (ticks == 0) {
					val respawnTile = tekton.getRespawnTile();
					val steps = RouteFinder.findRoute(tekton.getX(), tekton.getY(), tekton.getPlane(), tekton.getSize(), new TileStrategy(respawnTile.getX(), respawnTile.getY()), true);
