...
public final class OlmRoom extends BossChunk implements CycleProcessPlugin, FullMovementPlugin, PlayerCombatPlugin {
     ...
     */
    @Setter
    private Location center, entrance;
    /**
     * The Gear Olm NPC.
     */
    ...
    public static final int getMovementDirection(@NotNull final Player player, @NotNull final Location tile) {
        val offsetX = player.getX() - tile.getX();
        val offsetY = player.getY() - tile.getY();
        if (offsetX < 0) {
    ...
    public void rise() {
        ...
        World.spawnObject(
                headObject = new WorldObject(29880, 10, side == LEFT ? leftSideHeadObject.getRotation() : rightSideheaDObject.getRotation(),
                        getLocation(side == LEFT ? leftSideHeadObject : rightSideheaDObject)));
        World.spawnObject(leftClawObject = new WorldObject(29883, 10,
                side == LEFT ? leftSideLeftClawObject.getRotation() : rightSideLeftClawObject.getRotation(),
                getLocation(side == LEFT ? leftSideLeftClawObject : rightSideLeftClawObject)));
        World.spawnObject(rightClawObject = new WorldObject(29886, 10,
                side == LEFT ? leftSideRightClawObject.getRotation() : rightSideRightClawObject.getRotation(),
                getLocation(side == LEFT ? leftSideRightClawObject : rightSideRightClawObject)));
        WorldTasksManager.schedule(new WorldTask() {
            ...
            public void run() {
                ...
                if (ticks++ == 0) {
                    ...
                    World.spawnObject(headObject = new WorldObject(29881, 10,
                            side == LEFT ? leftSideHeadObject.getRotation() : rightSideheaDObject.getRotation(),
                            getLocation(side == LEFT ? leftSideHeadObject : rightSideheaDObject)));
                    World.spawnObject(leftClawObject = new WorldObject(29884, 10,
                            side == LEFT ? leftSideLeftClawObject.getRotation() : rightSideLeftClawObject.getRotation(),
                            getLocation(side == LEFT ? leftSideLeftClawObject : rightSideLeftClawObject)));
                    World.spawnObject(rightClawObject = new WorldObject(29887, 10,
                            side == LEFT ? leftSideRightClawObject.getRotation() : rightSideRightClawObject.getRotation(),
                            getLocation(side == LEFT ? leftSideRightClawObject : rightSideRightClawObject)));
                    val xOffset = side == LEFT ? 3 : 0;
                    ...
                    leftClaw.spawn();
                    val phases = new IntArrayList(new int[]{GreatOlm.ACID, GreatOlm.FLAME, GreatOlm.CRYSTAL});
                    val lastPhase = phase;
                    ...
                    if (!olm.isPenultimatePhase()) {
                        val message = "The Great Olm rises with the power of " + (phase == GreatOlm.ACID ? "<col=00ff00>Acid" : phase == GreatOlm.FLAME ? "<col=ff0000>Flame" : "<col=006000>Crystals") + "</col>.";
    ...
    private void fall() {
        ...
        WorldTasksManager.schedule(() -> {
            World.spawnObject(new WorldObject(29882, 10, side == LEFT ? leftSideHeadObject.getRotation() : rightSideheaDObject.getRotation(),
                    getLocation(side == LEFT ? leftSideHeadObject : rightSideheaDObject)));
    ...
    public void process() {
        ...
        for (val player : getPlayers()) {
            if (containsPool(player.getLocation())) {
                ...
                if (player.getAttributes().containsKey("lastOlmPool")) {
                    if (player.getAttributes().get("lastOlmPool").equals(player.getLocation())) {
                        player.putBooleanAttribute("PerfectOlm", false);
                    }
                }
                player.getAttributes().put("lastOlmPool", player.getLocation());
            }
        }
    }
