		...
		WorldTasksManager.schedule(new WorldTask() {
			...
			public void run() {
				...
				if (stage == 0) {
					...
					val possibleTargets = tekton.getPossibleTargets(EntityType.PLAYER);
					...
					if (tekton.isEnteredCombat()) {
                        if (!tekton.isBoostedStats()) {
                            val combat = tekton.getCombatDefinitions();
					...
					if (CombatUtilities.isWithinMeleeDistance(tekton, target)) {
						...
						engage = true;
                        val obj = new WorldObject(30023, 10, 0, tekton.getX(), tekton.getY(), tekton.getPlane());
                        tekton.setBlock(obj);
