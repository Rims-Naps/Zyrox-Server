...
public class CaptainRimor extends NPCPlugin {
    ...
    public void handle() {
        ...
        bind("Layouts", (player, npc) -> {
            ...
            player.getDialogueManager().start(new Dialogue(player, npc) {
                @Override
                public void buildDialogue() {
                    options(new DialogueOption("What are layouts?", key(10)),
                            new DialogueOption("Unlock slots.", key(100)),
                            new DialogueOption("Manage layouts.", key(200)));
                    ...
                    npc("So for example, a possible layout is:<br>F S C C P P C S C F<br>" +
                            "The letters mark the type of the room in the order they will come in.");
                    npc("The letter C stands for a combat room, which can be one of the following rooms:<br>" +
                            "Tekton, Vasa Nistirio, Guardians, Skeletal mystics, Lizardmen shaman, Muttadiles, Vanguards or Vespula.");
                    npc("The letter P stands for a puzzle room, which can be one of the following rooms:<br>" +
                            "Ice demon, Creature keeper(also known as thieving), Tightrope or Crabs.");
                    npc("The letter S stands for a scavenger room. There are two types of scavenger rooms.<br>" +
                            "Lastly, the letter F stands for a farming room. There are two types of farming rooms as well.");
                    ...
                    npc("So if you wish to not get a layout which comes with three puzzles, you can disable the layouts which have three puzzles and that will never occur as long as you" +
                            " lead the party.").executeAction(key(1));

                    val unlockedSlotsCount = player.getNumericAttribute("raids layout count").intValue();
                    ...
                    if (unlockedSlotsCount >= 10) {
                        ...
                        npc(Colour.RED.wrap("WARNING: ") + "Unlocking a slot costs 2.5m gold pieces per slot. This is a permanent unlock, meaning you cannot get your money back.").executeAction(() -> {
                            ...
                            player.sendInputInt("How many slots would you like to unlock?(1-" + (10 - unlockedSlotsCount) + ")", value -> {
                                ...
                                val affordableAmount = inventory.getAmountOf(ItemId.COINS_995) / 2_500_000;
                                val unlockedAmount = Math.min(affordableAmount, value);
                                if (unlockedAmount <= 0) {
                                    player.sendMessage("You cannot afford any unlocks.");
                                    return;
                                }
                                val unlockedCount = player.getNumericAttribute("raids layout count").intValue();
                                val unlockableCount = 10 - unlockedCount;
                                val toUnlock = Math.min(unlockedAmount, unlockableCount);
                                inventory.deleteItem(new Item(ItemId.COINS_995, toUnlock * 2_500_000));
                                player.addAttribute("raids layout count", unlockedCount + toUnlock);
                                player.getDialogueManager().start(new NPCChat(player, npc.getId(),
                                        "Thank you, I've unlocked " + toUnlock + " slot" + (toUnlock == 1 ? "" : "s") +
                                                " for you. You now have " + (toUnlock + unlockedCount) + "/10 slots unlocked."));
                            });
                        });
                    }
    ...
    private List<String> generateList(@NotNull final Player player) {
        val options = RaidPattern.getLayoutCodes();
        val filteredOptions = new ObjectArrayList<String>();
        val currentSettings = player.getNumericAttribute("disabled raids layouts").intValue();
        for (int i = 0; i < options.length; i++) {
            val option = options[i];
            val chars = option.split(" - ")[0].replaceAll("[^FSCP]", Strings.EMPTY);
            val builder = new StringBuilder();
            for (val c : chars.toCharArray()) {
                if (c == 'C') {
            ...
            val firstHalfOfString = string.substring(0, string.length() / 2);
            val secondHalfOfString = string.substring((string.length() / 2) + 1);
            val isDisabled = ((currentSettings >> i) & 0x1) == 0x1;
    ...
    private void sendInterface(@NotNull final Player player) {
        val filteredOptions = generateList(player);
        val currentSettings = player.getNumericAttribute("disabled raids layouts").intValue();
        val unlockedSlotsCount = player.getNumericAttribute("raids layout count").intValue();
        player.getDialogueManager().start(new OptionsMenuD(player, "Select the layouts to disable(" + Integer.bitCount(currentSettings) + "/" + unlockedSlotsCount + ")",
                filteredOptions.toArray(new String[0])) {
            @Override
            public void handleClick(int slotId) {
                val currentSettings = player.getNumericAttribute("disabled raids layouts").intValue();
                val isEnabled = ((currentSettings >> slotId) & 0x1) == 0;
                val unlockedSlotsCount = player.getNumericAttribute("raids layout count").intValue();
