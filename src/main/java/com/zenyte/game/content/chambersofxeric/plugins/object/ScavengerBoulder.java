...
public class ScavengerBoulder implements ObjectAction {
    ...
    public void handleObjectAction(final Player player, final WorldObject object, final String name, final int optionId, final String option) {
        player.getRaid().ifPresent(raid -> {
            if (option.equalsIgnoreCase("Push")) {
                ...
                val blockingObject = (LargeScavengerRoom.BlockingObject) object;
                val direction = player.getX() < object.getX() ? Direction.EAST : player.getX() > object.getX() ? Direction.WEST
                                                                                                               : player.getY() > object.getY() ?
                                                                                                                 Direction.SOUTH : Direction.NORTH;
                if (blockingObject.getLevelRequired() == -1) {
                    val dir = Utils.getMoveDirection(direction.getOffsetX(), direction.getOffsetY());
                ...
                player.getActionManager().setAction(new Action() {
                    ...
                    public boolean start() {
                        if (player.getSkills().getLevel(Skills.STRENGTH) < blockingObject.getLevelRequired()) {
                    ...
                    public int processWithDelay() {
                        ...
                        for (val player : blockingObject.getInteractingPlayers()) {
                        ...
                        val boulder = new BoulderNPC(object, direction, destination -> {
    ...
    private static final class BoulderNPC extends NPC {
        ...
        public void processNPC() {
            val moved = addWalkSteps(getX() + spawnDirection.getOffsetX(), getY() + spawnDirection.getOffsetY(), 1, true);
