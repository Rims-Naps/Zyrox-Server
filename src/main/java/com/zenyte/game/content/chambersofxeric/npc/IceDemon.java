...
public final class IceDemon extends RaidNPC<IceDemonRoom> implements CombatScript {
    ...
    private boolean lastHitWasFireSpell;
    ...
    protected void drop(final Location tile) {
        ...
        if (!killer.getBooleanAttribute("has_taken_damage_from_ice_demon") && !killer.getBooleanAttribute("elite-combat-achievement43")) {
            killer.putBooleanAttribute("elite-combat-achievement43", true);
            EliteTasks.sendEliteCompletion(killer, 43);
        }
        if (lastHitWasFireSpell && !killer.getBooleanAttribute("elite-combat-achievement52")) {
            killer.putBooleanAttribute("elite-combat-achievement52", true);
            EliteTasks.sendEliteCompletion(killer, 52);
        }
        if (!killer.getBooleanAttribute("ice_demon_range_pray") && !killer.getBooleanAttribute("elite-combat-achievement53")) {
            killer.putBooleanAttribute("elite-combat-achievement53", true);
            EliteTasks.sendEliteCompletion(killer, 53);
        }
    }
    ...
    public void handleIngoingHit(final Hit hit) {
        ...
        if (!player.getBooleanAttribute("elite-combat-achievement52")) {
            val weapon = hit.getWeapon();
            if (weapon == CombatSpell.FLAMES_OF_ZAMORAK || weapon == CombatSpell.FIRE_WAVE
                    || weapon == CombatSpell.FIRE_BOLT || weapon == CombatSpell.FIRE_BLAST
                    || weapon == CombatSpell.FIRE_STRIKE || weapon == CombatSpell.FIRE_SURGE) {
                lastHitWasFireSpell = true;
            } else {
                lastHitWasFireSpell = false;
            }
        }
    ...
    public float getXpModifier(final Hit hit) {
        ...
        if (weapon == CombatSpell.FLAMES_OF_ZAMORAK || weapon == CombatSpell.FIRE_WAVE
            ...
            hit.setDamage((int) (hit.getDamage() / 3F));
        }
	...
	public void processNPC() {
		if (stage == 0) {
			...
			percentage = (int) (((float) lifepoints / maxLifepoints) * 100F);
			...
			if (lifepoints <= 0) {
                ...
                for (val player : room.getPlayers()) {
                    player.putBooleanAttribute("has_taken_damage_from_ice_demon", false);
                    player.putBooleanAttribute("ice_demon_range_pray", false);
                }
    ...
    public int attack(final Entity target) {
        val magicPrayer = new MutableBoolean();
        val rangedPrayer = new MutableBoolean();

        getPossibleTargets(EntityType.PLAYER).forEach(t -> {
            val player = (Player) t;
            val prayers = player.getPrayerManager();
            if (prayers.isActive(Prayer.PROTECT_FROM_MISSILES)) {
                rangedPrayer.setTrue();
                player.putBooleanAttribute("ice_demon_range_pray", true);
            } else if (prayers.isActive(Prayer.PROTECT_FROM_MAGIC)) {
                magicPrayer.setTrue();
            }
        });
        val style = (magicPrayer.isTrue() && rangedPrayer.isTrue()) || (magicPrayer.isFalse() && rangedPrayer.isFalse()) ? Utils.random(1) : magicPrayer.isTrue() ? 1 : 0;
        setAnimation(attack);
        val targetTile = new Location(target.getLocation());
        if (style == 0) {
            World.sendSoundEffect(getMiddleLocation(), rangedAttackStartSound);
            val delay = World.sendProjectile(this, targetTile, proj);
            World.sendSoundEffect(targetTile, new SoundEffect(rangedAttackEndSound.getId(), rangedAttackEndSound.getRadius(), proj.getProjectileDuration(this.getLocation(),
                    targetTile) - 15));
            WorldTasksManager.schedule(() -> {
                ...
                for (val p : getRoom().getRaid().getPlayers()) {
                    if (p.getLocation().withinDistance(targetTile, 1)) {
                        p.applyHit(new Hit(IceDemon.this, getRandomMaxHit(IceDemon.this, (int) (30 * (room.getRaid().isChallengeMode() ? 1.5F : 1F)), RANGED, p), HitType.REGULAR));
                    }
                }
            ...
            WorldTasksManager.schedule(() -> {
                ...
                for (val p : getRoom().getRaid().getPlayers()) {
                    if (p.getLocation().withinDistance(targetTile, 1)) {
                        p.applyHit(new Hit(IceDemon.this, getRandomMaxHit(IceDemon.this, (int) (25 * (room.getRaid().isChallengeMode() ? 1.5F : 1F)), MAGIC, p), HitType.REGULAR));
