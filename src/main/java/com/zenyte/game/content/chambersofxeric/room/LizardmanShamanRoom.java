...
public final class LizardmanShamanRoom extends RaidArea implements CycleProcessPlugin {
     ...
     */
    private static final Location[][] lizardmanSpawnLocations = new Location[][]{
            new Location[]{
                    new Location(3273, 5270, 0), new Location(3281, 5269, 0), new Location(3278, 5262, 0),
                    new Location(3273, 5255, 0)
            },
            new Location[]{
                    new Location(3306, 5260, 0), new Location(3306, 5267, 0), new Location(3315, 5267, 0),
                    new Location(3316, 5259, 0)
            },
            new Location[]{
                    new Location(3340, 5258, 0), new Location(3346, 5259, 0), new Location(3341, 5268, 0),
                    new Location(3347, 5270, 0)
            }
    };
     ...
     */
    private static final Location[][] spiritTendrilSpawnLocations = new Location[][]{
            new Location[]{
                    new Location(3279, 5249, 0),
                    new Location(3280, 5249, 0)
            },
            new Location[]{
                    new Location(3311, 5251, 0),
                    new Location(3310, 5251, 0)
            },
            new Location[]{
                    new Location(3342, 5251, 0),
                    new Location(3343, 5251, 0)
            }
    };
     ...
     */
    private static final int[][][] safePolygons = new int[][][] {
            new int[][]{
                    { 3290, 5263 },
                    { 3290, 5261 },
                    { 3291, 5260 },
     ...
     */
    private static final Location[] extraSpiritTendrils = new Location[]{
            new Location(3289, 5262, 0), new Location(3289, 5261, 0)
    };
    /**
     * A 2D array containing coordinates to where the spawns blocking the exit are.
     */
    private static final Location[][] blockingExitLocations = new Location[][]{
            new Location[]{
                    new Location(3268, 5263, 0), new Location(3269, 5263, 0), new Location(3268, 5264, 0),
                    new Location(3269, 5264, 0)
            },
            new Location[]{
                    new Location(3311, 5275, 0), new Location(3311, 5274, 0), new Location(3312, 5275, 0),
                    new Location(3312, 5274, 0)
            },
            new Location[]{
                    new Location(3353, 5265, 0), new Location(3352, 5265, 0), new Location(3353, 5264, 0),
                    new Location(3352, 5264, 0)
            }
    };
    /**
     ...
     */
    @Getter private final RSPolygon safePolygon;

    public LizardmanShamanRoom(final RaidRoom type, final Raid raid, final int rotation, final int size, final int regionX,
        ...
        val polyPoints = safePolygons[index];
        val intArray = new int[polyPoints.length][2];
        for (int i = 0; i < polyPoints.length; i++) {
            val tile = getLocation(polyPoints[i][0], polyPoints[i][1], toPlane);
    ...
    public void loadRoom() {
        ...
        while (shamans.size() < amount) {
            shamans.add((LizardmanShaman) new LizardmanShaman(raid, this, NpcId.LIZARDMAN_SHAMAN_7573 + Utils.random(1), getLocation(tiles.get(Utils.random(tiles.size() - 1)))).spawn());
        }
    ...
    public boolean isDead() {
        for (val lizardman : this.shamans) {
            if (!lizardman.isDead()) {
                return false;
            }
        }
     ...
     */
    private void wipe() {
        for (val tendril : spiritTendrilSpawnLocations[index]) {
            World.removeObject(World.getObjectWithType(getLocation(tendril), 10));
        }
        if (index == 0) {
            for (val tendril : extraSpiritTendrils) {
                World.removeObject(World.getObjectWithType(getLocation(tendril), 10));
            }
        }
    }
    ...
    private static final class BlockingSpawn extends NPC {
        ...
        BlockingSpawn(final Location tile) {
            ...
            World.getRegion(getLocation().getRegionId()).clip(getPlane() & 0x3, getX() & 0x3f, getY() & 0x3f);
        }
        ...
        public void sendDeath() {
            ...
            WorldTasksManager.schedule(new WorldTask() {
                @Override
                public void run() {
                    World.getRegion(getLocation().getRegionId()).unclip(getPlane() & 0x3, getX() & 0x3f, getY() & 0x3f);
