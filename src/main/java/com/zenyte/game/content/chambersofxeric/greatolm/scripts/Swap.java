...
public final class Swap implements OlmCombatScript {
	...
	public void handle(final GreatOlm olm) {
		...
		int pairCount = (int) Math.min(4, Math.max(1, Math.floor(chamberSize / 2F)));
		while (pairCount-- > 0) {
			...
			if (!everyone.isEmpty()) {
				...
				secondary = null;
				tile = olm.randomLocation(GreatOlm.ENTIRE_CHAMBER);
			}
				...
				case 2:
					...
					pairs.forEach(pair -> {
						...
						if (distance == 0) {
							...
							val secondaryPlayer = pair.getSecondary();
							final Location secondary = secondaryPlayer == null ? pair.getTile()
                              : secondaryPlayer.getTemporaryAttributes().get("deathSpot") instanceof Location
                              ? new Location((Location) pair.getSecondary().getTemporaryAttributes().get("deathSpot"))
                              : new Location(secondaryPlayer.getLocation());
							if (secondary == null) {
							...
							pair.getPrimary().putBooleanAttribute("PerfectOlm", false);
							...
							if (pair.getSecondary() != null) {
								...
								pair.getSecondary().putBooleanAttribute("PerfectOlm", false);
	...
	private static final class PlayerPair {
		...
		@Getter private final Player primary;
		@Getter @Setter private Player secondary;
		@Getter @Setter private Location tile;
		@Getter private final Graphics colour;
