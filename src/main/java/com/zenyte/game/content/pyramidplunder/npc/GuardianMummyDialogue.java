...
public class GuardianMummyDialogue extends Dialogue {
    ...
    public void buildDialogue() {
        val chargeKey = player.carryingItem(PHARAOH_SCEPTRE.first()) ? CHARGE_KEY
                : player.carryingAny(PHARAOH_SCEPTRE.getAllButFirst()) ? REMOVE_CHARGE_KEY : MISSING_SCEPTRE_KEY;
        {
            ...
            player("If I gave you back some of the artefacts I've taken from the tomb, would you recharge the" +
                    " sceptre for me?");
            npc("*sigh* Oh alright. But only if the sceptre is fully empty, I'm not wasting the King's magic." +
                    "..");
            options("Recharge the sceptre with...",
                    new DialogueOption("Gold artefacts?", () -> {
                    ...
                    new DialogueOption("Actually, I'm more interested in plundering the tombs.",
                            () -> setKey(1)));
        ...
        if (!onlyChargeDialogue) {
            options(1, "Play the 'Pyramid Plunder' minigame?",
                    new DialogueOption("That sounds like fun, what do I do?", key(GAME_EXPLANATION_KEY)),
                    new DialogueOption("Not right now."),
                    new DialogueOption("I know what I'm doing - let's get on with it.", key(READY_KEY)),
                    new DialogueOption("I want to charge or remove charges from my sceptre.", key(chargeKey)));

            {
                ...
                npc("There are eight treasure rooms, each subsequent room requires higher thieving skills to both" +
                        " enter the room and thieve from the urns and other containers.");
                ...
                npc("There are four possible exits, you must open the door before finding out whether it is the " +
                        "exit" +
                        " or not.");
                npc("Opening the doors require picking their locks. Having a lockpick will make this easier.")
                        .executeAction(() -> setKey(SECOND_OPTION_KEY));
            }
            {
            ...
            options(SECOND_OPTION_KEY, "Do you have any more questions?",
                    new DialogueOption("How do I leave the game?", key(LEAVE_GAME_KEY)),
                    new DialogueOption("Tell me about these artefacts.", key(EXPLAIN_ARTEFACTS_KEY)),
                    new DialogueOption("What do I do with the artefacts I collect?", key(SELL_ARTEFACTS_KEY)),
                    new DialogueOption("I'm ready to give it a go now.", key(READY_KEY)));

            {
                player(LEAVE_GAME_KEY, "How do I leave the game?");
                npc("If at any point you decide you need to leave just use a glowing door.");
                npc("The game will end and you will be taken out of the pyramid.")
                        .executeAction(() -> setKey(SECOND_OPTION_KEY));
            }
            {
                player(EXPLAIN_ARTEFACTS_KEY, "Tell me about these artefacts.");
                npc("There are a number of different artefacts, of three main types. The least valuable are the" +
                        " pottery statuettes and scarabs, and the ivory combs.");
                npc("Next are the stone scarabs, statuettes and seals, and finally the gold versions of those" +
                        " artefacts.");
                npc("They are not old, but are well made. The urns contains all types of artefacts while the " +
                        "chest and sarcophagus contain stone and gold artefacts.")
                        .executeAction(() -> setKey(SECOND_OPTION_KEY));
            }
            {
                ...
                npc("He usually slinks about near the pyramid north-east of Sophanem. I expect he's trying to get" +
                        " some poor fools to steal things from that pyramid as well.");
                npc("I expect he'll give you some more gold for some than others.")
                        .executeAction(() -> setKey(SECOND_OPTION_KEY));
            }
    ...
    public static void uncharge(final Player player) {
        if (player.carryingAny(PHARAOH_SCEPTRE.getAllButFirst())) { // Check if players inv or equipment has a sceptre
            return;
        }
        val container = player.getInventory().containsAnyOf(PHARAOH_SCEPTRE.getAllButFirst()) ?
                player.getInventory().getContainer()
                : player.getEquipment().getContainer();
        val sceptre = container.getAny(PHARAOH_SCEPTRE.getAllButFirst());
        if (sceptre == null) {
            return;
        }
        val slot = container.getSlot(sceptre);
        if (slot == -1) {
            return;
        }
    ...
    private void charge(final Player player, final int key) {
        val container = player.getInventory().containsItem(PHARAOH_SCEPTRE.first()) ? player.getInventory().getContainer()
                : player.getEquipment().getContainer();
        val slot = container.getSlotOf(PHARAOH_SCEPTRE.first());
        if (slot == -1) {
            return;
        }
        val materials = new ArrayList<Item>();
        if (key == GOLD_KEY) {
        ...
        val amount = getNeededMaterialAmount(key);
    ...
    private List<Item> getCarriedMaterials(final Player player, final int key, final PlunderRewardTier tier) {
        val materials = new ArrayList<Item>();
        val amountNeeded = getNeededMaterialAmount(key);
        for (PlunderReward plunderReward : PlunderReward.get(tier)) {
            val rewardIds = new int[] { plunderReward.getItemId(), ItemDefinitions.get(plunderReward.getItemId()).getNotedId()};
            for (int rewardId : rewardIds) {
                if (player.carryingItem(rewardId)) {
                    val inventory = player.getInventory();
                    val item = inventory.getAny(rewardId);
                    if (item == null) {
                        continue;
                    }
                    val id = item.getId();
                    val amount = Math.min(amountNeeded -  materials.size(), inventory.getAmountOf(id));
