...
public class SpeartrapObject implements ObjectAction {
    ...
    public void handleObjectAction(Player player, WorldObject object, String name, int optionId, String option) {
        ...
        val trap = SpearTrap.get(currentRoom.getRoomId());
    ...
    private Direction getDirection(@NotNull final Player player, @NotNull WorldObject object, @NotNull final SpearTrap trap) {
        val horizontal = trap.isHorizontal();
        if (horizontal) {
    ...
    public void handle(final Player player, final WorldObject object, final String name, final int optionId, final String option) {
        val roomId = player.getVarManager().getBitValue(PlunderDoor.ROOM_VARBIT);
        if (roomId == 0) {
        ...
        val currentRoom = PlunderRoom.get(roomId);
        val trap = SpearTrap.get(currentRoom.getRoomId());
        val rot = object.getRotation();
        val tile = trap.within(object.getX(), object.getY()) ? object : object.transform(rot == 1 ? Direction.NORTH : rot == 3 ? Direction.SOUTH : rot == 0 ? Direction.WEST : Direction.EAST);
        player.setRouteEvent(new TileEvent(player, new TileStrategy(tile, 1,
                trap.isHorizontal() ? (RouteEvent.BLOCK_FLAG_NORTH | RouteEvent.BLOCK_FLAG_SOUTH) : (RouteEvent.BLOCK_FLAG_EAST | RouteEvent.BLOCK_FLAG_WEST)),
                getRunnable(player, object, name, optionId, option), getDelay()));
    }

    private void attempt(@NotNull final Player player, @NotNull WorldObject object, @NotNull final PlunderRoom room,
                         @NotNull final SpearTrap trap) {
        player.lock(6);
    ...
    private void succeed(@NotNull final Player player, @NotNull WorldObject object, @NotNull final PlunderRoom room,
                         @NotNull final SpearTrap trap) {
