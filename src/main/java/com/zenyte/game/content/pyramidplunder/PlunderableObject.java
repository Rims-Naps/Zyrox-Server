...
public interface PlunderableObject {
    ...
    default void reward(final Player player) {
        val floorBoost = 1 + player.getVarManager().getBitValue(PlunderDoor.ROOM_VARBIT) * 0.05;
        val max = getRewardTiers().size() - 1;
        val randomIndex = Math.min(max, Utils.random((int) ((max + 1) * floorBoost)));
        val tier = getRewardTiers().get(randomIndex);
        val possibleRewards = PlunderReward.get(tier);
        val reward = Utils.getRandomCollectionElement(possibleRewards);
    ...
    default boolean rollForSceptre(final Player player, final WorldObject object) {
        if (Utils.random(749) == 0) {
            ...
            player.lock(1);
            giveSceptre(player);
            WorldBroadcasts.broadcast(player, BroadcastType.RARE_DROP, sceptre.generateResult(),
    ...
    default void giveSceptre(final Player player) {
        val sceptreItem = sceptre.generateResult();
        val result = player.getInventory().addItem(sceptreItem);
        ...
        player.getDialogueManager().start(new Dialogue(player, NpcId.GUARDIAN_MUMMY) {
            ...
            public void buildDialogue() {
                if (result.getResult() == RequestResult.NOT_ENOUGH_SPACE) {
                    item(sceptreItem, "You find a golden sceptre but don't have space to carry the sceptre." +
                            " Clear some space in your backpack and pick the sceptre up, if you want it...");
                } else {
