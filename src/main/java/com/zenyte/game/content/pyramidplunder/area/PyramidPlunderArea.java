...
@Slf4j
public class PyramidPlunderArea extends Area implements CycleProcessPlugin, PartialMovementPlugin, TeleportMovementPlugin {
    ...
    public void process() {
        try {
            val tick = getAreaTimer().intValue();
        ...
        try {
            for (Player player : getPlayers()) {
                ...
                val currentPlunderTick = player.getVarManager().getBitValue(TIMER_VARBIT);
    ...
    public void processMovement(Player player, Location destination) {
        ...
        val currentRoom = player.getVarManager().getBitValue(ROOM_VARBIT);
        if (currentRoom < 1 || currentRoom > 8) {
            updateRoomInfo(player, destination);
            return;
        }
        val plunderRoom = PlunderRoom.rooms[currentRoom - 1];
    ...
    private final void updateRoomInfo(@NotNull final Player player, @NotNull final Location destination) {
        for (val room : PlunderRoom.rooms) {
