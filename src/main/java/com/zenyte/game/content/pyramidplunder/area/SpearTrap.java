...
public enum SpearTrap {
    FIRST_TRAP(1, new Location(1927, 4472, 0), false),
    SECOND_TRAP(2, new Location(1954, 4473, 0), false),
    THIRD_TRAP(3, new Location(1976, 4465, 0), false),
    FOURTH_TRAP(4, new Location(1930, 4452, 0), true),
    FIFTH_TRAP(5, new Location(1961, 4444, 0), true),
    SIXTH_TRAP(6, new Location(1926, 4427, 0), false),
    SEVENTH_TRAP(7, new Location(1944, 4424, 0), false),
    EIGHTH_TRAP(8, new Location(1974, 4423, 0), false);
    ...
    SpearTrap(final int roomId,
              final Location southwesternCorner, final boolean horizontal) {
        this.roomId = roomId;
    ...
    public boolean execute(@NotNull final Player player, final int x, final int y) {
        if (within(x, y)) {
            ...
            val trapObject = findTrap(currentTile);
            val baseDelay = 1;
            val movementDirection = findEmptyMovementTile(currentTile);
            val destination = currentTile.transform(movementDirection);
            ...
            WorldTasksManager.schedule(() -> {
                ...
                val object = World.getObjectWithType(trapObject, 10);
                ...
                val forceMovement = new ForceMovement(player.getLocation(), 10, destination, 30, movementDirection.getDirection());
    ...
    private final Direction findEmptyMovementTile(@NotNull final Location trapActivationTile) {
        for (val direction : Direction.mainDirections) {
            val position = trapActivationTile.transform(direction);
    ...
    public boolean within(final int x, final int y) {
        val swx = southwesternCorner.getX();
        val swy = southwesternCorner.getY();
    ...
    private final WorldObject findTrap(@NotNull final Location trapHitTile) {
        ...
        val objectUnderneath = World.getObjectWithType(trapHitTile, 10);
        ...
        for (val direction : Direction.mainDirections) {
            val position = trapHitTile.transform(direction);
            val object = World.getObjectWithType(position, 10);
