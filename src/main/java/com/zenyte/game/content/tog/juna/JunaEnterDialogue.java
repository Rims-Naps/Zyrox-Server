...
public class JunaEnterDialogue extends Dialogue {
    ...
    public static final ImmutableLocation INSIDE_CAVE_TILE = new ImmutableLocation(3253, 9516, 2);
     ...
     */
    private static final int ENTER_CAVE_INTERVAL_DAY = 7;
    ...
    public void buildDialogue() {
        ...
        if (player.getEquipment().getId(EquipmentSlot.WEAPON) != -1) {
            ...
            val lastAttempt = LocalDate.parse(player.getAttributeOrDefault(LAST_ATTEMPT_DATE_ATTR, LocalDate.ofYearDay(1969, 1).toString()));
            val daysLeft = ENTER_CAVE_INTERVAL_DAY - ChronoUnit.DAYS.between(lastAttempt, LocalDate.now());
            npc("I will not permit any adventurer to access the tears more than once a week. Come back in " + daysLeft + " days.");
        }
    }

    private void enter() {
        val firstTileDistance = player.getLocation().getTileDistance(OUTSIDE_CAVE_TILE);
        val secondTileDistance = player.getLocation().getTileDistance(INSIDE_CAVE_TILE);
        val lastTileDistance = INSIDE_CAVE_TILE.getTileDistance(CAVE_CENTER_TILE);
        player.setRun(false);
    ...
    public static boolean hasTimePassed(@NotNull final Player player) {
        ...
        return ChronoUnit.DAYS.between(lastAttempt, today) >= ENTER_CAVE_INTERVAL_DAY;
    }
}
