...
public class AgilityPyramidArea extends SouthernDesertArea implements CycleProcessPlugin, FullMovementPlugin,
    ...
    public void processMovement(Player player, Location destination) {
        val insideLowerPyramid = polygons.get(0).contains(destination);
        val insideHigherPyramid = polygons.get(1).contains(destination);
        player.getVarManager().sendBit(AgilityPyramid.MOVING_BLOCK_VARBIT, insideLowerPyramid ? destination.getPlane() :
                insideHigherPyramid ? destination.getPlane() + 2 : 0);
    }
}
