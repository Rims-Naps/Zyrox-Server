...
public class Cooking extends Action {
    ...
    public int processWithDelay() {
        ...
        if (isBurning()) {
            ...
            val message = data.getMessage().equals("") ? "You successfully cook the " + ItemDefinitions.get(data.getCooked()).getName().toLowerCase() + "." : data.getMessage();
            if (data.equals(CookingData.WILD_PIE)) {
                ...
                player.getAchievementDiaries().update(WildernessDiary.FISH_AND_COOK_DARK_CRAB, 0x2);
            } else if (data.equals(CookingData.BASS)) {
                player.getAchievementDiaries().update(KandarinDiary.CATCH_AND_COOK_BASS, 0x2);
                ...
                player.getAchievementDiaries().update(KourendDiary.CATCH_ANGLERFISH, 0x2);
            }
            ...
            val item = new Item(data.getCooked(), 1);
    ...
    private boolean check() {
        return (!player.isDead() && !player.isFinished() && CookingData.hasRequirements(player, data) && amount > 0)
                && World.getRegion(object.getRegionId()).containsObject(object.getId(), object.getType(), object);
    }
    ...
    private boolean isBurning() {
        ...
        val hasGauntlets = player.getEquipment().getId(EquipmentSlot.HANDS) == 775;
        val burnLevel = data.getBurnLevel();
        // TODO: "Level at which stops burning" differs per food per circumstance.
        val burnChance = burnLevel - (hasGauntlets ? 5 : 0) - (object.getId() == 21302 ? 5 : 0) - player.getSkills().getLevel(Skills.COOKING) - (BoosterPerks.isActive(player, BoosterPerks.COOKING) ? Utils.random(100) <= 20 ? 5 : 0 : 0);
