...
public class SpearOnHaysack implements PairedItemOnItemPlugin {
    ...
    public ItemPair[] getMatchingPairs() {
        ...
        for (val spear : spears) {
            pairs.add(ItemPair.of(6057, spear));
        }
        return pairs.toArray(new ItemPair[0]);
    }

    private static final int[] spears = new int[] {
            1237, 1239, 1241, 1243, 1245, 1247, 1249, 1251, 1253, 1255, 1257, 1259, 1261, 1263, 4158, 4159, 4580, 4582, 4726, 4910, 4911, 4912, 4913, 5016, 5704, 5706, 5708, 5710
            , 5712, 5714, 5716, 5718, 5720, 5722, 5724, 5726, 5728, 5730, 5734, 5736, 7809, 11824, 20158, 20397, 22610
    };

    private static final int[] permittedSpears = new int[] {
            1237, 1251, 5704, 5718
    };

    @Override
    public void handleItemOnItemAction(final Player player, final Item from, final Item to, final int fromSlot, final int toSlot) {
        val hay = from.getId() == 6057 ? from : to;
        val spear = hay == from ? to : from;
