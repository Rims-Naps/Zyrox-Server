...
public class SeedVaultContainer extends Container {
    private transient final SeedVault seedVault;
    ...
    public ContainerResult add(final Item requestedItem) {
        final Item item = requestedItem == null ? null
                : new Item(requestedItem.getId(), requestedItem.getAmount(), requestedItem.getAttributesCopy());
        ...
        if (predicate != null) {
            if (!predicate.test(null)) {
                val res = new ContainerResult(item, ContainerState.ADD);
        ...
        val placeholderId =
                ItemDefinitions.getOrThrow(PlaceholderRedirections.builder.getOrDefault(definitions.getId(),
                        definitions.getId())).getPlaceholderId();
            ...
            if (stackable && policy == ContainerPolicy.NORMAL || policy == ContainerPolicy.ALWAYS_STACK) {
                ...
                if (slot != -1 || placeholderId != -1 && (slot = getSlotOf(placeholderId)) != -1) {
                    ...
                    if (!existingItem.hasAttributes()) {
                        ...
                        result.setResult(
                                result.getSucceededAmount() != requestedAmount ? RequestResult.OVERFLOW :
        ...
        if (item.hasAttributes() && placeholderSlot == null || policy == ContainerPolicy.NEVER_STACK
                || (slot = getSlotOf(id)) == -1 && placeholderSlot == null && stackable && policy == ContainerPolicy.NORMAL
        ...
        result.setSucceededAmount(requestedAmount - item.getAmount());
        result.setResult(result.getSucceededAmount() != requestedAmount ? RequestResult.OVERFLOW :
                RequestResult.SUCCESS);
        return result;
    }
    ...
    public ContainerResult remove(final Item requestedItem, final boolean placeholder, Player player) {
        ...
        val phId = ItemDefinitions.getOrThrow(PlaceholderRedirections.builder.getOrDefault(definitions.getId(),
                definitions.getId())).getPlaceholderId();
        val itemId = placeholder && phId != -1 ? phId : id;
        val phIdSlots = getSlotsOf(phId);
        ...
        for (final int slot : slots) {
            ...
            set(slot, amount > item.getAmount() ? new Item(id, amount - item.getAmount(), item.getAttributesCopy()) :
                    null);
        ...
        result.setSucceededAmount(requestedAmount - item.getAmount());
        result.setResult(result.getSucceededAmount() != requestedAmount ? RequestResult.NOT_ENOUGH_ITEMS :
                RequestResult.SUCCESS);
        return result;
    }
    ...
    public void deposit(final Player player, final Container container, final int slotId, int amount) {
        ...
        if (!definitions.isStackable() && container.getPolicy() != ContainerPolicy.ALWAYS_STACK) {
            ...
            for (; i < length; i++) {
                ...
                final ContainerResult containerResult = seedVault.add(new Item(item.getId(), 1,
                        invItem.getAttributesCopy()));
            ...
            final ContainerResult containerResult = seedVault.add(new Item(item.getId(), amount,
                    item.getAttributesCopy()));
            final RequestResult result = containerResult.getResult();
            if (result == RequestResult.FAILURE) {
                return;
            }
    ...
    public void withdraw(final Player player, final Container container, int slot, int amount,
                         final boolean note, final boolean placeholder) {
        ...
        val stackable =
                definitions.isStackable() || (!item.hasAttributes() && note && definitions.getNotedId() != -1 && ItemDefinitions.get(definitions.getNotedId()).isStackable());
        ...
        if (!item.hasAttributes() && (stackable || policy == ContainerPolicy.ALWAYS_STACK)) {
            final int existingAmount = (note && definitions.getNotedId() != -1) ?
                    container.getAmountOf(definitions.getNotedId()) : container.getAmountOf(item.getId());
            if (existingAmount + amount < 0) {
                amount = Integer.MAX_VALUE - existingAmount;
            }

            if (!stackable || existingAmount == 0) {
                val freeSlots = container.getFreeSlotsSize();
                if (!stackable) {
                    if (freeSlots < amount) {
                        amount = freeSlots;
                    }
            ...
            val id =
                    !item.hasAttributes() && note && !definitions.isNoted() && definitions.getNotedId() != -1
                            ? definitions.getNotedId()
                            : item.getId();
            final ContainerResult containerResult = seedVault.remove(new Item(item.getId(), amount,
                    item.getAttributesCopy()), placeholder);
            final RequestResult result = containerResult.getResult();
            ...
            for (; i < length; i++) {
                ...
                val id = note && definitions.isNoted() && definitions.getNotedId() != -1
                        ? definitions.getNotedId()
                        : item.getId();
