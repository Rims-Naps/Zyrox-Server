...
public class LimestoneBrickCrafting implements PairedItemOnItemPlugin {
    ...
    public void handleItemOnItemAction(Player player, Item from, Item to, int fromSlot, int toSlot) {
        ...
        val crafting = skills.getLevel(Skills.CRAFTING);
        ...
        val successProbability = BASE_SUCCESS_PROBABILITY + (crafting * successPerLevel);
        val succeeded = Utils.randomDouble() <= successProbability;

        player.setAnimation(animation);
        inventory.deleteItem(ItemId.LIMESTONE, 1);
        if (succeeded) {
            skills.addXp(Skills.CRAFTING, 6);
            ...
            skills.addXp(Skills.CRAFTING, 1.5);
    ...
    public ItemPair[] getMatchingPairs() {
        return new ItemPair[] {
                ItemPair.of(ItemId.CHISEL, ItemId.LIMESTONE)
        };
    }
}
