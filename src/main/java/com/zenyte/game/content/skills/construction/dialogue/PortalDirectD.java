	...
	public void handleClick(int slotId) {
		...
		try {
			...
			for (Item teleportRune : teleportRunes) {
				if (teleportRune.getId() == 1963) 
					runes[idx++] = new Item(1964, teleportRune.getAmount() * 100);
				else
					runes[idx++] = new Item(teleportRune.getId(), teleportRune.getAmount() * 100);
			}
			if (player.getSkills().getLevel(Skills.MAGIC) < portal.getTeleport().getLevel() || !player.getInventory().containsItems(runes)) {
				...
				player.sendMessage("You need " + builder.toString() + " and " + portal.getTeleport().getLevel() + " Magic to direct the portal there.");
				return;
			}
