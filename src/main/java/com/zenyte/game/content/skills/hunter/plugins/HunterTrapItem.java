...
public class HunterTrapItem extends ItemPlugin implements FloorItemPlugin {
    ...
    @Override
    public void handle() {
        bind("Lay", (player, item, slotId) ->
                TrapType.findByItem(item.getId()).ifPresent(type -> player.getActionManager().setAction(new ItemTrapSetupAction(type, null, slotId, new Location(player.getLocation())))));
    }
    ...
    public int[] getItems() {
        ...
        for (val type : TrapType.getValues()) {
            val id = type.getItemId();
            if (id < 0) {
                continue;
            }
            list.add(id);
