...
public final class TuraelAssignmentD extends Dialogue {
	...
	public void buildDialogue() {
		if (player.getSlayer().getAssignment() != null) {
			...
			npc("You need to kill " + player.getSlayer().getAssignment().getAmount() + " "
					+ player.getSlayer().getAssignment().getTask().toString() + ".");
			val data = player.getSlayer().getAssignment().getTask() instanceof BossTask ? null
					: ((RegularTask) player.getSlayer().getAssignment().getTask());
			if (data == null || (player.getSlayer().getMaster() == SlayerMaster.KRYSTILIA
					|| data.getCertainTaskSet(SlayerMaster.TURAEL) == null
					|| data.getCertainTaskSet(SlayerMaster.TURAEL).getMaximumAmount() < player.getSlayer().getAssignment().getAmount())) {
				npc("You're still hunting " + player.getSlayer().getAssignment().getTask().toString() + ", you have "
						+ player.getSlayer().getAssignment().getAmount() + " to go.");
				npc("Although it's not an assignment that I'd normally give..."
						+ " I guess I could  give you a new assignment, if you'd like.");
		...
		val task = slayer.generateTask(SlayerMaster.TURAEL);
