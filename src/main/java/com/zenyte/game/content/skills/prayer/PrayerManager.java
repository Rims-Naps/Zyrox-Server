...
public class PrayerManager {
	...
	@Getter
	private transient final Object2IntOpenHashMap<Prayer> activePrayers = new Object2IntOpenHashMap<>();
	...
	public void togglePrayer(int id, final boolean quickPrayers) {
		...
		val activating = player.getVarManager().getBitValue(prayer.getVarbit()) == 0;
		if (activating) {
	...
	public void activatePrayer(final Prayer prayer) {
        if ((prayer == Prayer.PROTECT_FROM_MAGIC || prayer == Prayer.PROTECT_FROM_MISSILES || prayer == Prayer.PROTECT_FROM_MELEE) &&
                player.getNumericTemporaryAttribute("prayer delay").longValue() > System.currentTimeMillis()) {
	...
	public void toggleQuickPrayers() {
		...
		val on = player.getVarManager().getBitValue(4103) == 1;
		val empty = quickPrayerSettings == 0;
        if (on) {
	...
	public void process() {
		...
		val resistance = 60 + (player.getBonuses().getBonus(Bonuses.Bonus.PRAYER) * 2);
	...
	public boolean hasRequirements(final Prayer prayer, final boolean checkPoints) {
		...
		val settings = player.getSettings();
		...
		if (player.getTemporaryAttributes().get(Sotetseg.SOTETSEG_PRAYER_DISABLE_ATTRIBUTE_KEY) != null && (long) player.getTemporaryAttributes().get(Sotetseg.SOTETSEG_PRAYER_DISABLE_ATTRIBUTE_KEY) > Utils.currentTimeMillis() && (prayer.equals(Prayer.PROTECT_FROM_MAGIC) || prayer.equals(Prayer.PROTECT_FROM_MISSILES) || prayer.equals(Prayer.PROTECT_FROM_MELEE))) {
	...
	public int drainPrayerPoints(final int amount) {
		...
		if (player.getArea() instanceof VorkathInstance) {
			player.putBooleanAttribute("drainedPrayerDuringVorkath", true);
		}
		val points = getPrayerPoints();
		if ((points - amount) >= 0) {
	...
	public int drainPrayerPoints(final double percentage, final int minimumDrain) {
		val currentPoints = getPrayerPoints();
		val amount = Math.max((int) (currentPoints * (percentage / 100F)), minimumDrain);
	...
	public void restorePrayerPoints(final int amount) {
		val level = player.getSkills().getLevelForXp(Skills.PRAYER);
		val points = getPrayerPoints();
		if (points >= level) {
			return;
		}
