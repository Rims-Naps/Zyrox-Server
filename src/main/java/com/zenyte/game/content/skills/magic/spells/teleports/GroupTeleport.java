...
public enum GroupTeleport implements TeleportSpell, Teleport {
    ...
    private final void perform(final Player player) {
        ...
        val state = new SpellState(player, definitions.getLevel(), definitions.getRunes());
        if (!state.check()) {
            return;
        }
        player.findCharacters(3, Player.class, p ->
                                                       !p.isLocked() && !p.isDead() && p.getBooleanSetting(Setting.ACCEPT_AID)
                                                               && !p.getInterfaceHandler().containsInterface(InterfacePosition.CENTRAL)
                                                               && !p.getInterfaceHandler().containsInterface(InterfacePosition.DIALOGUE)
                                                               && p != player
        ).forEach(p -> Teleother.request(player, p, teleport));
        teleport(player);
    }
