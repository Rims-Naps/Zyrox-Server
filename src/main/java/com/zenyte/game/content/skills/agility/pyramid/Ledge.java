...
public class Ledge implements FailableAgilityPyramidObstacle, IrreversibleDirection {
    ...
    private void cross(final Player player, final WorldObject object, final boolean success) {
        val walkDirection = object.getFaceDirection().getCounterClockwiseDirection(6);
        val destination = player.getLocation().transform(walkDirection, 5);
        val sound = new SoundEffect(2451, 1, 20, success ? 5 : 2);
            ...
            public void run() {
                if (ticks == 0) {
                    ...
                    player.getAppearance().setRenderAnimation(new RenderAnimation(RenderAnimation.STAND,
                            slideAnimation.getId(), RenderAnimation.RUN));
                    player.setAnimation(Animation.STOP);
                    player.addWalkSteps(destination.getX(), destination.getY(), -1, false);
                }
                if (!success) {
                    val fallDestination =
                            AgilityPyramidArea.getLowerTile(player.getLocation().transform(object.getFaceDirection(),
                                    2));
                    if (ticks == 3) {
