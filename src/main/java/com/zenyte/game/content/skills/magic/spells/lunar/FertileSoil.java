...
public final class FertileSoil implements ObjectSpell {
	...
	public boolean spellEffect(final Player player, final WorldObject object) {
        val spot = player.getFarming().getPatch(object).isPresent() ? player.getFarming().create(object) : null;
        if (spot == null) {
        ...
        if (spot.getState().equals(PatchState.DEAD)) {
            player.getDialogueManager().start(new PlayerChat(player, "I don't think fertilizing this will bring it back to life."));
            return false;
        }
        ...
        val type = player.getInventory().containsItem(21622, 2) ? 3 : 2;
        ...
        this.addXp(player, Skills.FARMING, 18);
