...
public final class HunterTrap {
    ...
    @Nullable @Getter @Setter private Runnable removalRunnable;
     ...
     */
    public HunterTrap(@NotNull final Player player, @Nullable final HunterDummyNPC npc, @NotNull final Location location, @NotNull final List<WorldObject> defaultObjects,
                      @NotNull final List<WorldObject> builtObjects, @NotNull final List<WorldObject> collapsedObjects, @NotNull final TrapType type) {
    ...
    public void process() {
        ...
        val currentTick = ticks.getAndIncrement();
        for (val tickRunnable : tickRunnables) {
    ...
    public Optional<Runnable> findCollapseRunnable() {
        val runnable = Utils.findMatching(tickRunnables, tickRunnable -> tickRunnable.tick == TimeUnit.MINUTES.toTicks(3));
        if (runnable == null) {
