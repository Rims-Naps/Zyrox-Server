...
public class BrimhavenSixStones implements Shortcut {
	...
	private static final Location[] STONES = {
			new Location(2649, 9561, 0),
			new Location(2649, 9560, 0),
			new Location(2648, 9560, 0),
			new Location(2647, 9560, 0),
			new Location(2647, 9559, 0),
			new Location(2647, 9558, 0)
	};
	...
	public void startSuccess(final Player player, final WorldObject object) {
		player.lock();
		val start = object.getId() == 21738;
		val destination = start ? END : START;
		WorldTasksManager.schedule(new WorldTask() {
			...
			public void run() {
				if (ticks >= 0 && ticks <= 10 && ticks % 2 == 0) {
					val index = start ? ticks / 2 : 5 - ticks / 2;
