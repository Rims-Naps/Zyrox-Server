...
public class FarmingStorageInterface extends Interface {
    ...
    private void setDefaultQuantityOption(final Player player, final int option) {
        ...
        VarCollection.FARMING_EQUIPMENT_STORAGE_DEFAULT.update(player);
    }

    private static final void remove(@NotNull final Player player, @NotNull final Storable storable, int option) {
        val storage = player.getFarming().getStorage();
        val op = storage.getOption(option);
        val item = storage.getItem(storable);
        switch (op) {
            case EXAMINE:
                if (item == null) {
                    Examine.sendItemExamine(player, storable.getBaseId());
                } else {
