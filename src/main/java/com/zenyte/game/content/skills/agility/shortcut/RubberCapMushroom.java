...
public class RubberCapMushroom implements Shortcut, Failable {
    ...
    public void handleObjectAction(final Player player, final WorldObject object, final String name,
        ...
        final Runnable runnable = () -> player.getDialogueManager().start(new Dialogue(player) {
            ...
            public void buildDialogue() {
                options("You might hurt yourself if you do that",
                        new DialogueOption("I don't care, I'm doing it.", () -> handle(player, object, 0)),
                        new DialogueOption("I'd better not in that case"));
            }
        });

        player.setRouteEvent(new TileEvent(player, new TileStrategy(getRouteEvent(player, object), distance(object)),
                runnable, 1));
    }
    ...
    private void start(@NotNull final Player player, final boolean fail) {
        ...
        val distance = player.getLocation().getTileDistance(MUSHROOM_CENTER_TILE) + 1;

        WorldTasksManager.schedule(() -> player.addWalkSteps(MUSHROOM_CENTER_TILE.getX(), MUSHROOM_CENTER_TILE.getY()
                , distance, false));
