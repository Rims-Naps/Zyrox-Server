...
public class FletchingDefinitions {
	...
	public enum AmmunitionFletchingData {
		...
		AMETHYST_DART(AmmunitionType.DART, 90, 20, new Item(31036), new Item(31034), FEATHER),
			...
			for (val data : AmmunitionFletchingData.VALUES) {
				loop: for (val i : data.getMaterials()) {
					if (i.getId() == from.getId()
							|| (data == WOLFBONE_TIPS && from.getId() == ItemId.CHISEL && to.getId() != ItemId.CHISEL)
							|| (data == OGRE_ARROW_SHAFT && from.getId() == ItemId.KNIFE && to.getId() != ItemId.KNIFE)) {
						for (val o : data.getMaterials()) {
							if (o.getId() == to.getId() && (i != o || data == OGRE_ARROW_SHAFT || data == WOLFBONE_TIPS)
									|| (data == WOLFBONE_TIPS && to.getId() == ItemId.CHISEL && from.getId() != ItemId.CHISEL)
									|| (data == OGRE_ARROW_SHAFT && to.getId() == ItemId.KNIFE && from.getId() != ItemId.KNIFE))
								return data;
						}
						continue loop;
					}
	...
	public enum BowFletchingData {
		...
		static {
			for (BowFletchingData data : BowFletchingData.VALUES_ARR)
