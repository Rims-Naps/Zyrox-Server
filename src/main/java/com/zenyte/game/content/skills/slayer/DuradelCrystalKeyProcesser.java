 ...
 */
public class DuradelCrystalKeyProcesser extends DropProcessor {
	...
	public void attach() {
		...
		put(ItemId.CRYSTAL_KEY, new PredicatedDrop("This drop is can only be obtained when killing the monster during a slayer assignment, assigned by Duradel."));
	}
	...
	public Item drop(final NPC npc, final Player killer, final Drop drop, final Item item) {
		if (killer.getSlayer().getAssignment() != null) {
			...
			if (!drop.isAlways() && killer.getSlayer().isCurrentAssignment(npc) && killer.getSlayer().getAssignment().getMaster().equals(SlayerMaster.DURADEL)) {
		...
		definitionsList.parallelStream().forEach(definition -> {
			...
			for (val task : RegularTask.VALUES) {
				for (val t : task.getTaskSet()) {
					if (t.getSlayerMaster().equals(SlayerMaster.DURADEL)) {
