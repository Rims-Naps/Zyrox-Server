...
public final class SpellState {
	...
	private static final RuneResource[] RESOURCES = new RuneResource[] { STAFF_RESOURCE, TOME_RESOURCE, BRYOPHYTAS_RESOURCE,
			INVENTORY_RESOURCE, RUNE_POUCH_RESOURCE, SECONDARY_RUNE_POUCH_RESOURCE };
	...
	@Nullable private MagicSpell spell;
	...
	public boolean check(final boolean inform) {
		...
		for (val rune : runes) {
			...
			val requiredAmount = rune.getAmount();
			...
			if (amount != requiredAmount) {
				if (inform) {
					player.sendMessage("You do not have enough " + ItemDefinitions.getOrThrow(id).getName() + "s to " +
                            "cast this spell.");
				}
				return false;
			}
	...
	private int checkRuneResource(final RuneResource resource, int id, final int amount) {
		...
		if (resource == STAFF_RESOURCE || resource == TOME_RESOURCE || resource == BRYOPHYTAS_RESOURCE) {
			val result = getResource(resource, id, amount);
			if (result != null) {
			...
			val withinLastManStanding = varManager.getBitValue(3923) == 1 || varManager.getBitValue(5314) == 1;
	...
	private Resource getResource(final RuneResource source, final int id, final int amount) {
		val affected = source.getAffectedRunes();
		if (affected != null && !ArrayUtils.contains(affected, id)) {
			return null;
		}
		val held = source.getAmountOf(player, id, amount);
		if (held <= 0) {
			return null;
		}
		val resource = new Resource(id, held, source);
