...
public class ImplingJarPlugin extends ItemPlugin {
    ...
    public void handle() {
        bind("Loot", (player, item, slotId) -> {
            val jar = ImplingJar.get(item.getId());
            if (jar == null) {
                return;
            }
            val breakJar = Utils.random(1, 10) == 1;
            ...
            for (val it : jar.generateLoot(player)) {
    ...
    public enum ImplingJar {
        ...
        CRYSTAL(ItemId.CRYSTAL_IMPLING_JAR, CrystalImplingJarTable.table, new ClueDrop(ClueItem.ELITE.getScrollBox(), 50)),
