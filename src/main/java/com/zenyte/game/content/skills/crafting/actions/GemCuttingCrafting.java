...
public class GemCuttingCrafting extends Action {
	...
	private final int slot, amount;
	private int cycle;

	public static final boolean successful(final Player player, final int requirement) {
		val baseChance = 5d / 833 * player.getSkills().getLevel(Skills.CRAFTING);
		val reqChance = 0.5 - (requirement * 0.0093);
		return Utils.randomDouble() < baseChance + reqChance;
	}

	@Override
	public boolean start() {
		val level = data.equals(GemCuttingData.AMETHYST) ? data.getLevel() + (2 * slot) : data.getLevel();
