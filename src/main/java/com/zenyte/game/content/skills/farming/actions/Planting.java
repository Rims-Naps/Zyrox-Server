...
public class Planting extends Action {
    ...
    public static final Animation SPADE_ANIM = new Animation(830);
    public static final Animation SEED_DIPPING_ANIM = new Animation(2291);
    public static final SoundEffect SPADE_PLANTING_SOUND = new SoundEffect(1470);
    public static final SoundEffect SEED_PLANTING_SOUND = new SoundEffect(2432);
    ...
    public boolean start() {
        ...
        if (player.getSkills().getLevel(Skills.FARMING) < product.getLevelRequired()) {
        ...
        if (patch == FarmingPatch.WEISS_HERB_PATCH) {
            if (!OldFirePit.FirePit.WEISS_FIRE.isBuilt(player)) {
                player.getDialogueManager().start(new PlainChat(player, "The crops can't grow in areas of extreme cold. " +
                        "Perhaps you should look into warming up the area around first."));
                return false;
            }
        }
        if (patch == FarmingPatch.HARMONY_HERB) {
            if (!DiaryReward.MORYTANIA_LEGS4.eligibleFor(player)) {
                player.sendMessage("To unlock this patch you must need to unlock the Morytania Elite Diaries.");
                return false;
            }
        }
        if (type == PatchType.SPIRIT_TREE_PATCH) {
            ...
            if (level < 99) {
                val spiritTreeCount = player.getFarming().getGrownCount(PatchType.SPIRIT_TREE_PATCH, spot -> !spot.getState().equals(WEEDS));
                if (spiritTreeCount >= 2 || spiritTreeCount >= 1 && level < 91) {
                    player.getDialogueManager().start(new ItemChat(player, new Item(6063),
                            "You can only have " + (level < 91 ? "one" : "two") + " spirit tree" + (level < 91 ? "" :
                                    "s") + " planted at a time."));
                    return false;
                }
            }
    ...
    public int processWithDelay() {
        ...
        player.getSkills().addXp(Skills.FARMING, product.getPlantExperience());
        val item = new Item(product.getSeed());
        if (product.getType() == PatchType.ALLOTMENT) {
            if (player.getEquipment().getId(EquipmentSlot.AMULET) == 21160) {
                if (Utils.random(3) == 0) {
                    val uses = player.getNumericAttribute("amulet of bounty uses").intValue() + 1;
