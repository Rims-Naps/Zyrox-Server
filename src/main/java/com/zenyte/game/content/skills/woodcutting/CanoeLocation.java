...
public enum CanoeLocation {
	...
	BARBARIAN_VILLAGE(new Location(3109, 3415, 0), 34),
	EDGEVILLE(new Location(3128, 3504, 0), 39),
	...
	static {
		for (val location : VALUES) {
			LOCATIONS.put(location.getComponentId(), location);
		}
	}

	private CanoeLocation(Location tile, int componentId) {
		this.tile = tile;
	...
	public static void sendInterfaceConfiguration(final Player player, final int bit) {
		val canoe = CanoeDefinitions.getDefinitionByBitValue(bit);
		if (canoe == null) {
		    return;
        }
