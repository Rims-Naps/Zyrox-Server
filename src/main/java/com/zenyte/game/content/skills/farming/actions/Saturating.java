...
public final class Saturating extends Action {
    ...
    public static final PatchState[] allowedStates = new PatchState[] {
            PatchState.WEEDS, PatchState.GROWING, PatchState.WATERED, PatchState.DISEASED
    };
    ...
    public boolean start() {
        ...
        val compost = spot.getCompostFlag();
        if (compost.isPresent()) {
            val flag = getCompostType(item);
            assert flag != null;
            //We allow re-saturating plants with better compost types.
            if (flag.ordinal() <= compost.get().ordinal()) {
                player.sendMessage("This " + spot.getPatch().getType().getSanitizedName() + " has already been treated " +
                        "with " + compost.get().toString().toLowerCase() + ".");
                return false;
            }
        }
        if (item.getId() == 22994 || item.getId() == 22997) {
            if ((item.getCharges() & 0xFFFF) == 0) {
    ...
    public int processWithDelay() {
        ...
        player.getSkills().addXp(Skills.FARMING,
                flag == PatchFlag.COMPOST ? 18 : flag == PatchFlag.SUPERCOMPOST ? 26 : 36);
        if (item.getId() == 22994 || item.getId() == 22997) {
            item.setCharges((item.getCharges() >> 16) << 16 | ((item.getCharges() & 0xFFFF) - 1));
            if ((item.getCharges() & 0xFFFF) == 0) {
                item.setCharges(0);
                item.setId(22994);
    ...
    private final PatchFlag getCompostType(final Item item) {
        ...
        return id == 6032 ? PatchFlag.COMPOST : id == 6034 ? PatchFlag.SUPERCOMPOST : id == 21483 ?
                PatchFlag.ULTRACOMPOST : null;
    }
