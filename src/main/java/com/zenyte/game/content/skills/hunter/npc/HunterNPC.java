...
abstract class HunterNPC extends NPC {
    ...
    @NotNull final TrapPrey prey;
    @NotNull final TrapType trapType;
    @Nullable HunterTrap trap;
    @Nullable TrapInteractType interactType;
    @Nullable PreyInteractState interactState;
    int delay;
     ...
     */
    @NotNull private final Location nextMovementTile = new Location(0);
    ...
    protected boolean canMove(final int lastX, final int lastY, final int dir) {
        ...
        val x = Utils.DIRECTION_DELTA_X[dir];
        val y = Utils.DIRECTION_DELTA_Y[dir];
    ...
    protected void checkTraps() {
        ...
        val possibleTargets = getPossibleTargets(EntityType.NPC);
        ...
        val dummy = (HunterDummyNPC) target;
        val trapReference = dummy.getTrap();
        if (trapReference == null) {
            return;
        }
        val randomTrap = trapReference.get();
    ...
    protected boolean isPotentialTarget(final Entity entity) {
        ...
        val npc = (HunterDummyNPC) entity;
        if (npc.getId() == trapType.getActiveDummyNpcId()) {
            val trapReference = npc.getTrap();
            if (trapReference == null) {
                return false;
            }
            ...
            val player = trap.getPlayer().get();
            return player != null && !player.isNulled() && player.getSkills().getLevel(Skills.HUNTER) >= prey.getLevel();
        }
        return false;
    }
    ...
    protected static double successProbability(final int level, @NotNull final HunterPrey prey) {
        val baseRequirement = prey.baseRequirement();
        val baseChance = prey.baseCatchRate() / 255F;
        val neverFailLevel = prey.neverFailLevel();
        val adjustmentPercentage = 1 - baseChance;
        val successPerLevel = adjustmentPercentage / ((float) neverFailLevel - baseRequirement);
    ...
    protected static boolean success(@NotNull final HunterTrap trap, @NotNull final HunterPrey prey) {
        val player = trap.getPlayer().get();
        if (player == null || player.isNulled() || player.getSkills().getLevel(Skills.HUNTER) < prey.baseRequirement()) {
            return false;
        }
        return Utils.randomDouble() < successProbability(player.getSkills().getLevel(Skills.HUNTER), prey);
    }
}
