...
public class BirdhouseCreationAction extends Action {
    ...
    @NotNull private final BirdHouseType house;
    private int amount;

    @Override
    public boolean start() {
        val skills = player.getSkills();
        if (skills.getLevel(Skills.CRAFTING) < house.getCraftingRequirement()) {
    ...
    private boolean containsMaterials(Inventory inventory) {
        ...
        if (!inventory.containsItem(house.getLogsId()) || !inventory.containsItem(ItemId.CLOCKWORK)) {
            player.getDialogueManager().start(new DoubleItemChat(player, new Item(house.getLogsId()), new Item(ItemId.CLOCKWORK), "You don't have the right ingredients:<br>" +
                    "1 x " + ItemDefinitions.getOrThrow(house.getLogsId()).getName() + "<br>" +
                    "1 x Clockwork"));
            return false;
        }
        return true;
    ...
    public int processWithDelay() {
        ...
        player.getSkills().addXp(Skills.CRAFTING, house.getCraftingExperience());
        amount--;
        return 1;
    }
}
