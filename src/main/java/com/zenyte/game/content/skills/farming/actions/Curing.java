...
public class Curing extends Action {
    ...
    public boolean start() {
        ...
        if (spot.isTreePatch() || spot.getPatch().getType() == PatchType.BUSH_PATCH) {
            ...
            player.sendFilteredMessage("You start pruning the "
                    + (spot.getPatch().getType() == PatchType.BUSH_PATCH ? "bush" : "tree") + "...");
            player.setAnimation(inventory.containsItem(FarmingConstants.MAGIC_SECATEURS)
                    || equipment.getId(EquipmentSlot.WEAPON) == FarmingConstants.MAGIC_SECATEURS.getId() ? magicPruneAnimation : pruneAnimation);
            player.sendSound(pruneSound);
            player.sendSound(delayedPruneSound);
            delay(4);
        } else {
            val containsCure = inventory.containsItem(FarmingConstants.PLANT_CURE);
            val spell = new SpellState(player, Objects.requireNonNull(Magic.getSpell(Spellbook.LUNAR, "cure plant", ObjectSpell.class)));
            if (!containsCure) {
                ...
                val containsSecateurs = inventory.containsItem(FarmingConstants.SECATEURS) ||
                        inventory.containsItem(FarmingConstants.MAGIC_SECATEURS);
