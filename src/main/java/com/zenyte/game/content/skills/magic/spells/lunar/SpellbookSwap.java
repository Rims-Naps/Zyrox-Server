...
public final class SpellbookSwap implements DefaultSpell {
	...
	public boolean spellEffect(final Player player, int optionId, final String option) {
        ...
        if (player.inArea("Tournament Lobby") || player.inArea("Tournament Zone")) {
            player.sendMessage("You cannot use spellbook swap inside the tournament zones.");
            return false;
        }
		if (optionId > 1) {
			optionId -= 2;
		...
		player.getDialogueManager().start(new OptionDialogue(player, "Select a Spellbook", 
				new String[] {
					"Regular spellbook",
					"Ancient spellbook",
					"Arceuus spellbook"
				}, new Runnable[] {
					() -> handleSpell(player, 0),
					() -> handleSpell(player, 1),
					() -> handleSpell(player, 3),
				}));
		return false;
	}
	...
	private void handleSpell(final Player player, final int option) {
		...
		val state = new SpellState(player, this);
		if (!state.check()) {
		    return;
        }
