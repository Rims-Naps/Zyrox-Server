 ...
 */
public final class CrumblingWall implements Obstacle {
	...
	public void startSuccess(final Player player, final WorldObject object) {
		val destination = new Location(object.getX() + 1, object.getY(), object.getPlane());
		...
		WorldTasksManager.schedule(() -> {
			if (destination.getX() == 2543) {
				...
				player.addAttribute("barb_lap_count", player.getNumericAttribute("barb_lap_count").intValue() + 1);
				val laps = player.getNumericAttribute("barb_lap_count").intValue();
				player.sendMessage("You have ran "+ Colour.RED.wrap(laps) + (laps == 1 ? " lap " : " laps ") + "on the Barbarian Course.");
			}
