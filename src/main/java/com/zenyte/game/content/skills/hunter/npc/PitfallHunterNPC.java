...
public class PitfallHunterNPC extends HunterNPC implements Spawnable {
    ...
    public NPC spawn() {
        ...
        Utils.findEmptySquare(originalTile.transform(Utils.random(10) - 7, Utils.random(10) - 7, 0), 5, getSize(), Optional.empty())
                .ifPresent(tile -> setRespawnTile(new ImmutableLocation(tile)));
        return super.spawn();
    }
    ...
    private void fallIn(@NotNull final Player player, @NotNull final WorldObject trap) {
        val rot = trap.getRotation();
        val dir = (rot & 0x1) == 0 ? (getY() > trap.getY() ? SOUTH : NORTH) : (getX() > trap.getX() ? WEST : EAST);
        setFaceEntity(null);
        ...
        HunterUtils.findTrap(TrapType.PITFALL, trap, npc -> HunterUtils.getTrapOwnershipPredicate().test(player, npc))
                .ifPresent(hunterTrap -> hunterTrap.setState(TrapState.PROCESSING));
    ...
    private void jumpOver(@NotNull final Player player, @NotNull final WorldObject trap) {
        ...
        val py = getY();
        val z = getPlane();
        val dir = (rot & 0x1) == 0 ? (py > trap.getY() ? SOUTH : NORTH) : (px > trap.getX() ? WEST : EAST);
        val destination = new Location(dir == WEST ? (px - 4) : dir == EAST ? (px + 4) : px, dir == SOUTH ? (py - 4) : dir == NORTH ? (py + 4) : py, player.getPlane());
        val type = Objects.requireNonNull(Utils.findMatching(FlyingCreatureProjectile.values, value -> value.prey == prey));
        val firstProj = dir == EAST || dir == NORTH ? type.right : type.left;
        val secondProj = dir == EAST || dir == NORTH ? type.left : type.right;
        val mainProjectileStart = new Location(getLocation());
        val mainProjectileEnd = new Location(destination);
        val secondProjectileStart = new Location(mainProjectileStart);
        val secondProjectileEnd = new Location(mainProjectileEnd);
        val firstYOffset = dir == EAST || dir == WEST ? 1 : 0;
        val secondYOffset = dir == NORTH || dir == SOUTH ? 1 : 0;
        setFaceEntity(null);
        setAnimation(jump);
    ...
    private Optional<Location> calculateDestination(@NotNull final Player player, @NotNull final WorldObject leaptTrap) {
        val rot = leaptTrap.getRotation();
        val tx = leaptTrap.getX();
        val ty = leaptTrap.getY();
        val nx = getX();
        val ny = getY();
        val z = getPlane();
        if ((rot & 0x1) == 1) {
    ...
    public void follow(@NotNull final Player player, @NotNull final WorldObject leaptTrap) {
        val optionalDest = calculateDestination(player, leaptTrap);
        if (!optionalDest.isPresent()) {
            return;
        }
        val dest = optionalDest.get();
        if (getLocation().getDistance(dest) > MAX_FOLLOW_DISTANCE) {
            return;
        }
        if (target != null && player == target.get() && leaptTrap.matches(lastLeaptTile)) {
            WorldTasksManager.schedule(() -> player.sendMessage("The " + shortenedName + " won't jump the same pit twice in a row."), 2);
            return;
        }
        combat.reset();
        ...
        setRouteEvent(new PitfallTileEvent(this, new TileStrategy(dest), () -> {
            ...
            lock();
            WorldTasksManager.schedule(() -> {
                val trap = HunterUtils.findTrap(TrapType.PITFALL, leaptTrap, npc -> HunterUtils.getTrapOwnershipPredicate().test(player, npc)).orElseThrow(RuntimeException::new);
    ...
    private enum FlyingCreatureProjectile {
        LARUPIA(TrapPrey.SPINED_LARUPIA, 939, 940),
        KYATT(TrapPrey.SABRE_TOOTHED_KYATT, 937, 938),
        GRAAHK(TrapPrey.HORNED_GRAAHK, 941, 942);
        ...
        private final Projectile left, right;
    ...
    private static final class PitfallTileEvent extends NPCTileEvent {
        ...
        public boolean process() {
            ...
            if (!entity.hasWalkSteps()) {
                val steps = RouteFinder.findRoute(entity, 1, strategy, true);
