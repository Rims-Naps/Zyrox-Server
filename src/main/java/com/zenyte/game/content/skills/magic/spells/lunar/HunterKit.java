...
public final class HunterKit implements DefaultSpell {
	...
	public boolean spellEffect(final Player player, final int optionId, final String option) {
		...
		WorldTasksManager.schedule(() -> {
			...
			val result = player.getInventory().addItem(new Item(11159, 1));
			result.onFailure(item -> World.spawnFloorItem(item, player));
			player.sendMessage(result.getResult() == RequestResult.SUCCESS ? "A hunter kit was added to your inventory."
					: "A hunter kit was dropped under you.");
		}, 6);
		return true;
	}
