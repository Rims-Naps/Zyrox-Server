...
public final class ItemTrapSetupAction extends Action {
    ...
    @NotNull private final TrapType trapType;
    @Nullable private FloorItem floorItem;
    ...
    public boolean start() {
        ...
        if (floorItem != null) {
            ...
            val id = trapType.getItemId();
            Preconditions.checkArgument(id != -1);
            val item = new Item(id);
            val result = player.getInventory().deleteItem(slot, item);
            if (result.isFailure()) {
                return false;
            }
    ...
    public int processWithDelay() {
        ...
        World.destroyFloorItem(floorItem);
        val dummy = new HunterDummyNPC(trapType.getActiveDummyNpcId(), location);
        val object = new WorldObject(trapType.getObjectId(), 10, 2, location);
        val collapsedObject = new WorldObject(trapType.getCollapsedObjectId(), 10, 2, location);
        val trap = new HunterTrap(player, dummy, location, Collections.emptyList(), Collections.singletonList(object), Collections.singletonList(collapsedObject), trapType);
        dummy.setTrap(trap);
        final Runnable consumer = () -> {
            ...
            if (player != null && !player.isNulled()) {
                player.sendFilteredMessage("The " + trapType.toString() + " that you laid has fallen over.");
            }
        };
