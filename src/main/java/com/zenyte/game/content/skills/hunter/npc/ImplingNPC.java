...
import com.zenyte.game.content.skills.hunter.GlobalImplings;
...
public class ImplingNPC extends Bird implements Spawnable {
    ...
    private boolean puroSpawn;
    private boolean rarePuroSpawn;
    ...
    public ImplingNPC(int id, Location tile, Direction facing, int radius) {
        super(id, tile, facing, tile != null && tile.getRegionId() == 10307 ? 10 : 8192);
        this.originalId = id;
        setSpawned(true);
    }
    ...
    private final boolean calculateRetreat(final int offset) {
        ...
        val movingX = (int) Math.round(middle.getX() + (getSize() + distanceToRetreat) * Math.cos(angle));
        val movingY = (int) Math.round(middle.getY() + (getSize() + distanceToRetreat) * Math.sin(angle));
    ...
    public void processNPC() {
        if (retreatingFrom != null) {
            ...
            if (!calculateRetreat(0)) {
                val multiplier = Utils.random(1) == 0 ? -1 : 1;
    ...
    public ImplingNPC setPuroSpawn(final boolean puro) {
        this.puroSpawn = puro;
        return this;
    }
    
    public ImplingNPC setRarePuroSpawn(final boolean rarePuroSpawn) {
        this.rarePuroSpawn = rarePuroSpawn;
        return this;
    }
    ...
    public void finish() {
        ...
        if (!puroSpawn) {
            GlobalImplings.spawnRandomImpling();
        } else {
            if (rarePuroSpawn) {
                PuroPuroArea.spawnRareImpling(getRespawnTile());
            } else {
                setRespawnTask();
            }
        }
