...
public class MisthalinSteppingStone implements Shortcut {
	...
	public void startSuccess(final Player player, final WorldObject object) {
		...
		val faceDirection = Utils.getFaceDirection(object.getX() - player.getX(), object.getY() - player.getY());
		val destination = getStartPosition(player, object).transform(Direction.values[((faceDirection + 0x80) & 0x7FF) >> 8], 4);
	...
	private Location getStartPosition(final Player player, final WorldObject object) {
		if (object.getId() == 5949) {
			val north = new Location(3221, 9556);
			return player.getY() >= 9555 ? north : north.transform(0, -4, 0);
		} else {
			val east = new Location(3208, 9572);
