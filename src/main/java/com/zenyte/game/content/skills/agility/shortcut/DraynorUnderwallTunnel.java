...
public class DraynorUnderwallTunnel implements Shortcut {
	...
	public boolean preconditions(final Player player, final WorldObject object) {
		if (!DiaryReward.EXPLORERS_RING2.eligibleFor(player)) {
			player.sendMessage("You need to have completed the medium Lumbridge and Draynor diaries to use this shortcut.");
			return false;
		}
		return true;
	}
