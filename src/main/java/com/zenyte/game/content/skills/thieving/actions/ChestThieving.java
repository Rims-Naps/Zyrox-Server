...
public class ChestThieving extends Action {
    ...
    public int processWithDelay() {
        val hasLockpick = player.getInventory().containsItem(new Item(ItemId.LOCKPICK));
    ...
    private void success() {
        ...
        WorldTasksManager.schedule(() -> {
            player.getSkills().addXp(Skills.THIEVING, chest.getExperience());
            addLoot();
            spawnEmptyChest();
    ...
    private boolean isChestLooted() {
        val attribute = "thieving_chest_last_looted_" + object.getPositionHash();
        
        if (!player.getAttributes().containsKey(attribute)) {
            return false;
        }
        
        val lastLootedTime = player.getNumericAttribute(attribute).longValue();
