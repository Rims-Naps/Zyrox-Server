...
public class BoneGrinding extends Action {
    ...
    @AllArgsConstructor
    enum Bonemeal {
        REGULAR_BONEMEAL(Bones.BONES, 4255),
        BAT_BONEMEAL(Bones.BAT_BONES, 4256),
        BIG_BONEMEAL(Bones.BIG_BONES, 4257),
        BURNT_BONEMEAL(Bones.BURNT_BONES, 4258),
        BURNT_JOGRE_BONEMEAL(Bones.BURNT_JOGRE_BONES, 4259),
        BABYDRAGON_BONEMEAL(Bones.BABYDRAGON_BONES, 4260),
        DRAGON_BONEMEAL(Bones.DRAGON_BONES, 4261),
        WOLF_BONEMEAL(Bones.WOLF_BONES, 4262),
        MONKEY_BONEMEAL(Bones.MONKEY_BONES, 4267),
        JOGRE_BONEMEAL(Bones.JOGRE_BONES, 4271),
        ZOGRE_BONEMEAL(Bones.ZOGRE_BONES, 4852),
        FAYRG_BONEMEAL(Bones.FAYRG_BONES, 4853),
        RAURG_BONEMEAL(Bones.RAURG_BONES, 4854),
        OURG_BONEMEAL(Bones.OURG_BONES, 4855),
        SHAIKAHAN_BONEMEAL(Bones.SHAIKAHAN_BONES, 5615),
        DAGANNOTH_BONEMEAL(Bones.DAGANNOTH_BONES, 6728),
        WYVERN_BONEMEAL(Bones.WYVERN_BONES, 6810),
        LAVA_DRAGON_BONEMEAL(Bones.LAVA_DRAGON_BONES, 11922),
        SUPERIOR_DRAGON_BONEMEAL(Bones.SUPERIOR_DRAGON_BONES, 22116),
        WYRM_BONEMEAL(Bones.WYRM_BONES, 22754),
        DRAKE_BONEMEAL(Bones.DRAKE_BONES, 22756),
        HYDRA_BONEMEAL(Bones.HYDRA_BONES, 22758);
    ...
    public boolean start() {
        val status = player.getNumericAttribute("ectofuntus bone status").intValue();
        val boneId = player.getNumericAttribute("ectofuntus grinded bone").intValue();
        switch(stage) {
            case ADDING_BONES:
                if (status == 1) {
        ...
        if (bones == null) {
            val id = player.getNumericAttribute("ectofuntus grinded bone").intValue();
            if (id == 0) {
                return false;
            }
            bones = new Item(id);
    ...
    public int processWithDelay() {
        ...
        switch(stage) {
            ...
            case COLLECTING:
                val boneId = player.getNumericAttribute("ectofuntus grinded bone").intValue();
                val bonemeal = Utils.findMatching(Bonemeal.values,
                        meal -> Utils.findMatching(meal.bones.getBones(), b -> b.getId() == boneId) != null);
                if (bonemeal == null) {
                    return -1;
                }
