...
public class TuraelD extends Dialogue {
	...
	public void buildDialogue() {
		...
		if (player.getSlayer().getCompletedTasks() == 0 && player.getSlayer().getAssignment() == null) {
			val task = player.getSlayer().generateTask(SlayerMaster.TURAEL);
			options(TITLE, "Who are you?", "Have you any rewards for me, or anything to trade?", "Let's talk about the difficulty of my assignments.", "Er... Nothing...").onOptionOne(() -> setKey(10)).onOptionTwo(() -> setKey(20)).onOptionThree(() -> setKey(200)).onOptionFour(() -> setKey(300));
			player(10, "Who are you?");
			npc("I'm a Slayer Master. I train adventurers to learn " + "the weaknesses of seemingly invulnerable monsters. " + "To learn how, you need to kill specific monsters. " + "I'll identify suitable targets and assign you a quota.");
			...
			npc("You'll also need this enchanted gem - " + "it allows Slayer Masters like myself to contact " + "you and update you on your progress. " + "Don't worry if you lose it; " + "you can buy another from any Slayer Master.").executeAction(() -> {
			...
			options(TITLE, "I need another assignment.", "Have you any rewards for me, or anything to trade?", "Let's talk about the difficulty of my assignments.", "Er... Nothing...")
			.onOptionOne(() -> {
			    if (player.getSlayer().getAssignment() == null) {
                    ...
                    player.getDialogueManager().start(new Dialogue(player, npc) {
                        @Override
                        public void buildDialogue() {
                            npc(40, "Your slayer task is to slay " + task.getTask().toString() + ". You'll need to kill " + task.getAmount() + " of them. ");
            ...
            for (final RegularTask data : RegularTask.VALUES) {
                if (player.getSlayer().getAssignment().getTask() == data) {
                    if (data.getCertainTaskSet(SlayerMaster.TURAEL) == null || data.getCertainTaskSet(SlayerMaster.TURAEL).getMaximumAmount() < player.getSlayer().getAssignment().getAmount()) {
                        ...
                        npc("Although it's not an assignment that I'd normally give..." + " I guess I could  give you a new assignment, if you'd like.");
		...
		if (player.getSlayer().isCheckingCombat()) {
			npc("The Slayer Masters will take your combat level into account " + "when choosing tasks for you," + " so you shouldn't get anything too hard.");
			...
			npc("Okay, from now on, all the Slayer Masters will take your combat level " + "into account when choosing tasks for you, so you shouldn't get anything too hard.");
		}
