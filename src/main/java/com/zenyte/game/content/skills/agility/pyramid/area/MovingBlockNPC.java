...
public class MovingBlockNPC extends NPC {
    ...
    public void onMovement() {
        val area = GlobalAreaManager.getArea(AgilityPyramidArea.class);
        val players = area.getPlayers();
        if (players.isEmpty()) {
            return;
        }
        for (val player : players) {
            if (player.isLocked() || player.getPlane() != getPlane() || !Utils.collides(player.getX(), player.getY(), player.getSize(), getX(), getY(), getSize())) {
                continue;
            }
            val horizontal = spawnDirection == Direction.EAST || spawnDirection == Direction.WEST;
            val destTile = new Location(horizontal ? (respawnTile.getX() + 4) : player.getX(), horizontal ? player.getY() : (respawnTile.getY() + 4), player.getPlane() - 1);
    ...
    void slide(@NotNull final Direction direction) {
        val destination = getLocation().transform(direction, 2);
    ...
    private void push(final Player player, final Location destination) {
        player.lock();
        player.stopAll();
        val ticks = (int) player.getLocation().getDistance(destination);
        ...
        player.setForceMovement(new ForceMovement(destination, ticks * 30,
                backwardsDirection.getDirection()));
