...
public class WineCreation extends Action {
    ...
    public static final class WineOnGrapesItemAction implements PairedItemOnItemPlugin {
        ...
        public void handleItemOnItemAction(final Player player, final Item from, final Item to, final int fromSlot, final int toSlot) {
            val jug = from.getId() == 1937 ? from : to;
            val grapes = from == jug ? to : from;
        ...
        public ItemPair[] getMatchingPairs() {
            return new ItemPair[] {
                    ItemPair.of(1987, 1937), ItemPair.of(20749, 1937)
            };
        }
    }
    ...
    public boolean start() {
        val level = grapes == 20749 ? 65 : 35;
        if (player.getSkills().getLevel(Skills.COOKING) < level) {
    ...
    public int processWithDelay() {
        player.getInventory().deleteItemsIfContains(new Item[] {
                new Item(1937, 1), new Item(grapes, 1)
        }, () -> {
