...
public final class Construction {
    ...
    @Getter
    private transient final Map<Player, Integer> catsOnBlanket = new HashMap<Player, Integer>();
    @Expose
    @Getter
    ...
    @Getter
    @Setter
    private transient boolean buildingMode, renderDoorsOpen, teleportInside;
    @Getter
    private transient int chunkX, chunkY, yardSpace, yardOffset;
    @Expose
    @Getter
    @Setter
    ...
    private void createLandscape() {
        for (int x = 0; x < yardSpace; x++) {
            for (int y = 0; y < yardSpace; y++) {
                ...
                try {
                    MapBuilder.copySquare(allocatedArea, 1, Room.DUNGEON_LAND.getChunkX(), Room.DUNGEON_LAND.getChunkY(),
                            decoration == 2 ? 2 : decoration == 3 ? 3 : 0, chunkX + x, chunkY + y, 0, 0);
    ...
    private void createRooms() {
        for (final RoomReference reference : references) {
            ...
            addRoof(reference);
            try {
                MapBuilder.copySquare(allocatedArea, 1, reference.getRoom().getChunkX() + (decoration >= 4 ? 8 : 0),
                        reference.getRoom().getChunkY(), decoration & 0x3, chunkX + reference.getX() - yardOffset, chunkY + reference.getY() - yardOffset, reference.getPlane(), reference.getRotation());
    ...
    private void addDoors() {
        for (final RoomReference room : references) {
            ...
            final int regionY = (regionId & 0xFF) << 6;
    ...
    public void roomPreview(final RoomReference reference, final boolean remove) {
        ...
        for (int x = 0; x < 8; x++) {
            for (int y = 0; y < 8; y++) {
                final WorldObject[] objects = region.getObjects(decoration & 0x3, realChunkX * 8 + x & 0x3F, realChunkY * 8 + y & 0x3F);
                if (objects != null) {
                    for (final WorldObject object : objects) {
                        if (object == null) {
                            continue;
                        ...
                        if (reference.getPlane() == 0 || defs.containsOption(4, "Build")) {
                            ...
                            previewObject.setRotation((object.getRotation() + reference.getRotation()) & 0x3);
                            if (remove) {
    ...
    private int generateYardSpace() {
        ...
        return level >= 75 ? 9 : 5 + (int) (level / 15f);
    }
