...
public final class ToolLeprechaunItemPlugin implements ItemOnNPCAction {
	...
	public void handleItemOnNPCAction(final Player player, final Item item, final int slot, final NPC npc) {
		player.getDialogueManager().start(new Dialogue(player, npc) {
            ...
            public void buildDialogue() {
                val noted = getNoted(player, item);
                if (!noted.isPresent()) {
                ...
                player.getInventory().addItem(noted.get()).onFailure(remainder -> World.spawnFloorItem(remainder,
                        player));
            }
        });
	}
	...
	private Optional<Item> getNoted(final Player player, final Item unnoted) {
	    ...
	    val id = unnoted.getId();
        val herb = Herb.CLEAN_HERBS.get(id);
        if (herb != null) {
            return Optional.of(new Item(unnoted.getDefinitions().getNotedOrDefault(),
                    player.getInventory().getAmountOf(unnoted.getId())));
        }
        for (final FarmingProduct product : FarmingProduct.values) {
            if (product == null)
                continue;
            val prod = product.getProduct();
            if (prod == null)
                continue;
            if (prod.getId() == id) {
                return Optional.of(new Item(unnoted.getDefinitions().getNotedOrDefault(),
