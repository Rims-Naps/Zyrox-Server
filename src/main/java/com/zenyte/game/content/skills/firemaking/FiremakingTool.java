...
public enum FiremakingTool {
    ...
    TINDERBOX(new Animation(733), 590),
    TRAINING_BOW(new Animation(6713), 9705),
    BOW(new Animation(6714), 841, 839),
    OAK_BOW(new Animation(6715), 843, 845),
    WILLOW_BOW(new Animation(6716), 847, 849),
    MAPLE_BOW(new Animation(6717), 851, 853),
    YEW_BOW(new Animation(6718), 855, 857),
    MAGIC_BOW(new Animation(6719), 859, 861),
    SEERCULL(new Animation(6720), 6724);
    ...
    public static Optional<FiremakingTool> getAvailableTool(final Player player) {
        ...
        for (int i = 0; i < inventory.getContainer().getContainerSize(); i++) {
            ...
            if (item == null)
                continue;
            for (val tool : values) {
                if (ArrayUtils.contains(tool.bowIds, item.getId()))
