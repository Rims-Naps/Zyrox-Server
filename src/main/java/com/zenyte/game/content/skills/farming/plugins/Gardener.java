...
public class Gardener extends NPCPlugin {
    ...
    private String[] tips = new String[] {
            "Hops are good for brewing ales. I believe there's a brewery up in Keldagrim somewhere, and I've heard " +
            ...
            "If you need to get rid of your fruit trees for any reason, all you have to do is chop them down and then" +
                    " dig up the stump.",
            "Don't just throw away your weeds after you've raked a patch - put them in a compost bin and make some " +
                    "compost.",
            "Vegetables, hops and flowers need constant watering - if you ignore my advice, you will sooner or later " +
                    "find yourself in possession of a dead farming patch.",
            ...
            "Bittercap mushrooms can only be grown in a special patch in Morytania, near the Mort Myre swamp. There " +
                    "the ground is especially dank and suited to growing poisonous fungii.",
            "Applying compost to a patch will not only reduce the chance that your crops will get diseased, but you " +
                    "will also grow more crops to harvest.",
            "You can put up to ten potatoes, cabbages or onions in vegetable sacks, although you can't have a mix in " +
                    "the same sack.",
            "There is a special patch for growing Belladonna - I believe that is is somewhere near Draynor Manor, " +
                    "where the ground is a tad 'unblessed'.",
            "The only way to cure a bush or a tree of disease is to prune away the diseased leaves with a pair of " +
                    "secateurs. For all other crops I would just apply some plant-cure.",
            "There are six main Farming areas - Elstan's area near Falador, Dantaera's area near Catherby, Kragen's " +
                    "area near Ardougne, Lyra's area in Morytania, Marisi's area in Hosidius and Alan's area in the Farming Guild.",
            "Tree seeds must be grown in a plantpot of soil into a sapling, and then transferred to a tree patch to " +
                    "continue growing to adulthood.",
            "Supercompost is far better than normal compost, but more expensive to make. You need to rot the right " +
                    "type of item; show me an item, and I'll tell you if it's super-compostable or not.",
            "You can put up to five tomatoes, strawberries, apples, bananas or oranges into a fruit basket, although " +
                    "you can't have a mix in the same basket.",
            "If you want to make your own sacks and baskets you'll need to use the loom that's near the Farming shop " +
                    "in Falador. If you're a good enough craftsman, that is."
    };
    ...
    public void handle() {
        bindOptions(option -> option.startsWith("Pay"), (player, npc, option) -> {
            val patch = FarmingPatch.getPatchByGardener(npc.getId(), option.getId()).orElseThrow(RuntimeException::new);
            val spot = player.getFarming().create(patch);
            val state = spot.getState();
                ...
                if (npc.getId() == 8535) {
                    player.getDialogueManager().start(new Dialogue(player, npc) {
                        ...
                        public void buildDialogue() {
                            ...
                            options(new DialogueOption("The Cactus Patch", () -> startPaymentDialogue(player, npc, FarmingPatch.FARMING_GUILD_CACTUS, true)),
                                    new DialogueOption("The Northern Allotment Patch", () -> startPaymentDialogue(player, npc, FarmingPatch.FARMING_GUILD_NORTH_ALLOTMENT, true)),
                                    new DialogueOption("The Southern Allotment Patch", () -> startPaymentDialogue(player, npc, FarmingPatch.FARMING_GUILD_SOUTH_ALLOTMENT, true)),
                                    new DialogueOption("The Bush Patch", () -> startPaymentDialogue(player, npc, FarmingPatch.FARMING_GUILD_BUSH, true)));
                        }
                    });
                    return;
                }
        ...
        bind("Talk-to", (player, npc) -> {
            ...
            val set = FarmingPatch.getPatchSetByGardeners(npc.getId()).orElseThrow(RuntimeException::new);
            assert !set.isEmpty();
            //If multiple patches tied to the gardener, it is guaranteed to be an allotment patch.
            val multiple = set.size() > 1;
            val patch = set.stream().findFirst().orElseThrow(RuntimeException::new);
            val spot = player.getFarming().create(patch);
            val state = spot.getState();
            val chopOptions = spot.getProduct().isTree() && (state == GROWN || state == DEAD || state == DISEASED || state == REGAINING_PRODUCE);
            player.getDialogueManager().start(new Dialogue(player, npc) {
                ...
                private void options() {
                    options(TITLE, new DialogueOption(chopOptions ? "Would you chop my tree down for me?" : "Would you look after my crops for me?", key(5)),
                            new DialogueOption("Can you give me any farming advice?", key(100)),
                            new DialogueOption("Can you sell me something?", key(200)),
                            new DialogueOption("I'll come back another time.", key(300)));
                }
                ...
                private void buildWatchOverOptions() {
                    val message = player(5, "Would you look after my crops for me?");
                    if (multiple) {
                        npc("I might - which patch were you thinking of?");
                        val list = new ArrayList<DialogueOption>();
                        for (val patch : set) {
                            list.add(new DialogueOption("The " + patch.getDescription().toLowerCase(),
                                    () -> startPaymentDialogue(player, npc, patch, false)));
                        }
                ...
                private void buildChopOptions() {
                    ...
                    options(TITLE, new DialogueOption("Yes, you're right - I'll do it myself.", key(10)),
                            new DialogueOption("I can't be bothered - I'd rather pay you to do it.", key(20)));
                    ...
                    options(TITLE, new DialogueOption("Here's 200 Coins - chop my tree down please.", key(30)),
                            new DialogueOption("I don't want to pay that much, sorry.", key(40)));
                    player(30, "Here's 200 Coins - chop my tree down please.")
                            .executeAction(() -> player.getInventory().ifDeleteItem(new Item(995, 200),
                ...
                private void buildSellOptions() {
                    ...
                    options(TITLE, new DialogueOption("Some plant cure.", key(210)),
                            new DialogueOption("A bucket of compost.", key(220)),
                            new DialogueOption("A rake.", key(230)),
                            new DialogueOption("A plant pot.", key(240)),
                            new DialogueOption("(See more items)", key(205)));
                    options(205, TITLE, new DialogueOption("A watering can.", key(250)),
                            new DialogueOption("A gardening trowel.", key(260)),
                            new DialogueOption("A seed dibber.", key(270)),
                            new DialogueOption("(See previous items)", key(202)),
                            new DialogueOption("Forget it.", key(290)));
                    ...
                    if (!player.getInventory().containsItem(new Item(995, 25))) {
                        ...
                        player(215, "Yes, that sounds like a fair price.").executeAction(purchase(new Item(995, 25),
                                FarmingConstants.PLANT_CURE));
                        item(FarmingConstants.PLANT_CURE, npc.getName(player) + " hands over your purchase.");
                    }


                    player(220, "A bucket of compost.");
                    npc("A bucket of compost, eh? I might have one spare... tell you what, I'll sell it to you for 35" +
                            " coins if you like.");
                    if (!player.getInventory().containsItem(new Item(995, 35))) {
                        ...
                        player(225, "Yes, that sounds like a fair price.").executeAction(purchase(new Item(995, 35),
                    ...
                    player(230, "A rake.");
                    npc("A rake, eh? I might have one spare... tell you what, I'll sell it to you for 15" +
                            " coins if you like.");
                    if (!player.getInventory().containsItem(new Item(995, 15))) {
                        ...
                        player(235, "Yes, that sounds like a fair price.").executeAction(purchase(new Item(995, 15),
                    ...
                    if (!player.getInventory().containsItem(new Item(995, 40))) {
                        ...
                        player(245, "Yes, that sounds like a fair price.").executeAction(purchase(new Item(995, 40),
                                FarmingConstants.PLANT_POT));
                        item(FarmingConstants.PLANT_POT, npc.getName(player) + " hands over your purchase.");
                    }


                    player(250, "A watering can.");
                    npc("A watering can, eh? I might have one spare... tell you what, I'll sell it to you for 25 " +
                            "coins if you like.");
                    if (!player.getInventory().containsItem(new Item(995, 25))) {
                        player("I don't have enough money for that, I'm afraid.");
                    } else {
                        ...
                        player(255, "Yes, that sounds like a fair price.").executeAction(purchase(new Item(995, 25),
                                FarmingConstants.WATERING_CAN));
                        item(FarmingConstants.WATERING_CAN, npc.getName(player) + " hands over your purchase.");
                    }


                    player(260, "A gardening trowel.");
                    npc("A gardening trowel, eh? I might have one spare... tell you what, I'll sell it to you for 15 " +
                            "coins if you like.");
                    if (!player.getInventory().containsItem(new Item(995, 15))) {
                        player("I don't have enough money for that, I'm afraid.");
                    } else {
                        ...
                        player(265, "Yes, that sounds like a fair price.").executeAction(purchase(new Item(995, 15),
                                FarmingConstants.GARDENING_TROWEL));
                        item(FarmingConstants.GARDENING_TROWEL, npc.getName(player) + " hands over your purchase.");
                    }


                    player(270, "A seed dibber.");
                    npc("A seed dibber, eh? I might have one spare... tell you what, I'll sell it to you for 15 " +
                            "coins if you like.");
                    if (!player.getInventory().containsItem(new Item(995, 15))) {
                        player("I don't have enough money for that, I'm afraid.");
                    } else {
                        ...
                        player(275, "Yes, that sounds like a fair price.").executeAction(purchase(new Item(995, 15),
                ...
                private Runnable purchase(final Item coins, final Item reward) {
                    return () -> player.getInventory().ifDeleteItem(coins,
    ...
    private void startChopDownDialogue(final Player player, final NPC npc, final FarmingPatch patch) {
        ...
        player.getDialogueManager().start(new Dialogue(player, npc) {
            ...
            public void buildDialogue() {
                ...
                val cost = spot.getPatch().getType() == PatchType.REDWOOD_PATCH ? 3000 : 200;
    ...
    private void startPaymentDialogue(final Player player, final NPC npc, final FarmingPatch patch,
                                      final boolean quickPay) {
        player.getDialogueManager().finish();
        val spot = player.getFarming().create(patch);
        player.getDialogueManager().start(new Dialogue(player, npc) {
            @Override
            public void buildDialogue() {
                ...
                options(TITLE, new DialogueOption("Okay, it's a deal.", key(15)),
                        new DialogueOption("No, that's too much.", key(25)));
    ...
    private boolean containsPayment(final Player player, final Item[] items) {
        ...
        for (val item : items) {
            if (item == null) continue;
            val id = item.getId();
            val notedId = item.getDefinitions().getNotedOrDefault();
            var count = inventory.getAmountOf(id);
            if (notedId != id) {
