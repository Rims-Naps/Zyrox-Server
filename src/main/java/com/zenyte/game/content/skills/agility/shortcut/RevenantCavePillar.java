...
public class RevenantCavePillar implements Shortcut {
	...
	public Location getRouteEvent(final Player player, final WorldObject object) {
		val vertical = object.getRotation() == 0;
	...
	public void startSuccess(Player player, WorldObject object) {
		val vertical = object.getRotation() == 0;
		val direction = vertical ? player.getY() > object.getY() ? ForceMovement.SOUTH : ForceMovement.NORTH : player.getX() <= object.getX() ? ForceMovement.EAST : ForceMovement.WEST;
		player.lock();
		WorldTasksManager.schedule(new WorldTask() {
			...
			public void run() {
				if (ticks == 0) {
					val destination = direction == ForceMovement.NORTH ? new Location(player.getX(), player.getY() + 2, player.getPlane()) : direction == ForceMovement.SOUTH ? new Location(player.getX(), player.getY() - 2, player.getPlane()) : direction == ForceMovement.EAST ? new Location(player.getX() + 2, player.getY(), player.getPlane()) : new Location(player.getX() - 2, player.getY(), player.getPlane()); 
