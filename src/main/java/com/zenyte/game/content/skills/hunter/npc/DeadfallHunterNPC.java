...
public class DeadfallHunterNPC extends HunterNPC implements Spawnable {
    ...
    private int calculateFallingTrapId(@NotNull final WorldObject object) {
        ...
        val preyObject = prey.getObject();
        Preconditions.checkArgument(preyObject != null);
        val objects = preyObject.getObjects();
    ...
    public void interact() {
        ...
        val originalTrap = Objects.requireNonNull(trap.getDefaultObjects().get(0));
        val rotation = originalTrap.getRotation();
        val location = originalTrap.transform(rotation == 0 ? 1 : 0, rotation == 3 ? 1 : 0, 0);
        var flag = RouteEvent.BLOCK_FLAG_EAST | RouteEvent.BLOCK_FLAG_WEST;
        if (rotation != 0) {
