...
public class FishingSpotPlugin extends NPCPlugin {
    ...
    public void handle() {
        bind("Catch", new OptionHandler() {
                    ...
                    public void handle(Player player, NPC npc) {
                        ...
                        val delay = World.sendProjectile(player.getLocation(), npc, throwingProjectileFrom);
                        WorldTasksManager.schedule(() -> {
                            World.sendGraphics(splashGraphics, npc.getLocation());
                            LakeMolchArea.handleCaughtSpot(npc);
                            val delay2 = World.sendProjectile(npc, player.getLocation(), throwingProjectileTo);
                            WorldTasksManager.schedule(() -> {
                                if (player.inArea(LakeMolchArea.class)) {
                                    reward(player);
                                    if (Utils.random(5) == 0) {
                    ...
                    public void click(final Player player, final NPC npc, final NPCOption option) {
                        ...
                        if (player.getEquipment().containsItem(ItemId.CORMORANTS_GLOVE)) {
                            player.sendMessage("You need to wait for your cormorant to return before trying to catch more fish.");
                            return;
                        }
                        if (!AerialFishingTools.hasBird(player)) {
                            player.sendMessage("Those fish are too far out from the land for you to catch alone.");
                            return;
                        }
                        if (!AerialFishingTools.hasBait(player)) {
                            player.sendMessage("It wouldn't be fair to send the cormorant out with nothing to reward it.");
                            return;
                        }
    ...
    private void reward(Player player) {
        ...
        int doubleFish = 0;
        var diaryChance = 0 ;
        for (val entry : Fishing.diaryRewardMap.entrySet()) {
        ...
        //Fish
        int hunterLevel = skills.getLevel(Skills.HUNTER);
        int fishingLevel = skills.getLevel(Skills.FISHING);
    ...
    private void tryGreaterSiren(int hunterLevel, int fishingLevel, Skills skills, Inventory inven, int doubleFish, Player player) {
        if(hunterLevel >= AerialFish.GREATER_SIREN.getHunterLevel() && fishingLevel >= AerialFish.GREATER_SIREN.getFishingLevel()) {
            skills.addXp(Skills.FISHING, AerialFish.GREATER_SIREN.getFishingXP());
            skills.addXp(Skills.HUNTER, AerialFish.GREATER_SIREN.getHunterXP());
    ...
    private void tryMottledEel(int hunterLevel, int fishingLevel, Skills skills, Inventory inven, int doubleFish) {
        if(hunterLevel >= AerialFish.MOTTLED_EEL.getHunterLevel() && fishingLevel >= AerialFish.MOTTLED_EEL.getFishingLevel()) {
            skills.addXp(Skills.FISHING, AerialFish.MOTTLED_EEL.getFishingXP());
            skills.addXp(Skills.HUNTER, AerialFish.MOTTLED_EEL.getHunterXP());
    ...
    private void tryCommonTench(int hunterLevel, int fishingLevel, Skills skills, Inventory inven, int doubleFish) {
        if(hunterLevel >= AerialFish.COMMON_TENCH.getHunterLevel() && fishingLevel >= AerialFish.COMMON_TENCH.getFishingLevel()) {
            skills.addXp(Skills.FISHING, AerialFish.COMMON_TENCH.getFishingXP());
            skills.addXp(Skills.HUNTER, AerialFish.COMMON_TENCH.getHunterXP());
    ...
    private void tryBlueGill(Skills skills, Inventory inven, int doubleFish) {
        skills.addXp(Skills.FISHING, AerialFish.BLUEGILL.getFishingXP());
        skills.addXp(Skills.HUNTER, AerialFish.BLUEGILL.getHunterXP());
