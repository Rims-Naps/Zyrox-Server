...
public class FiremakingAction extends Action {
    ...
    public boolean start() {
        ...
        val burnDelay = player.getTemporaryAttributes().remove("BurnDelay");
        final Long time = !(burnDelay instanceof Long) ? null : (Long) burnDelay;
        val quickFire = time != null && time > Utils.currentTimeMillis();
        val levelDifference = player.getSkills().getLevel(Skills.FIREMAKING) - data.getLevel();
        val successPercentage = levelDifference * 7;
        val random = Utils.random(1, 3);
    ...
    public int processWithDelay() {
        val location = new Location(player.getX(), player.getY(), player.getPlane());
        val item = World.getRegion(location.getRegionId()).getFloorItem(data.getLogs().getId(), location, player);
    ...
    private void light() {
        ...
        val object = new WorldObject(data.getObjectId(), 10, 1, location);
        World.spawnTemporaryObject(object, ASHES, 200);
        if (data.equals(Firemaking.WILLOW)) {
            player.getAchievementDiaries().update(FaladorDiary.CHOP_BURN_WILLOW_LOGS, 0x2);
            ...
            player.getAchievementDiaries().update(VarrockDiary.CHOP_AND_BURN_YEW_LOGS, 0x2);
        } else if (data.equals(Firemaking.OAK)) {
            player.getAchievementDiaries().update(LumbridgeDiary.CHOP_AND_BURN_LOGS, 0x2);
            player.getAchievementDiaries().update(FremennikDiary.CHOP_AND_BURN_OAK_LOGS, 0x2);
        } else if (data.equals(Firemaking.MAGIC)) {
            player.getAchievementDiaries().update(WildernessDiary.CUT_AND_BURN_MAGIC_LOGS, 0x2);
            ...
            player.getAchievementDiaries().update(WesternProvincesDiary.CHOP_AND_BURN_TEAK_LOGS, 0x2);
        } else if (data.equals(Firemaking.MAHOGANY)) {
            player.getAchievementDiaries().update(WesternProvincesDiary.CHOP_AND_BURN_MAHOGANY_LOGS, 0x2);
            player.getAchievementDiaries().update(MorytaniaDiary.CHOP_AND_BURN_MAHOGANY_LOGS, 0x2);
        }
        ...
        player.getSkills().addXp(Skills.FIREMAKING, data.getXp() * (pyromancer ? 2 : 1));
