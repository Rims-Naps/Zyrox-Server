...
public class SeedPack extends ItemPlugin {
    ...
    public void handle() {
        bind("Check", ((player, seedPack, slotId) -> {
            val highRolls = seedPack.getNumericAttribute(FarmingContract.SEED_PACK_HIGH_ROLLS_ATTR).intValue();
            val mediumRolls = seedPack.getNumericAttribute(FarmingContract.SEED_PACK_MEDIUM_ROLLS_ATTR).intValue();
            val lowRolls = seedPack.getNumericAttribute(FarmingContract.SEED_PACK_LOW_ROLLS_ATTR).intValue();
            val total = highRolls + mediumRolls + lowRolls;
            val totalString = total > 1 ? total + " sets" : total + " set";
