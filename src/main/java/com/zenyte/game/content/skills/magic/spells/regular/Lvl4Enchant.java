...
public class Lvl4Enchant implements ItemSpell {
	...
	public boolean spellEffect(final Player player, final Item item, final int slot) {
		val data = JewelleryEnchantmentItem.getDataByMaterial(item, JewelleryEnchantmentType.LVL4_ENCHANTMENT);
		if (data != null && JewelleryEnchantment.check(player, data)) {
			...
			player.sendMessage("This spell can only be cast on diamond amulets, rings, necklaces and bracelets and on " + "shapes in the Mage Training Arena.");
			return false;
		}
	}
