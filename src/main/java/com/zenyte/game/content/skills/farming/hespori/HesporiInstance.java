...
public final class HesporiInstance extends DynamicArea implements FullMovementPlugin, DeathPlugin, LootBroadcastPlugin {
    ...
    private int originalPrayerPoints;

    /**
     * The chunk coordinates for the Hespori instance.
     */
    private static final int CHUNK_X = 154, CHUNK_Y = 1259;
    ...
    void finish() {
        ...
        val spot = owner.getFarming().create(patch);
        spot.setValue(HESPORI_PATCH_GROWN);
        spot.refresh();
        owner.getBossTimer().finishTracking("Hespori");
        if (owner.getBossTimer().getBossTimer("Hespori") < 50 && !owner.getBooleanAttribute("elite-combat-achievement67")) {
            owner.putBooleanAttribute("elite-combat-achievement67", true);
            EliteTasks.sendEliteCompletion(owner, 67);
        }
        if (originalPrayerPoints == owner.getPrayerManager().getPrayerPoints() && !owner.getBooleanAttribute("elite-combat-achievement59")) {
            owner.putBooleanAttribute("elite-combat-achievement59", true);
            EliteTasks.sendEliteCompletion(owner, 59);
        }
        if (owner.getBossTimer().getBossTimer("Hespori") < 40 && !owner.getBooleanAttribute("master-combat-achievement46")) {
            owner.putBooleanAttribute("master-combat-achievement46", true);
            MasterTasks.sendMasterCompletion(owner, 46);
        }
    }
    ...
    public void constructed() {
        ...
        owner.getAttributes().put("amount_of_flowers_killed", "0");
        originalPrayerPoints = owner.getPrayerManager().getPrayerPoints();
        for (int i = 0; i < 4; i++) {
            val position = Flower.FlowerPosition.values[i];
            val flower = new Flower(this, position);
            flower.spawn();
            flower.bloom();
            flowers[i] = flower;
        }
    ...
    public void enter(Player player) {
        val attr = player.getTemporaryAttributes();
    ...
    public boolean processMovement(Player player, int x, int y) {
        val entanglement = player.getTemporaryAttributes().remove(Hespori.entanglementAttrKey);
        if (entanglement instanceof Integer) {
            val count = (int) entanglement;
