...
public class LeatherCrafting extends Action {
	...
	public static boolean isMaterial(Player player, Item from, Item to) {
		for (int i = 0; i < 2; i++)
			if ((from.getId() == MATERIALS[2][i].getId() || to.getId() == MATERIALS[2][i].getId()) && (from.getId() == 2370 || to.getId() == 2370))
				return true;
		for (int i = 0; i < 2; i++)
			if ((from.getId() == MATERIALS[3][i].getId() || to.getId() == MATERIALS[3][i].getId()) && (from.getId() == 10113 || to.getId() == 10113))
				return true;
		for (int i = 0; i < 2; i++)
			if ((from.getId() == MATERIALS[10][i].getId() || to.getId() == MATERIALS[10][i].getId()) && (from.getId() == CraftingDefinitions.CHISEL.getId() || to.getId() == CraftingDefinitions.CHISEL.getId()))
				return true;
		if ((from.getId() == CraftingDefinitions.THREAD.getId() || to.getId() == CraftingDefinitions.THREAD.getId()) && (from.getId() == CraftingDefinitions.NEEDLE.getId() || to.getId() == CraftingDefinitions.NEEDLE.getId()))
		...
		for (int i = 0; i < MATERIALS.length; i++) {
			for (int x = 0; x < MATERIALS[i].length; x++)
				if ((from.getId() == MATERIALS[i][x].getId() || to.getId() == MATERIALS[i][x].getId()) && (from.getId() == CraftingDefinitions.NEEDLE.getId() || to.getId() == CraftingDefinitions.NEEDLE.getId()) && from.getId() != CraftingDefinitions.THREAD.getId() && to.getId() != CraftingDefinitions.THREAD.getId())
					return true;
		}
		return false;
	}
	...
	public boolean start() {
		if (player.getSkills().getLevel(Skills.CRAFTING) < LEVELS[category][slotId]) {
	...
	public int processWithDelay() {
		...
		if (ticks == 0) {
		...
		} else if (ticks == (BoosterPerks.isActive(player, BoosterPerks.CRAFTING) ? (Utils.random(100) <= 5 ? 2 : 3) : 3)/* || ticks == 3*/) {
			...
			player.getSkills().addXp(Skills.CRAFTING, EXPERIENCE[category][slotId]);
