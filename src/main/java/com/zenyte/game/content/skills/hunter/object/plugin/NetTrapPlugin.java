...
public class NetTrapPlugin implements ObjectAction, ItemOnObjectAction {
    ...
    public void handleObjectAction(Player player, WorldObject object, String name, int optionId, String option) {
        switch (option) {
            ...
            case "Check": {
                ...
                val type = TrapType.NET_TRAP_SWAMP_LIZARD;
                val trap = HunterUtils.findTrap(type, object, null).orElseThrow(RuntimeException::new);
                player.getActionManager().setAction(new CheckPlacedTrap(trap, false));
                break;
            }
            case "Dismantle": {
                val type = TrapType.NET_TRAP_SWAMP_LIZARD;
                val failedTrap = HunterUtils.findTrap(type, object, null);
                ...
                val pair = HunterUtils.findNetTrapPair(object);
                val trap = HunterUtils.findTrap(type, pair.getNet(), null).orElseThrow(RuntimeException::new);
    ...
    public Object[] getItems() {
        return new Object[] {
                ItemId.SMALL_FISHING_NET, ItemId.ROPE
        };
    }

    @Override
    public Object[] getObjects() {
        val set = new ObjectOpenHashSet<>();
        for (val trap : NetTrap.getValues()) {
        ...
        val prey = new TrapPrey[] {
                TrapPrey.SWAMP_LIZARD, TrapPrey.ORANGE_SALAMANDER, TrapPrey.RED_SALAMANDER, TrapPrey.BLACK_SALAMANDER
        };

        for (val preyType : prey) {
            val objectType = Objects.requireNonNull(preyType.getObject());
            for (val object : objectType.getObjects()) {
                set.add(object);
            }
