...
public class JumpGap implements FailableAgilityPyramidObstacle, IrreversibleDirection {
    ...
    private void cross(final Player player, final WorldObject object, final boolean success) {
        ...
        val destination = player.getLocation().transform(walkDirection, success ? 3 : 1);
        player.faceObject(object);
        player.sendSound(new SoundEffect(success ? 2465 : 2463, 1, 20));
        player.setAnimation(success ? jumpAnim : fallAnim);
        WorldTasksManager.schedule(() ->
                player.setForceMovement(new ForceMovement(destination, 30, walkDirection.getDirection())));

        WorldTasksManager.schedule(() -> {
            if (!success) {
                player.setAnimation(Animation.STOP);
            }
            player.setLocation(success ? destination :
                    AgilityPyramidArea.getLowerTile(destination.transform(object.getFaceDirection(), 2)));
        }, success ? 1 : 7);
    }
