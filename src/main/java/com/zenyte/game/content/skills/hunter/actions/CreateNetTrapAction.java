...
public class CreateNetTrapAction extends ConstructableHunterTrapAction {
    ...
    @Nullable private WorldObject netTrap;
    @Nullable private NetTrap trapConstant;
    ...
    public int processWithDelay() {
        ...
        val existingPossibleTrap = HunterUtils.findTrap(BOX_TRAP, netTrap, null).orElseGet(() -> HunterUtils.findTrap(BIRD_SNARE, netTrap, null).orElse(null));
        if (existingPossibleTrap != null) {
            val runnable = existingPossibleTrap.getRemovalRunnable();
            if (runnable != null) {
                runnable.run();
            } else {
        ...
        val type = trapConstant.getType();
        val dummy = new HunterDummyNPC(type.getActiveDummyNpcId(), netTrap);
        val bentYoungTree = new WorldObject(trapConstant.getBentYoungTree(), this.youngTree.getType(), this.youngTree.getRotation(), this.youngTree);
        val rotation = this.youngTree.getRotation();
        val offsetLocation = rotation == 0 || rotation == 1 ? this.youngTree : this.netTrap;
        val collapsedAnimatedTree = new WorldObject(type.getCollapsedAnimatedObjectId(), this.youngTree.getType(), this.youngTree.getRotation(), offsetLocation);
        val trap = new HunterTrap(player, dummy, this.youngTree, Collections.singletonList(this.youngTree), Arrays.asList(bentYoungTree, netTrap), Collections.singletonList(collapsedAnimatedTree), type);
        dummy.setTrap(trap);
        ...
        val collapsedTree = new WorldObject(type.getCollapsedObjectId(), this.youngTree.getType(), this.youngTree.getRotation(), offsetLocation);
    ...
    TrapType[] trapTypes() {
        return new TrapType[] {
                TrapType.NET_TRAP_SWAMP_LIZARD, NET_TRAP_ORANGE_SALAMANDER, NET_TRAP_RED_SALAMANDER, NET_TRAP_BLACK_SALAMANDER
        };
    }
}
