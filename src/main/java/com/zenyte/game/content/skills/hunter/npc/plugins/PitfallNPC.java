...
public class PitfallNPC extends NPCPlugin implements ItemOnNPCAction {
    ...
    private static final void tease(@NotNull final Player player, @NotNull final NPC npc) {
        player.getActionManager().setAction(new Action() {
            ...
            public boolean start() {
                val name = npc.getName(player).toLowerCase();
            ...
            public int processWithDelay() {
                val teasedNPC = HunterUtils.getTeasedNPC(player);
                if (teasedNPC.isPresent()) {
                    val creature = teasedNPC.get();
    ...
    public int[] getNPCs() {
        return new int[] {
                NpcId.SPINED_LARUPIA, NpcId.HORNED_GRAAHK, NpcId.SABRETOOTHED_KYATT
        };
    }
