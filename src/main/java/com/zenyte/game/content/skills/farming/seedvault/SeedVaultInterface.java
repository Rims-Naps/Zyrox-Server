...
public class SeedVaultInterface extends Interface implements SwitchPlugin {
    ...
    private static final int INPUT_SCREEN_CLOSE_SCRIPT = 101, SEARCH_TYPE = 11;
    ...
    public void open(Player player) {
        ...
        val vaultSize = ContainerType.SEED_VAULT.getSize();
        dispatcher.sendComponentSettings(getInterface(), getComponent("Categories"), 0, 10, CLICK_OP1);
        dispatcher.sendComponentSettings(getInterface(), getComponent("Container"), 0, vaultSize, CLICK_OP1, CLICK_OP2,
                CLICK_OP3, CLICK_OP4, CLICK_OP5, CLICK_OP6, CLICK_OP7, CLICK_OP8, CLICK_OP9, CLICK_OP10, DRAG_DEPTH2,
                DRAG_TARGETABLE);
        GameInterface.SEED_VAULT_INVENTORY.open(player);
        player.getVarManager().sendBit(CATEGORY_VARBIT, 0);
        val container = player.getSeedVault().getContainer();
    ...
    protected void build() {
        bind("Categories", ((player, slotId, itemId, option) -> player.getVarManager().sendBit(CATEGORY_VARBIT,
                slotId)));
        bind("Container", ((player, seedSlot, itemId, optionId) -> {
            val option = SeedVaultExchangeOption.of(optionId);
            switch (option) {
                case REMOVE_ALL_PLACE:
                    player.getSeedVault().releasePlaceholders();
                    break;
                case NOTE_OR_REMOVE_PLACE:
                    val def = ItemDefinitions.getOrThrow(itemId);
                ...
                case SELECTED:
                    ...
                    break;
                case X:
                    player.sendInputInt("How much would you like to deposit?", amt -> withdraw(player, seedSlot, amt,
                            false));
                    break;
                case EXAMINE:
                    break;
                case FAVORITE:
                    val favouriteSlot = FavouriteSlot.getBySeedSlot(player, seedSlot);
    ...
    private void switchItem(final Player player, final int fromSlot, final int toSlot) {
        ...
        if (toSlot == ContainerType.SEED_VAULT.getSize()) {
            val existingSlot = FavouriteSlot.getBySeedSlot(player, fromSlot);
            if (existingSlot.isPresent()) {
                unfavourite(player, existingSlot.get(), fromSlot);
                return;
            }
            val favouriteSlot = FavouriteSlot.getFreeSlot(player);
            if (favouriteSlot.isPresent()) {
        ...
        val container = vault.getContainer();
        val fromItem = container.get(fromSlot);
        val toItem = container.get(toSlot);
        if (fromItem == null || toItem == null) {
            return;
        }
        val categoryMatches = isSameCategory(fromItem, toItem);
        val bothFavourites = FavouriteSlot.getBySeedSlot(player, fromSlot).isPresent()
                && FavouriteSlot.getBySeedSlot(player, toSlot).isPresent();
    ...
    private void favourite(final Player player, final int seedSlot) {
        ...
        val seed = player.getSeedVault().getContainer().get(seedSlot);
    ...
    private String getSeedName(final Item seed) {
        val seedDef = seed.getDefinitions();
    ...
    private int getCategory(@NotNull final Item item) {
        val categoryString = getCategoryString(item).orElse("NAN");
    ...
    public static Optional<String> getCategoryString(@NotNull final Item item) {
        val definitions = ItemDefinitions.getOrThrow(item.getDefinitions().getUnnotedOrDefault());
        val params = definitions.getParameters();
        if (params == null) {
            return Optional.empty();
        }
        val category = params.get(709);
        if (category == null) {
            return Optional.empty();
        }
