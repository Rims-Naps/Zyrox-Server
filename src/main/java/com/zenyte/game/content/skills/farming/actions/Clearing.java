...
public class Clearing extends Action {
    ...
    public boolean start() {
        if (spot.getPatch() == FarmingPatch.FARMING_GUILD_REDWOOD) {
            player.getDialogueManager().start(new PlainChat(player, "This looks like a rather complicated job to take on yourself.<br>Perhaps Alexandra is willing to clear this out " +
                    "for you instead."));
            return false;
        }
        ...
        val tree = spot.getProduct().isTree();
        val product = spot.getProduct();
        if ((state == PatchState.GROWING || state == PatchState.WATERED) && tree) {
            val permission = player.getTemporaryAttributes().remove(permissionAttr) != null;
            if (!permission) {
        ...
        if (state == PatchState.WEEDS) {
            ...
            player.getDialogueManager().start(new PlayerChat(player, "Dig up these healthy plants? Why would I " +
                    "want to do that?"));
            return false;
    ...
    public int processWithDelay() {
        ...
        if (tree) {
            val state = spot.getState();
            if (spot.getPatch().getType() != PatchType.CALQUAT_PATCH && spot.getPatch().getType() != PatchType.SPIRIT_TREE_PATCH && (state == PatchState.GROWN || state == PatchState.REGAINING_PRODUCE)) {
                player.sendMessage("You are too late - the tree has already regrown.");
                return -1;
            }
        }
        if ((tree && spot.getProduct().getTreeDefinitions() == TreeDefinitions.FRUIT_TREE) ||
                Utils.random(5) <= 3) {
            ...
            if (type == PatchType.HESPORI_PATCH) {
                if (value == 8) {
                    val seed = new Item(22881 + ((Utils.random(2) * 2)), Utils.random(1, 2));
                    ...
                    NPCDrops.forEach(NPCDrops.getTable(8583), drop -> {
                        val item = new Item(drop.getItemId(), Utils.random(drop.getMinAmount(),
                                drop.getMaxAmount()));
                    ...
                    spot.refresh();
                    player.getSkills().addXp(Skills.FARMING, product.getHarvestExperience());
                }
            }
            return -1;
        }
    ...
    private final int getRootsAmount() {
        val level = player.getSkills().getLevelForXp(Skills.FARMING);
