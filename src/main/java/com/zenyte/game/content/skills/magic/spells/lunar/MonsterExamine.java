...
public final class MonsterExamine extends Interface implements NPCSpell {
	...
	private static final Graphics PGFX = new Graphics(1059, 0, 32), NGFX = new Graphics(736);
	...
	private static final class MonsterExamineChaseAction extends Action {
        ...
        boolean pathfind() {
            ...
            if (player.isProjectileClipped(target, false)
                    || !withinRange(target, target.getSize())
                    || (target.hasWalkSteps() && !withinRange(target.getNextPosition(target.isRun() ? 2 : 1), target.getSize()))) {
                appendWalksteps();
        ...
        final boolean withinRange(final Position targetPosition, final int targetSize) {
            ...
            if (distanceX == -npcSize - 10 && distanceY == -npcSize - 10
                    || distanceX == targetSize + 10 && distanceY == targetSize + 10
                    || distanceX == -npcSize - 10 && distanceY == targetSize + 10
                    || distanceX == targetSize + 10 && distanceY == -npcSize - 10) {
                return false;
            }

            return !(distanceX > targetSize + 10
                    || distanceY > targetSize + 10
                    || distanceX < -npcSize - 10
                    || distanceY < -npcSize - 10);
        }
        ...
        protected boolean isWithinAttackDistance() {
            ...
            if (checkProjectile && ProjectileUtils.isProjectileClipped(player, target,
                    player.getNextPosition(player.isRun() ? 2 : 1), target, false)) {
                return false;
            }
            int maxDistance = 10;
    ...
    private static final String getStats(final NPC npc) {
        val combat = npc.getCombatDefinitions();
        val statDefs = combat.getStatDefinitions();
        return "<col=c6a901>Stats</col><br>" + (npc.getCombatLevel() <= 0 ? "" : ("Combat level : " + npc.getCombatLevel()+ "<br>")) + "Hitpoints : "
                + npc.getHitpoints() + "<br>" + "Attack : " + statDefs.get(StatType.ATTACK) + "<br>" + "Defence : "
                + statDefs.get(StatType.DEFENCE) + "<br>" + "Strength : " + statDefs.get(StatType.STRENGTH) + "<br>"
                + "Magic : " + statDefs.get(StatType.MAGIC) + "<br>" + "Ranged : " + statDefs.get(StatType.RANGED)
                + (combat.getMaxHit() <= 0 ? "" : ("<br>" + "Max standard hit : " + combat.getMaxHit())) + "<br>" + "Main attack style: "
                + Utils.formatString(combat.getAttackStyle().toString()) + "<br>";
    }
	
	private static final String getAggressiveStats(final NPC npc) {
		val combat = npc.getCombatDefinitions();
		val statDefs = combat.getStatDefinitions();
        return "<col=c6a901>Aggressive Stats</col><br>" + "Attack speed : " + combat.getAttackSpeed() + "<br>"
                + "Stab bonus : " + statDefs.get(StatType.ATTACK_STAB) + "<br>" + "Slash bonus : "
                + statDefs.get(StatType.ATTACK_SLASH) + "<br>" + "Crush bonus : " + statDefs.get(StatType.ATTACK_CRUSH)
                + "<br>" + "Magic bonus : " + statDefs.get(StatType.ATTACK_MAGIC) + "<br>" + "Ranged bonus : "
                + statDefs.get(StatType.ATTACK_RANGED) + "<br>" + "Strength bonus : " + statDefs.get(StatType.MELEE_STRENGTH_BONUS)
                + "<br>" + "Ranged strength bonus : " + statDefs.get(StatType.RANGED_STRENGTH_BONUS) + "<br>"
                + "Attack bonus : " + statDefs.get(combat.getAttackStatType()) + "<br>";
	}
	
	private static final String getDefensiveStats(final NPC npc) {
        val combat = npc.getCombatDefinitions();
        val statDefs = combat.getStatDefinitions();
        return "<col=c6a901>Defensive Stats</col><br>" + "Stab bonus : " + statDefs.get(StatType.DEFENCE_STAB)
                + "<br>" + "Slash bonus : " + statDefs.get(StatType.DEFENCE_SLASH) + "<br>" + "Crush bonus : "
                + statDefs.get(StatType.DEFENCE_CRUSH) + "<br>" + "Magic bonus : " + statDefs.get(StatType.DEFENCE_MAGIC)
                + "<br>" + "Ranged bonus : " + statDefs.get(StatType.DEFENCE_RANGED) + "<br>";
	}
    ...
    public void open(Player player) {
        val attr = player.getTemporaryAttributes().get("monster examine npc");
        ...
        dispatcher.sendClientScript(1179,
                getStats(npc) + "|" + getAggressiveStats(npc) + "|" + getDefensiveStats(npc) + "|" + getOtherAttributes(npc) + "|" + npc.getDefinitions().getName());
    }
