...
public final class StatRestorePotShare implements ItemSpell {
	...
	private static final Potion[] VALID_POTIONS = new Potion[] {
			Potion.SANFEW_SERUM, Potion.ANTIFIRE_POTION, Potion.SUPER_ANTIFIRE_POTION, Potion.EXTENDED_ANTIFIRE,
			Potion.EXTENDED_SUPER_ANTIFIRE, Potion.ANTIPOISON, Potion.SUPERANTIPOISON, Potion.ANTIDOTE_PLUS,
			Potion.ANTI_VENOM, Potion.ANTI_VENOM_PLUS, Potion.ENERGY_POTION, Potion.SUPER_ENERGY_POTION,
			Potion.RESTORE_POTION, Potion.SUPER_RESTORE_POTION, Potion.PRAYER_POTION, Potion.STAMINA_POTION
	};
	...
	public boolean spellEffect(final Player player, final Item item, final int slot) {
	    val consumable = Consumable.consumables.get(item.getId());
        ...
        val potion = (Potion) consumable;
        int count = 0;
	    val doses = potion.getDoses(item.getId());

        val characters = player.findCharacters(1, Player.class, p2 ->
                p2 != player && !p2.isDead() && p2.getBooleanSetting(Setting.ACCEPT_AID) && p2.getDuel() == null);

        for (int i = characters.size() - 1; i >= 0; i--) {
            val p2 = characters.get(i);
            if (count >= doses) {
                break;
            }
            count++;
        ...
        val leftover = potion.getItem(dosesLeft);
