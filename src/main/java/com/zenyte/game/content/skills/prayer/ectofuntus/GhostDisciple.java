    ...
    public void handle() {
        bind("Talk-to", (player, npc) -> player.getDialogueManager().start(new Dialogue(player, npc) {
            ...
            public void buildDialogue() {
                val eligible = DiaryReward.MORYTANIA_LEGS1.eligibleFor(player) && DiaryReward.MORYTANIA_LEGS2.eligibleFor(player) && DiaryReward.MORYTANIA_LEGS3.eligibleFor(player);
                if (!eligible) {
                    player("Hello. I'd like to claim my ecto-tokens.");
                    val tokens = player.getNumericAttribute("ecto-tokens").intValue();
                    if (tokens == 0) {
