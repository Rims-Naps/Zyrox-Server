...
public class MasterScrollBookInterface extends Interface {
    ...
    public void open(Player player) {
        ...
        for (val teleport : ScrollBookTeleport.values) {
    ...
    private final void refresh(@NotNull final Player player, @NotNull final Item book, @NotNull final ScrollBookTeleport teleport) {
        val vm = player.getVarManager();
        val count = Math.max(0, Math.min(MAXIMUM_TELEPORTS_COUNT, getCount(book, teleport.getScrollId())));
        if (teleport != ScrollBookTeleport.WATSON) {
            vm.sendBit(teleport.getVarbit(), count);
        } else {
            vm.sendBit(8252, count >> 8);
            vm.sendBit(8253, count & 0xFF);
        }
    ...
    private final void setFavourite(@NotNull final Player player, @NotNull final ScrollBookTeleport teleport) {
        ...
        player.getDialogueManager().start(new Dialogue(player) {
            ...
            public void buildDialogue() {
                val label = Utils.formatString(teleport.toString());
                options("Set " + label + " teleport as your default teleport?",
    ...
    static final void removeFavourite(@NotNull final Player player) {
        player.getDialogueManager().start(new Dialogue(player) {
            @Override
            public void buildDialogue() {
                options("Remove your default teleport?",
                        new DialogueOption("Yes", () -> {
    ...
    static final Optional<ScrollBookTeleport> getFavourite(@NotNull final Player player) {
        val favouriteIndex = player.getNumericAttribute(favouriteKey).intValue();
        if (favouriteIndex == 0) {
    ...
    private final void handle(@NotNull final Player player, @NotNull final ScrollBookTeleport teleport, final int option) {
        ...
        switch (ScrollOption.values.get(option - 1)) {
            ...
            case SET_AS_DEFAULT:
                ...
                break;
            case REMOVE:
                val count = getCount(book, teleport.getScrollId());
                if (count <= 0) {
                ...
                player.sendInputInt("How many scrolls would you like to remove? (0-" + count + ")", value -> {
                    ...
                    val currentCount = getCount(book, teleport.getScrollId());
                    val removableCount = Math.max(0, Math.min(currentCount, value));
                    if (removableCount <= 0) {
                        return;
                    }
                    ...
                    player.sendMessage("You remove " + removableCount + " " + ItemDefinitions.getOrThrow(teleport.getScrollId()).getName() + (removableCount == 1 ? "" : "s")
                            + " from your Master Scroll Book.");
                    refresh(player, book, teleport);
                });
                break;
        }
    ...
    private static final boolean isEmpty(@NotNull final Item scrollBook) {
        val attrs = scrollBook.getAttributes();
