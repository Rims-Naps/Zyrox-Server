...
public class MasterScrollBook extends ItemPlugin implements PairedItemOnItemPlugin {
    ...
    public void handle() {
        ...
        bind("Teleport", (player, item, slotId) -> {
            val optionalFavourite = MasterScrollBookInterface.getFavourite(player);
    ...
    public void handleItemOnItemAction(Player player, Item from, Item to, int fromSlot, int toSlot) {
        val book = from.getId() == ItemId.MASTER_SCROLL_BOOK_EMPTY || from.getId() == ItemId.MASTER_SCROLL_BOOK ? from : to;
        val scrolls = from == book ? to : from;
        val count = MasterScrollBookInterface.getCount(book, scrolls.getId());
        val addableCount = Math.min(MasterScrollBookInterface.MAXIMUM_TELEPORTS_COUNT - count, scrolls.getAmount());
        if (addableCount <= 0) {
        ...
        player.sendMessage("Your Master Scroll Book is now storing " + (count + addableCount) + " " + ItemDefinitions.getOrThrow(scrolls.getId()).getName() +
                ((count + addableCount) == 1 ? "" : "s") + ".");
    ...
    public int[] getItems() {
        return new int[] {
                ItemId.MASTER_SCROLL_BOOK, ItemId.MASTER_SCROLL_BOOK_EMPTY
        };
    }
