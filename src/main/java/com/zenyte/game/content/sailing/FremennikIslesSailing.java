...
public class FremennikIslesSailing {
	...
	public enum SailingDestination {
		...
		MISCELLANIA(1372, 1373, "Miscellania", new Location(2631, 3693, 0), new Location(2578, 3844, 0)),
		WATERBIRTH_ISLAND(2344, 2345, "Waterbirth Island", new Location(2620, 3686, 0), new Location(2546, 3758, 0)),
		NEITIZNOT(5764, 5765, "Neitiznot", new Location(2646, 3709, 0), new Location(2310, 3783, 0)),
		JATIZSO(5766, 5767, "Jatizso", new Location(2646, 3709, 0), new Location(2419, 3781, 0));

		private final int toAnimation, fromAnimation;
		private final String areaName;
		private final Location rellekka, destination;
	...
	public static final void sail(final Player player, final SailingDestination destination) {
		val atDestination = player.inArea(destination.areaName);
		player.getInterfaceHandler().sendInterface(InterfacePosition.CENTRAL, INTERFACE_ID);
		val animation = atDestination ? destination.fromAnimation : destination.toAnimation;
		player.getPacketDispatcher().sendComponentAnimation(INTERFACE_ID, SHIP_COMPONENT_ID, animation);
		val duration = (int) TimeUnit.MILLISECONDS.toTicks(AnimationDefinitions.get(animation).getDuration());
