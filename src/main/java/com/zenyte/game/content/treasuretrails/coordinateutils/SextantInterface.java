...
import static com.zenyte.game.content.treasuretrails.clues.CoordinateClue.LONGITUDE_OFFSET;
import static com.zenyte.game.content.treasuretrails.clues.CoordinateClue.LATITUDE_OFFSET;
import static com.zenyte.game.content.treasuretrails.clues.CoordinateClue.MINUTE_TO_COORDINATE_MULTIPLIER;
...
public class SextantInterface extends Interface {
    ...
    public void open(Player player) {
        ...
        val framePosition = Utils.random(SextantPosition.FRAME_POSITIONS_COUNT - 1);
        val armPosition = Math.max(framePosition,
                Math.min(SextantPosition.ARM_POSITIONS_COUNT - (SextantPosition.FRAME_POSITIONS_COUNT - ARM_RIGHT_OFFSET - framePosition),
                        Utils.random(SextantPosition.ARM_POSITIONS_COUNT - 1)));
        val sextantPosition = new SextantPosition(framePosition, armPosition);
    ...
    private final void refreshSextantPosition(@NotNull final Player player) {
        ...
        val armPosition = Math.max(sextantPosition.getFramePosition(),
                Math.min(SextantPosition.ARM_POSITIONS_COUNT - (SextantPosition.FRAME_POSITIONS_COUNT - ARM_RIGHT_OFFSET - sextantPosition.getFramePosition()),
                        sextantPosition.getArmPosition()));
    ...
    private final int getSunVerticalPosition(@NotNull final Player player) {
        val sextantPosition = getSextantPosition(player);
        val sunOffset = (Math.min(COORD_CAP, player.getX()) >> SUN_OFFSET_COORD_RATIO) - SUN_OFFSET_COORD_RATIO;
        val armPosition = Math.max(sextantPosition.getFramePosition(),
                Math.min(SextantPosition.ARM_POSITIONS_COUNT - (SextantPosition.FRAME_POSITIONS_COUNT - ARM_RIGHT_OFFSET - sextantPosition.getFramePosition()),
                        sextantPosition.getArmPosition()));
    ...
    private final void sendCoordinates(@NotNull final Player player) {
        ...
        val totalLatitudeMinutes = (int) Math.floor(xOffset * MINUTE_TO_COORDINATE_MULTIPLIER);
        val totalLongitudeMinutes = (int) Math.floor(yOffset * MINUTE_TO_COORDINATE_MULTIPLIER);
        ...
        val longitudeMessage = longitudeDegrees + " degree" + (longitudeDegrees == 1 ? "" : "s") + " " + longitudeMinutes +
                " minute" + (longitudeMinutes == 1 ? "" : "s") +" " + (south ? "south" : "north");
        val latitudeMessage = latitudeDegrees + " degree" + (latitudeDegrees == 1 ? "" : "s") + " " + latitudeMinutes +
                " minute" + (latitudeMinutes == 1 ? "" : "s") +" " + (west ? "west" : "east");
    ...
    private final void move(@NotNull final Player player, @MagicConstant(intValues = {-1, 1}) final int horizontalOffset, @MagicConstant(intValues = {-1, 0, 1}) final int verticalOffset) {
        ...
        val framePosition = Math.max(0, Math.min(currentSextantPosition.getFramePosition() + verticalOffset, SextantPosition.FRAME_POSITIONS_COUNT - 1));
        val horizontalConditionalOffset = verticalOffset == 0 || framePosition != currentSextantPosition.getFramePosition() ? horizontalOffset : 0;
        val armPosition = Math.max(0, Math.min(currentSextantPosition.getArmPosition() + horizontalConditionalOffset, SextantPosition.ARM_POSITIONS_COUNT - 1));
        val newSextantPosition = new SextantPosition(framePosition, horizontalConditionalOffset == 0 ? armPosition : (Math.max(framePosition,
                Math.min(SextantPosition.ARM_POSITIONS_COUNT - (SextantPosition.FRAME_POSITIONS_COUNT - 2 - framePosition), armPosition))));
