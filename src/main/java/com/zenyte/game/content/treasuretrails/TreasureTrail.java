...
public class TreasureTrail {
    ...
    private static final Optional<ClueLevel> getLevel(@NotNull final Item item) {
        val clueItem = ClueItem.getMap().get(item.getId());
    ...
    public static final void setClue(@NotNull final Item item, final Clue clue) {
        val level = Objects.requireNonNull(getLevel(item).orElseThrow(RuntimeException::new));
        val range = Utils.random(1, 1);
    ...
    public static final Clue setRandomClue(@NotNull final Item item) {
        val level = Objects.requireNonNull(getLevel(item).orElseThrow(RuntimeException::new));
        val clue = TreasureTrailType.random(level);
        val range = Utils.random(level.getStepsRange().getFirst(), level.getStepsRange().getLast());
        item.resetAttributes();
        item.setAttribute(Constants.CLUE_SCROLL_NAME, level == ClueLevel.MASTER && clue instanceof CrypticClue ? (((CrypticClue) clue).selectMasterCrypticClues()) : Collections.singletonList(clue.getEnumName()));
        item.setAttribute(Constants.CLUE_SCROLL_TOTAL_STEPS, range);
        return clue;
    ...
    public static final List<CrypticClue> getCrypticClues(@NotNull final Item item) {
        ...
        val attr = item.getAttribute(Constants.CLUE_SCROLL_NAME);
        assert attr instanceof List;
        @SuppressWarnings("unchecked") val list = (List<String>) attr;
        val typedList = new ObjectArrayList<CrypticClue>();
        for (val element : list) {
            val typedClue = getClues().get(element);
    ...
    private static final boolean advance(@NotNull final Item item) {
        val currentSteps = item.getNumericAttribute(Constants.CLUE_SCROLL_CURRENT_STEPS).intValue();
        val totalSteps = item.getNumericAttribute(Constants.CLUE_SCROLL_TOTAL_STEPS).intValue();
    ...
    private static final <T> boolean findAndProgress(@NotNull final Player player, @NotNull final Class<T> clazz, @NotNull final Predicate<T> predicate, Consumer<ClueEntry> progressConsumer) {
        val entry = findClueScroll(player, clazz, predicate);
    ...
    private static final <T> Optional<ClueEntry> findClueScroll(@NotNull final Player player, @NotNull final Class<T> clazz, @NotNull final Predicate<T> predicate) {
        ...
        for (val entry : inventory.getContainer().getItems().int2ObjectEntrySet()) {
            ...
            val list = getCluesList(item);
            if (list == null) {
                continue;
            }
            for (int i = 0; i < list.size(); i++) {
                val constant = list.get(i);
                val clueScroll = getClues().get(constant);
                val challenge = clueScroll.getChallenge();
    ...
    public static final void progress(@NotNull final Player player, @NotNull final ClueEntry entry, @NotNull final Optional<Dialogue> optionalDialogue) {
        val list = getCluesList(entry.getItem());
        assert list != null;
        int index = -1;
        if (list.size() > 1) {
    ...
    public static final void progressTripleCryptic(@NotNull final Player player, @NotNull final Item item) {
        val entry = player.getInventory().getContainer().getItems().int2ObjectEntrySet().stream().filter(e -> e.getValue().equals(item)).findFirst();
        if (!entry.isPresent()) {
            return;
        }
    ...
    private static final void progress(@NotNull final Player player, @NotNull final Item item, final int slot, @NotNull final Optional<Dialogue> optionalDialogue, final int index) {
        if (index != -1) {
            val dialogue = optionalDialogue.orElseGet(() -> new EmptyDialogue(player));
            player.getDialogueManager().start(dialogue);
            if (item.getNumericAttribute(Constants.CLUE_SCROLL_STAGE).intValue() == (0x1 | 0x2 | 0x4)) {
                player.getInventory().deleteItem(slot, item);
            }
            if (dialogue.getOnBuild() == null) {
                val tornPiece = new Item(19837 + index);
        ...
        val advanceStep = TreasureTrail.advance(item);
        val dialogueItem = advanceStep ? item : new Item(Objects.requireNonNull(ClueItem.getMap().get(item.getId())).getCasket());
        if (!advanceStep) {
            val inventory = player.getInventory();
    ...
    public static final void playSong(@NotNull final Player player, @NotNull final String songName) {
        ...
        findClueScroll(player, SongRequest.class, clue -> clue.getSong().equalsIgnoreCase(songName)).ifPresent(entry -> {
            val optionalNPC = World.findNPC(Constants.CECILIA_NPC_ID, player.getLocation(), Constants.NPC_SEEK_DISTANCE);
    ...
    public static final boolean dig(@NotNull final Player player) {
        val entry = findClueScroll(player, DigRequest.class, clue -> clue.getLocation().getTileDistance(player.getLocation()) <= 1)
                .orElseGet(() -> findClueScroll(player, HotColdChallenge.class,
                        clue -> Math.max(Math.abs(clue.getCenter().getX() - player.getX()), Math.abs(clue.getCenter().getY() - player.getY())) < Constants.MASTER_HOT_COLD_RADIUS).orElse(null));
        if (entry == null ||
                (entry.getClueScroll() instanceof HotColdClue && entry.getClueScroll().level() == ClueLevel.BEGINNER &&
                        Math.max(Math.abs(((HotColdChallenge) entry.getChallenge()).getCenter().getX() - player.getX()),
                Math.abs(((HotColdChallenge) entry.getChallenge()).getCenter().getY() - player.getY())) >= Constants.BEGINNER_HOT_COLD_RADIUS)) {
            return false;
        }
        val guardian = new MutableObject<TreasureGuardianNPC>();
        val challenge = entry.getChallenge();
    ...
    private static final boolean interruptedByGuardians(@NotNull final Player player, @Nullable final TreasureGuardianNPC type) {
        ...
        val guardianAttr = player.getTemporaryAttributes().get(Constants.GUARDIAN_NPC_LIST);
        ...
        if (guardianAttr == null) {
            val tile = getSpawnTile(player.getLocation());
            val list = new ArrayList<TreasureGuardian>();
            if (type == TreasureGuardianNPC.GUARDIAN) {
                ...
                if (player.isMultiArea()) {
                    val usedHashSet = new IntOpenHashSet();
                    for (int a = 0; a < 3; a++) {
                        val t = a == 0 ? tile : getSpawnTile(player.getLocation(), position -> !usedHashSet.contains(position.getPosition().getPositionHash()));
    ...
    public static final Optional<HotColdResult> getHotColdResult(@NotNull final Player player, final boolean beginner) {
        val slot = TreasureTrail.findHotColdClue(player, beginner);
        if (!slot.isPresent()) {
            return Optional.empty();
        }
        val clueItem = Objects.requireNonNull(player.getInventory().getItem(slot.getAsInt()));
        val list = getCluesList(clueItem);
        if (list == null) {
            throw new IllegalStateException();
        }
        for (val constantName : list) {
            val clueScroll = TreasureTrail.getClues().get(constantName);
            val challenge = clueScroll.getChallenge();
            Preconditions.checkArgument(challenge instanceof HotColdChallenge);
            val hc = (HotColdChallenge) challenge;
            val center = hc.getCenter();
            val tile = player.getLocation();
            val distance = Math.max(Math.abs(center.getX() - tile.getX()), Math.abs(center.getY() - tile.getY()));
            val temperature = DeviceTemperature.get(clueScroll.level() == ClueLevel.BEGINNER, distance);
    ...
    private static final Location getSpawnTile(@NotNull final Location tile, @Nullable final Predicate<Position> predicate) {
        return Utils.findEmptySquare(tile, Constants.SPAWN_TILE_SEEK_RADIUS, Constants.SPAWN_TILE_NPC_SIZE,
                Optional.of(t -> !ProjectileUtils.isProjectileClipped(null, null, tile, t, true) && !tile.matches(t) && (predicate == null || predicate.test(t)))).orElse(tile);
    }
    ...
    private static final Item[] buildEquipmentArray(@NotNull final Player player) {
        val equipment = new Item[ContainerType.EQUIPMENT.getSize()];
        val entries = player.getEquipment().getContainer().getItems();
        for (int a = ContainerType.EQUIPMENT.getSize() - 1; a >= 0; a--) {
            val entry = entries.get(a);
    ...
    public static final void emoteChallenge(@NotNull final Player player, @NotNull final Emote emote) {
        ...
        findClueScroll(player, EmoteRequest.class, clue -> {
            ...
            val emotes = clue.getEmotes();
            if (emotes.size() == 2 && emotes.get(1) == emote) {
                val uri = player.getTemporaryAttributes().get("spawned uri npc");
            ...
            val invokeUri = emotes.get(0) == emote;
            if (invokeUri) {
                if (clue.isAgents()) {
                    val existingAgent = player.getTemporaryAttributes().get("double agent clue");
                    val containsAgent = existingAgent instanceof DoubleAgent;
                    val defeatedAgent = containsAgent && (((DoubleAgent) existingAgent).isDead() || ((DoubleAgent) existingAgent).isFinished());
                    if (!containsAgent) {
                        val agent = new DoubleAgent(clue.getLevel(), getSpawnTile(player.getLocation()), player, clue);
                        agent.spawn();
    ...
    public static final boolean searchKeyObject(@NotNull final Player player, @NotNull final WorldObject object, @NotNull final String option) {
        return findAndProgress(player, KeyRequest.class,
                clue -> Utils.findMatching(clue.getValidObjects(), s -> s.getTile().matches(object) && option.equalsIgnoreCase(s.getOption())) != null, entry -> progress(player, entry, Optional.empty()));
    }
    ...
    public static final boolean search(@NotNull final Player player, @NotNull final WorldObject object, @NotNull final String option) {
        return findAndProgress(player, SearchRequest.class, clue -> Utils.findMatching(clue.getValidObjects(), s -> s.getTile().matches(object) && option.equalsIgnoreCase(s.getOption())) != null,
                entry -> progress(player, entry, Optional.empty()));
    }
    ...
    private static final boolean talkChallenge(@NotNull final Player player, @NotNull final NPC npc) {
        val transmogrifiedId = player.getTransmogrifiedId(npc.getDefinitions(), npc.getId());
        val clueEntry = findClueScroll(player, TalkChallengeRequest.class, clue -> ArrayUtils.contains(clue.getValidNPCs(), transmogrifiedId));
        clueEntry.ifPresent(clue -> {
            Preconditions.checkArgument(clue.getChallenge() instanceof TalkChallengeRequest);
            val talkChallenge = (TalkChallengeRequest) clue.getChallenge();
    ...
    private static final boolean talkPuzzleBox(@NotNull final Player player, @NotNull final NPC npc) {
        val transmogrifiedId = player.getTransmogrifiedId(npc.getDefinitions(), npc.getId());
        val clueEntry = findClueScroll(player, PuzzleRequest.class, clue -> ArrayUtils.contains(clue.getValidNPCs(), transmogrifiedId));
        clueEntry.ifPresent(clue -> {
            val puzzleboxSlot = findPuzzleBox(player, clue.getClueScroll().getEnumName());
            if (puzzleboxSlot.isPresent()) {
                val inventory = player.getInventory();
                val puzzleboxItem = inventory.getItem(puzzleboxSlot.getAsInt());
            ...
            player.getDialogueManager().start(new Dialogue(player, npc) {
                ...
                public void buildDialogue() {
                    ...
                    val puzzle = new Item(Puzzle.random(clue.getClueScroll().level()).getPuzzleBox());
                    puzzle.setAttribute(Constants.CLUE_SCROLL_NAME, Collections.singletonList(clue.getClueScroll().getEnumName()));
                    val currentSteps = clue.getItem().getNumericAttribute(Constants.CLUE_SCROLL_CURRENT_STEPS).intValue();
                    val totalSteps = clue.getItem().getNumericAttribute(Constants.CLUE_SCROLL_TOTAL_STEPS).intValue();
                    ...
                    npc("Well done, you've found me!<br>" +
                            "Solve the puzzle to continue trailing:").executeAction(() -> player.getInventory().addOrDrop(puzzle));
    ...
    private static final boolean talkLightBox(@NotNull final Player player, @NotNull final NPC npc) {
        val transmogrifiedId = player.getTransmogrifiedId(npc.getDefinitions(), npc.getId());
        val clueEntry = findClueScroll(player, LightBoxRequest.class, clue -> ArrayUtils.contains(clue.getValidNPCs(), transmogrifiedId));
        clueEntry.ifPresent(clue -> {
            val lightboxSlot = findLightBox(player, clue.getClueScroll().getEnumName());
            if (lightboxSlot.isPresent()) {
                val inventory = player.getInventory();
                val lightboxItem = inventory.getItem(lightboxSlot.getAsInt());
            ...
            player.getDialogueManager().start(new Dialogue(player, npc) {
                ...
                public void buildDialogue() {
                    ...
                    val puzzle = new Item(ItemId.LIGHT_BOX);
                    puzzle.setAttribute(Constants.CLUE_SCROLL_NAME, Collections.singletonList(clue.getClueScroll().getEnumName()));
                    npc("Well done, you've found me!<br>" +
                            "Solve the puzzle to continue trailing:").executeAction(() -> player.getInventory().addOrDrop(puzzle));
    ...
    public static final boolean withoutHotColdClue(@NotNull final Player player, final boolean beginner) {
        ...
        for (val entry : inventory.getContainer().getItems().int2ObjectEntrySet()) {
            ...
            for (val constantName : list) {
                val constant = getClues().get(constantName);
    ...
    public static final OptionalInt getKey(@NotNull final Player player, @NotNull final NPC npc) {
        val clue = findClueScroll(player, KeyRequest.class, predicate -> ArrayUtils.contains(predicate.getValidNPCs(), player.getTransmogrifiedId(npc.getDefinitions(), npc.getId())));
    ...
    public static final boolean talk(@NotNull final Player player, @NotNull final NPC npc) {
        ...
        val transmogrifiedId = player.getTransmogrifiedId(npc.getDefinitions(), npc.getId());
        val clueEntry = findClueScroll(player, TalkRequest.class, clue -> ArrayUtils.contains(clue.getValidNPCs(),
                transmogrifiedId) && (clue.getPredicate() == null || clue.getPredicate().test(player)));
    ...
    public static final void kill(@NotNull final Player player, @NotNull final NPC npc) {
        findAndProgress(player, KillRequest.class, clue -> ArrayUtils.contains(clue.getValidNPCs(), player.getTransmogrifiedId(npc.getDefinitions(), npc.getId())),
                entry -> progress(player, entry, Optional.empty()));
    }
