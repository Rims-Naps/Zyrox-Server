...
public class ClueRewardInterface extends Interface {
    ...
    public void open(Player player) {
        val loot = player.getTemporaryAttributes().remove("treasure trails loot");
        if (!(loot instanceof List)) {
            throw new IllegalStateException("No loot list provided.");
        }
        @SuppressWarnings("unchecked cast")
        val rewards = (List<Item>) loot;
        val container = new Container(ContainerPolicy.ALWAYS_STACK, ContainerType.BARROWS_CHEST, Optional.of(player));
    ...
    public void close(final Player player, final Optional<GameInterface> replacement) {
        val container = player.getTemporaryAttributes().remove("treasure trails rewards container");
        if (!(container instanceof Container)) {
            return;
        }
        val typedContainer = (Container) container;
        val inventory = player.getInventory();
        val log = player.getCollectionLog();
        for (val itemEntry : typedContainer.getItems().int2ObjectEntrySet()) {
