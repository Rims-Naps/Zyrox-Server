...
public class WatsonPlugin extends NPCPlugin {
    ...
    public void handle() {
        bind("Talk-to", (player, npc) -> player.getDialogueManager().start(new Dialogue(player, npc) {
            ...
            public void buildDialogue() {
                ...
                val depositedCluesHash = player.getNumericAttribute("Watson handed in clue scrolls hash").intValue();
                val inventory = player.getInventory();
                val canHandInEasy = (depositedCluesHash & 0x1) == 0 && inventory.containsItem(ClueItem.EASY.getClue(), 1);
                val canHandInMedium = (depositedCluesHash & 0x2) == 0 && inventory.containsItem(ClueItem.MEDIUM.getClue(), 1);
                val canHandInHard = (depositedCluesHash & 0x4) == 0 && inventory.containsItem(ClueItem.HARD.getClue(), 1);
                val canHandInElite = (depositedCluesHash & 0x8) == 0 && inventory.containsItem(ClueItem.ELITE.getClue(), 1);
                val canDepositAny = canHandInEasy || canHandInMedium || canHandInHard || canHandInElite;
                val depositableCluesCount = BooleanUtils.toInteger(canHandInEasy) + BooleanUtils.toInteger(canHandInMedium)
                        + BooleanUtils.toInteger(canHandInHard) + BooleanUtils.toInteger(canHandInElite);
                val canDepositMoreThanOne = depositableCluesCount > 1;

                options(TITLE, new DialogueOption("Who are you?", key(100)),
                        new DialogueOption("Have you heard of Sherlock?", key(200)),
                        new DialogueOption("So what are master clue scrolls anyway?", key(300)),
                        canDepositAny ? new DialogueOption("I have something for you.", key(400)) : null,
                        new DialogueOption("Goodbye.", key(500)));
                ...
                options(TITLE, new DialogueOption("Have you heard of Sherlock?", key(200)),
                        new DialogueOption("So what are master clue scrolls anyway?", key(300)),
                        canDepositAny ? new DialogueOption("I have something for you.", key(400)) : null,
                        new DialogueOption("Goodbye.", key(500)));
                ...
                options(TITLE,
                        canHandInEasy ? new DialogueOption("Hand over easy clue.", () -> setKey(deposit(player, ClueItem.EASY, false) ? 410 : 460)) : null,
                        canHandInMedium? new DialogueOption("Hand over medium clue.", () -> setKey(deposit(player, ClueItem.MEDIUM, false) ? 420 : 460)) : null,
                        canHandInHard ? new DialogueOption("Hand over hard clue.", () -> setKey(deposit(player, ClueItem.HARD, false) ? 430 : 460)) : null,
                        canHandInElite ? new DialogueOption("Hand over elite clue.", () -> setKey(deposit(player, ClueItem.ELITE, false) ? 440 : 460)) : null,
                        new DialogueOption(canDepositMoreThanOne ? "Hand over all clues." : "Cancel.", () -> {
        ...
        bind("Check", (player, npc) -> player.getDialogueManager().start(new Dialogue(player, npc) {
            ...
            public void buildDialogue() {
                ...
                if ((depositedCluesHash & 0x1) == 0x1) {
                    builder.append("an easy clue scroll, ");
                }
                if ((depositedCluesHash & 0x2) == 0x2) {
                    builder.append("a medium clue scroll, ");
                }
                if ((depositedCluesHash & 0x4) == 0x4) {
                    builder.append("a hard clue scroll, ");
                }
                if ((depositedCluesHash & 0x8) == 0x8) {
                ...
                npc(builder.length() == 0 ? "You haven't given me any clue scrolls yet." : "You have given me " + builder.toString() + " so far.");
            }
        }));
    ...
    private final boolean depositAll(@NotNull final Player player) {
        val depositedCluesHash = player.getNumericAttribute("Watson handed in clue scrolls hash").intValue();
        val containsEasy = (depositedCluesHash & 0x1) == 0 && deposit(player, ClueItem.EASY, true);
        val containsMedium = (depositedCluesHash & 0x2) == 0 && deposit(player, ClueItem.MEDIUM, true);
        val containsHard = (depositedCluesHash & 0x4) == 0 && deposit(player, ClueItem.HARD, true);
        val containsElite = (depositedCluesHash & 0x8) == 0 && deposit(player, ClueItem.ELITE, true);
    ...
    private final boolean deposit(@NotNull final Player player, @NotNull final ClueItem type, final boolean plural) {
        ...
        val inventory = player.getInventory();
        val success = inventory.deleteItem(type.getClue(), 1).getSucceededAmount() > 0;
        if (success) {
            val ordinal = type.ordinal() - 1;
            val depositedCluesHash = player.getNumericAttribute("Watson handed in clue scrolls hash").intValue() | (1 << ordinal);
            val completed = depositedCluesHash == (0x1 | 0x2 | 0x4 | 0x8);
