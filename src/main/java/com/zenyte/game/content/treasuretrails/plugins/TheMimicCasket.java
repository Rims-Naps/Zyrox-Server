...
public class TheMimicCasket extends ItemPlugin {
    ...
    private static final List<ImmutableItem> bonusLoot = Arrays.asList(
            new ImmutableItem(ItemId.DEATH_RUNE, 480, 600),
            new ImmutableItem(ItemId.BLOOD_RUNE, 400, 500),
            new ImmutableItem(ItemDefinitions.getOrThrow(ItemId.GRIMY_RANARR_WEED).getNotedOrDefault(), 25),
            new ImmutableItem(ItemDefinitions.getOrThrow(ItemId.RAW_MANTA_RAY).getNotedOrDefault(), 25),
            new ImmutableItem(ItemDefinitions.getOrThrow(ItemId.WINE_OF_ZAMORAK).getNotedOrDefault(), 25));

    private static final List<ImmutableItem> thirdAgeLoot = Arrays.asList(
            new ImmutableItem(ItemId._3RD_AGE_FULL_HELMET),
            new ImmutableItem(ItemId._3RD_AGE_PLATEBODY),
            new ImmutableItem(ItemId._3RD_AGE_PLATELEGS),
            new ImmutableItem(ItemId._3RD_AGE_PLATESKIRT),
            new ImmutableItem(ItemId._3RD_AGE_KITESHIELD),
            new ImmutableItem(ItemId._3RD_AGE_RANGE_COIF),
            new ImmutableItem(ItemId._3RD_AGE_RANGE_TOP),
            new ImmutableItem(ItemId._3RD_AGE_RANGE_LEGS),
            new ImmutableItem(ItemId._3RD_AGE_VAMBRACES),
            new ImmutableItem(ItemId._3RD_AGE_MAGE_HAT),
            new ImmutableItem(ItemId._3RD_AGE_ROBE_TOP),
            new ImmutableItem(ItemId._3RD_AGE_ROBE),
            new ImmutableItem(ItemId._3RD_AGE_AMULET),
            new ImmutableItem(ItemId._3RD_AGE_DRUIDIC_ROBE_TOP),
            new ImmutableItem(ItemId._3RD_AGE_DRUIDIC_ROBE_BOTTOMS),
            new ImmutableItem(ItemId._3RD_AGE_DRUIDIC_CLOAK),
            new ImmutableItem(ItemId._3RD_AGE_LONGSWORD),
            new ImmutableItem(ItemId._3RD_AGE_BOW),
            new ImmutableItem(ItemId._3RD_AGE_WAND),
            new ImmutableItem(ItemId._3RD_AGE_DRUIDIC_STAFF),
            new ImmutableItem(ItemId._3RD_AGE_CLOAK),
            new ImmutableItem(ItemId._3RD_AGE_PICKAXE),
            new ImmutableItem(ItemId._3RD_AGE_AXE));
    ...
    public void handle() {
        bind("Open", (player, item, container, slotId) -> {
                    ...
                    public void buildDialogue() {
                        options("Do you want a chance of a Mimic boss fight?",
                                ...
                                new DialogueOption("No, I don't want those.", () -> {
                                    ...
                                    val casketId = item.getNumericAttribute("The Mimic original casket").intValue();
            ...
            if (item.getNumericAttribute("The Mimic slain").intValue() == 1) {
                var remainingRolls = item.getNumericAttribute("The Mimic rolls").intValue();
                val originalCasketId = item.getNumericAttribute("The Mimic original casket").intValue();
                val tier = Objects.requireNonNull(ClueReward.getTier(originalCasketId));
                val isFirstAttempt = remainingRolls == (tier == ClueLevel.MASTER ? 6 : 5);
                val isRingLoot = Utils.random(isFirstAttempt ? 25 : (40 + ((6 - remainingRolls) * 4))) == 0;
                if (isRingLoot) {
                    remainingRolls--;
                }
                player.getInventory().deleteItem(slotId, item);
                val rewards = Objects.requireNonNull(ClueReward.getTable(originalCasketId));
                val loot = new ObjectArrayList<Item>();
                if (isRingLoot) {
                    loot.add(new Item(ItemId.RING_OF_3RD_AGE));
                }
                val rate = 228 + ((6 - remainingRolls) * 22);
                if (Utils.random(rate - 1) == 0) {
                    val randomBonusLoot = thirdAgeLoot.get(Utils.random(thirdAgeLoot.size() - 1));
                    val element = new Item(randomBonusLoot.getId());
                    loot.add(element);
                    ...
                    if (Utils.random(1) == 0) {
                        val randomBonusLoot = bonusLoot.get(Utils.random(bonusLoot.size() - 1));
                        val element = new Item(randomBonusLoot.getId(), Utils.random(1) == 0 ? randomBonusLoot.getMinAmount() : randomBonusLoot.getMaxAmount());
                        loot.add(element);
                        player.sendMessage("The Mimic adds a little extra reward for you: " + Colour.RED.wrap(element.getAmount() + " x " + element.getName()));
                    }
                }
