...
public class PatchyNPC extends NPCPlugin implements ItemOnNPCAction {
    ...
    public void handleItemOnNPCAction(Player player, Item item, int slot, NPC npc) {
        val clothing = Objects.requireNonNull(Utils.findMatching(SewableClothing.values, v -> v.sewnItem == item.getId()));
        player.getDialogueManager().start(new Dialogue(player, npc) {
            @Override
            public void buildDialogue() {
                npc("Would you like to have that unsewn? That'll cost you " + DISCONNECT_COST + " coins.");
                options("Unsew the " + item.getName().toLowerCase() + "?",
                        new DialogueOption("Yes", () -> unsew(player, clothing)),
    ...
    public Object[] getItems() {
        ...
        for (val value : SewableClothing.values) {
    ...
    private enum SewableClothing {
        ...
        private final int baseItem, attachmentItem;
    ...
    public void handle() {
        bind("Sew", (player, npc) -> player.getDialogueManager().start(new Dialogue(player, npc) {
            ...
            public void buildDialogue() {
                ...
                options(TITLE, new DialogueOption("White bandana eyepatch", () -> sew(player, SewableClothing.WHITE_BANDANA_EYEPATCH)),
                        new DialogueOption("Red bandana eyepatch", () -> sew(player, SewableClothing.RED_BANDANA_EYEPATCH)),
                        new DialogueOption("Blue bandana eyepatch", () -> sew(player, SewableClothing.BLUE_BANDANA_EYEPATCH)),
                        new DialogueOption("Brown bandana eyepatch", () -> sew(player, SewableClothing.BROWN_BANDANA_EYEPATCH)),
                        new DialogueOption("More options", key(3)));
                options(TITLE, new DialogueOption("Hat eyepatch", () -> sew(player, SewableClothing.HAT_EYEPATCH)),
                        new DialogueOption("Crabclaw hook", () -> sew(player, SewableClothing.CRABCLAW_HOOK)),
                        new DialogueOption("Cavalier and mask", () -> sew(player, SewableClothing.CAVALIER_AND_MASK)),
                        new DialogueOption("Beret mask", () -> sew(player, SewableClothing.BERET_MASK)),
                        new DialogueOption("More options", key(4)));
                options(TITLE, new DialogueOption("Partyhat & specs", () -> sew(player, SewableClothing.PARTYHAT_AND_SPECS)),
                        new DialogueOption("Top hat & monocle", () -> sew(player, SewableClothing.TOP_HAT_AND_MONOCLE)),
                        new DialogueOption("Pirate hat & patch", () -> sew(player, SewableClothing.PIRATE_HAT_AND_PATCH)),
                        new DialogueOption("More options", key(2)));
            }
        }));
    }
    ...
    private final void sew(@NotNull final Player player, @NotNull final SewableClothing piece) {
        ...
        if (inventory.getAmountOf(ItemId.COINS_995) < CONNECT_COST || !inventory.containsItem(piece.baseItem, 1) || !inventory.containsItem(piece.attachmentItem, 1)) {
            val baseName = ItemDefinitions.getOrThrow(piece.baseItem).getName().toLowerCase();
            val attachmentName = ItemDefinitions.getOrThrow(piece.attachmentItem).getName().toLowerCase();
            val completeName = ItemDefinitions.getOrThrow(piece.sewnItem).getName().toLowerCase();
            player.getDialogueManager().start(new NPCChat(player, 3215, "I'll need " + CONNECT_COST + " coins, " + Utils.getAOrAn(baseName) + " " + baseName + " and " +
                    "" + Utils.getAOrAn(attachmentName) + " " + attachmentName + " to sew the " + completeName + "."));
            return;
        }
