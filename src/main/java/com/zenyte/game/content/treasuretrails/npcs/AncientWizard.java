...
public class AncientWizard extends TreasureGuardian implements CombatScript {
    ...
    public void processNPC() {
        block: if (++ticks > 50 || !owner.getLocation().withinDistance(this, 15)) {
            val list = owner.getTemporaryAttributes().get("TT Guardian NPC");
            if (list instanceof Collection) {
                for (val n : ((Collection) list)) {
    ...
    public int attack(final Entity target) {
        switch(getId()) {
            case 7307: {
                val spell = CombatSpell.BLOOD_BLITZ;
                setAnimation(spell.getAnimation());
                val proj = spell.getProjectile();
                val time = proj.getProjectileDuration(getLocation(), target);
                target.setGraphics(new Graphics(spell.getHitGfx().getId(), time, 0));
                val damage = getRandomMaxHit(this, 18, MAGIC, target);
            ...
            case 7308: {
                val defs = AmmunitionDefinitions.RUNITE_BOLT;
                setAnimation(new Animation(7552));
                val proj = defs.getProjectile();
