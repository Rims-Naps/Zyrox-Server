...
public class StrangeDevice extends ItemPlugin {
    ...
    public void handle() {
        bind("Feel", (player, item, container, slotId) -> {
            val result = TreasureTrail.getHotColdResult(player, item.getId() == ItemId.STRANGE_DEVICE_23183);
            if (!result.isPresent()) {
                player.getDialogueManager().start(new ItemChat(player, item, "The strange device is inactive."));
                return;
            }
            val res = result.get();
            val temperature = res.getTemperature();
            val clueItem = res.getClue();
            ...
            val lastDistance = clueItem.getAttribute("Hot Cold Distance");
            ...
            if (!(lastDistance instanceof Number) || temperature == DeviceTemperature.MASTER_VISIBLY_SHAKING || temperature == DeviceTemperature.BEGINNER_VISIBLY_SHAKING) {
                ...
                val dist = ((Number) lastDistance).intValue();
                if (dist == distance) {
    ...
    public int[] getItems() {
        return new int[]{
                ItemId.STRANGE_DEVICE, ItemId.STRANGE_DEVICE_23183
        };
    }
