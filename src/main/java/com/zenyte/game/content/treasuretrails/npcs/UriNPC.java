...
public class UriNPC extends TreasureGuardian {
    ...
    private static final List<String> quotes = Arrays.asList("What's cheese?",
            ...
            "A great captain is always willing to change course.",
    ...
    @NotNull public static final String randomQuote() {
    ...
    public static final void spawnUri(@NotNull final Player player, @NotNull final EmoteRequest clue) {
        val square = Utils.findEmptySquare(player.getLocation(), 2, 1,
                Optional.of(tile -> !tile.matches(player.getLocation())
                        && !ProjectileUtils.isProjectileClipped(null, null, player.getLocation(), tile, true) && clue.getPolygon().contains(tile)));
        val tile = square.orElse(player.getLocation());
        val uri = new UriNPC(player, tile, clue);
    ...
    @Getter private final EmoteRequest clue;
    @Getter @Setter
    ...
    public void onFinish(final Entity source) {
        ...
        World.getPlayer(username).ifPresent(player -> {
            ...
            val attr = player.getTemporaryAttributes().get("spawned uri npc");
            if (attr == UriNPC.this) {
