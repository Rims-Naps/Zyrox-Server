...
public class FaloTheBardNPC extends NPCPlugin implements ItemOnNPCAction {
    ...
    public void handleItemOnNPCAction(final Player player, final Item item, final int slot, final NPC npc) {
        ...
        if (list == null) {
            val clue = TreasureTrail.findFaloClue(player);
            if (!clue.isPresent()) {
                player.sendFilteredMessage("Nothing interesting happens.");
                return;
            }
            val scroll = clue.get();
            val challenge = (FaloTheBardChallenge) scroll.getChallenge();
            player.getDialogueManager().start(new Dialogue(player, npc) {
                @Override
                public void buildDialogue() {
                    item(item, "You show your object to Falo.");
                    val requirements = challenge.getTask().getItemRequirements();
                    ...
                    npc("Yes yes yes! That is it, thank you so much " + player.getName() + "." +
                            " The memories have come flooding back. I get why the lyrics are what they are now.").executeAction(() -> {
    ...
    private void talk(@NotNull final Player player, @NotNull final NPC npc) {
        player.getDialogueManager().start(new Dialogue(player, npc) {
            ...
            public void buildDialogue() {
                ...
                val challenge = (FaloTheBardChallenge) scroll.getChallenge();
                val clueItem = scroll.getItem();
                val stage = clueItem.getNumericAttribute("Falo the Bard Stage").intValue();
                if (stage == 0) {
