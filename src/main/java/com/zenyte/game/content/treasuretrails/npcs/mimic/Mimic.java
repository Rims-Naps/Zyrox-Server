...
final class Mimic extends NPC implements CombatScript {
    ...
    private enum Candy {
        ...
        private final void execute(@NotNull final Location middle, @NotNull final Location tile, @NotNull final Consumer<Location> consumer) {
            val time = Candy.PINK.projectile.getProjectileDuration(middle, tile);
    ...
    public static final int CHALLENGE_MIMIC = 7979, ATTACKABLE_MIMIC = 8633;
    ...
    Mimic(@NotNull Location tile, @NotNull final MimicInstance instance) {
        ...
        this.combat = new NPCCombat(this) {
            ...
            protected boolean checkAll() {
                ...
                if (npc.getRetreatMechanics().process(target) || !attackable()) {
                    return false;
                }
    ...
    private List<Location> findLocations(@NotNull final Location tile) {
        val list = new ObjectArrayList<Location>();
        int tryCount = 500;
        list.add(tile);
        while (list.size() < 6) {
            if (--tryCount <= 0) {
            ...
            val location = new Location(tile, 3);
            if (Utils.collides(getLocation(), getSize(), location, 1, 0) || list.contains(location) || ProjectileUtils.isProjectileClipped(null, null, tile, location, true) || !World.isFloorFree(location, 1)) {
                continue;
            }
            list.add(location);
        }
    ...
    public void onDeath(@NotNull final Entity source) {
        try {
            instance.player().ifPresent(player -> player.getBossTimer().finishTracking("Mimic"));
            instance.finish();
        } catch (Exception e) {
    ...
    private void clearThirdAgeMonsters() {
        for (val monster : thirdAgeMonsters) {
    ...
    private void iterateCollidingPlayers() {
        for (val player : instance.getPlayers()) {
            if (!Utils.collides(getX(), getY(), getSize(), player.getX(), player.getY(), player.getSize())) {
                continue;
            }
            val damageCounter = player.getNumericTemporaryAttribute("mimic damage counter").intValue();
            val lastTick = player.getNumericTemporaryAttribute("mimic last damage tick").longValue();
            val reset = lastTick != WorldThread.WORLD_CYCLE;
            val damage = Math.min(6, 1 + (reset ? 0 : (damageCounter / 3)));
    ...
    public int attack(@NotNull Entity target) {
        val attackType = Utils.random(isWithinMeleeDistance(this, target) ? 2 : 1);
        if (attackType == 2) {
            setAnimation(melee);
            ...
            WorldTasksManager.schedule(() -> {
                val tiles = findLocations(new Location(target.getLocation()));
                for (int i = tiles.size() - 1; i >= 0; i--) {
                    val candy = Candy.immutableCandyList.get(i);
    ...
    private void spawnIfAbsent(@NotNull final Location tile, @MagicConstant(valuesFromClass = ThirdAgeMonster.class) final int id) {
        val index = id - ThirdAgeMonster.WARRIOR;
        var monster = thirdAgeMonsters[index];
