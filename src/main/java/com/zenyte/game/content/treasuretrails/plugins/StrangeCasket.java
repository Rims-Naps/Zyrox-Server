...
public class StrangeCasket implements ObjectAction {
        ...
        if (option.equalsIgnoreCase("Search")) {
            player.getDialogueManager().start(new Dialogue(player) {
                ...
                public void buildDialogue() {
                    options(TITLE,
                            new DialogueOption("Search.", () -> search(player)),
                            new DialogueOption(TreasureTrail.isMimicEnabled(player) ? "Stop receiving the Mimic." : "Start receiving the Mimic.", () -> {
    ...
    private static final void search(@NotNull final Player player) {
        ...
        val item = Utils.findMatching(player.getInventory().getContainer().getItems().values(), it -> it.getId() == ItemId.MIMIC && it.getNumericAttribute("The Mimic slain").intValue() == 0);
        if (item == null) {
            player.getDialogueManager().start(new PlainChat(player, "You find nothing in the strange casket."));
            return;
        }
        player.getDialogueManager().start(new Dialogue(player) {
            @Override
            public void buildDialogue() {
                val message = npc(1, "The Mimic has... found you!<br>Will you fight now?", key);
