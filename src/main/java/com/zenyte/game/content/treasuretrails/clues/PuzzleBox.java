...
public final class PuzzleBox {
    ...
    private int emptySlot() {
        val slots = currentPuzzle.getAvailableSlots();
        assert slots.size() == 1;
    ...
    public void checkCompletion() {
        ...
        for (int i = 0; i < 24; i++) {
            val optionalValue = enumList.getValue(i);
    ...
    private Item findPuzzleItem() {
        ...
        val item = Utils.findMatching(entrySet, entry -> entry.getValue().getId() == puzzleBox);
    ...
    private final void generatePuzzle(final int id) {
        ...
        complete = false;
        currentPuzzle.clear();
        val firstPieceItemId = getEnum().getValue(0).orElseThrow(RuntimeException::new);
        for (int i = 0; i < 24; i++) {
