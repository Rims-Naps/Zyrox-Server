...
public interface SwitchPlugin {
    ...
    default Interface bind(final String fromComponent, final String toComponent, final Interface.SwitchHandler plugin) {
        if (!(this instanceof Interface))
            throw new RuntimeException("You can only bind the plugins on interfaces.");
        val inter = (Interface) this;
        val id = inter.getComponent(fromComponent) | inter.getComponent(toComponent) << 16;
