...
public class BattlestaffEnchantmentInterface extends Interface {
    ...
    private enum Battlestaff {
        AIR_BATTLESTAFF(ItemOnItemAction.ItemPair.of(1397, 1405)),
        WATER_BATTLESTAFF(ItemOnItemAction.ItemPair.of(1395, 1403)),
        EARTH_BATTLESTAFF(ItemOnItemAction.ItemPair.of(1399, 1407)),
        FIRE_BATTLESTAFF(ItemOnItemAction.ItemPair.of(1393, 1401)),
        LAVA_BATTLESTAFF(ItemOnItemAction.ItemPair.of(3053, 3054), ItemOnItemAction.ItemPair.of(21198, 21200)),
        MUD_BATTLESTAFF(ItemOnItemAction.ItemPair.of(6562, 6563)),
        STEAM_BATTLESTAFF(ItemOnItemAction.ItemPair.of(11787, 11789), ItemOnItemAction.ItemPair.of(12795, 12796)),
        SMOKE_BATTLESTAFF(ItemOnItemAction.ItemPair.of(11998, 12000)),
        MIST_BATTLESTAFF(ItemOnItemAction.ItemPair.of(20730, 20733)),
        DUST_BATTLESTAFF(ItemOnItemAction.ItemPair.of(20736, 20739));
    ...
    private static final void enchant(@NotNull final Player player, @NotNull final Battlestaff battlestaff) {
        ...
        iterator : for (int i = 0; i < 28; i++) {
            val item = inventory.getItem(i);
            if (item == null) {
                continue;
            }
            for (val pair : battlestaff.itemPairs) {
        ...
        val name = battlestaff.toString().toLowerCase().replace("_", " ");
        player.getInterfaceHandler().closeInterface(GameInterface.BATTLESTAFF_ENCHANTMENT);
        val cost = DiaryReward.KANDARIN_HEADGEAR4.eligibleFor(player) ? 20000 : DiaryReward.KANDARIN_HEADGEAR3.eligibleFor(player) ? 30000 : 40000;
        if (slot == -1) {
            player.sendMessage("You need " + Articles.get(name) + " " + name + " to enchant.");
            return;
        }
        if (!inventory.containsItem(995, cost)) {
            player.sendMessage("You need at least " + Utils.format(cost) + " coins to enchant the battlestaff.");
            return;
        }
