...
public class ThessaliaMakeOverInterface extends Interface {
    ...
    protected void build() {
        ...
        bind("Select body/legs colour", (player, slotId, itemId, option) -> {
            val topwear = player.getVarManager().getBitValue(3945) == 0;
        ...
        bind("Finish make-over", (player, slotId, itemId, option) -> {
            val topwear = player.getVarManager().getBitValue(3945) == 0;
            val style = player.getNumericTemporaryAttribute(topwear ? "SelectedBodyStyle" : "SelectedLegsStyle").intValue();
            val armStyle = player.getNumericTemporaryAttribute("SelectedArmStyle").intValue();
            val colour = player.getNumericTemporaryAttribute(topwear ? "SelectedBodyColour" : "SelectedLegsColour").intValue();
            val npcId = player.getNumericTemporaryAttribute("ThessaliaNPCId").intValue();
