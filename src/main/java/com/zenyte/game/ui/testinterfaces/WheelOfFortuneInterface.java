...
public class WheelOfFortuneInterface extends Interface {
    ...
    public void open(Player player) {
        val wheelOfFortune = player.getWheelOfFortune();
        var type = wheelOfFortune.isPendingReward() ? 1 : 0;
        if (!wheelOfFortune.isPendingReward()) {
            wheelOfFortune.roll();
        }

        val container = wheelOfFortune.getContainer();
        val reward = wheelOfFortune.getPrize();
