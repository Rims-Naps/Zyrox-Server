...
public class ChartershipInterface extends Interface {
    ...
    private void charter(final Player player, final CharterLocation destination) {
        val location = CharterLocation.getLocation(player.getLocation());
        if (location == null || destination == null) {
            return;
        }
        val cost = new Item(995, destination.getCosts()[location.ordinal()]);
        player.getInterfaceHandler().closeInterface(InterfacePosition.CENTRAL);
        if (!player.getInventory().containsItem(cost)) {
            player.getDialogueManager().start(new PlainChat(player, "You don't have enough gold in your inventory to sail<br>to " + destination.toString() + "."));
            return;
        }
        player.getDialogueManager().start(new Dialogue(player) {
            @Override
            public void buildDialogue() {
                plain("To sail to " + destination.toString() + " from here will cost you " + cost.getAmount() + " gold.<br><br>Are you sure you want to pay that?");
