...
public class HairdresserInterface extends Interface {
    ...
    protected void build() {
        bind("Select hair/beard style", (player, slotId, itemId, option) -> {
            var haircut = player.getVarManager().getBitValue(4146) == 1;
        ...
        bind("Finish make-over", player -> {
            ...
            val slot = player.getNumericTemporaryAttribute(haircut ? "SelectedHairStyle" : "SelectedBeardStyle").intValue();
            val style = haircut ? HairStyle.getStyle(slot, player.getAppearance().isMale()) : BeardStyle.getStyle(slot);
            val colour = player.getNumericTemporaryAttribute(haircut ? "SelectedHairColour" : "SelectedBeardColour").intValue();
            val npcId = player.getNumericTemporaryAttribute("HairdresserNPCId").intValue();
