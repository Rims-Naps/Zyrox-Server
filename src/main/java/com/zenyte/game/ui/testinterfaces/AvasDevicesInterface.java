    ...
    protected void build() {
        ...
        bind("Purchase ava's assembler", (player, slotId, itemId, option) -> {
            ...
            final IntConsumer consumer = amount -> {
                ...
                val space = player.getInventory().getFreeSlots();
                val coins = player.getInventory().getAmountOf(995);
                val heads = player.getInventory().getAmountOf(21907);
                if (amount > space) {
                    amount = space;
                }
                if ((amount * cost) > coins) {
                ...
                player.getInventory().deleteItemsIfContains(new Item[] {
                        new Item(995, finalAmount * cost),
                        new Item(21907, finalAmount)
                }, () -> player.getInventory().addOrDrop(new Item(22109, finalAmount)));
            };
            if (option == 3) {
                player.sendInputInt("How many would you like to purchase?", consumer::accept);
                return;
