...
public class ExperienceTrackerInterface extends Interface {
    ...
    private static final int[] varbits = new int[]{
            POSITION_VARBIT, SIZE_VARBIT, SPEED_VARBIT, SKILL_VARBIT, COLOR_VARBIT, PROGRESS_BAR_VARBIT, DURATION_VARBIT, GROUP_VARBIT, TRACKER_VARBIT
    };
    private static final int[] SKILLS = new int[]{
            ATTACK, STRENGTH, RANGED, MAGIC, DEFENCE, HITPOINTS, PRAYER, AGILITY, HERBLORE, THIEVING,
            CRAFTING, RUNECRAFTING, MINING, SMITHING, FISHING, COOKING, FIREMAKING, WOODCUTTING, FLETCHING, SLAYER, FARMING, CONSTRUCTION,
            HUNTER
    };
    ...
    public void open(final Player player) {
        ...
        dispatcher.sendComponentSettings(getInterface(), getComponent("Configure skill"), 0, Skills.SKILLS.length, CLICK_OP1, CLICK_OP2, CLICK_OP3, CLICK_OP4);
        ...
        dispatcher.sendComponentSettings(getInterface(), getComponent("Set colour"), 1, Enums.EXPERIENCE_TRACKER_COLOURS.getSize(), CLICK_OP1, CLICK_OP2, CLICK_OP3,
                CLICK_OP4, CLICK_OP5, CLICK_OP6, CLICK_OP7, CLICK_OP8);
    ...
    protected void build() {
        bind("Configure skill", (player, slotId, itemId, option) -> {
            if (slotId < 0 || slotId >= Skills.SKILLS.length) {
        ...
        bind("Save goal", (player, slotId, itemId, option) -> {
            val skill = player.getNumericTemporaryAttribute("SelectedSkillTracker").intValue();
            val type = player.getNumericTemporaryAttribute("SelectedSkillTrackerType").intValue();
            val startpoint = player.getVarManager().getValue(261);
            val endpoint = player.getVarManager().getValue(262);
    ...
    private void setTrackerPoint(@NotNull final Player player, final int option, final boolean start) {
        ...
        if (option == 6 || option == 7) {
            player.sendInputInt("Set tracker start point: " + (option == 6 ? "(skill level)" : "(XP value)"), amount -> {
                ...
                val value = option == 6 ? Skills.getXPForLevel(amount) : amount;
                player.getVarManager().sendVar(config, value);
            });
        } else {
            val value = skill == 23 ? player.getSkills().getTotalXp() : (int) player.getSkills().getExperience(SKILLS[skill]);
            player.getVarManager().sendVar(config, value);
        }
    }
