...
public class DiangoItemRetrievalService extends Interface {
    ...
    private boolean refresh(@NotNull final Player player) {
        val trackedHolidayItems = player.getTrackedHolidayItems();
        val set = new BitSet(trackedHolidayItems.size());
        for (val entry : Enums.DIANGO_ITEM_RETRIEVAL.getValues().int2IntEntrySet()) {
        ...
        val varps = new int[4];
        for (int i = 0; i < 4; i++) {
            val values = set.get(i * 32, (i + 1) * 32).toLongArray();
    ...
    protected void build() {
        bind("Interact with item", (player, slotId, itemId, option) -> {
            ...
            val trackedItems = player.getTrackedHolidayItems();
