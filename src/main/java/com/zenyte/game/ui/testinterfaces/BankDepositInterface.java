...
public class BankDepositInterface extends Interface {
    ...
    private static final int SCRIPT_RESET_INVENTORY_TAB_SPRITE = 915, INVENTORY_TAB = 3;
    ...
    public void open(Player player) {
        ...
        player.getPacketDispatcher().sendComponentSettings(id, getComponent("Interact with Item"), 0,
                ContainerType.INVENTORY.getSize(), CLICK_OP1, CLICK_OP2, CLICK_OP3, CLICK_OP4, CLICK_OP5,
                CLICK_OP6, CLICK_OP9, CLICK_OP10, DRAG_DEPTH1, DRAG_TARGETABLE);
    }
    ...
    protected void build() {
        ...
        bind ("Interact with Item", (player, slotId, itemId, option) -> {
            ...
            if (op.equals(ItemOption.WITHDRAW_SELECTOR)) {
                val quantity = Math.max(1, player.getVarManager().getValue(1794));
            ...
            if (op.equals(ItemOption.EMPTY)) {
                if (itemId == ItemId.COAL_BAG_12019) {
                    val coalAmount = item.getNumericAttribute("coal").intValue();
                    if (coalAmount <= 0) {
                    ...
                    val amountDeposited = player.getBank().add(new Item(ItemId.COAL, coalAmount)).getSucceededAmount();
        ...
        bind("Deposit loot", player -> {
            ...
            val carryingLootingBag = inventory.containsItem(ItemId.LOOTING_BAG) || inventory.containsItem(ItemId.LOOTING_BAG_22586);
            ...
            val container = lootingBag.getContainer();
