...
public class LootingBagInterface extends Interface {
    ...
    public void open(Player player) {
        val value = player.getLootingBag().getTotalValue();
        val text = value >= Integer.MAX_VALUE || value < 0 ? "Lots!" : player.getLootingBag().getContainer().isEmpty() ? "-" : Utils.format(value) + " coins";
        val obj = player.getTemporaryAttributes().get("deposit_looting_bag");
        if (obj instanceof Item) {
            ...
            if (player.getBooleanAttribute("received_looting_bag_warning")) {
                ...
                var slot = player.getInventory().getContainer().getSlotOf(LootingBag.CLOSED.getId());
                if (slot == -1) {
    ...
    protected void build() {
        ...
        bind("Store/Examine", (player, slotId, itemId, option) -> {
            ...
            if (!item.isTradable()) {
                player.sendMessage("Only tradable items can be put in the bag.");
                return;
            }
            if (!item.isEmblem()) {
                player.sendMessage("You can not put the Mysterious Emblem in the bag.");
                return;
            }
            ...
            val text = value >= Integer.MAX_VALUE || value < 0 ? "Lots!" : player.getLootingBag().getContainer().isEmpty() ? "-" : Utils.format(value) + " coins";
            if (option < 4) {
                val amount = option == 1 ? 1 : option == 2 ? 5 : player.getInventory().getAmountOf(item.getId());
    ...
    private static Object[] getValues(final Container container) {
        val values = new Integer[container.getContainerSize() + 1];
        values[0] = ContainerType.LOOTING_BAG.getId(); //inventory id
        for (int index = 1; index < values.length; index++) {
            val item = container.get(index - 1);
