...
public enum ClientProt {
	...
	private static final Int2ObjectOpenHashMap<ClientProt> CONSTANTS = new Int2ObjectOpenHashMap<>(100);

	static {
		for (final ClientProt constants : values()) {
			if (CONSTANTS.containsKey(constants.opcode)) {
				throw new RuntimeException("OVERLAPPING OPCODE: " + constants.opcode);
			}
			CONSTANTS.put(constants.opcode, constants);
		}
	}
	

	@Getter
	private final int opcode, size, limit;

	public static ClientProt get(final int opcode) {
		return CONSTANTS.get(opcode);
	}
	 ...
	 *             if the packet doesn't exist, throws illegal state exception.
	 */
	public static int getSize(final int opcode) throws IllegalStateException {
		val packet = CONSTANTS.get(opcode);
		if (packet == null) {
