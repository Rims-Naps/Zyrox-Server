...
public enum RepairableItem {
	 ...
	 * enum!
	 */
	FIRE_CAPE(50000, 6570, 20445),
	INFERNAL_CAPE(50000, 21295, 21287),
	FIRE_MAX_CAPE(50000, 13329, 20447),
	INFERNAL_MAX_CAPE(50000, 21285, 21289),
	AVAS_ASSEMBLER(75000, 22109, 21914),
	ASSEMBLER_MAX_CAPE(75000, 21898, 21916),
	...
	DRAGON_DEFENDER(40000, 12954, 20463),
	AVERNIC_DEFENDER(1000000, 22322, 22441),
	VOID_MAGE_HELM(40000, 11663, 20477),
	VOID_RANGER_HELM(40000, 11664, 20479),
	VOID_MELEE_HELM(40000, 11665, 20481),
	VOID_KNIGHT_TOP(45000, 8839, 20465),
	VOID_KNIGHT_ROBE(45000, 8840, 20469),
	VOID_KNIGHT_GLOVES(30000, 8842, 20475),
	ELITE_VOID_TOP(50000, 13072, 20467),
	ELITE_VOID_ROBE(50000, 13073, 20471),
	SARADOMIN_HALO(25000, 12637, 20537),
	ZAMORAK_HALO(25000, 12638, 20539),
	GUTHIX_HALO(25000, 12639, 20541),
	FIGHTER_HAT(45000, 10548, 20507),
	RANGER_HAT(45000, 10550, 20509),
	HEALER_HAT(45000, 10547, 20511),
	FIGHTER_TORSO(45000, 10551, 20513),
	PENANCE_SKIRT(20000, 10555, 20515);
	...
	public float getCost(Player player, Item item, boolean armourstand) {
		if (ordinal() <= VERACS_PLATESKIRT.ordinal()) {
			if (armourstand) {
				return getRepairCost() - ((1f - (player.getSkills().getLevel(Skills.SMITHING) / 200f)) * getRepairCost()) * (((item.getCharges() / 90000f)));
			} else {
				return getRepairCost() - (getRepairCost() * (((item.getCharges() / 90000f))));
			}
		} else {
