...
public class PacketDispatcher {
	...
	public void sendProjectile(final Position sender, final Position receiver, final Projectile projectile,
                               final int speed, final int offset) {
	...
	public void initFriendsList() {
        val friends = player.getSocialManager().getFriends();
        val list = new ArrayList<UpdateFriendList.FriendEntry>(friends.size());
        for (val friend : friends) {
	...
	public void initIgnoreList() {
        val ignores = player.getSocialManager().getIgnores();
        val list = new ArrayList<UpdateIgnoreList.IgnoreEntry>(ignores.size());
        for (val ignore : ignores) {
	...
	public void sendConfig(final int config, final int value) {
	    player.send(value >= Byte.MIN_VALUE && value <= Byte.MAX_VALUE ? new VarpSmall(config, value) :
                new VarpLarge(config, value));
	}
