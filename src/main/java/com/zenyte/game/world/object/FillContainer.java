...
public class FillContainer extends Action {
    ...
    public boolean start() {
        combo = FillCombinations.getDef(object.getName(player).toLowerCase());
        if (!check()) {
            return false;
        }
        return check();
    ...
    private void reward() {
        final String name = combo == FillCombinations.FLOUR_BIN ? object.getName(player) : object.getName();
