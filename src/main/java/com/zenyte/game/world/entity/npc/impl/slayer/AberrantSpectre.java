...
public class AberrantSpectre extends NPC implements Spawnable, CombatScript {
    ...
    public int attack(final Entity target) {
        ...
        if (!SlayerEquipment.NOSE_PEG.isWielding(player)) {
                ...
                for (int i = 0; i <= 6; i++) {
                    ...
                    if (i == 5) {
                        player.getPrayerManager()
                                .setPrayerPoints((int) (player.getPrayerManager().getPrayerPoints() * 0.4105263157894737));
                    } else {
            ...
            delayHit(npc, World.sendProjectile(npc, target, PROJECTILE), player,
                    new Hit(npc, getRandomMaxHit(npc, npc.getCombatDefinitions().getMaxHit(), MAGIC, target), HitType.MAGIC).onLand(hit -> {
                        player.setGraphics(GRAPHICS);
                        World.sendSoundEffect(new Location(player.getLocation()), impactSound);
                    }));
        }
