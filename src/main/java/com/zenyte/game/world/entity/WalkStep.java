...
public final class WalkStep {
	...
	public static final int getHash(final int dir, final int nextX, final int nextY, final boolean check) {
		return ((dir & 0x7) << 28) | (nextX & 0x3FFF) | ((nextY & 0x3FFF) << 14) | (check ? 1 << 31 : 0);
	}
	...
	public static final boolean check(final int hash) {
		return ((hash >> 31) & 0x1) == 1;
	}

	public static final boolean addWalkStepsInteract(final Entity entity, final int destX, final int destY, final int maxStepsCount, final int sizeX, final int sizeY,
	...
	public static final int[] calculatedStep(final Entity entity, int myX, int myY, final int destX, final int destY, final int lastX, final int lastY, final int sizeX,
			final int sizeY) {
		if (myX < destX) {
			myX++;
    ...
    private static final int withinRange(final int npcX, final int npcY, final int npcSize, final int targetX,
                                             final int targetY, final int targetSize) {
        val distanceX = npcX - targetX;
        val distanceY = npcY - targetY;
    ...
    public static boolean find(final Entity src, final Entity dest, int maxStepsCount,
                                         final boolean calculate) {
        final int[] srcPos = src.getLastWalkTile();
        val nextPos = dest.getLocation();
        final int x = nextPos.getX();
        final int y = nextPos.getY();
        final int srcSize = src.getSize();
        final int destSize = dest.getSize();

        val destSceneX = x + srcSize - 1;
        val destSceneY = y + srcSize - 1;

        while (maxStepsCount-- != 0) {
            final int[] srcScenePos = { srcPos[0] + srcSize - 1, srcPos[1] + srcSize - 1 };
            int o;
            val absDistX = srcPos[0] - x;
            val absDistY = srcPos[1] - y;

            val diagonal = (absDistX == -srcSize && absDistY == -srcSize) || (absDistX == -srcSize && absDistY == 1)
                    || (absDistX == 1 && absDistY == -srcSize) || (absDistX == 1 && absDistY == 1);
            if (diagonal) {
                if (srcScenePos[0] < destSceneX
	...
	public static boolean findBasicRoute(final Entity src, final Object dest, int maxStepsCount, final boolean calculate) {
		...
		val destSceneX = x + destSize - 1;
        val destSceneY = y + destSize - 1;
		while (maxStepsCount-- != 0) {
			final int[] srcScenePos = { srcPos[0] + srcSize - 1, srcPos[1] + srcSize - 1 };
			if (!Utils.isOnRangeExcludingDiagonal(srcPos[0], srcPos[1], srcSize, x, y, destSize, 0)) {
				if (srcScenePos[0] < destSceneX && srcScenePos[1] < destSceneY
	...
	public static final boolean calcFollow(final Entity entity, final Position target, final int maxStepsCount, final boolean calculate, final boolean intelligent,
			final boolean checkEntities) {
		if (intelligent) {
			val steps = RouteFinder.findRoute(entity.getX(), entity.getY(), entity.getPlane(), entity.getSize(),
					target instanceof WorldObject ? new ObjectStrategy((WorldObject) target)
							: target instanceof Location ? new TileStrategy((Location) target) : new EntityStrategy((Entity) target),
					true);
