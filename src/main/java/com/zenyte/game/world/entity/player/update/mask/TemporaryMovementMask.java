...
public final class TemporaryMovementMask extends UpdateMask {
	...
	public void writePlayer(final RSBuffer buffer, final Player player, final Player processedPlayer) {
		buffer.writeByte128(processedPlayer.getUpdateFlags().getFlags().contains(UpdateFlag.FORCE_MOVEMENT) ? 1 : (processedPlayer.isTeleported() ? 127 : (processedPlayer.getRunDirection() != -1 ? 2 : 1)));
	}
