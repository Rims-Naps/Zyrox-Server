...
public class Hans extends Dialogue {
	...
	public void buildDialogue() {
		val seconds = (int) (player.getVariables().getPlayTime() * 0.6);
		...
		npc("You've spent " + days + (days > 1 || days == 0 ? " days" : " day") + ", " + hours + (hours > 1 || hours == 0 ? " hours" : " hour") + ", " + minutes + (minutes > 1 || minutes == 0 ? " minutes" : " minute") + " in the world since you arrived in " + GameConstants.SERVER_NAME + ", " + Utils.format(player.getPlayerInformation().getDaysSinceRegistry()) + " days ago.");
		player(16, "Nothing.");
	}
