...
public class NightBeast extends SuperiorNPC implements CombatScript {
    ...
    public int attack(final Entity target) {
        ...
        if (attackCounter++ == 0 || !isWithinMeleeDistance(this, target)) {
    ...
    private final int sendMagicAttack(final Entity target) {
        this.setAnimation(this.getCombatDefinitions().getAttackAnim());
        CombatUtilities.delayHit(this, World.sendProjectile(this, target, MAGICAL_PROJ), target, new Hit(this, CombatUtilities.getRandomMaxHit(this, 31, MAGIC, target),
                HitType.MAGIC).onLand(hit -> target.setGraphics(hit.getDamage() <= 0 ? SPLASH : GFX)));
        return this.getCombatDefinitions().getAttackSpeed();
    }

    private final int sendGroupMagicAttack(final Entity target) {
        this.setAnimation(this.getCombatDefinitions().getAttackAnim());
        val tile = new Location(target.getLocation());

        for (int x = -1; x <= 1; x++) {
            for (int y = -1; y <= 1; y++) {
                val t = tile.transform(x, y, 0);
                if (!World.isFloorFree(t, 1))
                    continue;
                World.sendProjectile(this, t, MAGICAL_PROJ);
                val specificTime = MAGICAL_PROJ.getProjectileDuration(getLocation(), t);
