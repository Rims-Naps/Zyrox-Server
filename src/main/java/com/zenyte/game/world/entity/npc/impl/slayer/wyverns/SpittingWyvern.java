...
public class SpittingWyvern extends Wyvern {
    ...
    private static final Projectile rangedProjectile =
            new Projectile(1394, 0, 0, 120, 0, 8, 0, 5);
    ...
    public int attack(Entity target) {
        ...
        if (style == 0) {
            ...
            val delay = World.sendProjectile(this, target, rangedProjectile);
            val hitAmount = Utils.random(2) == 0 ? 4 : 1;
            val originalHit = ranged(target, 4);
            val hit = new Hit(originalHit.getSource(), originalHit.getDamage(), originalHit.getHitType());
            val hits = new Hit[hitAmount];
            val dealtDamage = new MutableBoolean();
