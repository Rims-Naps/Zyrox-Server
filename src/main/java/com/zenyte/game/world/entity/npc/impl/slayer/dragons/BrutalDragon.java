...
public class BrutalDragon extends NPC implements Spawnable, CombatScript {
    ...
    public boolean validate(final int id, final String name) {
        return name.equalsIgnoreCase("brutal green dragon") || name.equalsIgnoreCase("brutal blue dragon") || name.equalsIgnoreCase("brutal red dragon") || name.equalsIgnoreCase(
                "brutal black dragon");
    }
    ...
    public int attack(final Entity target) {
        val npc = this;
        val style = Utils.random(!isWithinMeleeDistance(npc, target) ? 1 : 2);
        if (style == 0) {
            ...
            val dragonfire = new Dragonfire(DragonfireType.CHROMATIC_DRAGONFIRE, 50,
                    DragonfireProtection.getProtection(player, true));
            ...
            val projectile = name.contains("green") ? greenDragonProjectile : name.contains("blue") ? blueDragonProjectile : name.contains("red") ? redDragonProjectile : blackDragonProjectile;
