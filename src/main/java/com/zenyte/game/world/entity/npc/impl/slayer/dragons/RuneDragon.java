...
public class RuneDragon extends NPC implements CombatScript, Spawnable {
    ...
    private int attackFromDistance(final Entity target) {
        val style = Utils.random(7);
        if (style <= 3) { //dragonfire
            return dragonfire(target);
        } else if (style <= 5) { //magic attack
            setAnimation(DISTANCE_ATTACK_ANIM);
            CombatUtilities.delayHit(this, World.sendProjectile(this, target, MAGIC_ATTACK_PROJ), target, new Hit(this, CombatUtilities.getRandomMaxHit(this,
        ...
        } else if (style == 6) { //lightning attack
            if (lightnings != null) {
                return dragonfire(target);
            }
            val base = new Location(target.getLocation());
            val offset = Utils.random(OFFSETS.length - 1);
            val locations = new Location[2];
        ...
        } else if (style == 7) { //life leech
            if (getHitpoints() == getMaxHitpoints()) {
                return dragonfire(target);
            }
            val projectile = AmmunitionDefinitions.ENCHANTED_ONYX_BOLT.getProjectile();
            val max = CombatUtilities.getRandomMaxHit(this, getCombatDefinitions().getMaxHit(), RANGED, target);
            val damage = Utils.random(max);
            setAnimation(DISTANCE_ATTACK_ANIM);
