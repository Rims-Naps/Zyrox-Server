...
public class AdamantDragon extends NPC implements CombatScript, Spawnable {
    ...
    private static final byte[][][] OFFSETS = new byte[][][] { new byte[][] { new byte[] { 2, 0 }, new byte[] { 0, 2 } },
            new byte[][] { new byte[] { 0, -2 }, new byte[] { 2, 0 } }, new byte[][] { new byte[] { -2, 0 }, new byte[] { 0, -2 } },
            new byte[][] { new byte[] { -2, 0 }, new byte[] { 0, 2 } } };
    ...
    private int attackFromDistance(final Entity target) {
        val style = Utils.random(5);
        if (style <= 2) { //dragonfire
            ...
            val dragonfire = new Dragonfire(DragonfireType.CHROMATIC_DRAGONFIRE, 50, DragonfireProtection.getProtection(player));
            val deflected = !perk ? 0 : ((int) Math.floor(dragonfire.getMaximumDamage() * modifier));
            delayHit(World.sendProjectile(this, target, DRAGONFIRE_PROJ), target,
                    new Hit(this, Utils.random(Math.max(0, dragonfire.getDamage() - deflected)), HitType.REGULAR).onLand(hit -> {
        ...
        } else if (style == 3) { //magic attack
            //TODO: Revisit this script: Is the damage calculation below actually correct? Makes no real sense.
            setAnimation(DISTANCE_ATTACK_ANIM);
            delayHit(World.sendProjectile(this, target, MAGIC_ATTACK_PROJ), target, new Hit(this, Utils.random(CombatUtilities.getRandomMaxHit(this,
                    getCombatDefinitions().getMaxHit(), MAGIC, target)), HitType.MAGIC).onLand(hit -> target.setGraphics(hit.getDamage() == 0 ? SPLASH : MAGIC_ATTACK_ONHIT)));
        } else if (style == 4) { //poison attack
            val location = new Location(target.getLocation());
            setAnimation(DISTANCE_ATTACK_ANIM);
            WorldTasksManager.schedule(() -> {
                var damage = Utils.random(14, 25);
                if (target instanceof Player) {
                    val player = (Player) target;
                ...
                val locations = new ArrayList<Location>(2);
                val amount = Utils.random(1, 2);
                val index = Utils.random(OFFSETS.length - 1);
                for (int i = 0; i < amount; i++) {
                    locations.add(location.transform(OFFSETS[index][0][i], OFFSETS[index][1][i], 0));
                }
                for (val l : locations) {
                    WorldTasksManager.schedule(() -> {
                        var dmg = Utils.random(14, 25);
                        if (target instanceof Player) {
                            val player = (Player) target;
                            if (player.getVariables().getTime(TickVariable.POISON_IMMUNITY) > 0) {
        ...
        } else if (style == 5) { //blood forfeit
            val projectile = AmmunitionDefinitions.ENCHANTED_RUBY_BOLT.getProjectile();
            val damage = (int) (Math.min(99, target.getHitpoints() * 0.2));//Do not allow the hit to deal more than 99 damage.
            setAnimation(DISTANCE_ATTACK_ANIM);
