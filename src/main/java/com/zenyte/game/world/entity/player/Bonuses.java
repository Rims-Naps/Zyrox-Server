...
public class Bonuses {
        ...
        if (player.getInterfaceHandler().isVisible(84)) {
            for (final Bonus bonus : Bonus.VALUES) {
                if (bonus.equals(Bonus.UNDEAD)) {
                    val necklace = player.getEquipment().getId(EquipmentSlot.AMULET);
                    val percentage = necklace == ItemId.SALVE_AMULET || necklace == ItemId.SALVE_AMULETI ? 15 : necklace == ItemId.SALVE_AMULET_E || necklace == ItemId.SALVE_AMULETEI ? 20 : 0;
                    val suffix = necklace == ItemId.SALVE_AMULET || necklace == ItemId.SALVE_AMULET_E ? " (melee)"
                            : necklace == ItemId.SALVE_AMULETI || necklace == ItemId.SALVE_AMULETEI ? " (all styles)" : Strings.EMPTY;
                    ...
                    if (helmet != null) {
                        val name = helmet.getName().toLowerCase();
                        if (name.contains("black mask") || name.contains("slayer helm")) {
                            val allStyles = name.contains("(i)");
                            percentage = "15";
    ...
    public int getDefenceBonus(final AttackType type) {
        val bonus = type.isMagic() ? Bonus.DEF_MAGIC : type.isRanged() ? Bonus.DEF_RANGE : type == AttackType.CRUSH ?
                Bonus.DEF_CRUSH : type == AttackType.SLASH ? Bonus.DEF_SLASH : Bonus.DEF_STAB;
