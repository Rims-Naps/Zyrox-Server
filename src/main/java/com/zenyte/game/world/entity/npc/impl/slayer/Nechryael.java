...
public class Nechryael extends NPC implements Spawnable, CombatScript {
    ...
    private static final byte[][] BASIC_OFFSETS = new byte[][] {
            { 0, -1 }, { -1, 0 }, { 0, 1 }, { 1, 0 }
    };
    ...
    public int attack(Entity target) {
        ...
        if (Utils.random(5) == 0) {
            ...
            while (true) {
                ...
                val offsets = BASIC_OFFSETS[Utils.random(BASIC_OFFSETS.length - 1)];
                if (World.isTileFree(tile = new Location(getX() + offsets[0], getY() + offsets[1], getPlane()), 1)) {
                    break;
                }
            }
            if (spawnCount <= 1) {
                val deathSpawn = new DeathSpawn(10, tile, Direction.SOUTH, 5);
                deathSpawn.spawn();
