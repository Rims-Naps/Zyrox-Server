...
public class Container {
	...
	public Container(final ContainerPolicy policy, final ContainerType type, final Optional<Player> player) {
		size = type.getSize();
		items = new Int2ObjectLinkedOpenHashMap<Item>(size);
		this.type = type;
	...
	@Getter protected transient IntAVLTreeSet availableSlots;
	@Getter
	@Setter
	...
	public void set(final int slot, final Item item) {
		...
		if (oldItem != null) {
		    val oldWeight = oldItem.getDefinitions().getWeight();
		    if (oldWeight > 0) {
                weight -= oldWeight;
            }
		}
		...
		if (item == null) {
		    ...
		    val itemWeight = item.getDefinitions().getWeight();
		    if (itemWeight > 0) {
                weight += itemWeight;
            }
			modifiedSlots.add(slot);
	...
	public int[] getSlotsOf(final int id) {
		...
		val array = list.toArray(new int[list.size()]);
		Arrays.sort(array);
		return array;
	}
	...
	public void shift() {
		...
		val shiftedMap = new Int2ObjectOpenHashMap<Item>(itemsSize);
		int index = 0;
		val length = getContainerSize();
	...
	public ContainerResult add(final Item requestedItem) {
		...
		int slot;
		if (item.hasAttributes() || policy == ContainerPolicy.NEVER_STACK
				|| (slot = getSlotOf(id)) == -1 && stackable && policy == ContainerPolicy.NORMAL || slot == -1) {
	...
	public MultiResult add(final Item... items) {
		val results = new ArrayList<ContainerResult>(items.length);
		for (int i = 0; i < items.length; i++) {
			val item = items[i];
			results.add(this.add(item));
		}
		val multiresult = new MultiResult(ContainerState.ADD, results.toArray(new ContainerResult[results.size()]));
		return multiresult;
	}
	...
	public MultiResult remove(final Item... items) {
		...
		val multiresult = new MultiResult(ContainerState.REMOVE, results.toArray(new ContainerResult[results.size()]));
		return multiresult;
	}
	...
	private void recalculateWeight() {
	    weight = 0;
	    for (val entry : this.items.int2ObjectEntrySet()) {
	        val item = entry.getValue();
	        if (item == null) {
	            continue;
            }
	        val weight = item.getDefinitions().getWeight();
	...
	public void withdraw(final Player player, final Container container, int slot, int amount) {
		...
		if (!item.hasAttributes() && (definitions.isStackable() || policy == ContainerPolicy.ALWAYS_STACK)) {
			...
			val realAmount = getAmountOf(item.getId());
			...
			final ContainerResult containerResult = definitions.isStackable() ? remove(slot, amount)
					: remove(new Item(item.getId(), amount, item.getAttributesCopy()));
