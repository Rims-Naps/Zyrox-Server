...
public class UncheckedEntityEvent extends RouteEvent<Player, EntityStrategy> {
    ...
    public boolean process() {
        ...
        if (!initiated || !strategy.getEntity().getLocation().matches(lastTile)) {
            ...
            entity.getPacketDispatcher().sendMapFlag(lastTile.getXInScene(entity),
                    lastTile.getYInScene(entity));
            return initiate();
        }
        if (!entity.hasWalkSteps()) {
            val steps = RouteFinder.findRoute(entity, entity.getSize(), strategy, true);
