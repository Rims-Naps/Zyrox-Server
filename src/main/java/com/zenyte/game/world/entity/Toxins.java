...
public final class Toxins {
	...
	@Setter
	@Getter
	private int damage, cycle, interval;
	@Expose
	@Setter
	private int diseaseCycle, diseaseInterval;
	...
	public void applyToxin(final ToxinType requestedType, final int damage, final boolean applyDamage) {
		...
		if (entity.getEntityType() == EntityType.PLAYER) {
			...
			if ((type == ToxinType.VENOM && player.getVariables().getTime(TickVariable.VENOM_IMMUNITY) > 0)
					|| (type == ToxinType.POISON && player.getVariables().getTime(TickVariable.POISON_IMMUNITY) > 0)) {
				return;
			}
			...
			if (type == ToxinType.VENOM && npc.getCombatDefinitions().isVenomImmune()
					|| type == ToxinType.POISON && npc.getCombatDefinitions().isPoisonImmune()) {
				return;
			}
			val immunityTypes = npc.getCombatDefinitions().getImmunityTypes();
