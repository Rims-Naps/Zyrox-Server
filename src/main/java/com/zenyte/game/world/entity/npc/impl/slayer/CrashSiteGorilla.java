...
abstract class CrashSiteGorilla extends NPC implements CombatScript {
    ...
    public void processNPC() {
        val combatStyle = combatDefinitions.getAttackStyle();
        ...
        if (combatStyle.isMelee()) {
            if (meleeMovementDelay > 0) {
                meleeMovementDelay--;
                val combatDelay = combat.getCombatDelay();
                if (combatDelay > 0) {
    ...
    public void applyHit(@NotNull final Hit hit) {
        ...
        if (combatStyle.isMelee()) {
            if (meleeSwitchTick < (WorldThread.WORLD_CYCLE - MELEE_SWITCH_DELAY)) {
                val availableAttackStyle = new ObjectArrayList<>(Arrays.asList(MAGIC, RANGED));
                val source = hit.getSource();
                if (source instanceof Player) {
                    val prayers = ((Player) source).getPrayerManager();
    ...
    private final int getDistance2D(@NotNull final Location source, final int sourceSize, @NotNull final Location target, final int targetSize) {
        val p1 = getAxisDistances(source, sourceSize, target);
        val p2 = getAxisDistances(target, targetSize, source);
    ...
    private final void onFailedCloseAttack(@NotNull final Entity target) {
        ...
        val isCorrectlyPrayedAgainst = target instanceof Player && ((Player) target).getPrayerManager().isActive(Prayer.PROTECT_FROM_MELEE);
        val nextPossibleAttacks = new ObjectArrayList<AttackType>();
    ...
    private final void onFailedDistancedAttack(@NotNull final Entity target, @NotNull final Prayer prayer) {
        //If the third "missed" hit is not currently prayed against, the gorilla will either keep using the same attack style for the next 3 attacks or switch to another style. If the third
        //"missed" hit was either a ranged or magic hit, however, the following attack will always be the same as the one just used, or a melee attack.
        val isCorrectlyPrayedAgainst = target instanceof Player && ((Player) target).getPrayerManager().isActive(prayer);
        val nextPossibleAttacks = new ObjectArrayList<AttackType>();
        if (isCorrectlyPrayedAgainst) {
            nextPossibleAttacks.addAll(possibleAttackStyles);
            nextPossibleAttacks.remove(combatDefinitions.getAttackStyle());
    ...
    private final void switchStyleTo(@NotNull final AttackType type, final boolean correctlyPrayed) {
        missedHits = 0;
        val previousType = combatDefinitions.getAttackType();
    ...
    private void meleeAttack(final Entity target) {
        ...
        val hit = new Hit(this, getRandomMaxHit(this, getCombatDefinitions().getMaxHit(), MELEE, target), HitType.MELEE);
    ...
    private void rangedAttack(final Entity target) {
        ...
        val hit = new Hit(this, getRandomMaxHit(this, getCombatDefinitions().getMaxHit(), RANGED, target), HitType.RANGED);
        ...
        val preciseDelay = rangedProjectile.getProjectileDuration(getLocation(), target.getLocation());
    ...
    private void magicAttack(final Entity target) {
        ...
        val hit = new Hit(this, getRandomMaxHit(this, getCombatDefinitions().getMaxHit(), MAGIC, target), HitType.MAGIC);
        ...
        val preciseDelay = magicProjectile.getProjectileDuration(getLocation(), target.getLocation());
    ...
    private void boulderAttack(final Entity target) {
        val collisionTile = new Location(target.getX(), target.getY(), target.getPlane());
        val from = collisionTile.transform(Direction.NORTH);
