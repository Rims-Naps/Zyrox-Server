...
public class ElderChaosDruid extends NPC implements CombatScript, Spawnable {
	...
	private static final byte[][] OFFSETS = new byte[][] { new byte[] { 1, 0 }, new byte[] { -1, 0 },
	...
	public void processNPC() {
        ...
        if (Utils.random(15) == 0) {
            ...
            if (target == null || !target.hasWalkSteps() || getLocation().withinDistance(target.getLocation(), 5)
                    || target instanceof Player && ((Player) target).inArea("Wilderness: Chaos Temple")) {
                return;
            }
            val location = new Location(getLocation());
	...
	public int attack(Entity target) {
		val projectile = CombatSpell.WIND_WAVE.getProjectile();
		setAnimation(CombatSpell.WIND_WAVE.getAnimation());
		setGraphics(CombatSpell.WIND_WAVE.getCastGfx());
        CombatUtilities.delayHit(this, World.sendProjectile(this, target, projectile), target, new Hit(this, CombatUtilities.getRandomMaxHit(this, getCombatDefinitions().getMaxHit(),
                MAGIC, target), HitType.MAGIC).onLand(hit -> target.setGraphics(hit.getDamage() == 0 ? SPLASH : CombatSpell.WIND_WAVE.getHitGfx())));
