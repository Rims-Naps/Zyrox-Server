...
public abstract class Dialogue {
	...
	@Getter @Setter private Runnable onCloseRunnable;
	@Getter @Setter private Runnable onBuild;

	/**
	 * @param dialogue
	 *            So I can assure a new dialogue is being made per class rather than reusing the same one per class.
	 */
	public Dialogue(final Player player, final int npcId) {
		this.player = player;
		dialogue = new Int2ObjectOpenHashMap<Message>();
		this.npcId = npcId;
		npc = null;
	}
	...
	public NPCMessage npcWithId(final int npcId, final String npcName, final String message, final Expression expression) {
		val baseDefinitions = NPCDefinitions.getOrThrow(npcId);
		val transmogrifiedId = player.getTransmogrifiedId(baseDefinitions, npcId);
	...
	public ItemMessage item(final int itemId, final String message) {
		return item(new Item(itemId), message);
	}
