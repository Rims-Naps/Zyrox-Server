    ...
    public int attack(final Entity target) {
        ...
        if (!SlayerEquipment.MIRROR_SHIELD.isWielding(player)) {
                ...
                for (int i = 0; i <= 6; i++) {
                    ...
                    if (i == 5) {
                        player.getPrayerManager()
                                .setPrayerPoints((int) (player.getPrayerManager().getPrayerPoints() * 0.4078947368421053));
                    } else {
            ...
            delayHit(npc, 0, player,
                    new Hit(npc, getRandomMaxHit(npc, npc.getCombatDefinitions().getMaxHit(), MELEE, target), HitType.MELEE));
        }
