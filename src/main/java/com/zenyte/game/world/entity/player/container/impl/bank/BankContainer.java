...
public final class BankContainer extends Container {
	...
	public ContainerResult add(final Item requestedItem) {
		...
		val placeholderId = ItemDefinitions.getOrThrow(PlaceholderRedirections.builder.getOrDefault(definitions.getId(), definitions.getId())).getPlaceholderId();
			...
			if (stackable && policy == ContainerPolicy.NORMAL || policy == ContainerPolicy.ALWAYS_STACK) {
				...
				if (slot != -1 || placeholderId != -1 && (slot = getSlotOf(placeholderId)) != -1) {
					...
					if (!existingItem.hasAttributes()) {
						...
						result.setResult(
								result.getSucceededAmount() != requestedAmount ? RequestResult.OVERFLOW : RequestResult.SUCCESS);
						return result;
					}
				}
			}
	...
	public ContainerResult remove(final Item requestedItem, final boolean placeholder, Player player) {
		...
		val phId = ItemDefinitions.getOrThrow(PlaceholderRedirections.builder.getOrDefault(definitions.getId(), definitions.getId())).getPlaceholderId();
	...
	public void withdraw(final Player player, final Container container, int slot, int amount, final boolean placeholder) {
		...
		val stackable =
                definitions.isStackable() || (!item.hasAttributes() && bank.getSetting(BankSetting.WITHDRAW_MODE) == 1 && definitions.getNotedId() != -1 && ItemDefinitions.get(definitions.getNotedId()).isStackable());
		...
		if (!item.hasAttributes() && (stackable || policy == ContainerPolicy.ALWAYS_STACK)) {
			...
			val id =
                    !item.hasAttributes() && bank.getSetting(BankSetting.WITHDRAW_MODE) == 1 && !definitions.isNoted() && definitions.getNotedId() != -1
			...
			for (; i < length; i++) {
				...
				val id = bank.getSetting(BankSetting.WITHDRAW_MODE) == 1 && definitions.isNoted() && definitions.getNotedId() != -1
						? definitions.getNotedId()
						: item.getId();
