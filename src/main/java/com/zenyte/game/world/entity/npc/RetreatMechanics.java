...
public class RetreatMechanics {
    ...
    protected boolean outOfBounds(@NotNull final Location other) {
        ...
        val tile = npc.getRespawnTile();
        val size = npc.getSize();
        val dx = other.getX() - tile.getX();
        val dy = other.getY() - tile.getY();
    ...
    protected Location retreatDestination(@NotNull final Entity target) {
        val distance = distance();
        val middle = npc.getMiddleLocation();
        val degrees = degrees(middle.getY() - target.getY(), middle.getX() - target.getX());
        val angle = Math.toRadians(degrees);
        val x = (int) Math.round(middle.getX() + (distance * Math.cos(angle)));
        val y = (int) Math.round(middle.getY() + (distance * Math.sin(angle)));
    ...
    private double degrees(final int offsetY, final int offsetX) {
        val degrees = Math.toDegrees(Math.atan2(offsetY, offsetX));
