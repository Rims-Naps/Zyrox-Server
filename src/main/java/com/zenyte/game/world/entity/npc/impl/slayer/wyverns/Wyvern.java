...
public abstract class Wyvern extends NPC implements CombatScript, Spawnable {
    ...
    protected void magicAttack(final Entity target) {
        ...
        val damage = magicAttackDamage(target);
    ...
    private void freeze(final Entity target) {
        if (target instanceof Player) {
            val player = (Player) target;
            val shield = player.getEquipment().getId(EquipmentSlot.SHIELD);
            val hasShield = shield == ItemId.ANCIENT_WYVERN_SHIELD || shield == ItemId.ANCIENT_WYVERN_SHIELD_21634;
    ...
    private boolean hasShield(final Entity target) {
        ...
        val player = (Player) target;
        val shield = player.getEquipment().getId(EquipmentSlot.SHIELD.getSlot());
    ...
    private static final ImmutableList<Integer> PROTECTIVE_SHIELDS = ImmutableList.of(
            ItemId.ELEMENTAL_SHIELD,
            ItemId.MIND_SHIELD,
            ItemId.DRAGONFIRE_SHIELD, ItemId.DRAGONFIRE_SHIELD_11284,
            ItemId.ANCIENT_WYVERN_SHIELD, ItemId.ANCIENT_WYVERN_SHIELD_21634,
            ItemId.DRAGONFIRE_WARD, ItemId.DRAGONFIRE_WARD_22003
    );
}
