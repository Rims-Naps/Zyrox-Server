...
public final class VeracCombat extends MeleeCombat {
	...
	public Hit getHit(final Player player, final Entity target, final double accuracyModifier, final double passiveModifier, double activeModifier, final boolean ignorePrayers) {
		val verac = CombatUtilities.hasFullBarrowsSet(player, "Verac's") && Utils.random(3) == 0;
		val damage = getRandomHit(player, target, getMaxHit(player, passiveModifier, 1, false), verac ? 100 : accuracyModifier);
		val hit = new Hit(player, verac ? (1 + damage) : damage, HitType.MELEE);
		if (verac) {
