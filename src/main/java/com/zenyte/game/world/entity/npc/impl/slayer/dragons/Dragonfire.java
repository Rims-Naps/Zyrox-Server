...
public class Dragonfire {
    ...
    public static abstract class DragonfireBuilder extends Dragonfire {
        public DragonfireBuilder(final DragonfireType type, final int maximumDamage,
                          final List<DragonfireProtection> protection) {
            this(type, maximumDamage, protection, DEFAULT_MSG_BUILDER);
        }

        public DragonfireBuilder(final DragonfireType type, final int maximumDamage,
                          final List<DragonfireProtection> protection, final MessageBuilder messageBuilder) {
        ...
        public float getAccumulativeTier() {
            ...
            for (val protection : affectedProtections) {
    ...
    public Dragonfire(final DragonfireType type, final int maximumDamage,
                      final List<DragonfireProtection> protection) {
        this(type, maximumDamage, protection, DEFAULT_MSG_BUILDER);
    }
     ...
     */
    private void filter() {
        val list = new ArrayList<DragonfireProtection>(3);
        for (val protection : this.protection) {
    ...
    @Getter private final int maximumDamage;
    ...
    public int getDamage() {
        ...
        reducedDamage = (int) (accumulativeTier >= type.fullProtectionTier ? 0 :
                (maximumDamage * (1 - (accumulativeTier / type.fullProtectionTier))));
        return reducedDamage;
    }
