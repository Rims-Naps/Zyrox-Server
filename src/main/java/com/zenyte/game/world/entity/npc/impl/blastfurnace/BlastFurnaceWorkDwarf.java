...
public class BlastFurnaceWorkDwarf extends NPC implements Spawnable {
    ...
    public void processNPC() {
        ...
        if(getId() == 6602 && NUMPTY_IDLE) {
            val roll = Utils.random(30);

            if(roll == 0) {
                NUMPTY_IDLE = false;

                if(getPosition().getX() == NUMPTY_RIGHT.getX())
                    addWalkSteps(NUMPTY_LEFT.getX(), NUMPTY_LEFT.getY());
                else
                    addWalkSteps(NUMPTY_RIGHT.getX(), NUMPTY_RIGHT.getY());
    ...
    public boolean validate(int id, String name) {
        return name.toLowerCase().equals("dumpy") || name.toLowerCase().equals("numpty");
    }
