...
public class PaydirtNPC extends NPC {
    ...
    @Getter private final List<Item> paydirt;
        ...
        if(getY() == 5660) {
            if (!player.isNulled()) {
                this.paydirt.forEach(item -> {
                    val id = item.getNumericAttribute("paydirt ore id").intValue();
                    if (id == 0) {
                        Paydirt.generate(player).ifPresent(pd -> {
                            player.getPaydirt().add(pd.getId());
                            player.getSkills().addXp(Skills.MINING, pd.getXp());
                        });
                    } else {
                        player.getPaydirt().add(id);
                        Paydirt.get(id).ifPresent(paydirt -> player.getSkills().addXp(Skills.MINING, paydirt.getXp()));
                    }
                });
                player.getVarManager().sendBit(5558, player.getPaydirt().size());

                /** Calculation for paydirt message upon entry into the sack. */
                val maxPaydirt = (player.getBooleanAttribute("motherlode_sack_upgrade") ? 162 : 81);
                val paydirtAmt = player.getPaydirt().size();
                if(paydirtAmt + 28 >= maxPaydirt)
