...
public final class LoyaltyManager {
    ...
    public static final int LOYALTY_POINTS_PER_INTERVAL = 20;
    ...
    public static final void onInitialization(final InitializationEvent event) {
        ...
        val currentLoyalty = player.getLoyaltyManager();
        currentLoyalty.login = Date.from(Instant.now());
        val saved = event.getSavedPlayer();
        val loyaltyManager = saved.getLoyaltyManager();
    ...
    public int getConsecutiveDaysLoggedIn(@NotNull final LocalDate from, @NotNull final LocalDate until, final int requiredDurationPerDay, @NotNull final TimeUnit timeUnit) {
        val startDay = from.toEpochDay();
        val endDay = until.toEpochDay();
        Preconditions.checkArgument(endDay > startDay, "End date cannot be after start date.");
        val necessaryDuration = timeUnit.toMillis(requiredDurationPerDay);
        int count = 0;
        for (long epochDay = endDay; epochDay >= startDay; epochDay--) {
            val date = LocalDate.ofEpochDay(epochDay);
            val duration = getLoggedInDuration(date);
    ...
    private long getLoggedInDuration(@NotNull final LocalDate day) {
        ...
        val endDate = Date.from(day.atStartOfDay(ZoneId.systemDefault()).toInstant());
        endDate.setTime(endDate.getTime() + TimeUnit.DAYS.toMillis(1));
        long totalMilliseconds = 0;
        for (val session : Objects.requireNonNull(this.sessions)) {
            val login = session.getLogin();
            val logout = session.getLogout();
            if (logout.before(date) || login.after(endDate)) {
                continue;
            }
            val start = (date.before(login) ? login : date).getTime();
            val end = (logout.after(endDate) ? endDate : logout).getTime();
    ...
    public void informSession(final int count) {
        ...
        val totalMinutes = count * LOYALTY_POINTS_INTERVAL_MINUTES;
        val hours = totalMinutes / 60;
        val remainingMinutes = totalMinutes % 60;
        val time = hours == 0 ? remainingMinutes + " minutes" : (hours + (hours == 1 ? " hour " : " hours ") + (remainingMinutes != 0 ? "and " : "") + (remainingMinutes == 0 ? "" :
                                                                                                                                         (remainingMinutes + " minutes")));
        val points = (int) (LOYALTY_POINTS_PER_INTERVAL * multiplier(count));
        setLoyaltyPoints(getLoyaltyPoints() + points);
        player.sendMessage(Colour.RS_GREEN.wrap("You receive " + points + " loyalty points for " + time.trim() + " of consecutive playtime. " +
                "You now have " + getLoyaltyPoints() + " loyalty points."));
    }
