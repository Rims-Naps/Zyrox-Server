...
public final class FlamesOfZamorakEffect implements SpellEffect {
	...
	public void spellEffect(final Entity player, final Entity target, final int damage) {
		if (target.getEntityType() == EntityType.PLAYER) {
			...
			if (p2.getSkills().getLevel(Skills.MAGIC) < p2.getSkills().getLevelForXp(Skills.MAGIC)) {
				return;
			}
		} else {
            final NPC npc = (NPC) target;
            val defs = NPCCDLoader.get(npc.getId());
            if (defs == null) {
                return;
            }
            if (npc.getCombatDefinitions().getStatDefinitions().get(StatType.MAGIC) < defs.getStatDefinitions().get(StatType.MAGIC))
                return;
        }
		target.drainSkill(Skills.MAGIC, 5f);
	}
