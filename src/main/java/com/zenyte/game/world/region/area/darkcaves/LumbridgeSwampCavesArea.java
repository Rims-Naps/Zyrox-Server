...
public class LumbridgeSwampCavesArea extends DarkArea implements PartialMovementPlugin {
	...
	private static final Location[] FLAMMABLE_LOCATIONS = new Location[] { new Location(3145, 9579, 0), new Location(3146, 9577, 0),
			new Location(3186, 9567, 0), new Location(3164, 9539, 0), new Location(3162, 9559, 0), new Location(3161, 9561, 0),
			new Location(3156, 9541, 0), new Location(3159, 9560, 0), new Location(3146, 9576, 0), new Location(3145, 9575, 0),
			new Location(3154, 9540, 0), new Location(3182, 9589, 0), new Location(3160, 9560, 0), new Location(3163, 9595, 0),
			new Location(3161, 9559, 0), new Location(3164, 9594, 0), new Location(3153, 9541, 0), new Location(3157, 9552, 0),
			new Location(3182, 9541, 0), new Location(3154, 9543, 0), new Location(3183, 9541, 0), new Location(3248, 9572, 0),
			new Location(3244, 9556, 0), new Location(3211, 9591, 0), new Location(3248, 9571, 0), new Location(3205, 9569, 0),
			new Location(3231, 9582, 0), new Location(3205, 9571, 0), new Location(3231, 9583, 0), new Location(3246, 9570, 0),
			new Location(3237, 9544, 0), new Location(3221, 9543, 0), new Location(3246, 9571, 0), new Location(3232, 9582, 0),
			new Location(3254, 9585, 0), new Location(3205, 9575, 0), new Location(3232, 9584, 0), new Location(3248, 9569, 0),
			new Location(3232, 9551, 0), new Location(3231, 9580, 0), new Location(3205, 9573, 0), new Location(3220, 9554, 0),
			new Location(3248, 9570, 0), new Location(3220, 9555, 0), };
	...
	public RSPolygon[] polygons() {
		return new RSPolygon[] { new RSPolygon(new int[][] { { 3162, 9601 }, { 3136, 9585 }, { 3147, 9534 }, { 3187, 9532 }, { 3198, 9546 },
				{ 3213, 9545 }, { 3226, 9536 }, { 3257, 9550 }, { 3264, 9585 }, { 3336, 9595 }, { 3330, 9658 }, { 3224, 9657 },
				{ 3221, 9625 }, { 3221, 9614 }, { 3208, 9605 } }, 0) };
	}
	...
	public void process() {
		...
		for (val player : players) {
			val attributes = player.getTemporaryAttributes();
			val flaring = attributes.get("lightsource flaring");
			if (flaring != null) {
				val flareTimer = player.getNumericTemporaryAttribute("lightsource flare count").intValue();
				if (flareTimer == 5) {
					val position = new Location(player.getNumericTemporaryAttribute("lightsource flare hash").intValue());
					if (player.getLocation().withinDistance(position, 3)) {
						attributes.remove("lightsource flaring");
						if (Utils.random(10) >= 3) {
							LightSource.extinguish(player, Integer.MAX_VALUE,
                                    LightSource.MAPPED_SOURCES.values().toArray(new LightSource[0]));
							player.sendMessage("The swamp gas explodes!");
							player.applyHit(new Hit(Utils.random(10, 20), HitType.REGULAR));
							refreshOverlay(player);
							continue;
						}
						val name = attributes.get("lightsource name");
			...
			if (player.getInterfaceHandler().isVisible(96)) {
				val ticksPassed = player.getNumericTemporaryAttribute("full darkess timer").intValue();
				if (ticksPassed == 5) {
			...
			if (Utils.random(1000) == 0) {
				val lightsources = LightSource.getLitLightSources(player, LightSourceItem.NONE);
			...
			if (Utils.random(10) == 0 && flaring == null) {
				val lightsources = LightSource.getLitLightSources(player, LightSourceItem.FLAMMABLE);
				if (lightsources.length == 0) {
					continue;
				}
				val location = player.getLocation();
				for (val flammableLocation : FLAMMABLE_LOCATIONS) {
					if (location.withinDistance(flammableLocation, 3)) {
						val name = lightsources[Utils.random(lightsources.length - 1)].toString().toLowerCase().replaceAll("_", " ");
    ...
    public boolean processMovement(Player player, int x, int y) {
        val tile = WALL_BEASTS.get(y | x << 14 | player.getPlane() << 28);
        if (tile == null) {
            return true;
        }
