 ...
 */
public class ApeAtollArea extends Area implements ContainerPlugin {
	...
	public RSPolygon[] polygons() {
		return new RSPolygon[] { new RSPolygon(new int[][]{
				...
				new RSPolygon(
						new int[][] { { 3012, 5496 }, { 3012, 5448 }, { 3035, 5448 }, { 3035, 5472 }, { 3070, 5472 }, { 3070, 5496 } },
						0) };
	}
	...
	public void enter(final Player player) {
		...
		val greegree = Greegree.MAPPED_VALUES.get(player.getEquipment().getId(EquipmentSlot.WEAPON));
		if (greegree == null) {
			return;
		}
	...
	public void leave(final Player player, boolean logout) {
	    if (GlobalAreaManager.get("Ape Atoll").inside(player.getLocation()) || GlobalAreaManager.get("Ape Atoll " +
                "Dungeon").inside(player.getLocation())) {
	        return;
        }
		if (player.getTemporaryAttributes().remove("greegree") != null) {
			val equipment = player.getEquipment();
			val item = equipment.getItem(EquipmentSlot.WEAPON);
			Greegree.reset(player);
			if (item == null || Greegree.MAPPED_VALUES.get(item.getId()) == null) {
				return;
			}
			player.getDialogueManager()
					.start(new PlainChat(player, "The Monkey Greegree wrenches itself from your hand as its power begins to fade..."));
	...
	public void onContainerModification(final Player player, final Container container, final Item previousItem, final Item currentItem) {
		...
		if (currentItem != null) {
			val greegree = Greegree.MAPPED_VALUES.get(currentItem.getId());
			if (greegree != null) {
				greegree.transform(player);
			}
		}
