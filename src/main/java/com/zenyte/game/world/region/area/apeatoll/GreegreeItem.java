...
public class GreegreeItem extends ItemPlugin implements EquipPlugin {
    ...
    public boolean handle(final Player player, final Item item, final int slotId, final int equipmentSlot) {
        val greegree = Greegree.MAPPED_VALUES.get(item.getId());
        if (greegree == null) {
            return false;
        }
        if (!GlobalAreaManager.get("Ape Atoll").inside(player.getLocation()) && !GlobalAreaManager.get("Ape " +
                "Atoll Dungeon").inside(player.getLocation())) {
    ...
    @Override
	public int[] getItems() {
		val list = new IntArrayList(Greegree.VALUES.length);
		for (val greegree : Greegree.VALUES) {
