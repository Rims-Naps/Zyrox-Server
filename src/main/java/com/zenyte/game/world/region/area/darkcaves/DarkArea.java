 ...
 */
public abstract class DarkArea extends Area implements CycleProcessPlugin, ContainerPlugin {
	
	protected void refreshOverlay(final Player player) {
		val brightness = Math.max(0, LightSource.getBrightness(player));
	...
	public void onContainerModification(final Player player, final Container container, final Item previousItem, final Item currentItem) {
		val previousSource = LightSource.getSource(previousItem);
		val currentSource = LightSource.getSource(currentItem);
