...
public class BonesTablet extends ItemPlugin {
    ...
    public static final int convertBones(@NotNull final Player player, final int transformedItem) {
        int amount = 0;
        for (val item : player.getInventory().getContainer().getItems().values()) {
            if (item == null) {
                continue;
            }
    ...
    public void handle() {
        bind("Break", (player, tablet, container, slotId) -> {
            val amount = convertBones(player, tablet.getId() == 8014 ? BONES : PEACHES);
            if (amount > 0) {
                ...
                val typeString = tablet.getId() == 8014 ? "bananas" : "peaches";
