...
public class RechargeDragonstoneJewelleryScroll extends ItemPlugin {
    ...
    public void handle() {
        bind("Read", (player, __, ___, slotId) -> {
            val containers = new Container[] {player.getInventory().getContainer(), player.getEquipment().getContainer()};
            int count = 0;
            int firstSuccessfulItem = -1;
            for (val ct : containers) {
                for (int i = ct.getContainerSize(); i >= 0; i--) {
                    val containerItem = ct.get(i);
    ...
    private final boolean rechargeDragonstoneItem(@NotNull final Item item) {
        ...
        val isSkillsNecklace = ArrayUtils.contains(SKILLS_NECKLACES, id);
        val isCombatBracelet = ArrayUtils.contains(COMBAT_BRACELETS, id);
