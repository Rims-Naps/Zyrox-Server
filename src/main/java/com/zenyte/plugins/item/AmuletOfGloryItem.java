...
public class AmuletOfGloryItem extends ItemPlugin {
            ...
            player.getDialogueManager().start(new Dialogue(player) {
                ...
                public void buildDialogue() {
                    options("Where would you like to teleport to?",
                            new DialogueOption("Edgeville", () -> new GloryTeleport(item, container, slotId, new Location(3087, 3489, 0)).teleport(player)),
                            new DialogueOption("Karamja", () -> new GloryTeleport(item, container, slotId, new Location(2918, 3176, 0)).teleport(player)),
                            new DialogueOption("Draynor Village", () -> new GloryTeleport(item, container, slotId, new Location(3105, 3251, 0)).teleport(player)),
                            new DialogueOption("Al Kharid", () -> new GloryTeleport(item, container, slotId, new Location(3293, 3163, 0)).teleport(player)),
    ...
    private static final class GloryTeleport implements Teleport {
        ...
        public void onArrival(final Player player) {
            val glory = AmuletOfGlory.get(item);
            val charges = glory.getCharges();
            val message = charges == 1 ?
                          ...
                          "<col=7F00FF>No charges were used since the amulet holds unlimited charges.";
            player.sendMessage(message);
            if (glory == AmuletOfGlory.ETERNAL) {
                return;
            }
            val result = glory.getResult();
