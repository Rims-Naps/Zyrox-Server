...
public class CoalBag extends ItemPlugin implements ItemOnItemAction {
    ...
    public static void emptyBag(final Item coalBag, final Player player) {
        val coal = coalBag.getNumericAttribute("coal").intValue();
        if (coal <= 0) {
        ...
        val amount = player.getInventory().addItem(new Item(453, coal)).getSucceededAmount();
    ...
    public static void emptyBagToBank(final Item coalBag, final Player player) {
        ...
        val amount = player.getBank().add(new Item(453, coal)).getSucceededAmount();
        coalBag.setAttribute("coal", coal - amount);
        player.sendMessage("You withdraw " + amount + " coal from the coal bag.");
    ...
    private void fillBag(final Item coalBag, final Player player) {
        val coal = coalBag.getNumericAttribute("coal").intValue();
        val cape = SkillcapePerk.SMITHING.isEffective(player);
        val limit = cape ? 36 : 27;
        if (coal >= limit) {
            player.sendMessage("Your coal bag cannot hold any more coal.");
            return;
        }
        val amount = player.getInventory().deleteItem(new Item(453, limit - coal)).getSucceededAmount();
        coalBag.setAttribute("coal", coal + amount);
        if (amount > 0) {
    ...
    public int[] getItems() {
        return new int[] {
                ITEM.getId(), Smelting.COAL.getId()
        };
    }
}
