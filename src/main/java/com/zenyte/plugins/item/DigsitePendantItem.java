...
public class DigsitePendantItem extends ItemPlugin {
            ...
            player.getDialogueManager().start(new Dialogue(player) {
                ...
                public void buildDialogue() {
                    options("Where would you like to teleport to?",
                            new DialogueOption("Digsite", () -> new PendantTeleport(item, container, slotId, digsiteLocation).teleport(player)),
                            new DialogueOption("Fossil Island", () -> new PendantTeleport(item, container, slotId, new Location(3763, 3870, 1)).teleport(player)),
                            new DialogueOption("Lithkren Dungeon", () -> new PendantTeleport(item, container, slotId, new Location(3547, 10456, 0)).teleport(player)),
    ...
    private static final class PendantTeleport implements Teleport {
        ...
        public void onArrival(final Player player) {
            val pendant = Pendant.get(item);
            val charges = pendant.getCharges();
            val message = charges == 1
                          ? "<col=7F00FF>You use your pendant's last charge.</col>"
                          : charges == 2 ? "<col=7F00FF>Your pendant has one charge left.</col>"
                          : "<col=7F00FF>Your pendant has " + Utils.format(charges - 1) + " charges left.</col>";

            player.sendMessage(message);
            val result = pendant.getResult();
