...
public class BugLantern extends ItemPlugin implements PairedItemOnItemPlugin {
    ...
    public void handleItemOnItemAction(Player player, Item from, Item to, int fromSlot, int toSlot) {
        val tinderbox = from.getId() == ItemId.TINDERBOX ? from : to;
    ...
    public ItemPair[] getMatchingPairs() {
        return new ItemPair[] {
                new ItemPair(ItemId.TINDERBOX, ItemId.UNLIT_BUG_LANTERN)
        };
    }
    ...
    public int[] getItems() {
        return new int[] {
                ItemId.LIT_BUG_LANTERN, ItemId.UNLIT_BUG_LANTERN
        };
    }
}
