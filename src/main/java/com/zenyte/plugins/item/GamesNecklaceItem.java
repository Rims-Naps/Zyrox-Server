...
public class GamesNecklaceItem extends ItemPlugin {
            ...
            player.getDialogueManager().start(new Dialogue(player) {
                ...
                public void buildDialogue() {
                    options("Where would you like to teleport to?",
                            new DialogueOption("Burthorpe", () -> new NecklaceTeleport(item, container, slotId, new Location(2898, 3552, 0)).teleport(player)),
                            new DialogueOption("Barbarian Outpost", () -> new NecklaceTeleport(item, container, slotId, new Location(2519, 3572, 0)).teleport(player)),
                            new DialogueOption("Corporeal Beast", () -> new NecklaceTeleport(item, container, slotId, new Location(2967, 4381, 2)).teleport(player)),
                            new DialogueOption("Tears of Guthix", () -> new NecklaceTeleport(item, container, slotId, new Location(3245, 9500, 2)).teleport(player)),
                            new DialogueOption("Wintertodt Camp", () -> new NecklaceTeleport(item, container, slotId, new Location(1627, 3941, 0)).teleport(player)));
                }
            });
        });
    ...
    private static final class NecklaceTeleport implements Teleport {
        ...
        public void onArrival(final Player player) {
            val necklace = GamesNecklace.get(item);
            val charges = necklace.getCharges();
            val message = charges == 1 ?
                          "<col=7F00FF>Your games necklace crumbles to dust.</col>" :
                          charges == 2 ?
                          "<col=7F00FF>Your games necklace has one charge left..</col>" :
                          "<col=7F00FF>Your games necklace has " + Utils.format(charges - 1) + " charges left.</col>";
    
            player.sendMessage(message);
            val result = necklace.getResult();
