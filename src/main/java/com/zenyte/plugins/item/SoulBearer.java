...
public class SoulBearer extends ItemPlugin implements PairedItemOnItemPlugin {
    ...
    public void handle() {
        bind("Fill", (player, item, container, slotId) -> {
            ...
            val minimum = Math.min(1000 - item.getCharges(), Math.min(inventory.getAmountOf(ItemId.SOUL_RUNE), inventory.getAmountOf(ItemId.BLOOD_RUNE)));
            if (minimum <= 0) {
                return;
            }
    ...
    public void handleItemOnItemAction(Player player, Item from, Item to, int fromSlot, int toSlot) {
        val inventory = player.getInventory();
        val bearer = from.getId() == ItemId.SOUL_BEARER ? from : to;
        val ensouledHead = bearer == from ? to : from;
        val amount = Math.min(inventory.getAmountOf(ensouledHead.getId()), bearer.getCharges());
        if (amount <= 0) {
            player.sendMessage("Your soul bearer is empty.");
            return;
        }
        val succeededAmount = player.getBank().add(new Item(ensouledHead.getId(), amount)).getSucceededAmount();
    ...
    public ItemPair[] getMatchingPairs() {
        val list = new ObjectArrayList<ItemPair>();
        for (val reanimation : Reanimation.values()) {
            list.add(ItemPair.of(ItemId.SOUL_BEARER, reanimation.getItemId()));
        }
