...
public class BottomlessCompostBucket extends ItemPlugin implements PairedItemOnItemPlugin {
    ...
    public void handle() {
        bind("Check", (player, item, container, slotId) -> {
            ...
            val uses = charges & 0xFFFF;
            val compostName = type == 0 ? "compost" : type == 1 ? "supercompost" : "ultracompost";
        ...
        bind("Fill/Check", (player, item, container, slotId) -> player.getDialogueManager().start(new Dialogue(player) {
            ...
            public void buildDialogue() {
                options(TITLE,
                        new DialogueOption("Fill.", () -> getHandler("Fill").handle(player, item, container, slotId)),
                        new DialogueOption("Check.", () -> getHandler("Check").handle(player, item, container, slotId)));
            }
        }));
        bind("Empty", (player, item, container, slotId) -> player.getDialogueManager().start(new Dialogue(player) {
            @Override
            public void buildDialogue() {
                item(item, "Emptying the bottomless compost bucket won't give you back any of the compost it holds - " + Colour.RED.wrap("it will all be lost forever") + ". Are " +
                        "you sure you wish to empty " + "it?");
        ...
        bind("Fill", (player, item, container, slotId) -> {
            val charges = item.getCharges();
            if ((charges & 0xFFFF) >= 9999) {
                player.sendMessage("Your bottomless compost bucket cannot hold any more compost.");
                return;
            }
            val inventory = player.getInventory();
            val compost = inventory.getAmountOf(6032) + inventory.getAmountOf(6033);
            val supercompost = inventory.getAmountOf(6034) + inventory.getAmountOf(6035);
            val ultracompost = inventory.getAmountOf(21483) + inventory.getAmountOf(21484);
            if (charges == 0) {
                ...
                if (differenceCount == 1) {
                    ...
                    player.getDialogueManager().start(new Dialogue(player) {
                        @Override
                        public void buildDialogue() {
                            val list = new ObjectArrayList<DialogueOption>();
                            if (compost != 0) {
    ...
    private final void fill(final Player player, final Item item, final int type) {
        val bucketType = item.getCharges() == 0 ? -1 : (item.getCharges() >> 16);
        ...
        val existing = item.getCharges() & 0xFFFF;
        final Runnable runnable = () -> {
            player.getDialogueManager().finish();
            val typeName = type == 0 ? "Compost" : type == 1 ? "Supercompost" : "Ultracompost";
            val inventory = player.getInventory();
            ...
            player.sendInputInt("How many " + typeName + " would you like to fill with? (1 - " +
                    Math.min(5000, compost) + ")", value -> fill(player, item, type, Math.min(5000, value)));
        };
        if (existing != 0) {
            runnable.run();
            return;
        ...
        player.getDialogueManager().start(new Dialogue(player) {
            @Override
            public void buildDialogue() {
                item(item, "Filling the bucket with compost will make the bucket<br>untradeable. If emptied, it can be made tradeable once<br>more - however <col=cc0000>ALL COMPOST " +
                        "CONTAINED<br><col=cc0000>WITHIN IS LOST.</col>");
    ...
    private void fill(final Player player, final Item item, final int type, final int amount) {
        ...
        player.getDialogueManager().start(new ItemChat(player, item,
                "You fill your bottomless compost bucket with " + (amount - requested) + " x " +
                        (type == 0 ? "compost" : type == 1 ? "supercompost" : "ultracompost") + ". " +
                        "Your bottomless compost bucket now contains a total of " + (item.getCharges() & 0xFFFF) + " uses."));
    }

    @Override
    public void handleItemOnItemAction(Player player, Item from, Item to, int fromSlot, int toSlot) {
        val bucket = from.getId() == 22994 || from.getId() == 22997 ? from : to;
        val compost = bucket == from ? to : from;
        val compostId = compost.getId();
    ...
    public ItemPair[] getMatchingPairs() {
        return new ItemPair[] {
                ItemPair.of(22994, 6032), ItemPair.of(22994, 6033), ItemPair.of(22994, 6034), ItemPair.of(22994, 6035), ItemPair.of(22994, 21483), ItemPair.of(22994, 21484),
                ItemPair.of(22997, 6032), ItemPair.of(22997, 6033), ItemPair.of(22997, 6034), ItemPair.of(22997, 6035), ItemPair.of(22997, 21483), ItemPair.of(22997, 21484),
        };
    }
}
