...
public class RingOfSuffering extends ItemPlugin implements PairedItemOnItemPlugin, ChargeExtension {
        ...
        bind("Recoil settings", (player, item, container, slotId) -> player.getDialogueManager().start(new Dialogue(player) {
            ...
            public void buildDialogue() {
                options(TITLE, new DialogueOption("Toggle recoil effect", () -> {
                    player.addAttribute("recoil effect", player.getBooleanAttribute("recoil effect") ?
                                                         0 :
                                                         1);
                    setKey(5);
                }), new DialogueOption("Discard recoil charges", () -> uncharge(player, item, container, slotId)), new DialogueOption("Cancel"));
                plain(5, !player.getBooleanAttribute("recoil effect") ?
                         "Your ring of suffering will now reflect damage." :
                         "Your ring of suffering will no longer reflect damage.");
            }
        }));
    }
                ...
                options("Are you sure you want to uncharge your <col=00080>" + item.getName() + "</col>?", "Yes, I'm sure.", "No.")
                        .onOptionOne(() -> {
                            if (container.get(slotId) == item) {
                                container.set(slotId, new Item(item.getId() == 20657 ?
                                                               19710 :
                                                               item.getId() == 20655 ?
                                                               19550 :
                                                               item.getId()));
    ...
    public void handleItemOnItemAction(final Player player, final Item from, final Item to, final int fromSlot, final int toSlot) {
        val recoil = ArrayUtils.contains(RECOILS, from.getId()) ?
                     from :
                     to;
        val recoilId = recoil.getId();
        val suffering = recoil == from ?
                        to :
                        from;
        val charges = suffering.getCharges();
        player.sendInputInt("How many rings do you wish to use?", value -> {
            val inventory = player.getInventory();
            val maxAmount = (int) Math.ceil((100000 - charges) / 40F);
            if (maxAmount == 0) {
                player.sendMessage("Your ring of suffering is already fully charged.");
                return;
            }
            val inInventory = (int) (inventory.getAmountOf(RECOILS) & 0x7FFFFFFF);
            val amount = Math.min(maxAmount, Math.min(inInventory, value));
            if (amount == 0) {
                return;
            }
            int total = 0;
            val result = inventory.deleteItem(recoilId, amount);
            total += result.getSucceededAmount();
            if (result.getSucceededAmount() != amount) {
                val remainder = amount - result.getSucceededAmount();
                val additionalResult = inventory.deleteItem(recoilId == RECOIL ?
                                                            NOTED_RECOIL :
                                                            RECOIL, remainder);
            ...
            player.sendMessage(
                    "You load your ring with " + total + " rings of recoil. It now has " + suffering.getCharges() + " recoil charges.");
            val sufferingId = suffering.getId();
