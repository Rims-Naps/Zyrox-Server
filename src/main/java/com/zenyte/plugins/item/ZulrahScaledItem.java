...
public class ZulrahScaledItem extends ItemPlugin {
	...
	public void handle() {
		bind("Dismantle", (player, item, slotId) -> {
			val name = item.getName().replace(" (uncharged)", "");
			player.getDialogueManager().start(new Dialogue(player) {
                @Override
                public void buildDialogue() {
                    item(item, "Dismantling the " + name + " is an irreversible process which grants you 20,000 Zulrah's scales.");
                    options("Dismantle the " + name + "?",
                            new DialogueOption("Dismantle it.", () -> {
