	...
	private static final class ActivateDialogue extends Dialogue {
        ...
        public void buildDialogue() {
            ...
            if (assignment == null) {
                ...
                npc("You're currently assigned to kill " + assignment.getTask().getTaskName() + "; only "
                        + assignment.getAmount() + " more to go. Your reward point tally is " + player.getNumericAttribute("slayer_points").intValue() + ".");
            }
            options();

            player(100, "Who are you?");
            npc("My name's " + Utils.formatString(player.getSlayer().getMaster().toString()) + ", I'm your Slayer Master.");
            options();

            player(200, "Where are you?");
            npc("You'll find me " + player.getSlayer().getMaster().getLocation() + ".<br>I'l be here when you need a new task.");
            options();
        ...
        private void options() {
            options(TITLE, new DialogueOption("How am I doing so far?", key(5)),
                    new DialogueOption("Who are you?", key(100)),
                    new DialogueOption("Where are you?", key(200)),
                    new DialogueOption("Got any tips for me?", key(300)),
                    new DialogueOption("Nothing really.", key(400)));
        }
    }
