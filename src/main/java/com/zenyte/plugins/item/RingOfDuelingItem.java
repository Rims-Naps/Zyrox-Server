...
public class RingOfDuelingItem extends ItemPlugin {
            ...
            player.getDialogueManager().start(new Dialogue(player) {
                ...
                public void buildDialogue() {
                    options("Where would you like to teleport to?",
                            new DialogueOption("Duel Arena", () -> new RingTeleport(item, container, slotId, new Location(3315, 3235, 0)).teleport(player)),
                            new DialogueOption("Castle Wars", () -> new RingTeleport(item, container, slotId, new Location(2440, 3090, 0)).teleport(player)),
                            new DialogueOption("Clan Wars", () -> new RingTeleport(item, container, slotId, new Location(3388, 3160, 0)).teleport(player)),
    ...
    private static final class RingTeleport implements Teleport {
        ...
        public void onArrival(final Player player) {
            val ring = RingOfDueling.get(item);
            val charges = ring.getCharges();
            val message = charges == 1 ?
                          "<col=7F00FF>Your ring of dueling crumbles to dust.</col>" :
                          charges == 2 ?
                          "<col=7F00FF>Your ring of dueling has one charge left..</col>" :
                          "<col=7F00FF>Your ring of dueling has " + Utils.format(charges - 1) + " charges left.</col>";
