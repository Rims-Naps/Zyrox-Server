...
public class XericsTalisman extends ItemPlugin implements PairedItemOnItemPlugin {
    ...
    public void handle() {
        ...
        bind("Rub", (player, item, container, slotId) -> player.getDialogueManager().start(new OptionsMenuD(player,
                "The talisman has " + item.getCharges() + " " + (item.getCharges() == 1 ? "charge" : "charges") + ".",
                "Xeric's Look-out", "Xeric's Glade", "Xeric's Inferno", "Xeric's Heart", "Xeric's Honour", "Nowhere") {
            @Override
        ...
        bind("Check", (player, item, container, slotId) -> player.sendMessage("The talisman has " + item.getCharges() + " " + (item.getCharges() == 1 ?
                                                                                                                         "charge" : "charges") + "."));
        bind("Inspect", (player, item, container, slotId) ->
                player.getDialogueManager().start(new ItemChat(player, item, "You inspect the tablet, and sense a magical power, running your fingers over the inscriptions, " +
                        "they tingle as you get a feel for Xeric's power. He must have stored one of his best spells in this tablet, " +
                        "perhaps he has some sort of device which could utilise it?")));
        bind("Uncharge", (player, item, container, slotId) -> {
            ...
            val talisman = new Item(13392, 1);
            val fangs = new Item(13391, item.getCharges());
            player.getInventory().addItem(talisman);
            player.getInventory().addOrDrop(fangs);
            player.getDialogueManager().start(new DoubleItemChat(player, talisman, fangs,
                    "You remove " + fangs.getAmount() + " lizard " + (fangs.getAmount() == 1 ? "fang" : "fangs") + " from your talisman."));
        });
    }
    ...
    public void handleItemOnItemAction(Player player, Item from, Item to, int fromSlot, int toSlot) {
        if (from.getId() == 21046 || to.getId() == 21046) {
            val tablet = from.getId() == 21046 ? from : to;
            val amulet = from == tablet ? to : from;
            if (player.getAttributes().containsKey("xeric's honour")) {
            ...
            player.getDialogueManager().start(new DoubleItemChat(player, tablet, amulet, "As the talisman and tablet are held together, a surge of power connects the two. " +
                    "After a brief moment, the tablet crumbles, but the eyes of the talisman glow stronger than ever."));
            return;
        }
        val amulet = (from.getId() == 13392 || from.getId() == 13393) ? from : to;
        val fangs = amulet == from ? to : from;
        if (amulet.getCharges() >= 1000) {
            player.sendMessage("Your Xeric's amulet is already fully charged.");
            return;
        }
        val amount = Math.min(1000 - amulet.getCharges(), fangs.getAmount());
        player.getInventory().deleteItem(new Item(fangs.getId(), amount));
        amulet.setCharges(amulet.getCharges() + amount);
        amulet.setId(13393);
        player.getInventory().refreshAll();
        player.getDialogueManager().start(new ItemChat(player, amulet,
                "Your talisman now has " + (amulet.getCharges() <= 10 ? (Utils.numNames[amulet.getCharges()].substring(1)) : amulet.getCharges())
                        + " charge" + (amulet.getCharges() == 1 ? "" : "s") + "."));
    }
    ...
    public ItemPair[] getMatchingPairs() {
        return new ItemPair[] {
                ItemPair.of(13391, 13392), ItemPair.of(13391, 13393), ItemPair.of(21046, 13392), ItemPair.of(21046, 13393)
        };
    }
    ...
    private static final class TalismanTeleport implements Teleport {
        ...
        public void onArrival(final Player player) {
            ...
            if (destination.getX() == 1643 && destination.getY() == 3671) { //Xeric's heart
