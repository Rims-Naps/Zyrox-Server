    ...
    private void bloom(final Player player) {
        ...
        for (int x = -1; x <= 1; x++) {
            for (int y = -1; y <= 1; y++) {
                ...
                val tile = player.getLocation().transform(x, y, 0);
                val object = World.getObjectWithId(tile, 3508);
                if (object == null) {
                    continue;
                }
                if (Utils.random(1) == 0) {
                    val spawnedObj = new WorldObject(3509, object.getType(), object.getRotation(), tile);
