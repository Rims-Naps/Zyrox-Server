...
public class SkillsNecklace extends ItemPlugin {
    ...
    public void handle() {
        ...
        bind("Farming Guild",
                (player, item, container, slotId) -> new NecklaceTeleport(item, container, slotId,
                        player.getSkills().getLevelForXp(Skills.FARMING) >= 45 ? new Location(1248, 3725, 0) : new Location(1248, 3719, 0)).teleport(player));
        bind("Rub", (player, item, container, slotId) -> player.getDialogueManager().start(new OptionsMenuD(player, "Where would you like to teleport to?",
            ...
            public void handleClick(int clickedSlot) {
                ...
                switch(clickedSlot) {
                    ...
                    case 4:
                        ...
                        break;
                    case 5:
                        new NecklaceTeleport(item, container, slotId,
                                player.getSkills().getLevelForXp(Skills.FARMING) >= 45 ? new Location(1248, 3725, 0) : new Location(1248, 3719, 0)).teleport(player);
                        break;
                }
            }
    ...
    private static final class NecklaceTeleport implements Teleport {
        ...
        public void onArrival(final Player player) {
            val necklace = SkillsNecklaceItem.get(item);
            val charges = necklace.getCharges();
            val message = charges == 1 ?
                          "<col=7F00FF>You use your skills necklace's last charge.</col>" :
                          charges == 2 ?
                          "<col=7F00FF>Your skills necklace has one charge left.</col>" :
                          "<col=7F00FF>Your skills necklace has " + Utils.format(charges - 1) + " charges left.</col>";
