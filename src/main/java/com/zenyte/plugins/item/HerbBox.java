...
public class HerbBox extends ItemPlugin {
    ...
    private enum BoxedHerb {
        ...
        private static final Optional<BoxedHerb> getReward() {
    ...
    public void handle() {
        bind("Take-one", (player, item, container, slotId) -> {
            ...
            val herb = new Item(BoxedHerb.getReward().orElseThrow(RuntimeException::new).id);
            if (count >= 9) {
        ...
        bind("Bank-all", (player, item, slotId) -> {
            player.getActionManager().setAction(new Action() {
                ...
                public int processWithDelay() {
                    val startingNumber = player.getNumericAttribute("herb box herbs count").intValue();
                    player.getAttributes().remove("herb box herbs count");
                    val result = player.getInventory().deleteItem(new Item(11738, 1)).getSucceededAmount();
                    if (result == 0) {
