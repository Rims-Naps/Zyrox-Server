...
public class CombatBracelet extends ItemPlugin {
            ...
            player.getDialogueManager().start(new Dialogue(player) {
                ...
                public void buildDialogue() {
                    options("Where would you like to teleport to?",
                            new DialogueOption("Warriors' Guild", () -> new BraceletTeleport(item, container, slotId, new Location(2882, 3547, 0)).teleport(player)),
                            new DialogueOption("Champions' Guild", () -> new BraceletTeleport(item, container, slotId, new Location(3192, 3367, 0)).teleport(player)),
                            new DialogueOption("Monastery", () -> new BraceletTeleport(item, container, slotId, new Location(3052, 3487, 0)).teleport(player)),
                            new DialogueOption("Ranging Guild", () -> new BraceletTeleport(item, container, slotId, new Location(2655, 3443, 0)).teleport(player)),
    ...
    private static final class BraceletTeleport implements Teleport {
        ...
        public void onArrival(final Player player) {
            val bracelet = CombatBraceletItem.get(item);
            val charges = bracelet.getCharges();
            val message = charges == 1 ?
                          "<col=7F00FF>You use your combat bracelet's last charge.</col>" :
                          charges == 2 ?
                          "<col=7F00FF>Your combat bracelet has one charge left.</col>" :
                          "<col=7F00FF>Your combat bracelet has " + Utils.format(charges - 1) + " charges left.</col>";
            player.sendMessage(message);
            val result = bracelet.getResult();
