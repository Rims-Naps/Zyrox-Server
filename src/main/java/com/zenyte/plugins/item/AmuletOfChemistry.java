        ...
        bind("Options", (player, item, container, slotId) -> {
           player.getDialogueManager().start(new Dialogue(player) {
               ...
               public void buildDialogue() {
                   val isStopping = player.getNumericAttribute("AOC: cancel combining when out of charges").intValue() == 1;
                   options(TITLE,
                           new DialogueOption("Check charges", () -> {
                               val uses = player.getNumericAttribute("amulet of chemistry uses").intValue();
                               player.sendMessage("Your amulet of chemistry has " + ((5 - uses)) + " charge" + (uses == 4 ? "" : "s") + " left.");
                           }),
                           new DialogueOption(!isStopping ? "Continue combining potions when amulet breaks"
                                   : "Stop combining potions when amulet breaks", () -> {
