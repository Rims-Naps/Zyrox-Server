...
public class MaxCape extends ItemPlugin {
    ...
    public void handle() {
        bind("Features", (player, item, container, slotId) -> player.getDialogueManager().start(new Dialogue(player) {
            ...
            public void buildDialogue() {
                ...
                list.add(new DialogueOption("Stamina Boost", () -> {
                    val lastConsumptionDate = player.getNumericAttribute("Stamina Boost Use");
                    val milliseconds = lastConsumptionDate.longValue();
                ...
                options(5, TITLE, new DialogueOption("Ring of life effect", key(50)),
                        new DialogueOption("Ring of life spawn", key(100)),
                        new DialogueOption("Back", () -> setKey(1)));
                val enabled = player.getBooleanAttribute("Skillcape ring of life teleport");
                plain(50, "Your cape will" + (enabled ? " " : " not ") + "currently teleport you to safety should " +
                        "your health reach dangerous levels.");
                options("Would you like to " + (enabled ? "disable" : "enable") + " this feature?",
                        new DialogueOption("Yes", () -> {
                            player.toggleBooleanAttribute("Skillcape ring of life teleport");
                            setKey(60);
                        }),
                        new DialogueOption("No"));
                plain(60, "Your cape will follow your instructions to" + (enabled ? " not " : " ") + "save you when " +
                                "your health is low.");
                options(100, "Choose a respawn destination.",
                        new DialogueOption("Ardougne", () -> {
                            player.setRespawnPoint(RespawnPoint.ARDOUGNE);
                            player.sendMessage(Colour.RED.wrap("Your respawn location has now been changed to " +
                                    "Ardougne."));
                        }),
                ...
                val isGatheringJunk = player.getAttributes().containsKey("avasDeviceRetrieve");
                if (isGatheringJunk) {
                    ...
                    plain(205, "You somehow communicate your message to the undead chicken. Henceforth it will " +
                            "gather up random metal items while you've got it equipped.");
                }
                options(300, "What would you like to search for?",
                        new DialogueOption("Pestle and Mortar", () -> {
                        ...
                        new DialogueOption("Mith Grapple & Crossbow", () -> {
                            val searches = player.getVariables().getGrappleAndCrossbowSearches();
                            ...
                            player.sendMessage("You search the cape and find a " + Colour.RS_RED.wrap("Bronze " +
                                            "crossbow") + " and " + Colour.RS_RED.wrap("Mithril grapple."));
                            val remaining = MAX_GRAPPLE_SEARCHES - (searches + 1);
                            if (remaining >= 1) {
                                val remainingString = remaining == 2 ? "twice more today" : "one more time today";
        ...
        bind("Other Teleports", (player, item, slotId) -> player.getDialogueManager().start(new Dialogue(player) {
            ...
            @Override
            public void buildDialogue() {
                options(TITLE, "Chinchompa Teleports", "Farming Guild")
                        .onOptionOne(() -> setKey(5))
                        .onOptionTwo(() -> ItemTeleport.MAX_CAPE_FARMING_GUILD.teleport(player));
                options(5, "Which hunting location would you like to teleport to?", "Carnivorous chinchompas (Feldip Hills)", "Black chinchompas (Wilderness)", "Cancel")
                        .onOptionOne(() -> teleport(player, ItemTeleport.MAX_CAPE_CARNIVEROUS_CHINCHOMPAS))
                        .onOptionTwo(() -> teleport(player, ItemTeleport.MAX_CAPE_BLACK_CHINCHOMPAS));
            }
        }));
            ...
            player.getDialogueManager().start(new OptionsMenuD(player, "Select a destination", "Warrior's Guild", "Fishing Guild", "Crafting Guild", "Farming Guild", "Otto's Grotto", "Chinchompas", "POH Portals") {
                ...
                public void handleClick(int slotId) {
                    if (slotId <= 4) {
                        ...
                        player.getDialogueManager().start(new OptionDialogue(player, "Which hunting location would " +
                                "you like to teleport to?", new String[] { "Carnivorous chinchompas (Feldip Hills)",
                                "Black chinchompas (Wilderness)", "Cancel" }, new Runnable[] {() -> teleport(player,
                                MAX_CAPE_CARNIVEROUS_CHINCHOMPAS), () -> teleport(player,
                                ItemTeleport.MAX_CAPE_BLACK_CHINCHOMPAS)}));
                    } else if (slotId == 6) {
    ...
    static final void sendSpellbookDialogue(final Player player) {
        if (player.getVariables().getSpellbookSwaps() >= 5) {
            player.getDialogueManager().start(new PlainChat(player, "You may only switch spellbooks five times " +
                    "per day. Try again tomorrow."));
            return;
        }
        val list = new ObjectArrayList<Dialogue.DialogueOption>();
        val currentSpellbook = player.getCombatDefinitions().getSpellbook();
        for (val spellbook : Spellbook.VALUES) {
            if (spellbook != currentSpellbook) {
                list.add(new Dialogue.DialogueOption(Utils.formatString(spellbook.toString()), () -> {
                    ...
                    val count = player.getVariables().getSpellbookSwaps() + 1;
                    player.getVariables().setSpellbookSwaps(count);
                    player.sendMessage(Colour.RED.wrap("You have changed your spellbook " + count + "/5 times " +
                            "today."));
                }));
            }
        }
    ...
    static final void sendPOHPortalTeleports(final Player player) {
        player.getDialogueManager().start(new OptionsMenuD(player, "House portal teleports",
                POHPortal.map.keySet().toArray(new String[0])) {
            @Override
            public void handleClick(int slotId) {
                val type = POHPortal.values[slotId];
