...
public class Chronicle extends ItemPlugin implements PairedItemOnItemPlugin {
    ...
    public void handle() {
        ...
        bind("Check Charges", (player, item, container, slotId) -> {
            val charges = player.getNumericAttribute("Chronicle charges").intValue();
    ...
    public void handleItemOnItemAction(final Player player, final Item from, final Item to, final int fromSlot, final int toSlot) {
        val card = from.getId() == 13658 ? from : to;
    ...
    private static final class ChronicleTeleport implements Teleport {
        ...
        public void onArrival(final Player player) {
            ...
            val message = charges == 1 ?
                          "<col=7F00FF>You use your chronicle's last charge.</col>" :
                          charges == 2 ?
                          "<col=7F00FF>Your chronicle has one charge left.</col>" :
                          "<col=7F00FF>Your chronicle has " + Utils.format(charges - 1) + " charges left.</col>";
