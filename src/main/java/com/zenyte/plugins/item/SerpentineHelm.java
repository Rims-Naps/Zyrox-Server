...
public class SerpentineHelm extends ItemPlugin {
	...
	public void handle() {
		bind("Restore", (player, item, slotId) -> {
			val charged = !item.getName().contains("(uncharged)");
			player.getDialogueManager().start(new Dialogue(player) {
				...
				public void buildDialogue() {
					...
					options("Do you wish to restore the " + item.getName().toLowerCase() + "?", "Yes", "No")
						.onOptionOne(() -> { 
							setKey(5);
		...
		bind("Uncharge", (player, item, slotId) -> {
			val replacement = item.getId() == 12931 ? UNCHARGED_HELMET : item.getId() == 13197 ? UNCHARGED_TANZANITE_HELMET : UNCHARGED_MAGMA_HELMET;
