...
public class CoinPouch extends ItemPlugin {
    ...
    private static final int COIN_PICKPOCKETING_MULTIPLIER = 5;
    
    @Override
    public void handle() {
        bind("Open", (player, item, slotId) -> {
            val pouch = com.zenyte.game.content.skills.thieving.CoinPouch.ITEMS.get(item.getId());
            open(player, pouch, 1);
        });
    ...
    private void open(final Player player, final com.zenyte.game.content.skills.thieving.CoinPouch pouch, final int amount) {
        val totalCoins = getCoinAmount(pouch, amount);
