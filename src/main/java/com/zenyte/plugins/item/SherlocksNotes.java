...
public class SherlocksNotes extends ItemPlugin implements PairedItemOnItemPlugin {
    ...
    @Override
    public void handle() {
        bind("Info", (player, item, container, slotId) ->
                player.getDialogueManager().start(new ItemChat(player, item, "Sherlock's notes can be used on any clue scroll to advance one stage further to completing it.<br>" +
                        "Just use the notes on a clue scroll in your inventory to activate it.")));
    }

    @Override
    public void handleItemOnItemAction(Player player, Item from, Item to, int fromSlot, int toSlot) {
        val paper = from.getId() == SHERLOCKS_NOTES ? from : to;
        val clue = paper == from ? to : from;
        val list = TreasureTrail.getCluesList(clue);
        if (list == null) {
