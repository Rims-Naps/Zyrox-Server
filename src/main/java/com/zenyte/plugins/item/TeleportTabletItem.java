...
public class TeleportTabletItem extends ItemPlugin {
	...
	public void handle() {
        ...
        bind("Revert", (player, item, slotId) -> {
           player.getDialogueManager().start(new Dialogue(player) {
               ...
               public void buildDialogue() {
                   options("Revert these tablets?",
                           new DialogueOption("Revert ALL - you will not get your scrolls back.", () -> {
                               player.getDialogueManager().finish();
                               val succeeded = player.getInventory().deleteItem(item).getSucceededAmount();
                               if (succeeded > 0) {
                                   val houseTabs = new Item(8013, succeeded);
                                   player.getInventory().addOrDrop(houseTabs);
                                   player.getDialogueManager().start(new ItemChat(player, houseTabs, "You revert " +
                                           "the tablets to their original form."));
                               }
                               ...
                               player.sendInputInt("How many do you wish to revert? " + item.getAmount(), value -> {
                                   val succeeded = player.getInventory().deleteItem(new Item(item.getId(), value)).getSucceededAmount();
                                   if (succeeded > 0) {
                                       val houseTabs = new Item(8013, succeeded);
                                       player.getInventory().addOrDrop(houseTabs);
		...
		bind("Toggle", (player, item, slotId) -> {
		    val tablet = TeleportTablet.getTeleportTab(item);
		    String current;
		    String target;
		    Setting setting;
		    int currentValue;
            ...
            val message = "Your default option is now the " + target + " teleport.";
            player.getDialogueManager().start(new Dialogue(player) {
                @Override
                public void buildDialogue() {
                    options("Spellbook/portal default: " + current,
                            new DialogueOption("Change to the " + target + ".", () -> {
