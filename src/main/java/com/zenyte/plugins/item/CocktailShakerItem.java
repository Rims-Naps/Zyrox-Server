	...
	public void handle() {
		...
		bind("Pour", (player, item, slotId) -> {
			...
			if(shaker == CocktailShaker.DRUNK_DRAGON || shaker == CocktailShaker.CHOCOLATE_SATURDAY) {
				String list = "";
				for(final int garnish : shaker.getGarnish()) {
					list += (ItemDefinitions.get(garnish).getName().toLowerCase().contains("pot") ?
							"cream, " : ItemDefinitions.get(garnish).getName().toLowerCase()+", ");
				}
				final String message = (shaker == CocktailShaker.CHOCOLATE_SATURDAY) ? "You need to heat then enqueue cream and chocolate dust to finish." :
						"You need to enqueue "+list+"and then heat to finish this cocktail.";
				player.getDialogueManager().start(new PlainChat(player, message));
			} else {
				player.getSkills().addXp(Skills.COOKING, shaker.getXp()-30);
			}
		});
		...
		bind("Add-ingreds", (player, item, slotId) -> {
			...
			final String message = (shaker == CocktailShaker.CHOCOLATE_SATURDAY) ? "You enqueue the finishing touches to this cocktail." :
				"You enqueue some pineapple chunks and cream. You need to warm this cocktail to finish.";
