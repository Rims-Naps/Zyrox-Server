...
public class NecklaceOfPassageItem extends ItemPlugin {
            ...
            player.getDialogueManager().start(new Dialogue(player) {
                ...
                public void buildDialogue() {
                    options("Where would you like to teleport to?",
                            new DialogueOption("Wizards' Tower", () -> new NecklaceTeleport(item, container, slotId, new Location(3113, 3177, 0)).teleport(player)),
                            new DialogueOption("The Outpost", () -> new NecklaceTeleport(item, container, slotId, new Location(2428, 3349, 0)).teleport(player)),
                            new DialogueOption("Eagles' Eyrie", () -> new NecklaceTeleport(item, container, slotId, new Location(3406, 3157, 0)).teleport(player)),
    ...
    private static final class NecklaceTeleport implements Teleport {
        ...
        public void onArrival(final Player player) {
            val necklace = NecklaceOfPassage.get(item);
            val charges = necklace.getCharges();
            val message = charges == 1 ?
                          "<col=7F00FF>Your necklace of passage crumbles to dust.</col>" :
                          charges == 2 ?
                          "<col=7F00FF>Your necklace of passage has one use left.</col>" :
                          "<col=7F00FF>Your necklace of passage has " + Utils.format(charges - 1) + " uses left.</col>";
