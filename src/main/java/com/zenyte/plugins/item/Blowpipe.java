...
public class Blowpipe extends ItemPlugin implements ChargeExtension {
    ...
    public static int getBlowpipeAmmunition(final Item blowpipe) {
        ...
        val darts = blowpipe.getNumericAttribute("blowpipeDartType").intValue();
        if (darts == 0) {
            return -1;
        }
        return darts;
    ...
    public void handle() {
        bind("Unload", (player, item, slotId) -> {
            ...
            val result = player.getInventory().addItem(new Item(type, darts));
            ...
            player.getDialogueManager().start(new Dialogue(player) {
                ...
                public void buildDialogue() {
                    this.destroyItem(item, "Are you sure you want to uncharge it?", "If you uncharge the blowpipe, " + "all scales and darts will fall out.").onYes(() -> {
                        ...
                        val removedDarts = container.add(new Item(type, darts)).getSucceededAmount();
                        val removedScales = container.add(new Item(12934, scales)).getSucceededAmount();
        ...
        bind("Dismantle", (player, item, slotId) -> {
            ...
            player.getDialogueManager().start(new OptionDialogue(player, "Dismantle the blowpipe for 20,000 Zulrah's scales?", new String[]{
                    "Yes, dismantle the blowpipe.", "No, don't dismantle it."
            }, new Runnable[]{
                    () -> {
                        if (player.getInventory().getItem(slotId) != item) return;
    ...
    public void removeCharges(final Player player, final Item item, final ContainerWrapper wrapper, int slotId, final int amount) {
        ...
        val bpstep = player.getNumericTemporaryAttribute("blowpipestep").intValue();
        if (bpstep == 3) {
    ...
    public void checkCharges(final Player player, final Item item) {
        ...
        val percentage = FORMATTER.format(scales / (float) 16383 * 100).replace(".0", "");
