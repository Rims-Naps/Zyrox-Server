...
public class ToxicStaffOfTheDead extends ItemPlugin implements PairedItemOnItemPlugin {
    ...
    public void handle() {
        bind("Uncharge", (player, item, container, slotId) -> {
            ...
            val scales = (int) Math.floor(item.getCharges() * SerpentineHelmetChargingAction.SCALES_TO_CHARGES_RATIO);
            if (scales > 0) {
    ...
    public void handleItemOnItemAction(Player player, Item from, Item to, int fromSlot, int toSlot) {
        val scales = from.getId() == 12934 ? from : to;
        val staff = from == scales ? to : from;
        ...
        val scalesRequired = (int) Math.ceil(amount * SerpentineHelmetChargingAction.SCALES_TO_CHARGES_RATIO);
        if (scalesRequired <= 0) {
    ...
    public ItemPair[] getMatchingPairs() {
        return new ItemPair[] {
                ItemPair.of(12904, 12934), ItemPair.of(12902, 12934)
        };
    }
}
