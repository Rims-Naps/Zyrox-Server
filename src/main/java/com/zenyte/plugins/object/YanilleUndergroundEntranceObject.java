            ...
            if (option.equals("Smoke")){
                player.getDialogueManager().start(new Dialogue(player) {
                    ...
                    public void buildDialogue() {
                        val on = player.getBooleanAttribute("SmokeOverlay");
                        options("Do you want to turn the smoke overlay back " + (on ? "off?" : "on?"), on ? "Yes, hide the smoke overlay." : "Yes, turn the smoke overlay on", "No, leave it " + (on ? "on." : "off"))
                                .onOptionOne(() -> player.toggleBooleanAttribute("SmokeOverlay"));
                    }
                });
                return;
            }
