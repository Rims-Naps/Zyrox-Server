...
public class MotherlodePaydirtHopper implements ObjectAction {
    ...
    private static final int PAYDIRT_NPC = NpcId.PAYDIRT;
    ...
    public void handle(final Player player, final WorldObject object, final String name, final int optionId, final String option) {
        player.setRouteEvent(new TileEvent(player, new TileStrategy(
                player.getLocation().getDistance(easternHopper)
                        < player.getLocation().getDistance(northernHopper) ?
                        easternHopper : northernHopper),
    ...
    public void handleObjectAction(Player player, WorldObject object, String name, int optionId, String option) {
        ...
        if (conveyorAmount.intValue() > 0) {
            player.getDialogueManager().start(new ItemChat(player, PAYDIRT_ITEM, "You've already got some pay-dirt in the machine." +
                    "<br>You can put more in once the last batch comes out."));
            return;
        }

        /** Paydirt in sack management */
        val MAX_AMOUNT = (player.getBooleanAttribute("motherlode_sack_upgrade") ? 162 : 81);
        var paydirt = player.getInventory().getAmountOf(PAYDIRT_ITEM.getId());
        val sackAmount = player.getPaydirt().size();
