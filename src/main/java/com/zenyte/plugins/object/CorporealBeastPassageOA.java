...
public final class CorporealBeastPassageOA implements ObjectAction {
	...
	public void handleObjectAction(final Player player, final WorldObject object, final String name, final int optionId,
		...
		if (option.equals("Go-through")) {
			...
			player.lock(1);
			player.setLocation(player.getX() < object.getX() ? new Location(object.getX() + 3, player.getY(), player.getPlane())
					: new Location(object.getX() - 1, player.getY(), player.getPlane()));
			val corp = World.findNPC(319, player.getLocation(), 25);
			if (corp.isPresent()) {
			    val c = corp.get();
