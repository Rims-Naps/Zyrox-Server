...
public class MagicHutDoor implements ObjectAction {
    ...
    public void handleObjectAction(final Player player, final WorldObject object, final String name, final int optionId, final String option) {
        val x = player.getX();
        val y = player.getY();
        val inHut = x >= 3187 && x <= 3194 && y >= 3958 && y <= 3962;
        val openOption = option.equalsIgnoreCase("Open");
        ...
        if (option.equals("Pick-lock") || openOption) {
            ...
            val success = Utils.interpolateSuccess(127, 217, 23, 99, player.getSkills().getLevelForXp(Skills.THIEVING));
            if (!success) {
    ...
    private void walkThrough(@NotNull final Player player, @NotNull final WorldObject object) {
        player.resetWalkSteps();
        val door = Door.handleGraphicalDoor(object, null);
        if (object.getY() > 3958) {
