...
public class ClosedCupboardObject implements ObjectAction {
    ...
    public void handleObjectAction(final Player player, final WorldObject object, final String name, final int optionId, final String option) {
        if (option.equalsIgnoreCase("Open")) {
            ...
            if (object.getPosition().getPositionHash() == new Location(3096, 3269, 1).getPositionHash()) { // morgan house in Draynor
                if (!player.getInventory().hasFreeSlots()) {
                    player.getDialogueManager().start(new PlainChat(player, "The cupboard contains garlic, but you don't<br><br>have room to hold it at the moment."));
                    return;
                }
    ...
    private WorldObject getObject(@NotNull final WorldObject current) {
        var obj = activeDrawers.get(current);
        if (obj == null) {
