...
public final class WoodcuttingShrine implements ObjectAction {
	...
	private static final class ShrineAction extends Action {
		...
		public int processWithDelay() {
			...
			giveRewards();
			player.addAttribute("shrine_count", player.getNumericAttribute("shrine_count").intValue() + 1);
		...
		private final void giveRewards() {
			if (Utils.random(299) == 0) {
				...
				player.getCollectionLog().add(piece);
				...
				if(petid != 0){
					...
					val name = petitem.getName().toLowerCase();
					player.getInventory().addOrDrop(petitem);
					WorldBroadcasts.broadcast(player, BroadcastType.PET, pet);
					player.sendMessage("You offer the egg to the shrine and receive " + name + "!");
				} else {
					player.sendMessage(Colour.RED.wrap("You have a funny feeling like you would have been followed..."));
		...
		private static void sendOpenedCount(final Player player) {
			val opened = player.getNumericAttribute("shrine_count").intValue();
			player.sendMessage("You have made " + (opened == 1 ? Colour.RED.wrap(opened) + " offering." : Colour.RED.wrap(opened) + " offerings."));
		}
