...
public class ObservatoryGrapple implements ObjectAction {
    ...
    public void handleObjectAction(final Player player, final WorldObject object, final String name, final int optionId, final String option) {
        ...
        if (skills.getLevel(Skills.AGILITY) < 23 || skills.getLevel(Skills.RANGED) < 24 || skills.getLevel(Skills.STRENGTH) < 28) {
            player.getDialogueManager().start(new PlainChat(player, "You need an Agility level of at least 23, a Ranged level of at least 24 & a Strength level of at least 28 to use " +
                    "this shortcut."));
            return;
        }
    ...
    public void handle(final Player player, final WorldObject object, final String name, final int optionId, final String option) {
        player.setRouteEvent(new TileEvent(player, new TileStrategy(new Location(2448, 3155, 0)), getRunnable(player, object, name,
