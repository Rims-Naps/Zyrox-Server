...
public class CastleWarsFlagObject implements ObjectAction {
    ...
    public void handleObjectAction(final Player player, final WorldObject object, final String name, final int optionId, final String option) {
        val saradomin = object.getId() == saradominFlag.getId() || object.getId() == saradominFlagStand.getId() || object.getId() == SARADOMIN_FLAG_GROUND;
        ...
        if(object.getId() == saradominFlag.getId() || object.getId() == zamorakFlag.getId()) {
            if (saradomin && team.equals(CastleWarsTeam.SARADOMIN) || !saradomin && team.equals(CastleWarsTeam.ZAMORAK)) {
                if(player.getEquipment().getId(EquipmentSlot.WEAPON) == SARADOMIN_FLAG.getId() && object.getId() == zamorakFlag.getId()
                        || player.getEquipment().getId(EquipmentSlot.WEAPON) == ZAMORAK_FLAG.getId() && object.getId() == saradominFlag.getId()) {
                    processSubmitCapture(team, player, object);
                } else {
                    val prefix = team.equals(CastleWarsTeam.SARADOMIN) ? "Saradomin" : "Zamorak";
        ...
        if(object.getId() == saradominFlagStand.getId() || object.getId() == zamorakFlagStand.getId()) {
            ...
            if(weapon == SARADOMIN_FLAG.getId() && object.getId() == saradominFlagStand.getId()
                    || weapon == ZAMORAK_FLAG.getId() && object.getId() == zamorakFlagStand.getId()) {
                return;
            }
    ...
    public void processFlagReturn(final CastleWarsTeam team, final WorldObject object) {
        val saradomin = object.getId() == saradominFlag.getId() || object.getId() == saradominFlagStand.getId() || object.getId() == SARADOMIN_FLAG_GROUND;
        val flagVarbit = saradomin ? CWarsOverlayVarbit.SARADOMIN_FLAG : CWarsOverlayVarbit.ZAMORAK_FLAG;;
    ...
    public void processFlagPickup(final CastleWarsTeam team, final Player player, final WorldObject object) {
        val saradomin = object.getId() == saradominFlag.getId() || object.getId() == saradominFlagStand.getId() || object.getId() == SARADOMIN_FLAG_GROUND;
        val hasWeapon = player.getEquipment().getId(EquipmentSlot.WEAPON) != -1;
        val hasShield = player.getEquipment().getId(EquipmentSlot.WEAPON) != -1;
        val flagVarbit = saradomin ? CWarsOverlayVarbit.SARADOMIN_FLAG : CWarsOverlayVarbit.ZAMORAK_FLAG;;
    ...
    public void processSubmitCapture(final CastleWarsTeam team, final Player player, final WorldObject object) {
        ...
        val scoreVarbit = weapon == SARADOMIN_FLAG.getId() ? CWarsOverlayVarbit.ZAMORAK_SCORE : CWarsOverlayVarbit.SARADOMIN_SCORE;
        val flagVarbit = weapon == SARADOMIN_FLAG.getId() ? CWarsOverlayVarbit.SARADOMIN_FLAG : CWarsOverlayVarbit.ZAMORAK_FLAG;
    ...
    public Object[] getObjects() {
        return new Object[] {
                saradominFlag.getId(), saradominFlagStand.getId(), SARADOMIN_FLAG_GROUND,
                zamorakFlag.getId(), zamorakFlagStand.getId(), ZAMORAK_FLAG_GROUND
        };
    }
}
