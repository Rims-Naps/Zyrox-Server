...
public class IronWinchObject implements ObjectAction {
	...
	private static final Animation turnAnimation = new Animation(4506);
	...
	public void handleObjectAction(Player player, WorldObject object, String name, int optionId, String option) {
		...
		if (optionId == 1) {
            ...
            val count = CharacterLoop.find(position.getWesternFireLocation(), 25, Player.class, __ -> true).size();
			player.getDialogueManager().start(new NPCChat(player, 5870,
					count == 0 ? "No adventurers are inside the cave." : (count + (count == 1 ? " adventurer is" : " adventurers are") + " inside the cave.")));
		}
	}
