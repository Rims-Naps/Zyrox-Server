...
public final class WildernessDitchObject implements ObjectAction {
	...
	public static final void jump(@NotNull final Player player, @NotNull final WorldObject object) {
		...
		val direction = ((object.getRotation() & 0x1) != 0) ? (player.getX() > object.getX()) ? Direction.WEST : Direction.EAST : (player.getY() > object.getY()) ? Direction.SOUTH : Direction.NORTH;
		player.lock(2);
		...
		player.setForceMovement(new ForceMovement(player.getLocation(), 33, direction == Direction.NORTH ? new Location(player.getX(), player.getY() + 3) : new Location(player.getX(), player.getY() - 3), 60, direction.getDirection()));
		player.setLocation(direction == Direction.NORTH ? new Location(player.getX(), player.getY() + 3): new Location(player.getX(), player.getY() - 3));
		player.setAnimation(ANIM);
		player.sendSound(sound);
	}
