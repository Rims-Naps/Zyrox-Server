...
public class StrangeMachine implements ObjectAction {
    public static final IntList requiredItems = IntArrayList.wrap(new int[] {
            ItemId.HAMMER, ItemId.WYVERN_VISAGE, ItemId.ELEMENTAL_SHIELD
    });
    ...
    private static boolean check(final Player player) {
        ...
        if (player.getSkills().getLevel(Skills.MAGIC) < 66 || player.getSkills().getLevel(Skills.SMITHING) < 66) {
    ...
    private static void reward(final Player player, final WorldObject object) {
        player.getInventory().deleteItemsIfContains(new Item[] {
                new Item(ItemId.ELEMENTAL_SHIELD),
                new Item(ItemId.WYVERN_VISAGE)
        }, () -> {
            World.sendObjectAnimation(object, defaultMachineAnim);
            val skills = player.getSkills();
            val inventory = player.getInventory();
            val reward = new Item(ItemId.ANCIENT_WYVERN_SHIELD_21634);
            skills.drainSkill(Skills.MAGIC, player.getSkills().getLevel(Skills.MAGIC));
            skills.addXp(Skills.MAGIC, 2000);
            skills.addXp(Skills.SMITHING, 2000);
            inventory.addItem(reward);
            player.getDialogueManager().start(new ItemChat(player, reward, "At great cost to your personal magical " +
                    "energies, you have crafted the wyvern visage and elemental shield into an ancient wyvern shield."));
        });
        player.unlock();
    }
