...
public class OreOnRockGolemPetNPCAction implements ItemOnNPCAction {
    ...
    public void handleItemOnNPCAction(Player player, Item item, int slot, NPC npc) {
        val combination = RockGolemCombination.get(item.getId());
        if (combination == null) {
            return;
        }
        ...
        val currentPet = player.getFollower().getPet();
    ...
    public Object[] getObjects() {
        return SKILLING_PETS_BY_SKILL.get(Skills.MINING).stream().map(SkillingPet::getPetId).toArray();
    }

    @Getter
    private enum RockGolemCombination {

        DEFAULT(ROCK_GOLEM_DEFAULT, 1480),
        TIN(ROCK_GOLEM_TIN, 438),
        COPPER(ROCK_GOLEM_COPPER, 436),
        IRON(ROCK_GOLEM_IRON, 440),
        BLURITE(ROCK_GOLEM_BLURITE, 668),
        SILVER(ROCK_GOLEM_SILVER, 442),
        COAL(ROCK_GOLEM_COAL, 453),
        GOLD(ROCK_GOLEM_GOLD, 444),
        MITHRIL(ROCK_GOLEM_MITHRIL, 447),
        GRANITE(ROCK_GOLEM_GRANITE, 6979, 6981, 6983),
        ADAMANTITE(ROCK_GOLEM_ADAMANTITE, 449),
        RUNITE(ROCK_GOLEM_RUNITE, 451),
        AMETHYST(ROCK_GOLEM_AMETHYST, 21347),
        LOVAKITE(ROCK_GOLEM_LOVAKITE, 13356),
        ELEMENTAL(ROCK_GOLEM_ELEMENTAL, 2892),
        DAEYALT(ROCK_GOLEMD_DAEYALT, 9632);
        ...
        static {
            for (val combination : ALL) {
                for (val ore : combination.getOres()) {
