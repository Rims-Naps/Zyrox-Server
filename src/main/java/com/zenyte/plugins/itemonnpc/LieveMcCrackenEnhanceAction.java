...
public class LieveMcCrackenEnhanceAction implements ItemOnNPCAction {
    public static final ImmutableMap<Integer, Integer> TRIDENT_MAPPING = ImmutableMap.of(ItemId.UNCHARGED_TOXIC_TRIDENT,
            ItemId.UNCHARGED_TOXIC_TRIDENT_E, ItemId.UNCHARGED_TRIDENT, ItemId.UNCHARGED_TRIDENT_E);
        ...
        if (item.getId() == ItemId.KRAKEN_TENTACLE || item.getId() == KRAKEN_TENTACLE_NOTED) {
            player.getDialogueManager().start(new Dialogue(player, npc) {
                ...
                public void buildDialogue() {
                    val tentacleAmount = player.getInventory().getAmountOf(item.getId());
                    val newAmount = player.getNumericAttribute("stored_kraken_tentacles").intValue() + tentacleAmount;
                    ...
                    options("Lieve will not return your tentacles.",
                            new DialogueOption("Give them to her.", () -> {
            ...
            val tentacleAmount = player.getNumericAttribute("stored_kraken_tentacles").intValue();
            player.getDialogueManager().start(new Dialogue(player, npc) {
                @Override
                public void buildDialogue() {
