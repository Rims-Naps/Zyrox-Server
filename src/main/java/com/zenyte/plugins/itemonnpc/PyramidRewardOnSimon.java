...
public class PyramidRewardOnSimon implements ItemOnNPCAction {
    ...
    public void handleItemOnNPCAction(Player player, Item item, int slot, NPC npc) {
        val artefact = PlunderReward.getByItem(item.getId());
        if (artefact != null) {
    ...
    private void handlePlunderReward(final Player player, final NPC npc, final Item item, final PlunderReward artefact) {
        player.getDialogueManager().start(new Dialogue(player, npc) {
            ...
            public void buildDialogue() {
                ...
                options("Do you want to sell your " + item.getName(),
                        new DialogueOption("Yes, show me the money.", key(SELL_ARTEFACT_KEY)),
                        new DialogueOption("No, I think I'll keep it."));
    ...
    private void handleAgilityReward(final Player player, final Item item) {
        ...
        val amount = inventory.getAmountOf(ItemId.PYRAMID_TOP);
        if (amount == 0) {
            return;
        }
        val rewardAmount = amount * SimonTempleton.COINS_PER_PYRAMID;
        player.getDialogueManager().start(new ItemChat(player, item,
                "You hand over the artefact(s) and Simon hands you " + rewardAmount + " coins."));
