...
public class ProspectorPercyD extends Dialogue {
    ...
    public ProspectorPercyD(final Player player, final int npcId) {
        ...
        if(!player.getBooleanAttribute("motherlode_upstairs"))
            options.add("Upper level access <col=600000>(40 nuggets)</col>");

        if(!player.getBooleanAttribute("motherlode_sack_upgrade"))
            options.add("Double space in paydirt sack <col=600000>(80 nuggets)</col>");
    ...
    public void buildDialogue() {
        ...
        options("What would you like to say?", "Is there anything else I can unlock here?", "Would you like to trade?", "Nevermind").
                onOptionOne( () -> setKey(5) ).
                onOptionTwo( () -> player.openShop("Prospector Percy's Nugget Shop") );
        ...
        options(5, "What would you like to buy?", options.toArray(new String[options.size()])).
                onOptionOne( () -> {
                    if(!player.getBooleanAttribute("motherlode_upstairs")) {
                        if (player.getInventory().containsItem(12012, 40)
                                && !player.getBooleanAttribute("motherlode_upstairs")
                                && player.getSkills().getLevelForXp(Skills.MINING) >= 72) {
                            setKey(10);
                        } else {
                            finish();
                            if (player.getBooleanAttribute("motherlode_upstairs")) {
                            ...
                            if (player.getSkills().getLevelForXp(Skills.MINING) < 72) {
        ...
        options(10, "Purchase access for <col=006000>40 golden nuggets</col>?", "Yes", "No").
                onOptionOne( () -> {
        ...
        options(20, "Upgrade sack for <col=006000>80 golden nuggets</col>?", "Yes", "No").
                onOptionOne( () -> {
