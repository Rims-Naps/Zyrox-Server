...
public class BankerD extends Dialogue {
	...
	public void buildDialogue() {
		...
		val partyTeleport = frequency > 0 && partyRoom.getVariables().isAnnouncements();

		if (partyTeleport) {
			options(TITLE, "I'd like to access my bank account, please.", "I'd like to collect items.", "What is this place?", "Can you teleport me to the Party room?")
					.onOptionOne(() -> GameInterface.BANK.open(player))
					.onOptionTwo(() -> GameInterface.GRAND_EXCHANGE_COLLECTION_BOX.open(player))
					.onOptionThree(() -> setKey(5)).onOptionFour(() -> setKey(25));
			npc(25, "Certainly.").executeAction(() -> TeleportCollection.FALADOR_PARTY_ROOM.teleport(player));
		} else {
			options(TITLE, "I'd like to access my bank account, please.", "I'd like to collect items.", "What is this place?")
					.onOptionOne(() -> GameInterface.BANK.open(player))
					.onOptionTwo(() -> GameInterface.GRAND_EXCHANGE_COLLECTION_BOX.open(player))
					.onOptionThree(() -> setKey(5));
		}
