...
public class TzHaarMejJalD extends Dialogue {
    ...
    public void buildDialogue() {
        if(exchange) {
            ...
            options("Would you like to trade your firecape?", "Trade my firecape for 8,000 tokkul", "Gamble my firecape for a pet <col=600000>(1/200)</col>", "Nevermind").onOptionOne( () -> {
                if(!player.getInventory().containsItem(TzHaar.FIRE_CAPE_ITEM)) {
                    player.sendMessage(Colour.RS_RED.wrap("You have no firecape to exchange!"));
                } else {
                    ...
                    player.getInventory().addItem(6529, 8000);
                    player.sendMessage(Colour.RS_GREEN.wrap("You have sold your firecape to TzHaar-Mej-Jal for 8,000 tokkul!"));
                }
            }).onOptionTwo( () -> gambleFirecape());

        } else {
            npc("I am the guardian of these Fight caves, can I help you JalYt?");
            options("Select an option", "How many waves are the Fight caves?", "Can I sell you a firecape?", "Nevermind").
                    onOptionOne( () -> setKey(10)).
                    onOptionTwo( () -> setKey(20));

            player(10, "How many waves are the Fight caves?");
            npc(11, "The fight caves are 63 waves, like authentic OSRS. <member=0><col=2188ff>Sapphire</col> donators and above start on wave 31.");

            player(20, "Can I sell you a firecape?");
            if(player.getInventory().containsItem(TzHaar.FIRE_CAPE_ITEM)) {
                npc(21, "You can sell me your firecape for 8,000 tokkul, or use it on me / exchange to gamble it for a Tzrek-Jad pet.");
            } else {
    ...
    public void gambleFirecape() {
        ...
        if(roll == 1) {
            ...
            WorldBroadcasts.broadcast(player, BroadcastType.GAMBLE_CAPE, TzHaar.FIRE_CAPE_ITEM);
        } else {
