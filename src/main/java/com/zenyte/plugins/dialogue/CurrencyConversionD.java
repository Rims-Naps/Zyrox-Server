...
public class CurrencyConversionD extends Dialogue {
    ...
    private Item conversion, toDelete, coins, tokens;

    public CurrencyConversionD(Player player, Item item, final int slotId) {
        super(player);
        this.item = item;
        this.slotId = slotId;
        conversion = item.getId() == 995 ? new Item(13204, item.getAmount() / 1000) : new Item(995, item.getAmount() > 2_147_483 ? 2_147_483_000 : item.getAmount() * 1000);
        toDelete = conversion.getId() == 995 ? new Item(13204, conversion.getAmount() / 1000) : new Item(995, conversion.getAmount() > 2_147_483 ? 2_147_483_000 : conversion.getAmount() * 1000);
    ...
    public void buildDialogue() {
        options("Exchange your " + item.getName().toLowerCase() + (item.getId() == 13204 ? "s" : "") + " for " + conversion.getName().toLowerCase() + (conversion.getId() == 13204 ? "s" : "") + "?", "Yes", "No")
                .onOptionOne(() -> {
                    ...
                    val succeeded = player.getInventory().deleteItem(toDelete).getSucceededAmount();
                    conversion = item.getId() == 995 ? new Item(13204, succeeded / 1000) : new Item(995, succeeded > 2_147_483 ? 2_147_483_000 : succeeded * 1000);
