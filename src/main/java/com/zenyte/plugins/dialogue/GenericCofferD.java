...
public class GenericCofferD extends Dialogue {
    ...
    private final static Item CASH_SMALL = new Item(1002, 1);
    private final static Item CASH_MED = new Item(1003, 1);
    private final static Item CASH_BIG = new Item(1004, 1);
    ...
    public void buildDialogue() {
        options("Select an Option", buildOptions()).onOptionOne(() -> {
            ...
            finish();
            if (player.getNumericAttribute(attribute).intValue() != 0 && player.getInventory().getAmountOf(995) != 0)
                handleDeposit();
        });
    }
    ...
    private void handleWithdraw() {
        ...
        val goldTilFull = Integer.MAX_VALUE - player.getInventory().getAmountOf(995);
        val amount = player.getNumericAttribute(attribute).intValue() > goldTilFull ? TextUtils.formatCurrency(goldTilFull) : TextUtils.formatCurrency(player.getNumericAttribute(attribute).intValue());
        player.sendInputInt("How much do you want to withdraw? ("+amount+")", withdrawAmt -> {
            if (withdrawAmt > player.getNumericAttribute(attribute).intValue())
                withdrawAmt = player.getNumericAttribute(attribute).intValue();
            ...
            if (player.getNumericAttribute(attribute).intValue() >= withdrawAmt) {
                ...
                val cofferNew = player.getNumericAttribute(attribute).intValue();
                val goldItem = cofferNew < 10000 ? (cofferNew < 1000 ? CASH_SMALL : CASH_MED) : CASH_BIG;
    ...
    private void handleDeposit() {
        val goldTilFull = Integer.MAX_VALUE - player.getNumericAttribute(attribute).intValue();
        val amount = player.getInventory().getAmountOf(995) > goldTilFull ? TextUtils.formatCurrency(goldTilFull) : TextUtils.formatCurrency(player.getInventory().getAmountOf(995));

        player.sendInputInt("How much do you want to deposit? ("+amount+")", depositAmt -> {
    
            if (depositAmt > player.getInventory().getAmountOf(995))
                depositAmt = player.getInventory().getAmountOf(995);
