...
public class CookingD extends SkillDialogue {
    ...
    public void run(final int slotId, final int amount) {
        ...
        val cookable = cookableList == null ? CookingData.getDataByProduct(items[slotId]) : cookableList.get(slotId);
        if (cookable != null) {
        ...
        if (object == null) {
            ...
            if(CookingCombination.CHOPPED.contains(comboElement)) {
                ...
                if(!player.getInventory().containsItem(1923, 1) && !combo.toString().toLowerCase().equals("chilli_con_carne")) {
            ...
            if (comboElement != null) {
                if (player.getSkills().getLevel(Skills.COOKING) >= comboElement.getLevel()) {
                    if (comboElement.getProduct().getAmount() > 1 && player.getInventory().getFreeSlots() < comboElement.getProduct().getAmount() - 1)
                        player.sendMessage("You need at least " + (comboElement.getProduct().getAmount() - 1) + " free " + ((comboElement.getProduct().getAmount() - 1) > 1 ? "slots" : "slot") + " to make this!");
                    else
                        player.getActionManager().setAction(new CookingCombo(comboElement, amount));
                } else {
