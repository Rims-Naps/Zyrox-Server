...
public final class SpiritTreeMenuD extends OptionsMenuD {
    ...
    public SpiritTreeMenuD(Player player) {
		super(player, "Spirit Tree Locations", SpiritTree.getAvailableOptions(player));
    }

	@Override
	public void handleClick(int slotId) {
	    val tree = SpiritTree.getTree(player);
	    val destinationTree = SpiritTree.get(slotId);
		val currentTree = SpiritTree.getTree(player);
		if (destinationTree == currentTree) {
            player.getDialogueManager().start(new NPCChat(player, 4982, "You're already here."));
		    return;
        }
		val optionalPredicate = destinationTree.getOptionalPredicate();
		if (optionalPredicate.isPresent()) {
		    val predicate = optionalPredicate.get();
		    if (!predicate.test(player)) {
		        return;
            }
        }
		val location = destinationTree.getLocation();
		player.lock(2);
