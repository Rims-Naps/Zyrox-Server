...
public class CatacombsProcessor extends DropProcessor {
    ...
    public void attach() {
        ...
        appendDrop(new DisplayedDrop(19679, 1, 1) {
            public double getRate(final Player player, final int id) {
                val definitions = NPCCDLoader.get(id);
                if (definitions == null) {
                    return 500;
                }
                return 500 - definitions.getHitpoints();
            }
        });
        ...
        appendDrop(new DisplayedDrop(19677, 1, 1) {
            public double getRate(final Player player, final int id) {
                val definitions = NPCCDLoader.get(id);
                return (int) (2F / 3F * (500 - Math.min(400, definitions.getHitpoints())));
            }
        });
    }

    public void onDeath(final NPC npc, final Player killer) {
        val rate = 500 - npc.getMaxHitpoints();
        ...
        if (!SuperiorMonster.superiorMonsters.contains(npc.getId()) || CatacombsOfKourend.polygon.contains(npc.getLocation())) {
            val shardRate = (int) (2F / 3F * (500 - Math.min(400, npc.getMaxHitpoints())));
    ...
    public static void dropTotemPiece(final Player killer, final NPC victim) {
        val baseCount = killer.getAmountOf(ItemId.DARK_TOTEM_BASE);
        val middleCount = killer.getAmountOf(ItemId.DARK_TOTEM_MIDDLE);
        val topCount = killer.getAmountOf(ItemId.DARK_TOTEM_TOP);
    ...
    @Override
    public int[] ids() {
        val set = new IntOpenHashSet(new int[]{7268, 7276, 7256, 7247, 7251, 7248, 7255, 7252, 7272, 7273, 7246, 7277, 7279, 7244, 7242,
