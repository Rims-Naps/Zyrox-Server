 ...
 */
public class RareDropTableProcessor extends DropProcessor {

    @Override
    public void attach() {
        for (final int id : allIds) {
            //Rare drop table
            val srdt = StandardRareDropTable.getTable(id);
            if (srdt == null) {
                continue;
            }
            this.appendDrop(new DisplayedDrop(2689, srdt.getRollsCount(), srdt.getRollsCount(), (float) srdt.getMaxRoll() / srdt.getRoll(), (player, npcId) -> npcId == id));
        ...
        for (final int id : allIds) {
            //Gem drop table
            val grdt = GemDropTable.getTable(id);
            if (grdt == null) {
                continue;
            }
            this.appendDrop(new DisplayedDrop(2690, 1, 1, (float) grdt.getMaxRoll() / grdt.getRoll(), (player, npcId) -> npcId == id));
        }
    }
    ...
    public void onDeath(final NPC npc, final Player killer) {
        ...
        val srdt = StandardRareDropTable.getTable(npc.getId());
        if (srdt != null) {
            if (StandardRareDropTable.roll(npc)) {
                for (int i = 0; i < srdt.getRollsCount(); i++) {
                    StandardRareDropTable.get(killer).ifPresent(drop -> {
                        ...
                        if (killer.getPrivilege().eligibleTo(Privilege.ADMINISTRATOR)) {
    ...
    @Override
    public int[] ids() {
        val list = IntLists.synchronize(new IntArrayList(100));
