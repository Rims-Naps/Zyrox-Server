...
public class ItemOnCastleWarsCatapult implements ItemOnObjectAction {
    ...
    public static final WorldObject ZAMORAK_CATAPULT = new WorldObject(4381, 11, 1, new Location(2384, 3117, 0));
    ...
    public void handleItemOnObjectAction(final Player player, final Item item, final int slot, final WorldObject object) {
        val broken = object.getId() == BROKEN_ZAMORAK_CATAPULT.getId() || object.getId() == BROKEN_SARADOMIN_CATAPULT.getId();
        val isSaradomin = CastleWars.getTeam(player).equals(CastleWarsTeam.SARADOMIN);
        val saradominObject = object.getId() == SARADOMIN_CATAPULT.getId() || object.getId() == BROKEN_SARADOMIN_CATAPULT.getId();
