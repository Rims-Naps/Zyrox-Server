...
public class RopeOnBattlementAction implements ItemOnObjectAction {
    ...
    public void handleItemOnObjectAction(final Player player, final Item item, final int slot, final WorldObject object) {
        ...
        val rotation = object.getId() == SARADOMIN_BATTLEMENT ? object.getRotation() + 2 : object.getRotation() - 2;
        val tileX = (object.getRotation() % 2 == 0) ? ((object.getRotation() == 2) ? object.getX() + 1 : object.getX() - 1) : object.getX();
        val tileY = (object.getRotation() % 2 == 0) ? object.getY() : ((object.getRotation() == 1) ? object.getY() + 1 : object.getY() - 1);
        val tile = new Location(tileX, tileY, 0);
        ...
        val ropeObject = new WorldObject(ROPE_OBJECT_ID, 4, rotation % 4, tile);
