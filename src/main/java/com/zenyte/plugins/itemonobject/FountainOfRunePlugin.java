...
public final class FountainOfRunePlugin implements ItemOnObjectAction {
	...
	private static final int[] BASIC_GLORIES = new int[] {
			1704, 1706, 1708, 1710, 1712, 11976
	};
	
	private static final int[] TRIMMED_GLORIES = new int[] {
            10362, 10360, 10358, 10356, 10354, 11966
	};
	
	private static final int[] SKILLS_NECKLACES = new int[] {
			11113, 11111, 11109, 11107, 11105, 11970
	};
	
	private static final int[] COMBAT_BRACELETS = new int[] {
			11126, 11124, 11122, 11120, 11118, 11974
	};

	private static final int[] RINGS_OF_WEALTH = new int[] {
			2572, 11988, 11986, 11984, 11982, 11980
	};

	private static final int[] TRIMMED_RINGS_OF_WEALTH = new int[] {
			12785, 20790, 20789, 20788, 20787, 20786
	};
            ...
            for (int i = 0; i < Utils.random(1, 2); i++) {
                for (val player : players) {
                    ...
                    if (Utils.random(2) == 0 && players.size() == 1)
                        continue;
                    val playerTile = player.getLocation();
                    Location tile = null;
                    int tryCount = 100;
                    while (--tryCount > 0) {
                    ...
                    val projectile = PROJECTILES.get(Utils.random(PROJECTILES.size() - 1));
	...
	public void handleItemOnObjectAction(final Player player, final Item item, int slot, final WorldObject object) {
		...
		WorldTasksManager.schedule(() -> {
			...
			for (int i = container.getContainerSize(); i >= 0; i--) {
				val containerItem = container.get(i);
				if (containerItem == null) {
					continue;
				}
				val id = containerItem.getId();
				val isUnchargedGlory = ArrayUtils.contains(BASIC_GLORIES, id);
				val isTrimmedUnchargedGlory = ArrayUtils.contains(TRIMMED_GLORIES, id);
				if (isUnchargedGlory || isTrimmedUnchargedGlory) {
					if (Utils.random(2499) == 0) {
						...
						player.getCollectionLog().add(new Item(ETERNAL_GLORY));
						continue;
					}
					containerItem.setId(isUnchargedGlory ? BASIC_CHARGED_GLORY : TRIMMED_CHARGED_GLORY);
					continue;
				}
				val isUnchargedRow = ArrayUtils.contains(RINGS_OF_WEALTH, id);
				val isTrimmedRow = ArrayUtils.contains(TRIMMED_RINGS_OF_WEALTH, id);
	...
	public Object[] getItems() {
		return new Object[] { 1704, 1706, 1708, 1710, 1712, 11976, 10360, 10358, 10356, 10354, 11966, 11113, 11111, 11109, 11107, 11105,
				11970, 11126, 11124, 11122, 11120, 11118, 11974, 2572, 11988, 11986, 11984, 11982, 11980,
				12785, 20790, 20789, 20788, 20787, 20786 };
	}
