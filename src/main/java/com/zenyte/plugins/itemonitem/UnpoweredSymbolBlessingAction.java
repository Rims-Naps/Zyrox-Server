...
public class UnpoweredSymbolBlessingAction implements PairedItemOnItemPlugin {
    ...
    public void handleItemOnItemAction(Player player, Item from, Item to, int fromSlot, int toSlot) {
        if (player.getSkills().checkLevel(Skills.PRAYER, 50, "bless the unholy symbol.")) {
            val book = from.getId() != ItemId.UNPOWERED_SYMBOL ? from : to;
            player.setAnimation(book.getId() == ItemId.UNHOLY_BOOK ? unholyBlessingAction : balanceBlessingAction);
            WorldTasksManager.schedule(() -> {
                val slot = from.getId() == ItemId.UNPOWERED_SYMBOL ? fromSlot : toSlot;
                player.getInventory().replaceItem(ItemId.HOLY_SYMBOL, 1, slot);
                player.getPrayerManager().drainPrayerPoints((int) (player.getPrayerManager().getPrayerPoints() * 0.10 + 2));
    ...
    public ItemPair[] getMatchingPairs() {
        return new ItemPair[]{
                ItemPair.of(ItemId.UNPOWERED_SYMBOL, ItemId.UNHOLY_BOOK),
                ItemPair.of(ItemId.UNPOWERED_SYMBOL, ItemId.BOOK_OF_BALANCE)
        };
    }
}
