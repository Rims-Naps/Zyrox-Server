...
public class FungicideFiller implements PairedItemOnItemPlugin {
    ...
    public void handleItemOnItemAction(final Player player, final Item from, final Item to, final int fromSlot, final int toSlot) {
        val filler = from.getId() == FILLER ? from : to;
        val can = filler == from ? to : from;
        if (can.getId() == 7421) {
    ...
    public ItemPair[] getMatchingPairs() {
        return new ItemPair[] {
                ItemPair.of(FILLER, 7421),
                ItemPair.of(FILLER, 7422),
                ItemPair.of(FILLER, 7423),
                ItemPair.of(FILLER, 7424),
                ItemPair.of(FILLER, 7425),
                ItemPair.of(FILLER, 7426),
                ItemPair.of(FILLER, 7427),
                ItemPair.of(FILLER, 7428),
                ItemPair.of(FILLER, 7429),
                ItemPair.of(FILLER, 7430),
                ItemPair.of(FILLER, 7431),
        };
    }
}
