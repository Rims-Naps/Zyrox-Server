...
public class ItemOnVolcanicForgeItemAction implements ItemOnObjectAction {
	...
	public void handleItemOnObjectAction(final Player player, final Item item, int slot, final WorldObject object) {
		val ward = item.getName().contains("shard") && item.getName().contains("Odium") ? 11926 : item.getName().contains("shard") && item.getName().contains("Malediction") ? 11924 : -1;
		if (ward != -1) {
			for (val id : VolcanicForge.WARDS.get(ward)) {
