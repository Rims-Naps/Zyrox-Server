...
public class InfernalEelCracking implements PairedItemOnItemPlugin {
    ...
    public void handleItemOnItemAction(Player player, Item from, Item to, int fromSlot, int toSlot) {
        player.getActionManager().setAction(new Action() {
            ...
            public int processWithDelay() {
                val random = Utils.random(ONYX_CHANCE * LAVA_SCALE_SHARD_CHANCE - 1);
                val inventory = player.getInventory();
                inventory.deleteItem(new Item(ItemId.INFERNAL_EEL));
                val item = random < ONYX_CHANCE ? new Item(ItemId.ONYX_BOLT_TIPS) : random < (ONYX_CHANCE + LAVA_SCALE_SHARD_CHANCE) ?
                        new Item(ItemId.LAVA_SCALE_SHARD, Utils.random(1, 5)) : new Item(ItemId.TOKKUL, Utils.random(15, 30));
    ...
    public ItemPair[] getMatchingPairs() {
        return new ItemPair[] {
                ItemPair.of(ItemId.HAMMER, ItemId.INFERNAL_EEL)
        };
    }
}
