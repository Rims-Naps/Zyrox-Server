...
public class AmylaseOnSuperEnergyAction implements ItemOnItemAction {
	...
	public void handleItemOnItemAction(final Player player, final Item from, final Item to, final int fromSlot, final int toSlot) {
		if (player.getSkills().getLevel(Skills.HERBLORE) < 77) {
			player.sendMessage("You need a Herblore level of at least 77 to brew stamina potions.");
			return;
		}
		val amylase = from.getId() == ItemId.AMYLASE_CRYSTAL ? from : to;
		val potion = from.getId() != ItemId.AMYLASE_CRYSTAL ? from : to;
		val dose = Integer.valueOf(potion.getName().substring(13, 14));
		if (amylase.getAmount() >= dose) {
			player.getDialogueManager().start(new StaminaPotCreationDialogue(player,
					new Item(potion.getId())));
		} else {
	...
	public ItemPair[] getMatchingPairs() {
		return new ItemPair[]{
				ItemPair.of(ItemId.SUPER_ENERGY4, ItemId.AMYLASE_CRYSTAL),
				ItemPair.of(ItemId.SUPER_ENERGY3, ItemId.AMYLASE_CRYSTAL),
				ItemPair.of(ItemId.SUPER_ENERGY2, ItemId.AMYLASE_CRYSTAL),
				ItemPair.of(ItemId.SUPER_ENERGY1, ItemId.AMYLASE_CRYSTAL)
		};
	}
