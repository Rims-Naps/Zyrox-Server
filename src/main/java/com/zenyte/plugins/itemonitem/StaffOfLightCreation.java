...
public class StaffOfLightCreation implements PairedItemOnItemPlugin {
    ...
    public void handleItemOnItemAction(final Player player, final Item from, final Item to, final int fromSlot, final int toSlot) {
        val light = from.getId() == 13256 ? from : to;
        val staff = from == light ? to : from;
        player.getDialogueManager().start(new Dialogue(player) {
            @Override
            public void buildDialogue() {
                doubleItem(staff, light, "Are you sure you wish to use the Saradomin's light on the staff? This irreversible " +
                        "process consumes both the light as well as the staff, upgrading both to a staff of light.");
