...
public class GuthixRestTeaMixing implements PairedItemOnItemPlugin {
    ...
    public enum PartialTea {
        HERB_TEA_MIX_1(ItemId.HARRALANDER),
        HERB_TEA_MIX_2(ItemId.GUAM_LEAF),
        HERB_TEA_MIX_3(ItemId.MARRENTILL),
        HERB_TEA_MIX_4(ItemId.HARRALANDER, ItemId.MARRENTILL),
        HERB_TEA_MIX_5(ItemId.HARRALANDER, ItemId.GUAM_LEAF),
        HERB_TEA_MIX_6(ItemId.GUAM_LEAF, ItemId.GUAM_LEAF),
        HERB_TEA_MIX_7(ItemId.GUAM_LEAF, ItemId.MARRENTILL),
        HERB_TEA_MIX_8(ItemId.HARRALANDER, ItemId.MARRENTILL, ItemId.GUAM_LEAF),
        HERB_TEA_MIX_9(ItemId.GUAM_LEAF, ItemId.GUAM_LEAF, ItemId.MARRENTILL),
        HERB_TEA_MIX_10(ItemId.GUAM_LEAF, ItemId.GUAM_LEAF, ItemId.HARRALANDER),
        COMPLETE_MIX(ItemId.GUAM_LEAF, ItemId.GUAM_LEAF, ItemId.MARRENTILL, ItemId.HARRALANDER);

        private static final PartialTea[] values = values();
        private final int[] ingredients;
        @Getter private final int teaId;
    ...
    public void handleItemOnItemAction(Player player, Item from, Item to, int fromSlot, int toSlot) {
        if (player.getSkills().getLevel(Skills.HERBLORE) < 18) {
        ...
        val herb = isHerb(from) ? from : to;
        val mix = herb == from ? to : from;
        val existingArray = mix.getId() == ItemId.CUP_OF_HOT_WATER ? new int[0] : Objects.requireNonNull(Utils.findMatching(PartialTea.values, tea -> tea.teaId == mix.getId())).ingredients;
        val ingredientsList = new IntArrayList(existingArray);
        ingredientsList.add(herb.getId());
        val ingredientsArray = ingredientsList.toIntArray();
        Arrays.sort(ingredientsArray);
        val upgradedTea = Utils.findMatching(PartialTea.values, tea -> Arrays.equals(tea.ingredients, ingredientsArray));
        if (upgradedTea == null) {
        ...
        player.getSkills().addXp(Skills.HERBLORE, 13.5F + (ingredientsList.size() * 0.5F));
    }
    ...
    public ItemPair[] getMatchingPairs() {
        ...
        for (val partialTea : PartialTea.values) {
