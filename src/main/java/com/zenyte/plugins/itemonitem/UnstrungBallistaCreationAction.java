...
public class UnstrungBallistaCreationAction implements ItemOnItemAction {
    ...
    public void handleItemOnItemAction(Player player, Item from, Item to, int fromSlot, int toSlot) {
        if (player.getSkills().getLevel(Skills.FLETCHING) < 73) {
            player.sendMessage("You need a Fletching level of at least 73 to do that.");
            return;
        }
        val ballista = from.getId() == 19595 || from.getId() == 19598 ? from : to;
        val spring = from.getId() == 19601 ? from : to;
    ...
    public ItemPair[] getMatchingPairs() {
        return new ItemPair[] {
                ItemPair.of(19595, 19601),
                ItemPair.of(19598, 19601)
        };
    }
