...
public class SacredEelDissecting implements PairedItemOnItemPlugin {
    ...
    public void handleItemOnItemAction(Player player, Item from, Item to, int fromSlot, int toSlot) {
        val knife = from.getId() == 946 ? from : to;
        val eel = knife == from ? to : from;
        ...
        player.getActionManager().setAction(new Action() {
            ...
            public boolean start() {
                if ((level = player.getSkills().getLevel(Skills.COOKING)) < 72) {
            ...
            public int processWithDelay() {
                val scales = Math.min(7, (3 + (level - 72 / 7))) + Utils.random(2);
                player.getSkills().addXp(Skills.COOKING, 100 + (scales * 3));
