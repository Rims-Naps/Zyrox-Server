...
public class SlayerStaffEnchanting implements PairedItemOnItemPlugin {
    ...
    public void handleItemOnItemAction(final Player player, final Item from, final Item to, final int fromSlot, final int toSlot) {
        val staff = from.getId() == 4170 ? from : to;
        val enchantment = staff == from ? to : from;
        player.getDialogueManager().start(new Dialogue(player) {
            ...
            public void buildDialogue() {
                ...
                options("Enchant the staff?", new DialogueOption("Yes.", () -> {
                    ...
                    player.getDialogueManager().start(new ItemChat(player, new Item(21255), "You enchant your slayer's staff with the enchantment scroll."));
