...
import com.zenyte.game.item.BossDropItem;
...
import static com.zenyte.game.item.BossDropItem.*;
...
public class ZulrahItemCreationAction implements ItemOnItemAction {
	...
	public void handleItemOnItemAction(final Player player, final Item from, final Item to, final int fromSlot, final int toSlot) {
		...
		if (item.equals(TOXIC_BLOWPIPE)) {
			if (player.getSkills().getLevelForXp(Skills.FLETCHING) < 53) {
			...
			if (player.getSkills().getLevelForXp(Skills.CRAFTING) < 52) {
			...
			if (player.getSkills().getLevelForXp(Skills.CRAFTING) < 59) {
				player.sendMessage("You need a Crafting level of at least 59 to do this.");
				return;
			}
			val isTrident = item.equals(TRIDENT_OF_THE_SWAMP) || item.equals(TRIDENT_OF_THE_SWAMP_E);
			if (isTrident) {
				val isCharged = SeaTrident.chargedMap.containsKey(from.getId()) || SeaTrident.chargedMap.containsKey(to.getId());
			...
			val itemInSlot = player.getInventory().getItem(slotId);
			player.getDialogueManager().start(new Dialogue(player) {
				...
				public void buildDialogue() {
					...
					options("Do you wish to proceed?", "Yes", "No")
						.onOptionOne(() -> {
	...
	@Override
	public int[] getItems() {
		return new int[]{12922, 12927, 12932, 1755, 11905, 11907, 11908, 11791, 12931, 13200, 13201,
				ItemId.UNCHARGED_TRIDENT_E, ItemId.TRIDENT_OF_THE_SEAS_E
		};
	}
}
