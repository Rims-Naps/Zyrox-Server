...
public final class RunecraftingTiaraEquip implements EquipPlugin {
    ...
    private static final Map<Integer, Integer> BITS = ImmutableMap.<Integer, Integer>builder().put(5527, 607).put(5529, 608).put(5531, 609).put(5533, 612).put(5535, 610).put(5537, 611).put(5539, 613).put(5541, 615).put(5543, 616).put(5545, 614).put(5547, 617).put(22121, 6220).build();
    ...
    public void onEquip(final Player player, final Container container, final Item equippedItem) {
        if (ArrayUtils.contains(SkillcapePerk.RUNECRAFTING.getCapes(), equippedItem.getId())) {
            for (val bitId : BITS.values()) {
                player.getVarManager().sendBit(bitId, 1);
            }
            return;
        }
        val bitId = BITS.get(equippedItem.getId());
    ...
    public void onUnequip(final Player player, final Container container, final Item unequippedItem) {
        ...
        val bitId = BITS.get(unequippedItem.getId());
        if (bitId == null || bitId == -1) {
            return;
        }
    ...
    @Override
    public int[] getItems() {
        return new int[]{5527, 5529, 5531, 5533, 5535, 5537, 5539, 5541, 5543, 5545, 5547, 22121,
                9765, 9766, 10645, 13280, 13282, 13342
        };
    }
