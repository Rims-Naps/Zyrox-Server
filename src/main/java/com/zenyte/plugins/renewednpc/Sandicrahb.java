...
public class Sandicrahb extends NPCPlugin {
        ...
        bind("Talk-to", (player, npc) -> {
            player.getDialogueManager().start(new Dialogue(player, npc) {
                ...
                public void buildDialogue() {
                    npc("Hello.");
                    options(TITLE,
                            new DialogueOption("Who are you?", key(5)),
                            new DialogueOption("Can you take me to " + (npc.getId() == 7483 ? "Crabclaw Isle" : "Mainland") + ".", key(50)));
    ...
    private final void travelRequest(@NotNull final Player player, @NotNull final NPC npc) {
        ...
        val toCrabclaw = npc.getId() == 7483;
        player.getDialogueManager().start(new Dialogue(player, npc) {
            @Override
            public void buildDialogue() {
                options(toCrabclaw ? "Travel to Crabclaw Isle?" : "Travel back to mainland?",
                        new DialogueOption("Yes." + (toCrabclaw ? " (10,000 coins)" : Strings.EMPTY), () -> {
