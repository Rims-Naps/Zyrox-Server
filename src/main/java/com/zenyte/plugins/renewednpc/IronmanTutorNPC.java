...
public class IronmanTutorNPC extends NPCPlugin {
    ...
    private void armour(final Player player, final NPC npc) {
        ...
        val armour = ZenyteGuide.STARTER_ITEMS[player.getGameMode().ordinal()];
        val items = Utils.concatenate(player.getInventory().getContainer().getItems().values().toArray(new Item[]{}), player.getEquipment().getContainer().getItems().values().toArray(new Item[]{}), player.getBank().getContainer().getItems().values().toArray(new Item[]{}));
        var added = 0;
        for (val item : armour) {
        ...
        val message = added == 0 ? "I think you've already got the whole set." : "There you go. Wear it with pride.";
