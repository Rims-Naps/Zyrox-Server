...
public class SimonTempleton extends NPCPlugin {
    public static final int COINS_PER_PYRAMID = 25_000;
    @Override
    public void handle() {
        bind("Talk-to", (player, npc) -> player.getDialogueManager().start(new Dialogue(player, npc) {
            ...
            public void buildDialogue() {
                if (player.carryingAny(PlunderReward.getItemIds())) {
                    ...
                    options("Do you want to sell all your artefacts?",
                            new DialogueOption("Yes, show me the money.", key(5)),
                            new DialogueOption("No, let me think about it.", key(10)));

                    player(5, "Yes, show me the money.").executeAction(() -> {
                        for (int itemId : PlunderReward.getItemIds()) {
                            val artefact = PlunderReward.getByItem(itemId);
                            val amount = player.getInventory().getAmountOf(itemId);
                            if (amount == 0) {
                                continue;
                            }
                    ...
                    player("No, I haven't.");
                    npc("Try harder, mate! It won't be long till some other critter turns up and snatches them " +
                            "out from under your eyes.");
                    player("I will, goodbye.");
                } else {
                    npc("G'day, mate. Got any new artefacts for me?");
                    val agilityArtefact = new Item(ItemId.PYRAMID_TOP, player.getInventory().getAmountOf(ItemId.PYRAMID_TOP));
                    options(TITLE,
                            new DialogueOption("Sell it", () -> {
                                player.getInventory().ifDeleteItem(agilityArtefact,
                                        () -> player.getInventory().addOrDrop(ItemId.COINS_995, COINS_PER_PYRAMID * agilityArtefact.getAmount()));
                                setKey(5);
                            }),
                    ...
                    val amountString = agilityArtefact.getAmount() == 1 ? "one" : "some";
                    val itemPronoun = (agilityArtefact.getAmount() == 1 ? "it" : "them");
