...
public class MonkOfEntrana extends NPCPlugin {
    ...
    public static boolean isAllowed(final Player player) {
        return Stream.of(player.getEquipment().getContainer().getItems().values(), player.getInventory().getContainer().getItems().values()).flatMap(Collection::stream)
                .noneMatch(MonkOfEntrana::isForbiddenOnEntrana);
    }

    public static final boolean isForbiddenOnEntrana(@Nullable final Item item) {
        return Objects.nonNull(item) && Objects.nonNull(item.getDefinitions()) && (item.isWieldable() && !isJewellery(item) && !ALLOWED_ITEMS.contains(item.getId())
                && !item.getName().startsWith("Graceful"));
    }
