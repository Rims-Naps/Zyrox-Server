...
public final class ShopInterface implements UserInterface {
	...
	public void handleComponentClick(final Player player, final int interfaceId, final int componentId, int slotId, final int itemId, final int optionId, final String option) {
		val obj = player.getTemporaryAttributes().get("Shop");
		if (!(obj instanceof GameShop)) {
			return;
		}
		val shop = (GameShop) obj;
		val item = interfaceId == INVENTORY_INTERFACE ? player.getInventory().getContainer().get(slotId) : shop.getContainer().get(slotId - 1);
		if (item == null) {
			return;
		}
		val defs = ItemDefinitions.get(item.getId());
		switch (interfaceId) {
		case INTERFACE:
			if (componentId == 16) {
				slotId--;
				...
				case 1:
					...
					val currency = shop.getCurrency() == 995 ? "coins" : shop.getCurrency() == 11849 ? "marks of grace" : "";
