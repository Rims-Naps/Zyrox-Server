...
public enum AnimationBase {
    ...
    public static final void pack() throws IOException {
        val archive = Game.getCacheMgi().getArchive(ArchiveType.BASES);
        for (val value : values()) {
            archive.addGroup(new Group(value.baseId, new File(new ByteBuffer(IOUtils.toByteArray(new FileInputStream(new java.io.File(value.path)))))));
        }
