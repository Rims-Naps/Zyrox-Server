...
public class HighDefinitionPets {
    ...
    private void packAnimationBase(@NotNull final AnimationBase base, final int[] anims) throws IOException {
        for (int anim : anims) {
            val file = new File("assets/pets/animation definitions/" + anim + ".dat");
...
*/
    ...
    public void packItems() throws IOException {
        ...
        for (val entry : sortedMap.int2ObjectEntrySet()) {
            ...
            def = new ItemDefinitions(realId++, new mgi.utilities.ByteBuffer(FileUtils.readFileToByteArray(file)));
        ...
        @AllArgsConstructor
        class ModelInfo {
            int id, zoom, rx, ry, rz, tx, ty;
        }

        val customItems = new Object2ObjectAVLTreeMap<String, ModelInfo>();
        ...
        for (val custom : customItems.object2ObjectEntrySet()) {
            assert def != null;
            def.setName(custom.getKey());
            val model = custom.getValue();
    ...
    public void packModels() throws IOException {
        ...
        for (val originalId : list) {
            val file = new File("assets/pets/models/" + originalId + ".dat");
    ...
    public void packNPCs() throws IOException {
        ...
        for (val entry : sortedMap.int2ObjectEntrySet()) {
            val file = entry.getValue();
            for (int i = 0; i < 1; i++) {
                val def = new NPCDefinitions(ChristmasMapPacker.npcId, new mgi.utilities.ByteBuffer(FileUtils.readFileToByteArray(file)));
                objSet.add(def);
