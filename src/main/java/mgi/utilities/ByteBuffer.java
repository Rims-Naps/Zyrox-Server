...
public class ByteBuffer {
    ...
    public final int readUnsignedSmart() {
        val value = this.buffer[this.position] & 0xff;
        if (value < 128) {
    ...
    public final int readShort() {
        this.position += 2;
        var value = ((this.buffer[this.position - 1] & 0xff) + (this.buffer[this.position - 2] << 8 & 0xff00));
        if (value > 0x7FFF) {
            value -= 0x10000;
        }
        return value;
    ...
    public final int readSignedMedInt() {
        this.position += 3;
        var value = ((this.buffer[this.position - 1] & 0xff) + (this.buffer[this.position - 2] << 8 & 0xff00) + (this.buffer[this.position - 3] << 16 & 0xff0000));
        if (value > 0x7FFFFF) {
            value -= 0x1000000;
        }
        return value;
   ...
   //     System.out.println("String: " + string);
    ...
    //    System.out.println("Strlen:" + strLen + " offset: " + strOffset);
    ...
    public Int2ObjectOpenHashMap<Object> readParameters() {
        ...
        val parameters = new Int2ObjectOpenHashMap<Object>(length);
        for (int index = 0; index < length; index++) {
            val stringInstance = readUnsignedByte() == 1;
            val key = readMedium();
    ...
    public void writeParameters(final Int2ObjectOpenHashMap<Object> parameters) {
        ...
        for (val parameter : parameters.int2ObjectEntrySet()) {
            val key = parameter.getIntKey();
