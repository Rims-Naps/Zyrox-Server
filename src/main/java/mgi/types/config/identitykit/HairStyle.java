...
public enum HairStyle {
    ...
    private static final Map<Integer, HairStyle> STYLES = new HashMap<>(VALUES.length);

    static {
        for (val style : VALUES) {
            STYLES.put(style.slotId & 0x3F | ((style.male ? 1 : 0) & 1) << 6, style);
        }
    }

    public static int getStyle(final int slotId, final boolean male) {
        val style = STYLES.get(slotId & 0x3F | ((male ? 1 : 0) & 1) << 6);
